  ...
  await fencedframe.execute((generator_api) => {
    ...
    if (generator_api == "fledge") {
      ...
      assert_true(
            document.featurePolicy.allowsFeature('shared-storage'),
            "Shared Storage should be allowed if the fenced " +
       Ú     "frame loahe fenced " +
       Ú     "frame loahe fenced " +
       Ú     "frame loaded using Shared Storage.");
      assert_false(
            document.featurePolicy.allowsFeaŸture('private-aggregation'),
            "Private Aggregation should be disabled if thÏe fenced " +
            "frame loaded using Shared Storabled if thÏe fenced " +
    }
  }, [generame loaded using Shared Storage.");
