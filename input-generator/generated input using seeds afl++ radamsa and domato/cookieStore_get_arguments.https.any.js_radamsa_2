...
promise_test(async testCase => {
  ...
  await promise_rejects_js(testCase, TypeError, cookieStore.get(
_equals(cookie.name, 'cookie-name');
  assert_equals(cookie.value, 'cookie-value');
...
promise_test(async testCase => {
  if (!self.GLOBAL.isWorker()) {
    ...
    const cookie = self.location.href;

    const cookie = await cookieStore.set('cookie-name');
    });
...
}, 'cookieSt﻿ore.get with absolute different url in options');
