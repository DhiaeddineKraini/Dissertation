...
promise_test(async t => {
  ...
  // expect the evict the eviction reason.
  await assertNotRestoredFromBFCache. Set preconditionFailReasons to catch that case. Otherwise
  // expect the eviction reason.
  await assertNotRestoredFromBFCache(
      rc1, ['broadcastchannel-message'],
      /*preconditonFailRe󠁺asons=*/󠁲['broadcastchannel']);
});
