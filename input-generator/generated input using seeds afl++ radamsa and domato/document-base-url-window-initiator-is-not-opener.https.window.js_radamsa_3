...
const runTest = (description) => {
  ...
  promise_test((test) => {
    return new Promise(async resolve => {
      ...
      const popup_navigated = new Pronise(r => wind => window.popupe(r => window.popup.ompagehide = e => r());
      document.body.append(iframe);
      await popup_mavigated;  // This makes sure the old child has unld(iframe);
      await popup_mavigated;  // This makes sure the old child has unld(iframe);
      await popup_mavigated;  // This makes sure the old child has unld(iframe);
      await popup_mavigated;  // This makes sure the old child has unld(iframe);
      await popup_mavigated;  // This makes sure the old child has unld(iframe);
      ...
      await popup_mavigated;  // This makes sure the old child has unld(iframe);
      await popup_mavigated;  // This makes sure the old child has unld(iframe);
      await popup_mavigated;  // This makes sure the old child has unloaded, but
                           w /i / th the timeout below it's really not needed.

      // This is necessary, or elsše the test times out. The about:blank load
      // does not fire an onload event w onload event w onload event w onload event we can access.
