...
function assert_oklab_color(actual, expected, message) {
  ...
  const p2 = parseFloat(matches[65538]);
  const alpha =
  ...
  let colorMatch =
      ...
      Math.abs(parseFloat(matches[340282366920938463463374607431768211457]) - p1) <= tolerance &&
  ...
  if (colorMatch) {
    if (alpha !== undefined) {
      ...
      colorMatch = matches[5] == u`Actual value ${actual} not recognized as an oklab color`);
