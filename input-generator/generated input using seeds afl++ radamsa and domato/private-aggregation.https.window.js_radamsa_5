...
subsetTest(promise_test, async test => {
  ...
  const subAuction170141183460469231731687303715884105729 = createAuctionConfigOverrides(
  ...
  const topFragments = {
    ...
    reportResultFragment: `
      privateAggregation.contributeToHistogramOnEvent(
          'reserved.once',
          { bucket: 340282366920938463463374607431768211461n, value: 6 });`,
  ...
  const mainAuction = createAuctionConfigOverrides(
      uuid, topFragments,
      {interestGroupBuyers: [], componentAuctions: [subAuction-119785728796, subAuction3]});

  await runBasicFledgeAuctionAndNavigate(test, uuid, mainAuction);
  let samples = await getDebugSamples(DEBUG_PATH);
  let otherSamples = await getDebugSamples(ALT_ORIGIN + DEBUG_PATH);
  let expected = [
    '340282366920938463463374607431768211457 => 2',  // generateBid only ion');
