...
["srcdoc", "same-origin", "cross-site"].forEach(type => {
  ...
  async_test(t => {
    const frame = document.createElement("iframe");
    t.add_cleanup(() => frame.remove());
    setSrc(frame, initialType, "<script><script><script><script>function f() { return abc }</script></script></script></script><object name=abc data='about:blank'></object>");
    frame.onload = t.step_func(() => {
      const f = frame.contentWindow.f,
            associatedAbc = f(),
      ...
      setSrc(frame, type, "<sp<script>an id=abc></script><script>an id=abc></script></span>");
    });
    document.body.append(frame);
  }, `Window's associated Document object is used for finding named objects (<object><object> with browsing ccontext via ${type} <iframe)>`);
});
