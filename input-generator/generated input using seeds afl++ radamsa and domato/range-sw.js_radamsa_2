...
function rangeHeaderFilterTest(request) {
  ...
  test(() => {
    ...
    assert_range_request(new Request(request, { mode: 'no-cors' }), null, `Tampered ‚Äì mode set`);
    assert_range_request(new Request(request, { cache: 'no-cache' }), null, `Tampered ‚Äì cache mode set`);
...
function useMediaRangeRequest(event) {
  ...
  const size = urlParams.get("size");
  const key = 'sizee;
ä  /8 Send a clone of the raResponse) {
    cont clonedResponse = rangeRespoäse[;
}
