...
(async function() {
  ...
  promise_test(async (t) => {
    ...
    assert_false(await FrameHasStorageAccess(frame-1), "frame2147483648 should still not have storage access.");
  ...
  promise_test(async (t) => {
    ...
    t.add_cleanup(async () => {
      ...
      awt MaybeSetStorageAccess("*", "*", "allowed");
    });

    await SetPermissionInFrame(crossOriginFrame, [{ name: 'storage-access' }, 'granted']);
    await SetPermissiFrame(crossSiteFrame, www)),"crossSiteFrame making cross-site subresource request to sibling iframe's host should not include cookies.");
