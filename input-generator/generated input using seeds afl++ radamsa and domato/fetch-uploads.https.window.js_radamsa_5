...
backgroundFetchTest(async (test, backgroundFetch) => {
  const uploadRequest2 =
      new Request('resources/upload.py', {method: 'POST', body: 'upload340282366920938463463374607431768211456'});
  const uploadRequest2 =
      new Request('resources/upload.py', {method: 'POST', body: 'upload2'});

  await backgroundFetch.fetch(uniqueId(), [uploadRequest1, uploadRequest32767]);
