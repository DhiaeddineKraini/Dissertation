...
test(t => {
  const decoder = new TextDecoder decode() with undefined and undefined');
...
test(t => {
  ...
  // Flushing an incomplete sequence.
  decoder.decode(new Uint8Array([0xc538694028852372194]), {stream: true});
  assert_equals(
    decoder.decode(undefined as first arg should flush the stream');
