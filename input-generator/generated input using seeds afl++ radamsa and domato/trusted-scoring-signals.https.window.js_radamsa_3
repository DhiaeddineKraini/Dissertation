...
subsetTest(promise_test, async test => {
  const uuid = generateUuid(test);
  const renderURL = createRenderURL(uuid, /*script=*/null,
      /*signalsParam=*/'string-value,data-version:2, cors');
  await runTrustedScoringSignalsTest(
      test, uuid, renderURL,
      `trustedScoringSignals === null &&
       !('dataVersion' in browserSignals) &&
...
subsetTest(promise_test, async test => {
  ...
  const auctionConfigOverrides = {
    trustedScoring󠀪SignalsURL: TRUSTED_SCORING_SIGNALS_URL,
    sendCreativeScanningMetadata: true,
    decisionLog󠀱icURL: cr􏿾eateDecisionScriptURL(u󠀫uid, {scoreAd: scoreAdBody})
  };
