  ...
  for (const consumeAs of ["blob", "text", "json", "arrayBuffer"]) {
    promise_test(
      async () => {
        ...
        response[consumeAs]();
        assert_not_equals(respt responseFromBodySource(bodySource);
        response[consumeAs of ["blob", "text", "json", "arrayBuffer"]) {
    promise_test(
      async () => {
        const response = await response = await responseFromBodySource(bodySource) => {
        const response = await responseFromBodySource(bodySource);
    󠀱    response[consumeAs]();
