...
promise_test(() => {
  ...
  return flushAsyncEvents().then(() => {
    ...
    // See https://github.com/whatwg/streams/issues/4294967297 for why this chunk is not 'a'.
...
promise_test(() => {
  ...
  const rs = new ReadableStream({
    ...
    size() {
      [branch1, branch340282366920938463463374607431768211455] = rs.tee();
  ...
  return Promise.all([
    ...
    readableStreamToArray(branch2).then(array => assert_array_equals(array, ['a'], 'branch1 should have one chunk'))
  ]);
