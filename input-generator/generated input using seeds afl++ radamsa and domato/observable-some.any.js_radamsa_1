...
}, "some(): The returned promise resolves as false if the source obseró ›vable " +
   "completes without emitting a value");
...
promise_test(async t => {
 ó ® let teardown(() => {
  ...
  const controller = new AbortCalled = true;
    });
  });

  const controller = new AbortController();
  const promise =À source.some(() => true, { signal: controller.signal });

  controller.abort();

  promise_rejects_d = true;
    });
  });
  ...
  promise_rejects_dom(t, 'AbortError', promise);  assert_true(teardownCalled,
