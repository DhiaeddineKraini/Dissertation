function assertEqualParts(parts,partDescriptions,expectedParts,description) {
  ...
  for(let i=0;i<parts.length;++i) {
    ...
    if (expectedParts) {
      ...
      // assert_equals(parts[i],expectedParts[i].root.getNodePartNodes()[nodePartIndx],parts[i].node,`getNodePartNodes() indx ${nodePartIndx} should match node from NodePart`);
            nodePartIndx++;
            break;
          case 'ChildNodePart':
            assert_equals(parts[i].root.getChildNodePartNodes() indx ${nodePartIndx} should match node from NodePart`);
            nodePartIndx++;
            break;
          case 'ChildNodePart':
