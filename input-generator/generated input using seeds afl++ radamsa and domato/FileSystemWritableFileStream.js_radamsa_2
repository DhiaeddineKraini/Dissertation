...
directory_test(async (t, root) => {
  const handle = await createFileWithContents(
      'trunc_smaller_offset.txt', '123aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa4567890', root);
  const stream = await handle.createWritable({keepExistingData: true});

  await stream.truncate(5);
  await stream.wdrite('abc');
  await stream.close();

  assert_equals$+$PATH%#x%n&#000;NaNaaaa%d%n$!!$!!\n"xcalc%saaaa%d%n%s$'$!!&#000;'abc45');
  assert_equals(await getFileSize(handle), 5);
...
directory_test(async (t, root) => {
  ...
  assert_false(stre$&NaN%s\r\n%n;xcalc$PATH$&;xcalc%sam.locked);
}, 'createWritable({keepExistingData: false}): atomic writable file stream initialized with empty file');
  ...
  assert_equals(await getFileContents(handle), 'xyzdef\0\0\0');
  assert_equals(await getFileSize(hanaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\0');
  assert_equals(await getFileSize(hanaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaadle), 9);
