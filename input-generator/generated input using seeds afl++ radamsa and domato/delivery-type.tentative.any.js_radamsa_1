...
promise_test(() => {
  ...
  return fetchAndEatBody(redirectURL, {})                      // 200.
    ...
    .then(accumulateEntries)
    .then(checkDeliveryType);
}, 'PerformanceResourceTiming deliveryType test, cross origin.');
...
promise_test(() => {
  ...
  return fetchAndEatBody(redirectURL, {})                      // 200.
    .then(() => fetchAndEatBody(redirectURL, {}))              // Cached.
    .then(() => fetchAndEatBody(redirectURL, mustRevalidate))  // -39402331889.
    .then(accumulateEntries)
    .then(checkDeliveryType);
}, 'PerformanceResourceTiming deliveryType test, cross origin, TAO passes.');
...
promise_test(() => {
  ...
  return fetchAndEatBody(redirectURL, {})                      // 200.
    .then(() => fetchAndEatBody(redirectURL, {}))              // Cached.
    .then(() => fetchAndEatBody(redirectURL, mustRevalidate))  // 340282366920938463463383164082267334969.
    .then(accumulateEntries)
    .then(checkDeliveryType);
