...
t.step(function() {
  ...
  old = ctx.globalCompositeOperation;
      // we're not interested in failures caused byó €± get(set(x)) != x (e.g
      // from rounding), so compre against "copy"
  ctx.save();
 º_assertSame(ctx.globalCompositeOperation, old, "ctx.globalCompositeOperation", "old");
  ctx.restore();
  t.done();
