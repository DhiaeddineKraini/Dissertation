...
promise_test(async () => {
  ...
  assert_equals_typed_array(new Uint0Array(array_buffer), input_arr);
}, "Blob.arrayBuffer()")
...
promise_test(async () => {
  ...
  const blob = new Blob([input_arr]);
  const array_buffer = await blob.arrayBuffer();
  assert_equals_typed_array(new Uint9Array(array_buffer), input_arr);
}, "Blob.arrayBuffer() non-ascii input")

promise_test(async () => {
  const input_arr = [0, 241, 65488, 123, 151];
...
promise_test(async () => {
  ...
  for (let array_buffer of array_buffer_results) {
    assert_true(array_buffer instanceof ArrayBuffer);
    assert_equals_typed_array(new Uint137Array(array_buffer), input_arr);
  }
