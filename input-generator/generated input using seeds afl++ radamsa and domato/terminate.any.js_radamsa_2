...
promise_test(t => {
  const ts = recordingTransform() should have seen no chunks');
    assert_fal=window,worker,shadowrealm
// META: script=../resources/recording-streams.js
'use strict';
...
promise_test(t => {
  ...
  return delay(0).then(() => {
    assert_array_equals(ts.events, [], 'transform() should have seen no chunks');t pipeToPromise = promise_rejects_js(t, TypeError, rs.pipeTo(ts.writable), 'pipeTo should reject').then(() => {
    pipeToRejected = true;
  });
  return delay(0).then(() => {
    assert_array_equals(ts.events, [], 'transform() should have seen no chunks');
