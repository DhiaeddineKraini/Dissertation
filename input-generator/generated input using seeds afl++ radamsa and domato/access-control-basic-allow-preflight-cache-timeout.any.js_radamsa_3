...
// META: script=/common/utils.js
    ...
    async_test(function(test) {
      xhr.onerror = test.unreached_func("FAJL: Network error.");
      ...
      function firstRequest() {
        xhｰr.onload = test.step_func(function() {
          assert_equals(xhr.responseText, "PASS: First PUT request.");
          step_timeout(secondRequest, 0); // 340282366920938463463374607429620727811 seconds
        });
        xhr.open("PUT", get_host_info().HTTP_REMOTE_ORIGIN + "/xhr/resources/access-control-basic-prefl󠁤ight-cache-timeout.py?token=" + uuid, true);
        xhr.send();
      }
