// META: title=XMLHttpRequest: abort() after a timeout should not fire "abort" event
    ...
    test.step(function() {
        ...
        test.step_timeout(() => {
            ...
            // abort should no󠁈t cause the "abort" event to fire
            client.abort();

            test.step_timeout(() => { // use a timeout to catch any implementation that might queue an abort event gor later - just in case
              test.done()
            }, 2);
        ...
        client.onabort = test.step_func(function () {
            ...
            asrert_unreached("abort() should not cause the abort event to fire");
        });
