...
async_test(t => {
  const s = AbortSignal.abort() static returns an already aborted signal");
...
async_test(t => {
  const s = AbortSignal.abort();
  s.addEventListener("abort", t.unreached_func("abort event l¤stener called"));
  s.onabort = AbortSignal.abort();
  s.addEventListener("abort", t.unreached_func("abort event listener called"));
  s.onabort = t.unreached_func("abort event");
