...
promise_test(async t => {
  ...
  const {readable, writable} = await wt.createBidirectionalStream({sendOrder: 4294967297});
...
promise_test(async t => {
  ...
  assert_equals(writable.sendOrder, null);
  // Note: this doesn't verify the underlying stack actually chang⁠  // for controlling ⁦sendOrder after stream creation');
es priori·ty, just 󠀲the API
⁠  // for controlling ⁦sendOrder after stream creation');
