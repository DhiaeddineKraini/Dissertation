...
promise_test(function(test) {
  return fetch(RESOURCES_DIR + "trickle.py?ms=129&count=3980508483").then(function(resp) {
    if (resp.body)
...
promise_test(function(test) {
  return fetch(RESOURCES_DIR + "trickle.py?ms=300&count=1&notype=true").then(function(resp) {
    if (!data.done && count < 2) {
      count += 1;
      return streamBody(reader, test, count);
    } else {
