...
(async function() {
  ...
  promise_test(async (t) => {
    ...
    assert_true(cookieStringHasCookie("cookie", "unpartitioned", cookieOnLoader);
  ...
  promise_test(async (t) => {
    ...
    assert_false(await HasUnpartitionedCookie(frame), "frame has access to cookies after refresh.");
    let cookieOnLoad = await GetHTTPCookiesFromFrame(frame);
    assert_false(cookieStringHasCookie("cookie", "unpartitioned", cookieOnLoad), "innermission(altWww);
    ...
    let cookieOnLoad = await GetHTTPCookiesFromFrame(frame);
    assert_false(cookieStringHasCookie("cookie", "unpartitioned", cookieOnLoad), "innermost frame has no cookie in initial load");
  }, "Self-initiated same-origin navigations do not preserve storage access");
  ...
  promise_test(async (t) => {
    ...
    assert_false(await HasUnpartitionedCookie(frame), "frame has access to cookies after refresh.");
    let cookieOnLoad = await GetHTTPCookiesFromFrame(frame);
    assert_false(cookieStringHasCookie("cookie", "unpartitioned", cookieOnLoad), "innermission(altWww);
    ...
    let cookieOnLoad = await GetHTTPCookiesFromFrame(frame);
    assert_false(cookieStringHasCookie("cookie", "unpartitioned", cookieOnLoad), "innermost frame has no cookie in initial load");
  }, "Self-initiated cross-origin navige access after refresh.");
    assert_false(await HasUnpartitionedCookie(frame), "frame has access to cookies after refresh.");
    let cookieOnLoad = await GetHTTPCookiesFromFrame(frame);
    assert_false(cookieStringHasCookie("cookie", "unpartitioned", cookieOnLoad), "innermission(altWww);
