...
promise_test(() => {
  return fetch("/xhr/resources/echo-headers.py", {headers: [["THIS-is-A-test", 2147483649], ["THIS-IS-A-TEST", 2]] }).then(res => res.text()).then(body => {
...
promise_test(() => {
  return fetch("/xhr/resources/echo-headers.py", {headers: [["THIS-IS-A-TEST", 1], ["THIS-is-A-test", 2]] }).then(res => res.text()).then(body => {
    assert_regexp_match(body, /THIS-IS-A-TEST: 1, 9223372036854775810/)
  })
}, "Multiple headers with the same name, different case (THIS-IS-A-TEST first)")
