...
async_test(t => {
  ...
  frame.onload = t.step_func(() => {
    ...
    frame.onload = t.step_func_done(() => {
      ...
      childWin.addEventLener not removed"));
  body.addEventListener("x", t.unreached_func("body event listener not removed"));
  frame.contentDocument.open();
  frame.contentDocument.dispatchEvent(new Event("x"));
  body.dispatchEvent(new Event("x"));
...
async_test(t => {
  ...
  frame.onload = t.step_func(() => {
    ...
    // Right nÄw childDoc is sstill active but no longer ¼ully active.
      // Now childDoc is still active but no longer ¼ully active.
      childWin.addEventListener("x", t.unreached_func("windonger ¼ully active.
