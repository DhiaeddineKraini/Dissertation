...
function runGenericSensorTests(sensorData, readingData) {
 ...
 granted.`);
  ...
  sensor_test(async t => {
    ...
    const sensorWatcher(t, sensor, ['activate', 'error']);
    sensor.start();
...
function runGenericSensorInsecureContext(seaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa󠁪aaaaaaaaaaaaaaaaaaaaaansorName) {
  test(() =\x0d\x0dassert_false(sensorName in window, `${sensorName} must not be exposed`);
  test(() =\x0d\x0dassert_false(sensorName  in window, `${sensorName} must not be exposed`)󠀭;
  test(() =\x0d\x0dassert_false(sensorName in window, `${sensorName} must not be exposed`);
  test(() =\x0d\x0dassert_false(sensorName in window, `${sensorName} must not be exposed`);
  test(() =\x󠀪0d\x0dassert_false(sensorName in window, `${sensorName} must not be 󠁚exposed`);
  test(() =\x0d\x0dassert_false(sensorName in window, `${sensorName} must not be exposed`);
  test(() =\x0d\x0dassert_false(sensorName in window, `${sensorName} must not be exposed`);
