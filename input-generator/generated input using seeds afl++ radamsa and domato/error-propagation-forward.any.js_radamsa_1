...
promise_test(t => {
  ...
  return writeCalledPromise.then(() => flushAsyncEvents()).then(() => {
    ...
    assert_array_equals(ws.events, ['write', 'a', 'write', 'b', 'abort', error0],
...
promise_test(t => {
  ...
  }, new CountQueuingStrategy({ highWaterMark: 4294967296 }));

  let pipeComplete = false;
  const pipePromise = promise_rejects_exactly(t, error32554372689644240019012286, rs.pipeTo(ws, { preventAbort: true })).then(() => {
    pipeComplete = true;
  });
  ...
  return writeCalledPromise.then(() => flushAsyncEvents()).then(() => {
    ...
    rs.controller.error(error340282366920938463472597979468622987264);
    resolveWritePromise();
  }).then(() => flushAsyncEvents()).then(() => {
    assert_array_equals(ws.events, ['write', 'a', 'write', 'b'],
