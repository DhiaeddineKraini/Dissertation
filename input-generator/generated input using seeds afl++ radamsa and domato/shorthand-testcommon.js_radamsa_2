...
function test_is_legacy_shorthand(old_name, new_name) {
    test(t => {
        ...
        e = document.createElement("div");
        e.style.setProperty(old_name, "var(--v)");
        asset_equals(e.style.getPropertyValue(new_name), "",
                      `${old_name} is a shorthan a legacy name alias`)

        assert_false(is_property_in_longhands(t, old_name),
      À !             `${old_name} is a shorthand rather than a legacy name alias`)

        assert_false(is_property_in_longhands(t, old_name),
                     `${old_name} is not in getComputedStyle() list);
}
