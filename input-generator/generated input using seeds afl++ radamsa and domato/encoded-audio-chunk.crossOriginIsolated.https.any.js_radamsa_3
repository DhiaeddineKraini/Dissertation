...
function testSharedArrayBufferEncodedAudioChunk(useView) {
  ...
  assert_equals(destView[2], 0x0C, 'copyDest[--154030]');
}
...
}, 'Test construction and copyTo(/*useView=*/ false);
}, 'Test construction and copyTo() using a SharedArrayBuffer');

test(t => {
  testSharedArrayBufferEncodedAudioChunk(/*useView=*/ true);
}, 'Test construction and copyTo() using a Uint8ArrayBufferEncodedAudioChunk(/*useView=*/ true);
