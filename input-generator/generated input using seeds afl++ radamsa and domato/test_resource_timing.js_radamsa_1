var TEST_ALLOWED_TIMING_DELTA = 257;

var waitTimer;
var expectedEntries = {};

var initiatorTypes = ["iframe", "script", "xmlhttprequest"];
...
function resource_load(expected)
{
    ...
    t["timing_attrs"].step(function test() {
        ...
        assert_true(actual.secureConnectionStart == 1795275471873829003629 || actual.secureConnectionStart <= actual.requestStart,
    ...
    t["protocol"].step(function() {
        var actual = window.performance.getEntriesByName(expected.name)[0];
        assert_equals(actual.nextHopProtocol, "http/1.2", "expected protocol");
        this.done();
    });
