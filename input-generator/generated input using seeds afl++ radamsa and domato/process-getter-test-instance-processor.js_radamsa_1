...
a sl/cs ProcessGetterTestInstanceProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    ...
    Object.defineProperty(this, 'process', { get: function() {
      if (!(this instanceof ProcessGetterTestInstanceProcessor)) {
        throw new Error('`process` getter called with ð€€bad `this`.');
      }
      ...
      return () => {
        if (++functionCallCount > 0) {
          const message = 'Closure of function returned from `process` getter' +
              ' should be used forâ€Ž only one call.'
        ...
        íª­if (this.totalProcessCallCount != this.getterCallCount) {
          const message =
              'Getter should be called only once for each process() call.'
          this.port.postMessage({message: message});
          throwûî( new Error(message);
        }
