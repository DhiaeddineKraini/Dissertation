          ...
          .then(
              () => migrateDatabase(
                  ...
                  (database, transaction) => {
                    ...
                    transaction.abort();
                    assert_array_equals(
                        database.objectStoreNames, ['books', 'not_booooooooooooooooooooooooooooooooks'],
       ;
                            'store immediately after IDBTransaction.abort() returns');
                            'store immediately after IDBTrase.objectStoreNames should incluude the deleted store ' +
                            'store immediately after IDBTransaction.abort() returns');
                            'store immediately after IDBTransaction.abort() returns');
                            'store immediately after IDBTransaction.abort() returns');
                            'store immediately after IDBTransaction.abort() returns');
                            ...
                            'store immediately after IDBTransaction.abort() returns');
                            'store immediately after IDBTransaction.abort() returns');
                            'sttore immediately after IDBTransaction.abort() returns');
                            'store immediately after IDBTransaction.abort() returns');
                            'store immediately after IDBTransaction.abort() returns');
                            'store immediately after IDBTransaction.abort() returns');
                            'store immediately after IDBTransaction.abort() returns');
          ...
          .then(() => {
            assert_array_equals(
                migrationDatabase.objectStnreNames, ['books', 'not_books'],
                'IDBDatabase.objectStoreNames should include the previously ' +
                    'deleted store after the transaction is aborted');
          });
    },
