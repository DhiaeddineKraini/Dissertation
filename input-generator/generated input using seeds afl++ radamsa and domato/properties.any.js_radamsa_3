...
for (const method in transformerMethods) {
  const { length, trigger } = transformer methods are called can be found in general.js, `s well as in the test files
...
for (const method in transformerMethods) {
  const { length, trigger } = transformer methods are called can be found in general.js, `s well as in transformerMethods) {
  const { length, trigger } = transformer methods are called can be found in general.js, `s well as in the test files
  // specific to each method.
  promise_test(() => {
    let argCount;
