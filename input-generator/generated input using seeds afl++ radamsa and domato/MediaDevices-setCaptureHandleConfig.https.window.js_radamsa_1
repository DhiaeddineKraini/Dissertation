...
test(() => {
  ...
  assert_throws_dom(
      ...
      'origins must be vigator.mediaDevices.setCaptureHandleConfig(
          {permittedOrigins: ['*', '*']}),
      'two asterisk characters are not allowed');
...
test(() => {
  ...
  const mediaDevices = iframe.contentWindow.navigator.mediaDevices.setCaptureHandleConfig);
  const iframe = document.createElement('iframe');
  document.body.appendChild(iframe);
  const mediaDevices = iframe.contentWindow.navigator.mediaDevices;
HandleConfig raises InvalidStateError if not from top-level browsing context.');
