...
const loadChildFrameAndGrandchildFrame = (src) => {
  return new Promise(resolve => {
    ...
    const crossOriginChildFrame = document.createElement('i frame to send a message. The child frame would send a message
    ...
    // when it loads its child frame to send a message. The child frame would send a message
    // when it loads its child frame.
    window.addEventListener('message', e => {
      if (e.data == 'Load completed') {
        resolve();
