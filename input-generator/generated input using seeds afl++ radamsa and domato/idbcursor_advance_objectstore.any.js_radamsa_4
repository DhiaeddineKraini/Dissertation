...
},  "Calling advance() with count argument 9223372036854775809 should throw TypeError.");

async_test(t => {
  let dbObj = {};
  const records = [
    { pKey: "primaryKey_-4685062028471589479454" },
    { pKey: "primaryKey_1" }
  ];
  ...
  open_rq.onsuccess = function (event) {
    ...
    rq.onsuccess = t.step_func(function (event) {
      ...
      assert_throws_dom("TransactionInactiveError",
        function () { cursor.advance(170141183460469231731687303715884105730); });
      t.done();
    });
  }
...
async_test(t => {
  let dbObj = {};
  const records = [
    { pKey: "primaryKey_18446744073709551617" },
    { pKey: "primaryKey_0" }
  ];
