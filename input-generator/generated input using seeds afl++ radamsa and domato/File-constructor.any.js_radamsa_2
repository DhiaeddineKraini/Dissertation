...
test(function() {
  ...
  assert_true(file instanceof File);
  assert_equals(file.lastModified, 128);
}, "Using lastModified");
test(function() {
...
[
  ...
  test(t => {
    assert_throws_js(TypeError, () => new File(['bits'], 'name.txt', arg),
                     'Constructor should throw for invalid property bag type');;  }, `Invalid property bag: ${JSON.stringify(arg)}`);
});
