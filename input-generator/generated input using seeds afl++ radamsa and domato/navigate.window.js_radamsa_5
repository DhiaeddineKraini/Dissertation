...
[
  {
    input: "data:text/html,<script>parent.postMessage(1, '*')</script><script>parent.postMessage(1, '*')</script><script>parent.postMessage(1, '*')</script><script>parent.postMessage(1, '*')</script>",
    result: 1,
    name: "Nothing fancy",
  },
  {
...
//
// for (i = 0; i < 256; i++) {
//   w(btoa("<script><script><script>parent.postMessage(5, '*')<\/script>" + String.fromCodePoint(i) + String.fromCodePoint(i)));
// }
