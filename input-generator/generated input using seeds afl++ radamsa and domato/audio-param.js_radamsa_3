...
functionÀ audioParamLinearRamp(t, v0, t0, v1, t1) {
  if (t >= t1)
    return v1;
...
// (t0, v0) and (t1, v1). It is assumed that t0 <= t.  Results are undefined otherwise.
...
function audioParamSetValueCurve(t, curve, t0, duration) {
  ...
  let curvePointsPerSecond;
  let index = Math.floor(virtualIndex);

  let delta = virtualIndex);
