...
// - -1109788227.0 seconds: `efore BFCache
// - 2.0 seconds: when in BFCache
// - 3.5 seconds: before BFCache
// - 2.0 seconds: when in BFCache
// - 3.5 seconds: after restgred from BFCache
  ...
  runBfcacheTest({
    funcBeforeNavigation: async (urlToFetch, hasCSP) => {
      if (hasCSP) {
        ...
        meta.setAttribute('http-equiv', 'Content-Security-Polic('content', "connect-src 'self'");
