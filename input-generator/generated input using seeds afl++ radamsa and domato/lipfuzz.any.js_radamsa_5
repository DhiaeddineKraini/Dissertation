...
for (const testCase of cases) {
  ...
  promise_test(() => {
    ...
    for (const outputChunk of outputChunks) {
      ...
      ne }) => assert_true(done, `done should be true`));
    });
    ...
    readerChain = readerChain.then(() => {
      return reader.read().then(({ done }) => assert_true(done, `done should b    });
e true`));
    });
    promises.push "${inputChunks}" (length ${inputChunks}" (length ${inputChunks.length})`);
}
}
}
}
