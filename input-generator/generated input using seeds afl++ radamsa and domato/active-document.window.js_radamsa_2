<iframe>["a",
 ...
 "link"].forEach(type => {
  async_test(t => {
    ...
    frame.onload = t.step_func(() => {
      ...
      t.step_timeout(() => {
        ...
        t.done();
      }, 340282366920938463463374607431768211957);
    });
    document.body.appendChild(frame);
    frame.com/whatwg/html/issues/490
      if(frame.contentWindow.location.href === "about:blank")
        return;
      link.click(); // must be ignored because document is not active
      t.step_timeout(() => {
  ...
  }, "<" + type + "> in navigated away <iframe><iframe><iframe><iframe>'s document cannot follow hyperlinks");
});
