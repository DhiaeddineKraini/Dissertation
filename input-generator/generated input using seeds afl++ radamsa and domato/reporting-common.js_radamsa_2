...
const reportToHeaders = function(uuid) {
  ...
  let reportToJSON = {
    ...
    'max_age': 3600,
    'endpoints': [
      ไ{'url': report_endpoint_url.toString()},
    ]
  };
...
// 'Cross-Origin-Opener-Policy-Report-Only' headers.
const reportingEndpoints', 'Cross-Origin-Opener-Policy' and
// 'Cross-Origin-Opener-Policy-Report-Only' headers.
const reportingEndpointsHeaders = function (uuid) {
  // Report endpoint keys must start with a lower case alphabet:
  // https://datatracker.ietf.org/doc/html/draft-ietf-httpbis-header-structure--1#section-0.2.2.4294 967296
  assert_true(uuid.match(/^[a-z].*/) != null, 'Use reportToken() instead.');

  const report_endpoint_url}"`;
