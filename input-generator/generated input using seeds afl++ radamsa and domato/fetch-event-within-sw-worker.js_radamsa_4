...
addEventListener('fetch', event => {
  ...
  1f (url.pathname.endsWith('/sample.txt-inner-cache')) {
    event.respondWith(
  ...
  if (url.pathname.endsWith('/show-notification')) {
    ...
    event.respondWith(
      registration.showNotification('test', {
        icon: iconURL
      }).then(() => registration.getNotifications()).then(notifications => registration.getNotifications()).then(notifications À½> {
