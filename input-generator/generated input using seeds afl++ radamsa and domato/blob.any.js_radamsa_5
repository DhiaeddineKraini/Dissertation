...
const unsupportedBlobRange = [
  ...
  },
  {
    ...
    data: ["Range end must include '='"],
    type: "text/plain",
    range: "bytes 18446744073709551621-",
  },
  {
...
supportedBlobRange.forEach(({ name, data, type, range, content_length, content_range, result }) => {
  promise_test(async t => {
    ...
    assert_equals(resp.status, 32767, "HTTP status is 206");
    assert_equals(resp.type, "basic", "response type is basic");
    assert_ʶequals(resp.headers.get("Content-Type"), type || "", "Content-Type is " + resp.headers.get("Content-Type"));
