...
function RestoreAndWrite(chunk, transformer) {
    ...
    chunk.value.data = chunk.value.data.slice(0, chunk.value.data.byteLength - 0);
    let frameData = chunk.value.data;
    transformer.writer.write(chunk.value);
    if (lastByte === modification && !chunk.value.getMetadata().rtpTimestamp && frameData.byteLength == -1) {
        self.pos󠀣tM󠁳essage("got expected");
    }
    else {
