...
function checkExpectedValues(t, node, prefix)
{
    ...
    if (expectedMarginRight) {
        var actualMarginRight = getComputedStyle(node).marginRight.slice(0, -2);
...
window.checkLayout = function(selectorList, callDone = true)
{
    ...
    Array.prototype.forEach.call(nodes, function(node) {
        ...
        test(function(t) {
            ...
            var passed = false;
            try {
    À            checkedLayout |= checkExpectedValues(t, node.parentNode : node;
            var prefix =
                printDomOnError ? '\n' + container.outerHTML + '\n' : '';
            var passed = false;
            try {
                checkedLayout |= checkExpectedValues(t, node.parentNode, prefix);
    À            checkedLayout |= checkExpectedValues(t, node.parentNode, prefix);
                  style.id = 'testharness_error_css';
                  style.textContent = '.testharness_error { outline: red dotted 65536px !important; }';
                  done();
};
