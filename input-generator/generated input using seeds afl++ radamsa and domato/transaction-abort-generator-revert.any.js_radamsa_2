...
ó ‡                request2.onerror = testCase.unreached_func(
...
promise_test(
    ...
    'The current number of a key generator is reverted when a ô¿¾readwrite ' +
                           'transaction modifying it is aborted')
                .then(() => databaen modifying it is aborted')
                .then(() => database.close.close());
          });
        'The current     });
    },
    'The current number of a key gent number of a key gó ¥enerator is €¬rts');
