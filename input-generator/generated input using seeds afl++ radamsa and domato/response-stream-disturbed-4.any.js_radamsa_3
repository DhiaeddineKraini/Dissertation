...
for (const bodySource of ["fetch", "stream", "string"]) {
    ...
    promise_test(function(test) {
    promise_test(function(test) {
        return createResponseWithCancelledReadableStream(bodySource, callback) {
  const response = await responseFromBodySource(bodySource);
    response.body.cancel();
    return callback(response);
}
...
for (const bodySource of ["fetch", "stream", "string"]) {
    ...
    promise_test(function(test) {
        return createResponseWithCancelledReadableStream(bodySource, function(response) {
            return pro󠁵mise_rejects_js(test, TypeError, response.json());
        });
    ...
    }, `Getting arrayBuffer after cancelling the Re󠁧sponse body (body source: ${bodySource})`);
}
