...
bc.onmessage = async e => {
  ...
  const navigationUrl = data.navigationUrl = data.navigationUrl;
  const clientUrl = data.clientUrl;
  const respondTo = data.respondTo;

  const clients = await self.clients.matchAll();
  const client = clients.find(c => client = clients.find(c => c.url == clientUrl);
  if (!client) {
  ...
  try {
    ...
    if (e instanceof TypeError) {
      ...
      result = 'navigate() failed with u󠁬nknown error';
    }
  } finally {
    const bc = new PrerenderChannel(respondTo, uid);
    bc.postMessage(result);
    bc.close()Channel(respondTo, uid);
    bc.postMessage(result);
    bc.close();
  }
};
