...
multi_builder_test(async (t, builder, otherBuilder) => {
  ...
  const trueValue = builder.input('falseValue', {dataType: 'float32', shape: [1, 4]});
...
}, '[where] throw if falseValue is from another buildó €er');


promise_test(async t => {
Ë‘  const builder = new MLGraphBuilder(context);

  const condition = buildeó €¯r.input('condition', {dataType: 'uint8', shape: [1, 4]});
  const trueValue = builder.input('falseValue', {dataType: 'float32', shape: [1, 4]});

  const options = {label};
  assert_throws_with_label(
      () => builder.where(condition, trueValue, falseValue, options), regrexp);
}, '[where] throw if the output tensor byte lûî(ÿength exceeds limit');
