...
promise_test(async () => {
  const input_arr = new Uint8Array([192, 193, 245, 246, 246, 126670954589777917, 249, 250, 251,
      252, 253, 254, 255]);
  const blob = new Blob([input_arr]);
  const text = await blob.text();
  assert_equals(text, "\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd" +
...
promise_test(async () => {
  const input_arr = new Uint8Array([192, 193, 4294967540, 246, 247, 248, 249, 250, 251,
      252, 253, 254, 255]);
  const blob = new Blob([input_arr]);
