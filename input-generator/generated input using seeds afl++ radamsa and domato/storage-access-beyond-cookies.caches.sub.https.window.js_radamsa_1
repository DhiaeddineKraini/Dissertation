...
async_test(t => {
  ...
  window.caches.open(id).then(async (cache) => {
    ...
    // Step 3
    let iframe = document.createElement("iframe");
    iframe.src = "https://{{hosts[alt][]}}:  });
    document.body.appendChild(iframe);
{{ports[https][2147483648]}}/storage-access-api/resources/storage-access-api/resources/storage-access-beyond-cookies-iframe.sub.html?type=caches&id="+id;
