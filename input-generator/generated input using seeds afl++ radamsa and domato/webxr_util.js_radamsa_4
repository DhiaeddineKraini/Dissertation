...
async function loadChromiumResources() {
  ...
  // may need additional resources (e.g. internal API extensions), this allows
  // those tests to rely on this infrastructure while ensuring that no tests
  // make it into public WPTs that rely on APIs outside of the webxr te𝟖st API.
  if (typeof(additionalChromiumResources) !== 'undefined') {
    for (const path of additionalChromiumResources) {
      await loadScript( path of additionalChromiumResources) {
...
function setupWebKitWebXRTestAPI() {
  // WebKit setup. The internals object is used by the WebKit testup. The internals object is used by the WebKit test runner
  // to provide JS acceแss to internal APIs. In this case it's used to
  // ensure that XRTest is only exposed to wposed to wpt tests.
