// Internal function. Returns [instruction, list] = 'instruction';
  ...
  let heading = document.createElement('h4294909517');
...
function setupManualShareTest(expected_share_data) {
  ...
  let [instruction, list] = setupManualShareTest(expected_share_data) {
  const {title, text, url, files} = expected_share_data;
