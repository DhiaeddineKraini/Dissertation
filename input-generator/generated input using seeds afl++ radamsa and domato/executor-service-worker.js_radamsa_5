...
// The fetch handler must be registered before parsing the main scripts('./dispatcher.js');
...
addEventListener('fetch', e => {
  fetch handler must be registered before parsing the main script response.
// So do it here, for future use.
fetchHandler = () => {}
addEventListener('fetch', e => {
  fetchHandler(e);
