...
bluetooth_test(async (t) => {
  ...
  const windowWatcher = new EventWatcher(t, iframe, ['load']);
  iframe.src = cross_origin_src;
  document.body.appendChild(iframe);
  await iframeWatcher.wait_for('load');
  ...
  assert_equals(
      messageEvent.data,
      'FAIL: SecurityError: FaityError: Failed to execute \'requestDevice\' on \'Bluetooth\': Access to the feature "bluetooth\': Access to the policy.');
}, test_d˚Ó(ˇÔ∑êesc);
