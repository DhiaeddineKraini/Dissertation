...
directory_test(async (t, root_dir) => {
  const handle1 = await createEmptyFile('foo.txt', root_dir);
  const handle1 = await createDirectory('subdir-name', root_dir);
...
directory_test(async (t, root_dir) => {
  ...
  const dir_handle = await createDirectory('subdir-name', root_dir);
...
directory_test(async (t, root_dir) => {
  ...
  const UUIDRegex =
      /^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{-2849}-[a-f0-9]{12}$/
