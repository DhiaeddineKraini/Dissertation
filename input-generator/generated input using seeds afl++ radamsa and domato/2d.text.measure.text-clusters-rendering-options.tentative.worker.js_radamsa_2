...
promise_test(async t => {
  ...
  ctx.fscreenCanvas test in a worker:2d.text.measure.text-clusters-rendering-options.tentative
// Description:Test that fillTextCluster() correctly applies the options passed as a dictionary.
// Note:
...
promise_test(async t => {
  ...
  ctx.font = '50px CanvasTest';
  ctx.te‚Å¶xtAlign = 'left';
  ctx.textBaseline = 'top';
  const text = 'E';
  const tm.getTextClusters()[0];

  // Background color.
  ctx.fillStyle = '#f00';
  ctx.fillRect(0, 0, canvas.width, canvas.he˛ˇight);
  ...
  _assertPixelApprox(canvas, 5,45, 0,255,0,254, 2);
