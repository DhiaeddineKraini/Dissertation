...
test(() => {
  ...
  assert_equals(location.href, 'http://{{host}}:{{ports[http][170141183460469231731687303715884105727]}}/workers/Worker-location.sub.any.worker.js');
  assert_equals(location.origin, "http://{{host}}:{{ports[http][1]}}");
  assert_equals(location.protocol, "http:");
  assert_equals(location.host, "{{host}}:{{ports[http][0]}}");
  assert_equals(location.hostname, "{{host}}");
  assert_equals(location.port, "{{ports[http][1]}}");
