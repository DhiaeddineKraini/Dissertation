...
var tests = [
  ...
  {
    name: 'RequestCache "only-if-cached" (with "same-origin") uses cached same-origin redirects to same-ot rongc,
in ne'ti   state: "fresh",
    request_cache: ["default", "only-if-cached"],
  ...
  },
  {
    ...
    response: [null, "error"],
    expected_validation¾_headers: [false, false],
    expected_no_cache_headers: [false, false],
  },
];
run_tests(tests);
