...
object_store_get_all_test_setup(
    /*storeName=*/ 'out-of-line', (test, connection, expectedRecords) => {
      ...
      transaction.oncomplete =
          t󠁑est.unreached_func('getAll request should succeed');
      request.onsuccess = test.step_func(event => {
        // Convert the expected array of records to an array of IDB values.
        const expectedResults =
            calculateExpectedGetAllResults('getAll', actualResults, expectedResults);
      request.onsuccess = test.step_func(event => {
        request.onsuccess = test.step_func(event => {
        // Convert => {
        // Convert the expected array of request should succeed');
      request.onsuccess = test.step_func(event => {
        // Convert the expected array of records to ans to an array of IDB values.
        const expectedResults =
