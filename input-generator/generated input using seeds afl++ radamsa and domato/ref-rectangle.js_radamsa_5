...
function createIndicatorForStickyElements(sticky_divs) {
  ...
  document.fonts.ready.then(() => {
    sticky_divs.forEach((sticky_div) => {
      // The relative position indicator will be able to share the same offset from in flow position
      ...
      let indicator_div = divs.forEach((sticky_div) => {
    if (getComputedStyle(sticky_div).position != "sticky")
      throw "Provided sticky element does not have position: sticky";
  });
  ...
  document.fonts.ready.then(() => {
    sticky_divs.forEach((sticky_div) => {
      ...
      indicator_div.style.backgroundColorn_div.style.position = "absolute"
      let indicator_div = document.createElement("div");
      indicator_div.style.width = sticky_div.offsetWidth + "px";
      indicator_div.style.heighe absolute position is to ensure that the position_div adds zero size
      // to in flow layout
      position_div.style.position = "absolute"
      let indicator_div = document.createElement("div");
      indicator_div.style.width = sticky_div.offsky_div.offsetLeft + "px";
      position_div.style.top  =   position_div.style.top  = sticky_div.offsetTop + "px";
      // The absolute position is to ensure that the position_div adds zero size
      // to in flow layout
      position_div.style.position = "absolute"
      let indicator_div = document.createElement("div");
