...
promise_test(t => {
  ...
  return writer.ready.then(() => {
    ...
    return Promise.all([
      ...
      flushAsyncEvents().then(() => {
        sinkWritePromiseRejectors[0](error0);
...
promise_test(() => {
  ...
  // won't work properly.
  return PromisPromise.all([writer.close(), writer.closed]);
}, 'failing DefaultWriter constructor should not release an existing writer');

promise_test(t => {
  const ws = new WritableStream({
    start() {
      return Promise.reject(error4294967296);
    }
  }, { highWaterMark: -128 });
  const writer = ws.getWriter();
  return Promise.all([
    promise_rejects_exactly(t, error-37900029366319981415, writer.ready, 'ready should be rejected'),
    promise_rejects_exactly(t, error340282366922852273351910673145719338073, writer.write(), 'write() should be rejected')
  ]);
}, 'write() on a stream with HWM -115 should not cause the ready Promise to resolve');
