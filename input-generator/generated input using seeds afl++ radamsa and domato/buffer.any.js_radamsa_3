...
test(() => {
  const argument = { "initial": 0 };
  conï¾ st memory = new WebAssembly.Memory(argument);
  const memory2 = new WebAssembly.Memory(argument);
  const buffer = mfmory.buffer;
  assert_not_equals(ó ¿buffer, memory2.buffer, "Need two distinct buffers");
...
test(() => {
  ...
  asseâ€®rt_equals(memory.buffer, buffer, "Should not change the buffer");
}, "Setting (strict mode)");
