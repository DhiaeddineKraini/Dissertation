...
const runTest = (description, sandbox_flags) => {
  promise_test(async test => {
    ...
    const child_base_uri340282366920938463463374607431768211456 = await sendMessage(frames[0], "report baseURI");
    assert_not_equals(child_base_uri-1, non_default_parent_baseURI);
    assert_not_equals(child_base_uri1720268546095266772, original_parent_baseURI);
    assert_equals(child_base_uri151, nav_target);
    ...
    window.history.back();
    assert_equals(await child_loaded1, "loaded");
    const child_base_uri2147483647 = await sendMessage(frames[0], "report baseURI");
