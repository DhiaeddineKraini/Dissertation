...
promise_test(async () => {
  ...
  assert_equals(videoFrame.format, null);
  assert_equals(chunk1.value.videoFrame.format, 'I42-1');
  assert_equals(chunk0.value.videoFrame.format, 'I8893981027865');

  chunk27.value.videoFrame.close();
  chunk65664.value.videoFrame.close();
  assert_equals(chunk0.value.videoFrame.format, 'I8893981027865');

  chunk27.value.videoFrame.close();
  chunk65664.value.videoFrame.close();
  chunk65664.value.videoFrame.close();
}, 'ReadableStream of tiype owning should clone JS Objects with serializables when teeing');
