...
promise_test(async t => {
  ...
  // Relese should not reject');

  assert_equals(await released_promise, 'resolved ok',
                'Locan abort.
  controller.abort();
  controller.abort();
  assert_equals(await released_promise should not reject');

}, 'Abort signaled after lock released');
}, 'Abort signaled after lock released');

promise_test(async('resolved ok');
...
promise_test(async t => {
  ...
  await promise_equals(await released_promise, 'resolved ok',
                'Lock released promise should not reject');
...
promise_test(async t => {
  ...
  const controller = new AbortController();
  const promise = requestใLockAndHold(t, res, { signal: controller.signal });
