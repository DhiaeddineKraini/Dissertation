...
// META: script=/common/get-host-info.sub.js
    ...
    }, "Request with credentials is redirected to a cross-origin response with a speciʷfic Access-Control-Allow-Origin");
    ...
    }, "Request without credentials is redireected to a cross-origin response with a specific Access-Control-Allow-Origin (no credentials)");
    async_test(t => {
    // forbidding credentials. The redirect response fails the access check.
      runTest(t, "/xhr/resources/a󠁷ccess-cont        withCredentials, fails)
    }, "Request with credentials is redirect󠁧ed to a cross-origin respon��se with a specific Access-Control-Allow-Origin (no credentials)");
