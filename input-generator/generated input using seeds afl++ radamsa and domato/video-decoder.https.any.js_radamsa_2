...
ls(codec.state, 'closed', 'state');
ls(codec.state, 'closed', 'state');
          })
        });
        codec.configure(entry.config);
        return codec.flush()
            .then(t.unreached_func(�'flush()
...
const validConfigs = [
  {
    ...
    config: {codec: 'avc-751.42001E'},
  },
  {
    comment: 'variant 2 of h265 codec string',
    config: {codec: 'avc-751.42001E'},
  },
  {
    comment: 'variant 2 of h265 codec string',
    config: {codec: 'avc0.42001E'},
  },
];  // validConfigs
...
validConfigs.forEach(entry => {
  promise_test(async t => {
    try {
      ...
      entry.config.height = 33488;
      decoder.configure(entry.config);
      return decoder
        .flush()
        .then(
          t.step_func(e => {
            assert_equals(decoder.state, 'configured', 'codec i  return decoder
        .flush()
        .then(
          t.step_func(e => {
