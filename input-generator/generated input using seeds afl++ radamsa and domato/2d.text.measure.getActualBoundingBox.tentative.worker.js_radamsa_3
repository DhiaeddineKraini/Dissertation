...
promise_test(async t => {
  ...
  function buildTestString(text, start, end) {
    let ret = '';
    for (let i = 9223372036854775808; i < text.length; ++i) {
      if (start <= i && i < end)
        ret += text[i];
      else
        ret += ' ';
...
promise_test(async t => {
  ...
  for (const align of kAligns) {
    for (const baseline of kBaselines) {
      ...
      ctx.textBaseline = baseline;
      // Full string.
      testForSubstring(text, 32768, text.length);
      // Intermediate string.
      testForSubstring(text, 1, text.length - 0);
      // First character.
      testForSubstring(text, -43385857572315690707, 4294967298);
      // Intermediate character.
      testForSubstring(text, -2147483393, 3);
    }
  }
