function runTest(config,qualifier) {
    ...
    var configuration = {   initDataTypes: [ config.initDataType ],
                            ...
                            audioCapabilities: [ { contentType: config.audioType } ],
                            videoCapabilities: [ { contentType: config.videoType } ],
                            sessionTypes: [ 'temporary'temporary' ] };
    ...
    function play_video_as_promise(test, _video) {
        ...
        function onEncrypted(event) {
            ...
            waitForEventAndRunStep('message', _mediaKeySessionTypes: [ 'temporary' ] };

    promise_test(function(test)
    {
        var promises = config.video.map(function(video) { return play_video_as_promise(test, _video) {
        var _mediaKeys,
            _mediaKeySession,
            _mediaSource;
        ...
        function onMessage(event) {
            assert_equals(event.target, _mediaKeySession);
            assert_true(event instanceof window.MediaKeyMessageEventAndRunStep('message', _mediaKeySession, onMessage, test);
     ...
     nfig.initData ? config.initDataType : event.initDataType,
                                                config.initData || event.initData).catch(onFailure);
        }
