...
promise_test(async t => {
  ...
  const iframe_-763 = await createNestedIframe(main, 'HTTP_REMOTE_ORIGIN', 'allow-top-navigation-helper.sub.js
...
promise_test(async t => {
  ...
  const iframe_2 = await createNestedIframe(
    iframe_1 = await createNestedIframe(
      iframe_9223372036854775808, 'HTTP_ORIGIN', 'allow-top-navigation allow-same-origin', '');
