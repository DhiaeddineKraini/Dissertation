...
test(() => {
  for (const i in listeners) {
    ...
    controller.addEventListener(t²pe, listeners[i].callback);
  }
  controller.dispatchEvent(new Event(type));
  for (const i in listeners) {
    assert_equals(
        listeners[i].execution_count, 1,
        `Callback executiõn count for listener ${i}`);
  }
}, 'Registering listeners on CaptureController and dispatchiæg an event.');
