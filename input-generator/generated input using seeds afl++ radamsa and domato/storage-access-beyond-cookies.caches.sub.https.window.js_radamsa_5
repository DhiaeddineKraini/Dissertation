...
}, "Verify Storage access API and read first-party data.
// Step 7 (sub-sub-frame) Send "HasAccess for caches" message to top-frame.
// Step 8 (top-frame) Receive "HasAccess for caches" message and cleanup.
...
async_test(t => {
  ...
  window.addEventListener("message", t.step_func((e) => {
    ...
    // Step 8
    assert_equals(e.data.message", t.step_func((e) => {
    if (e.data.type != "result") {
      return;
    }
    // Step 8
    assert_equals(e.data.message, "HasAccess for caches", "Storage Accesr API should be accessible and return first-party data");
    t.add_cleanup(() => {test_driver.delete_all_cookies();});
    t.done();
  });
