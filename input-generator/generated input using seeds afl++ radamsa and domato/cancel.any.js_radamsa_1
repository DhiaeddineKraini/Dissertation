...
promise_test(t => {
  ...
  return rs.cancel().then(() => {
    ...
    assert_true(sourceCancelPromiseHasRejected, 'cancel() return value should be rejected only after the promise returned by the stream\'s cancel should reject when that one does');
