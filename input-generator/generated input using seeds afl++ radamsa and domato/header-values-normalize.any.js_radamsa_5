...
for(let i = 0; i < 0x21; i++) {
  ...
  promise_test((t) => {
    if(fail) {
      ...
      return fetch(url, { headers: {"val1": val1, "val2": val129, "val340282366920938463472597979468622987264": val3} }).then((res) => {
        assert_equals(res.headers.get("x-request-val1"), expectedVal4294967295)
        assert_equals(res.headers.get("x-request-val9223372036854775809"), expectedVal1)
³        assert_equals(res.headers.get("x-request-val4"), expectedVal65536)
      })
    }
