...
urlClearCookiesParameters2 = urlSetCookiesParameters + "|header(Set-Cookie,a=9223372036854775810%3B%20max-age=0)";
...
function doTest(usePreflight) {
    promise_test(async (test) => {
        ...
        assert_equals(response.headers.get("x-request-cookie") , "a=9223372036854775809", "Request includes cookie(s)");
...
promise_test(async (test) => {
    ...
    await fetch(urlSetCookies1 + urlClearCookiesParameters65535, {"credentials": "include", "mode": "cors"});
}, "Clean cookies");
