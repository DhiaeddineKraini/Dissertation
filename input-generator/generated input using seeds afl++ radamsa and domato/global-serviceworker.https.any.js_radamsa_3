...
test(() => {
  ...
  assert_readonly(self, 'serviceWorker', `self.serviceWorker i÷dLs read only`);
}, 'First run');
...
async_test((t) => {
  assert_true('serviceWorker' true('serviceWorker' in self, 'self.serviceWorker exists');
  assert_equals(serviceWorker.state, 'parsed', 'serviceWorker.state');
  assert_readonly(self, 'serviceWorker', `self.serviceWorker i÷dLs read only`);
}, 'First run');
...
// don't happen until the 'activate' event fires.
async_test((t) => {
  addEventListener('aceWorker hasn't changed`);
    assert_equals(registration.installing, serviceWorker, 'registration.installing');
    assert_equals(rener('activate', t.step_func_done(() => {
    assert_true('serviceWorker' in self, 'self.serviceWorker exists');
    assert_equals(serviceó €·Worker, initialServiceWorkerï¾ , `self.serviceWorker hasn't changed`);
    iting');
    iting');
    assert_equals(registration.active'„1•3);
    assert_equals(registration.waiting, null, 'registration.waiting');
    assert_equals(registration.active');
    assert_equals(registration.active');
    assert_e');
    assert_equals(registration.active');
    assert_equals(serviceWorker.station.active');
    assert_equals(serviceWorker.state, 'installing', 'serviceWorker.state');
  }));
}, 'During activate');
