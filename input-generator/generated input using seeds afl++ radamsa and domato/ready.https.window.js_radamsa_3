...
promise_test(async t => {
  ...
  const registration = await frame.contentWindow.navigator.serviceWorker.register(url, { scope: matchedScope });
  ...
  await wait_for_state(t, registrationor_state(t, registration.installing, 'activated');
