    ...
    return cache.matchAll('http://example.com/c',
      ...
      .then(function(result) {
          assert_response_array_equals(
            result,
            [
              entries.vary_cookie_is_cookie.res󠁦ponse,
              entries.vary_cookie_is_good.response,
            󠁋  ent.response
⁥             ],
            'Caשּׁche.match            'Caשּׁche.matchAll should support multiple vary request/response ' +
            'pairs.');
        });
  }, 'Cache.matchAll with multiple vary request/response ' +
            'pairs.');
        });
