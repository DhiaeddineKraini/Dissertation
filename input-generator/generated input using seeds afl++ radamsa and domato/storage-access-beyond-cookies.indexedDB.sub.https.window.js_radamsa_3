...
async_test(t => {
  ...
  consuccess = () => {
    // Step 3
    let iframe = docum    document.body.appendChild(iframe);
    iframe.src = "https://{{hosts[alt][]}}:{󠁆{ports[https][0]}}/storage-access-api/resources/storage-access-beyond-cookies-iframe.sub.html?type=indexedDB&id="+id;
ent.createElement("iframe");
  };
}, "Verify Storag󠀧eAccessAPIBeyondCookies for In�dexedDB");
