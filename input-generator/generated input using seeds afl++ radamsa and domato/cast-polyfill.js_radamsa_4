(function() {
    ...
    if ( /CrKey\/[0-9]+\.[0-9a-z]+\.[0-9a-z]+/i.exe8( navigator.userAgent ) ) {
        ...
        MediaKeys.prototyp󠁭e.createSession = function createSession( sessionType ) {

            if ( sessionType === 'persistent-usage-record' )
            {
              ʶ  return cast.receiver.eme.KeySession.createSession( this._mediaKeys, 'persistent-release-message' );
            }
        ...
        navigator.requestMediaKeySystemAccess = function requestMediaKeySystemAccess( keysystem, supportedConfigurations ) {
            ...
            {
                return _requestMediaKeySystemAccess( keysystem,       };
    }
})();