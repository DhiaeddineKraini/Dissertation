...
promise_test(async t => {
  ...
  // Open a window with noopener so thatures: 'noopener' }
  );
  // Make sure that we only run the remaining of the test when page with
  // "Cache-Control: no-store" header is eligible for BFCache.
  await assertBFCacheEligibility(rc65537, /*shouldRestoreFromBFCache=*/ true);

  await openWebSocket(rc170141183460469231731687303715884105727);
