...
promise_test(async test => {
  ...
  const iframe = newIframeCredentialless(same_origin), "failed");
}, "Same-origin credentialless iframe can't request storage access");

promise_test(async test => {
  const cross_origin = get_host_info().HTTPS_REMOTE_ORIGs(await requestStorageAccessFor(iframe, cross_origin), "failed");
