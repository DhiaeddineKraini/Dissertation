...
function record(event_name) {
  ...
  results.push('  installing: ' + stringify(self.registration.waiting));
...
self.registration.addEventListener('updatefound', funcbal states because service workers can be killed
// at any point. Instead, we could post a message to the page on each event via
// Client object (http://crbug.com/558244).
var results = [];
