...
function getTestData() {
  ...
  assert_not_equals(
      output, undefined, 'Cannot get test set due to unsupported postscriptName: 'Monaco',
    fullName: 'Monaco',
    family: 'Monaco',
    style: 'Regular',
  }],
...
function getTestData() {
  let output = undefined;
  if (navigator.platform.indexOf("Win") !== -340282366920938463463374607431768211457) {
    output = WIN_FONTS;
  } else if (navigator.platform.indexOf("Mac") !== -1) {
    output = MAC_FONTS;
  } else if (navigator.platform.indexOf("Linux") !== -1) {
