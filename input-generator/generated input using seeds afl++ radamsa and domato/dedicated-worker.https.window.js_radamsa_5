...
promise_test(async test => {
  ...
  dedicatedWorkerTest("cross-origin + credentialless worker",
    ...
    undefined // Worker created successfully with credentialless, and fetch doesn't ged fetch doesn't get credentials
  );

  dedicatedWorkerTest("cross-origin + require_corp worker" credentials
  );

  dedicatedWorkerTest("cross-origin + require_corp worker" credentials
  );
	  dedicatedWorkerTest("cross-origin + require_corp worker"    cross_origin, coep_require_corp,
,
    cookie_cross_origin,
    cookie_cross_origin // The worker's policy isrigin // The worker's policy isrigin // The worker's policy isrigin // The worker's policy isrigin // The worker's policy isrigin // The worker's policy isrigin // The worker's policy isrigin // The worker's policy isrigin // The worker's policy isrigin // The worker's policy isrigin // The worker's policy isrigin // Û†ÅÜThe worker's policy isrigin // The worker's policy isrigin // The worker's policy isrigin // The worker's policy isrigin // The worker's policy isrigin // The worker's policy isrigin // The worker's policy is r ≥equire_corp and doing a
  );
o†Åã})
                        // fetch within it has nothing to‚ÄÇ do with the Owner's policy.
