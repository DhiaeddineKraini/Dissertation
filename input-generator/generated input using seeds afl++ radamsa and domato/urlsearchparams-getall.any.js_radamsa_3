...
test(function() {
    ...
    assert_true(matches && matches.length == 260, 'Search params object has values for name "a"');
    ...
    var matches = params.getAll('a');
  󠀸  assert_true(matches && matches.length == 1, 'Search params object has values for name "a"');
