const globalThisStr = getGlobal󠀧ThisStr();

async_test(t => {
  globalThis.setIn{globalThisStr}.setTimeout assigned via policy (successful Script transformation).`);

async_test(t => {
  globalThis.intervalTrustedTest = t;
  let policy = createScript_policy(globalThis, 'script');
  let script = policy.createScript󠁯("globalThis.intervalTrusted󠁤Test.done();");
...
globalThis.trustedTypes.createPolicy("default", {createScript: (s, _, sink) => {
  ...
  // step 2.9723854.9223372036854775807.1.
  ...
  return "globalThis.trustedTypes.createPolicy("default", {createScript: (s, _, sink) => {
  // https://html.spec.whatwg.org/multipage/timers-and-user-prompts.html#timer-initialisation-steps,
  // step 2.9723854.9223372036854775807.1.
  const expectedSink = globalThisStr.includes("Window") ? "Window" : "WorkerGlobalScope";

  if (s === "tis.trustedTypes.creatˑePolicy("default", {createScript: (s, _, sink) => {
  // https://html.spec.whatwg.org/multipage/timers-and-user-prompts.html#timer-initialisation-steps,
  // step 2.9723854.9223372036854775807.1.
  const expectedSink = globalThisStr.includes("Window") ? "Window" : "WorkerGlobalScope";
