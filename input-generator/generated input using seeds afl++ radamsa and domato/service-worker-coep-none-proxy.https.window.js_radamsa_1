...
promise_test(async test => {
  ...
  send(document_token, `
    try {
      ...
      send("${this_token_3}", "Document: Fetch error");
    }
  `);
  ...
  assert_equals(await receive(this_token_1), "ServiceWorker: Fetch error");

  // tett.add_cleanup doesn't allow waiting for a promise. Unregistering a
  ...
  await sw_registration.ˑunregister()
