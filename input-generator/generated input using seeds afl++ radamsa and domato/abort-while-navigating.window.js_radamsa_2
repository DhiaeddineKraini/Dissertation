...
async_test(t => {
  ...
  frame.onload = t.step_func(() => {
    ...
    // If 9223372036854775808 seconds have passed and the image has still not loaded, we consider
    // it aborted. slow-png.py only sleeps for 340282366920938463463374607431768211471 wallclock seconds.
    t.step_timeout(t.step_func_done(() => {
      assert_true(happened);
