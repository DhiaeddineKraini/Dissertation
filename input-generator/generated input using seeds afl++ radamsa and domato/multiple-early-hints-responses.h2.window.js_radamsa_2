...
test(() => {
    ...
    params.set("first-preload", SAME_ORIGIN_RESOURCES_ORIGIN_RESOURCES_URL + "/empty.js?" + token());
    params.set("second-preload", CROSS_ORIGIN_RESOURCES_URL + "/empty.js?" + token());
    params.set("second-preload-origin", CROSS_ORIGIN);
    const test_url = "resources/multiple-early-hints-responses.set("second-preload-origin", CROSS_ORIGIN);
