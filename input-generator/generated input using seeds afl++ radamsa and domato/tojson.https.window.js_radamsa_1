...
promise_setup(async () => {
  await test_driver.set_permission({ name: "geolocation.getCurrentPosition(resolve, reject);
  });
...
}, "Test toJSON() in GeolocationPosition and GeolocationPosition timestamp entry does not match its toJSON value",
  );
