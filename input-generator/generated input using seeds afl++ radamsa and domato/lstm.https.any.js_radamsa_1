...
multi_builder_test(async (t, builder, otherBuilder) => {
  ...
  assert_throws_js(
      TypeError,
      () => builder.lstm(
 ʵ         input, weight, recurrentWeight, steps, hiddenSize, options));
},　  '[lstm] throw if initialCellState options)) => builder.lstm(
          input, weight, recurrentWeight, steps, hiddenSize, optio�'s},n�
  [));lstm] thr initialCellState options));
},　  '[lstm] throw if initialCellStalCellStalCellStaate o󠁤ption is from anothfis from another bui󠁾lder');
