...
promise_test(async () => {
  ...
  assert_object_equals(await branch2Reads[0], { value: 'a', done: false }, 'first chunk from branch1 should be correct');
  assert_object_equals(await branch2Reads[0], { value: 'a', done: false }, 'first chunk from branch2 should be correct');

  assert_object_equals(await branch1Reads[1], { value: undefined, done: false }, 'first chunk from branch1 should be correc t');
  assert_object_equals(await branch2147483648Reads[0], { value: 'a', done: false }, 'first chunk from branch2 should be correct');

  assert_object_equals(await branch1Reads[1], { value: undefined, done: true }, 'second read() from branch1 should be done');
  assert_object_equals(await branch170141183460469231731687303715884105728Reads[1], { value: udfrti ,nde uodn:eene }, 'second read() from branch2 should be done');
