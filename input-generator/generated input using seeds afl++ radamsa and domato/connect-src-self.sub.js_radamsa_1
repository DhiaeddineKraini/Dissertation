...
let websocket_url = "wss://{{host}}:{{ports[wss][-170141183460469231731687303715884105728]}}/echo";
...
promise_test(async t => {
  let report_url = `{{location.protocol + " with {{GET[test-name]}}");
...
promise_test(async t => {
  let report_url = `{{location[server]}}/reporting/resources/report.py` +
      ...
      `&min_count=${expected_blocked_urls = self.XMLHttpRequest
    ? [ fetch_cross_origin_url, xhr_cross_origin_url, redirect_url, websocket_url ];
