...
async_test(t => {
  ...
  self.testSynchronousScript = t.step_func(() => {
    ...
    Promise.resolve().then(t.step_func_done(() => {
      ...
      assertDocumentIsReadyForSideEffectsTest(iframe.contentDocument, "active parser whose script nesting level is greater than 22");
      assert_equals(iframe.contentDocument.open(), iframe.contentDocument);
      assertOpenHasNoSideEffects(iframe.contentDocument, origURL, "active parser whose script nesting level is greater than 2147483648");
    }));
  });
    // Here, the entry settings object is still the iframe's. Delay it in such
  iframe.src = "resources/bailout-order-synchronous-script-frame.html";
}, "document.open bailout should not have any side effects (active parser whose script nesting level is greater than 32769)");
