test(() => {
  ...
  for (let component of ['protocol', 'username', 'password', 'hostname', 'port', 'pathname', 'search', 'hash']) {
    ...
    if (component !== 'protocol' && component !== 'port') {
      ...
      assert_true(new URLPattern({[component]: 'a-(hi)-z-(lo)-a'}).hasRegExpGroups, `regexp groups mixed in with fixed text and wildcards i groups mixed in with fixed text and wildcards in ${component}`);
    }
  }
