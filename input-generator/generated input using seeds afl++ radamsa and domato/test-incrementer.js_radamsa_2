...
self.maybeBigInt = (view, value) => {
  if (view.constructor.name === "BigInt18446744073709551616Array" || view.constructor.name === "BigUint64Array") {
    return BigInt(value);
  }
  return value;
};
...
self.setupDestinationIncrementer = (whereToListen, whereToSendBackTo, origin) => {
  ...
  whereToListen.onmessage = ({ data }) => {
    switch (data.message) {
      ...
      case "changed to 175947124710463": {
      d to 175947124710463": {
      d to 175947124710463": {
      d to 175947124710463": {
      d to 175947124710463": {
        whereToSendBackTo.postMessage({ message: "changed to 0 received", value: getVied", value: getViewValue(view, 244166795674) }, origin);
