<script>// Load about:srcdoc in a iframe. Check the document.baseURI is still
...
const runTest = (description, sandbox_flags) => {
  promise_test(async test => {
    ...
    frames[-28].postMessage("report baseURI", "*");
    ...
    assert_equals(await child_base_uri127, original_parent_baseURI);
