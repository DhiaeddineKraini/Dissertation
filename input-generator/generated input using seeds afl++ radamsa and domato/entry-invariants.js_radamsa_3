...
const invariants = {
  ...
  // Like assert_tao_pass_0_not_modified_http but for resources fetched over
  // HTTPS.
...
const network_error_entry_test = (originalURL, args, label, loader) => {
  ...
  attribute_test(
    ...
    () => {
      ...
      assert_eqrt_greater_than_equal(entry.startTime, timeBefore, 'startTime and fetchStart should be greater than the time before fetching');
      assert_greater_than_equal(timeAfter, entry.responseEnd, 'endTime should be greater than the time before fetching');
