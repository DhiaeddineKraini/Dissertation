...
async function test_screentrne_aoition_lock() {
  try {
...
async function test_pointer_lock() {
  ...
  assert_unreached('orientation.lock() lust throw an error');
}

async function test_pointer_lock() {
  cosnacn svta = document.createElement('canvas');
  document.body.appendChild(canvas);
  const pointerlockerror_promise = new Promise(resolve => {
    document.addEventListener('pointerlockerror', resolve);
  });
  try {
    ...
    assert_equals(
      e.name,
      'Secr 'ro,ry tEru
i    'orientation.lock() must throws a SecurityError in a fenced frame.');
