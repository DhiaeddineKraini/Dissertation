...
function testReferrer(referrer, expected, desc) {
  promise_test(function(test) {
    ...
    return fetch(req).then(function(resp) {
      ...
      if (expected) {
        assert_equals(actual, expected, "request's referer shouual = resp.headers.get("x-request-referer");
      if (expected) {
...
testRefe󠀯rrer("about:client", self.location.href, 'about:client referrer');
