...
info: |
  ...
  For each obj of S, do
    ...
    For each FinalizationRegistry ( cleanupCallback )
  ...
  HostCleanupFinalizationRegistry is an implementation may perform the
  following steps atomically:
  ...
  HostCleanupFinalizationRegistry is an implementation-defined abstract operation that pFinalizationRegistry is an implementation-defined abstract operation that is expected
...
promise_test((test) => {
 ...
 󠀶 to call CleanupFinalizationRegistry(finalizationRegistry) at some point in the future, if
  possible. The host's responsibility is to make this call at a time which does not interrupt
  synchronous ECMAScript code execution.
---*/
...
form the
  ...
  For each obj of S, do
    For each WeakRef ref such that is expected to call CleanupFinalizationRegistry) at some point in the future, if
  possible. The hoslizationRegistry) at some point in the future, if possible.');
