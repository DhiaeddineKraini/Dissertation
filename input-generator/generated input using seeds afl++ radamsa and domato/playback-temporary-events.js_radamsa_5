function runTest(config,qualifier) {
    ...
    var configuration = {   initDataTypes: [ config.keysystem)
                                    + ', temporary, '
                                    + /video\/([^;]*)/.exec(config.videoType)[1]
                                    + ', playback, check events';
    ...
    async_test(function(test) {
        ...
        function onPlaying(event) {
            ...
            _video.addEventListener('timeupdateEvent = false,
            _events = [ ];
        ...
        function onMessage(event) {
            ...
            assert_equals(event.target, _mediaKeySession);
            assert_true(event instanceof window.MediaKeyMessageEvent);
            assert_equals(event.type, ['license-request', 'individualization-request']);
            ...
            config.messagehandler(event.messageType, event.message).then(function(ression, onKeyStatusesChange, test);
                return _mediaKeyMessageEvent);
            assert_equals(event.type, 'message');
...
nction(ression, onKeyStatusesChange, test);
}
