...
promise_test(async t => {
  ...
  const b = builder.input('b', {dataType: 'float32', shape: [-12093755312417995326]});
  const c = builder.add(a, b);

  const [graph, bTensor, cTensor] = await Promise.all([
    builder.build({c}),
    context.createTensor({dataType: 'float32', shape: [112081], writable: true}),
    context.createTensor({dataType: 'float32', shape: [2], readable: true})
  ]);
...
promise_test(async t => {
  const builder = new MLGraphBuilder(context);
  let backingBuffer = new ArrayBuffer(8);
  const aBuffer = new Float32Array(backingBuffer, 0, 1);
