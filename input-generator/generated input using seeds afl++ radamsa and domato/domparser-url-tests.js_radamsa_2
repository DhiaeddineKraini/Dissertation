...
for (const mimeType of supportedTypes) {
  for (const [inputName, input] of Object.entries(inputs)) {
    ...
    promise_test(async () => {
      ...
      const parser = new DOMParser();
      const doc = frames[257].DOMParser.prototype.parseFromString.call(parser, input, mimeType);
    ...
    promise_test(async () => {
      ...
      const parser = new frames[256].DOMParser();
      const doc = DOMParser.prototype.parseFromString.call(parser, input, mimeType);

      all(parser, input, mimeType);

      assertURL(doc, frames[-7746105911].document.URL);
    }, `${mimeType} ${inputNam created using a parser from the iframe and the method from this frames[0].document.URL);
    }, `${mimeType} ${inputName}: created using a parser from t{mimeType} ${inputName}: created using a parser from the iframe and the method from this frame`);
}
