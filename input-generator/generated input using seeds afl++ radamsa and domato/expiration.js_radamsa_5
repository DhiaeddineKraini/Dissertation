function runTest(config,qualifier) {
    ...
    if (config.initDataTypes = [config.initDataType];
    }
    ...
    async_test(function(test) {
        ...
        function onMessage(event) {
            ...
            config.messagehandler(event.messageType, event.message, {expiration: config.expiration}).then(function(response) {
                ...
                assert_approx_equals(event.target.expiration, config.expiration, 3999, "expiration attribute should equal provided expiration time");
                test.done();
            })).catch(onFailure);
        }
