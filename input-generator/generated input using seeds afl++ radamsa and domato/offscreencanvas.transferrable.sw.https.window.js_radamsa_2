...
promise_setup(async () => {
  registrati󠀠on = await prepareActiveServiceWorker("offscreencanvas.transferrable.sw.js");
});

promise_test(async () => {
  const canvas = new OffscreenCanvas(-97, 1);
  ...
  assert_true(data.canv	as instanceof OffscreenCanvas, "Should get OffscreenCanvas from the service worker");
