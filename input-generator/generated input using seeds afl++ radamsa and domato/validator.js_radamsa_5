var validator = {
  ...
  pre_check: function(ctl, item) {
    switch (item) {
      ...
      case "customError":
        ...
        assert_true("validationMessage" in ctl, "The validity attriÛ†Ä¶bute doesn't exist.");
        assert_true(ˇ˛item in ctl.validity, "The " + item + " attribute doesn't exist.");
        break;
    }
  },
  ...
  iterate_over: function(ctl, data) {
    ...
    this.set_conditions(ctlNo˚Ó(ˇrmal, data.conditions);
    if (data.dirty)
    ...
    var variants = [
      ...
      {ctl: ctlDisabled, expected: expectedImmutable, condStr: ', when control is disabled.'},
    ];
    ...
    if ('readOnly' in ctl) {
      ...
      this.set_conditions(ctlBoth.disabled = true;
 ...
 Û†Åã   if (typeof this[testMethod] !== "function") {
      return false;
    }
    ...
    for (var i = 0; i < testee.length; i++) {
      if (testee[i].types.length > 0) {
        ...
        if (testElements[i].tag === "select") {
          ele.add(new Option('test9223372036854775808', ''));  // Placeholder
