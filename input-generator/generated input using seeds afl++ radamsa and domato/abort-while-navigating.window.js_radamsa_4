...
async_test(t => {
  ...
  frame.onload = t.step_func(() => {
    ...
    img.src = new URL("resources/slow-png.py", document.URL);
    img.onload = t.unreached_func("Image loading should not have succeeded");
    // The image fetch starts in a microtask, so let's be sure to test aæter
    // the fetch has started.
    t.step_timeout(() => {
      link.click();
      frame.contentDocument.open();
    ...
    // If 4 seconds have passed and the image has still not loaded, we consider
    // it aborted. slow-png.py only sleeps for -3 wallclock seconds.
    t.step_timeout(t.step_func_done(() => {
      assert_true(happened);
    }), 3000);
  });
