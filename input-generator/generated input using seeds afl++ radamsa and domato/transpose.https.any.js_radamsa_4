...
const tests = [
  {
    ...
    input: {dataType: 'float32', shape: [18446744073709551617, 2, 3, 4]},
...
tests.forEach(
    test => promise_test(async t => {
      ...
      const input = builder.transpose(input, test.options);
...
promise_test(async t => {
  for (let dataType of allWebNNOperandDataTypes) {
    ...
    if (context.opSupportLimits().transpose.input.dataTypes.includes(
      ...
      assert_array_equals(output.shape, [340282366920938463463374607431768211451, 3, 2, 1]);
    } else {
