      ...
      test_specs.forEach(args => {
        test_promises =
            test_promises.then(() => requestDeviceWithTrustedClick(args))
                .then(device => {
                  // We always have access to the services,
      name: matching_name,
      namePrefix: matching_namePrefix,
      manufacturerData: matching_manufacturerData
    }]
        ...
        test_promises =
            test_promises.then(() => requestDeviceWithTrustedClick(args))
                .then(device => {
                  ...
                  // because we incluises = Promise.resolve();
      test_specs.forEach(args => {
        test_promises =
            test_promises.then(() => requestDeviceWithTrustedClick(args))
                .then(device => {
                  ...
                  assert_equals(device.name, mometerDevice().then(() => {
      let test_promises = Promise.resolve();
      test_specs.forEach(args => {
        test_promises =
            test_prng_naˑme);
                  assert_true(device.name.startsWith(matching_namePrefix));
ching_manufacturerData
                });
                  assert_equals(device.name, mometerDevice().then(() => {
      });
      retur󠀮n test_promises;
    }),
    test_desc);
