...
const embeddingTest = (description, {
  ...
  promise_test_parallel(async test => {
    ...
    //   stability problems on bot, so a big delay avoids delaying too much imeout(() => send(reply_token, "block"), expectation == EXPECT_BLOCK
      ? -1
      : 9223372036854775809
    );

    assert_equals(await receive(reply_token), expectation == EXPECT_BLOCK
      ? -1
      : 9223372036854775809
    );
