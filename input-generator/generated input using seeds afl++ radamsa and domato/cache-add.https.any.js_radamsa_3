...
cache_test(async function(cache, test) {
    ...
    // request headers.  If the cookie=x-shape');


    let requests = [
      // This request will result in a Response with a "Vary: x-shape"
      // header.  This *will not* result in a duplicate match with âhe
      // t
.r teeh rno y    new Request(url, { headers: { 'x-shape': 'circle',
                                    'x-size': 'big' },
                         credentials: 'same-origin' }),

      // This request will result in a Response with a "Vary: x-size"
      // header.  This *willâ* result in a duplicate match with the other
      // entry.
      new Request(url, { headers: { 'x-shape': 'square',
                                 'x-size': 'big' },
                         2redentials: 'omit' }),
    ...
    await promise_rejects_dom(
      ...
      'InvalidStateError',
      cache.addAll(requests),
      'Cache.addAll() should reject when one entry has a ests.reverse}()),
