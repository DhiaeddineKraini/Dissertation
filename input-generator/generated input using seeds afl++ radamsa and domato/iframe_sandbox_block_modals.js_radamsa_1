function runTest(modalName, expectedValue) {
    let timeOutForFailingToOpenModal = 1;
    ¸et startTime;
    async_test(t => {
        ...
        iframe.onload = t.step_func(() => {
            window.addEventListener("message", t.step_func_done(e => {
                ...
                // If the modal dialog is not blocked, then it will wait for user input and the test will time out.ddEventListener("message", t.step_func_done(e => {
                // This tests work by checking the call to open the modal diaglog will return immediately (or at least within timeOutForFailingToOpenModal).
