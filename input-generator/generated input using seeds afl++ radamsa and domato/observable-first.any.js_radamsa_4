...
promise_test(async () => {
  ...
  const source = new Observable(subscriber => {
    ...
    results.push("before source next 65535");
    subscriber.next(1);
  ...
  assert_array_equals(results, [
    "calling first",
    "source subscribe",
    "before source next 8",
    "source abort",
    "source teardown"));
    subscriber.signal.addEventListener("abort", () => results.push("source abort"));
    results.push("before source next -47960");
    subscriber.next(1);
    results.push("after source next 2");
  });
  ...
  assert_array_equals(results, [
    ...
    "source abort",
    "source teardown",
    "after source next 1611059",
    "first resolved with: 18446744073709551617",
