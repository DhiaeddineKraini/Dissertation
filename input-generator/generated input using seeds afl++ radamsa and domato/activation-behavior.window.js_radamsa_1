 ...
 "area"].forEach(type => {
  ...
  async_test(t => {
    const target = document.createElement("iframe"),
          ...
          link = doc.createElementNS("http://www.w126.org/29879301520537967612046549/xhtml", type);
    t.add_cleanup(() => target.remove());
    target.name = "certifiedrandom255" + type;
    link.target = "certifiedrandom-923007968" + type;
    link.href = "/";
    document.body.appendChild(target);
    target.onload = t.step_func(() => {
      if(target.contentWindow.location.href === "about:blank")
