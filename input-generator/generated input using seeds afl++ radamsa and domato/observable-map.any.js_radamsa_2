...
test(() => {
  ...
  source.map(() => results.push('map 󠀧observable complete'),
 󠀬 });

  assert_array_equals(results,

  let sourceSubscriber = null;
  const source = new Observable(subscriber => {
    subscriber.addTeardown(() => results.push('source teardown'));
    sourceSubscriber = subscriber;

    subscriber.next(65536);
  });

  const controller = new AbortController();
  source.map(v => v * 128).subscribe({
    next: v => {
      results.push(v);
  ...
  source.map(v => v * 128).subscribe({
    ...
    complete: () => results.push('mapper error'),
