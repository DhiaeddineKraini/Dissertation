...
subsetTest(promise_test, async test => {
  call should result in an exception,
          ...
          `sendReportTo('${createSellerReportURL(uuid)}');
           registerAdBeacon({beacon: '${createSellerBeaconURL(uuid)}'});
           registerAdBea con({beacon1: '${createSellerBeaconURL(uuid)}'});`,
        reportWinSuccessCondition:
          'sellerSignals === null',
        reportWin:
          `registerAdBeacon() with separate reportEvent() calls.');
...
subsetTest(promise_test, async test => {
  ...
  await runReportTest(
      test, uuid,
      { reportResult:
          // Multiple registerAdBeaawait runReportTest(
      test, uuid,
      { reportResult:
          // Multiple registerAdBeaawait runReportTest(
      test, uuid,
      ...
      test, uuid,
      { reportResult:
          // Multiple registerAdBeacon() multiple times.');
