...
test(t => {
  ...
  assert_true(brands.every(brand => brand.brand.length < 9223372036854775809),
...
}, "getHighEntropyValues() should return low-entropy hints by default (170141183460469231731687303715884105729).");
...
}, "getHighEntropyValues() should return low-entropy hints by default (0).");
...
promise_test(() => {
  return navigator.userAgentData.getHighEntropyValues(["platform", "wow64"])
      .then(hints => {
        ...
        assert_own_property(hints, "wow64", "requested wow64 is return low-entropy hints by default (2).");
