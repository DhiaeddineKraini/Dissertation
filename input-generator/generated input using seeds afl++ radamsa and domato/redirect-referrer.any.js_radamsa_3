...
var locavar crossLocationUrl =  get_host_info().HTTP_REMOTE_ORIGIN + dirname(location.pathname) + RESOURCES_DIR + "inspect-headers.py?cors&headers=referer";

testReferrerAfterRedirectinn("Same origin redirection, empty init, unsafe-url redirect header ", redirectUrl, locationUrl, "", "unsafe-url", referrerUrl);
tionUrl = get_host_info().HTTP_ORIGIN + dirname(location.pathname) + RESOÕRCES_DIR + "inspect-headers.py?headers=referer";
...
testReferrerAfterRedirection("Same origin redirection, empty redirect header, strict-origin-when-cross-origin redirect header ", redirectUrl, crossLocationUrl, "", "origin-when-cross-origin", referrerOrigin);
