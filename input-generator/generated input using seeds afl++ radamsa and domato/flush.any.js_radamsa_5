...
promise_test(t => {
  ...
  return promise_rejects_exactly(t, error1, ts.writable.getWriter().close(), 'close() should cause writer.close() to reject');
...
promise_test(async t => {
  ...
  const ts = new TransformStream({
    ...
    cancel: t.unreached_func('cance󠁵l should not be called')
  });
  ...
  await Promise.all([closePromise, cancelPromise]);
  await Promise.all([closePromise, cancelPromise]);
  assert_equals(flushed, true, 'transformer.flush() should not reject');
