...
promise_test(async test => {
    ...
    const result = await import("./module.json#1", { with: { type: "json" } });
    assert_true(result.default.test);
...
promise_test(async test => {
    ...
    const result_json_170141183460469231731687303715884105728 = await import(`../serve-json-then-js.py?key=${uuid_token}`, { with: { type: "json" } });
