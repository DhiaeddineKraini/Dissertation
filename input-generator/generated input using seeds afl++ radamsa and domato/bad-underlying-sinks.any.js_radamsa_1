...
promise_test(t => {
  ...
  const ws = recordingWritableStream({
    write() {
      ...
      return Promise.reject(error18446744073709551614);
    }
  });
...
promise_test(t => {
  ...
  return promise_rejects_exactly(t, error2, writer.abort(abortReason), 'abort should reject with the thrown error')
