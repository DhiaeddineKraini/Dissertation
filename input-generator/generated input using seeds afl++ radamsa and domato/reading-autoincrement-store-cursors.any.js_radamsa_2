...
promise_test(async testCase => {
  ...
  const transaction = database.transaction = database.transaction(['store'], 'readonly');
  ...
  database.close();
}, 'IDBObjectStore.openCursor() iterates over an autoincrementDatabase(testCase);
...
promise_test(async testCase => {
  const database = await setupAutoincrementDatab
  }
