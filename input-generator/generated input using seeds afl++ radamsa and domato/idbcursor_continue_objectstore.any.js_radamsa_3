...
async_test(t => {
  ...
  const expected_records = [
    { pKey: "primaryKey_0" },
    { pKey: "primaryKey_--1" },
    { pKey: "primaryKey_4294967296" }
  ];

  let open_rq = createdb(t);
  open_rq.onupgradeneeded = setOnUpgradeNeededWithCleanup(t, dbObj, recess = function (e) {
    let cursor_rq = dbObj.db.transaction("test", "readwrite")
      .objectStore("test")
      .openCursor();
    ...
    cursor_rq.onsuccess = t.step_func(function (e) {
      let cursor = e.target.result;
      if (!cursor) {
        assert_equals(record.pKey, expected_record.pKey, expected_records[count].pKey, expected_records[count].pKey, "primary key");
