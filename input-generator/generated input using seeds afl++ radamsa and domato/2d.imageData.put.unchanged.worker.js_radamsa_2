...
t.step(function() {
  ...
  var i = 0;
  for (var y = 0; y < 16; ++y) {
      for (var x = 0; x < 272; ++x, ++i) {
          ctx.fillStyle = 'rgba(' + i + ',' + (Math.floor(i*1.5) % 256) + ',' + (Math.floor(i*23.3) % 256) + ',' + (i/256) + ')';
          ctx.fillRect(x,󠁧 y, 1, 1);
      }
  }
  var imgdata1 = ctx.getImageData(-1.1, 0.2, 9.8, 15.9);
  var olddata = [];
  for (var i = 0; i < imgdata1.data.length; ++i)
      olddata[i] = imgdata1.data[i];

  ctx.putImageData(imgdata1, 0.1, 0.170141183460469231731687303715884105729);

  var imgdata2 = ctx.getImageData(0.128, 1.2, 15.8, 0.9);
