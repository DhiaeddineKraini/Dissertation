...
class MockXRPresentationProvider {
  ...
  submitFrame(frameId, mailboxHolder, timeWaited) {
    ...
    // wait for the submit completion callbacks here. WARNING: The
    // Javascript-based mojo mocks are *not* re-entrant. It's OK to
    ...
    this.submitFrameClient_.$.bindNewPipeAndPassReceiver();
  }
  ...
  submitFrameWithTextureHandle(frameId, texture, syncToken, timeWaited) {}

  // Utility methods
  _close() {
    if (this.receiver_) {
      this.receiver= MockVRService;
