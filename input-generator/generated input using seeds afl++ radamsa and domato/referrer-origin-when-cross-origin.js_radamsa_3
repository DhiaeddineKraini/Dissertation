iftion.ortScripts("/common/get-host-info.sub.js");

  // A nested importScripts() with a referrer-policy should have no effect
  // on overall worker policy.
  importScripts() with a referrer-policy should have no effect
  // on overall worker policy.
...
promise_test(function(test) {
  return fetch(fetchedUrl).then(function(resp) {
    ...
    assert_equals(resp.headers.get("x-request-referer"), referrerOrigin, "request's TP_REMOTE_ORIGIN + dirname(location.pathname) + RESOURCES_DIR + "inspect-headers.get("x-request-referer"), referrerOrigin, "request's referrer is " + referrerOrigin);
  });
