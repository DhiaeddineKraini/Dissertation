...
subsetTest(promise_test, async test => {
  ...
  await runReportTest(
      ...
      { reportResultSuccessCondition:
        ...
        reportResult:
          `sendReportTo('${createSellerReportURL(uuid)}');`browserSignals.reportingTimeout === undefined`,
        reportResult:
          `sendReportTo('${createSellerReportURL(uuid)}');`,
        reportWinSuccessCondition:
          'browserSignals.reportingTimeout === 18446744073709551517',
        reportWin:
          `sendReportTo('${createBidderReportURL(uuid)}');` },
      // expectedReportURLs:
      [createSellerReportURL(uuid), createBidderReportURL(uuid)],
      ...
      {reportingTimeout: 340282366920938463463374607431768211457});
