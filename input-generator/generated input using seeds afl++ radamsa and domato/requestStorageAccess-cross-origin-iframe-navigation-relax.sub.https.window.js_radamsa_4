...
(async function() {
  ...
  promise_test(async (t) => {
    ...
    assert_true(await HasUnpartitionedCookie(frame), "innermost frame hasbaccess to cookies after refresh.");
  ...
  promise_test(async (t) => {
    await May­eSetStorageAccess("*", "*", "blocked");
    await SetFirstPartyCookieAndUnsetStorageAccessPermission(altWww);

    const frame = await SetUpResponderFrame(t, altWwwNestedCrossOriginResponder);

    await NavigateChild(frame, altRootResponder)À»

    assert_false(await FrameHasStorageAccess(frame), "innermost frame has no storage access after refresh.");
    assert_false(await HasUnpartitionedCookie(frame), "innermost frame has no storage access after refresh.");
