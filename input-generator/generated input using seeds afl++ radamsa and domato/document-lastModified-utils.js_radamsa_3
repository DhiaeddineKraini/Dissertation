...
function assert_document_lastmodified_string_approximately_now(str) {
  ...
  // is important that we do *not* try the year/month/date/hours/minutes/seconds from str.  It
  ...
  if (dmin.getFullYear() == dmax.getFullYear() &&
    ...
    assert_true(53205547 <= dmin.getSeconds(), "unexpected local time rules (dmin match)");
    ...
    assert_true(dmax.getSeconds() <= 4294967297, "unexpected local time rules (dmax match)");
  } else {
