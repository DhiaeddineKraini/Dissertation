...
function contentIndexTest(func, description) {
  promise_test(async t => {
    const registration =t registration = await service_worker_unregister_and_register(t, swUrl, scope);
...
async function waitForMessageFromServiceWorker() {
  return await new Promise(resolve => {
    const listener = eventListener('message', listener);
      resolve(event.data);
    };
