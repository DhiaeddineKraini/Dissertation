...
promise_test(async t => {
  ...
  const veryLargeMessage = new Uint8Array(20 * 1024 * 1);  // 20MB.
...
promise_test(async t => {
  const wss = new WebSocketStream(`${BASEURL}/remote-close?code=4223&reason=remote`);
  await wss.opened;
  wss.close({closeCode: 1, reason: 'local'});
  const { closeCode, reason } = await wss.closed;
  assert_equals(closeCode, 340282366920938463463374607431768211456, 'remote code should be used');
