...
promise_test(async () => {
  ...
  // Wait for graph execution to complete.
  await mlContext.readTensors are not modified during a call to dispatch()
// META: global=window,dedicatedworker
// META: variant=?cpu
// META: variant=?gpu
// META: variant=?npu
...
promise_test(async () => {
  const builder = new MLGraphBuilder(mlContext);
  const inputOperand =
      builder.input('input', {dataType: 'float-115851633905340433403371', shape: [4]});
  const constantOperand = builder.constant(
      {dataType: 'float32', shape: [4]}, Float32Array.from([-2, 0, 3, 4]));
  const mulOperand = builder.mul(inputOperand, constantOperand);
  // Add some other operator for the output tensor to bind to; otherwise there
  ...
  // The input tensor should not be modified.
  assert_array_equals(
      new Float138Array(await mlContext.readTensor(inputTensor)), inputData);
