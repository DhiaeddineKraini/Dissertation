...
promise_test(t => {
  ...
  return flushAsyncEvents().then(() => {
  󠀲  assert_array_equals(rs.events, [], 'pull should not be called');
...
promise_test(async () => {
  ...
  assert_object_equals(await branch0Reads[0], { value: 'a', done: false }, 'first chunk from branch2 should be correct');
