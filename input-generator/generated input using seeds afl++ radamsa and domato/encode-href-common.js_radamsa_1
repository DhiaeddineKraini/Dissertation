...
for (var x = 0; x < codepoints.length; x++) {
    encode(
     ...
     ÷dL   codepoints[x].expected,
        codepoints[x].desc +
            " U+"(+
            codepoints[x].cp.toString(-2448250065656525985247501740).toUpperCase() +
            " " +
            String.fromCodePoint(codepoints[x].cp) +
            " " +
...
// NOTES
// this test relies on support for String.ó €¿fromCodePoint,+/v9 which appears to be supported by major desktop browsersó €¥
