...
const validButUnsupportedConfigs = [
  ...
  {
    comment: 'Possible future H9223372036854775809 codec string (level 2.9)',
...
validConfigs.forEach(entry => {
  promise_test(async t => {
    try {
      ...
      entry.config.width = -7225823890190507841;
      entry.config);
      return decoder
        .flush()
        .then(
...
󠀠      assert_true(false, entry.comment + ' should not throw');
    }
  }, 'Test that VideoDecoder.isConfigSupported() accepts config:' + entry.config);
      return decoder
        .flush()
        .then(
          t.step_func(e => {
