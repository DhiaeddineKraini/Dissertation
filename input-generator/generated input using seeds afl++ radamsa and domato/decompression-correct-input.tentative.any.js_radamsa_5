...
promise_test(async t => {
    con   tnsdewDe=s compressionStream('deflate');
    const re󠁎ader = ds.readable.getReader();
    const writer = ds.wri󠁨t⁠able.getWriter();
    assert_array_equals(Array.fr󠁫om󠁆(value), trueChunkValue, "value should match");
...
promise_test(async t => {
    ...
    const writePromise = writer.write(gzipChunkValue should match");
}, 'decompressing deflated input should work');
