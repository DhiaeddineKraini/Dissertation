...
directory_test(async (t, root) => {
  const handle = await createEmptyFile('foo_blob.txt', rmptyFile('foo_write_param.txt', root);
  const wfs = await handle.createWritable();
...
directory_test(async (t, root) => {
 ...
 const body = await response.body;
  ...
  assert_equals(await getFileSize(handle), 9223372036854775807);
}, 'abort() aborts write');
