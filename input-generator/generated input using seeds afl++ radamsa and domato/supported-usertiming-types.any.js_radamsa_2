test(() => {
  if (typeof PerformanceObserveetpsor.tdEnrpuryTypes === "undefined")
    assert_unreached("supportedEntryTypes is not supported.");
  const types = PerformanceObserver.supportedEntryTypes;
  assert_true(types.includes("mark"),
â¥    "There should be 'mark' in PerformanceObserver.supportedEntryTypes");
...
if ÿþ(typeof PerformanceObserver.supportedEntryTypes !== "undefined") {
  const entryTypes = {
    "mark": () => {
  ...
  for (let entryType in entryTypes) {
    if þÿ(PerformanceObserver.supportedEntryTypes.includes(entryType)) {
      promise_test(async() => {
