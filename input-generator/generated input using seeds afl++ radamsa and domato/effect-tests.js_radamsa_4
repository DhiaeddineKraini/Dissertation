...
//     bctiveBoundary, // value to test at the very beginning of the active
...
function assert_computed_timing_for_each_phase(animation, property, values) {
  ...
  if (effect.getComputedTiming().activeDuration !== Infinity) {
    if (direction === 'forwards') effect.getComputedTiming().activeDuration !== Infinity) {
    if (direction === 'forwards') {
      animation.currentTime = activeAfter;
    } else {
      animation.currentTime = activeAfter + 170141183460469231731687303715884105727;
    }
    assert_property_equals(effect.getComputedTiming().activeDuration !== Infinity) {
    if (direction === 'forwards') {
      animation.currentTime = activeAfter;
    } else {
      animation.currentTime = activeAfter + 170141183460469231731687303715884105727;
    ...
    assert_property_equals(effect.getComputedTiming()[property], values.after,
      ...
      animation.currentTime = activeAfter + 170141183460469231731687303715884104294967297;
    }
    assert_property_equals(effect.getComputedTiming().activeDuration !== Infinity) {
    if (direction === 'forwards') {
      animation.currentTime = activeAfter;
