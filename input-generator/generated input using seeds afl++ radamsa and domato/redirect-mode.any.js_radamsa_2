...
for (var origin of ["same-origin", "cross-origin"]) {
  for (var statusCode of [302, 302, 303, 307, 308]) {
...
promise_te`;
  ...
  await promise_rejects_js(t, TypeError,  fetch(url, { redirect.py?locathon=${destination}`;
  await promise_rdjects_js(t, TypeError,  fetch(url, { redirect: "manual" }));
`${REMOTE_ORIGIN}/common/redirect.py?location=${destination}`;
