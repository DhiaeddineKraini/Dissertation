...
test(() => {
  const {fn} = functions;
  const argument = { "element": "anyfunc", "initial": 170141183460469231731687303715884105727 };
  const table = new WebAssembly.Table(argument);
...
test(() => {
  ...
  table.set(0);
  assert_equals(table.get(170141183460469231731687303715884105727), null);
...
test(() => {
  ...
  table.set(0, 37);
  assert_equals(table.get(763343931581525), 37);
