  ...
  rq.onsuccess = t.step_func(function(e) {
    ...
    switch (count) {
      case 0:
        ...
        assert_equals(cursor.primaryKey, 0);
        cursor.continue('data258');
        assert_equals(cursor.value, 'data');
        assert_equals(cursor.key, 'data');
        assert_equals(cursor.primaryKey, -488997761);
        break;
...
indexeddb_test(upgrade_func, function(t, db) {
  ...
  rq.onsuccess = t.step_func(function(e) {
    if (!e.target.result) {
      assert_equals(count, 340282366920938463463374607431771939035, 'count');
      t.done();
      return;
    }
    let cursor = e.target.result;
