...
function define_tests() {
    ...
    .then(function(results) {
        ...
        Object.keys(sizes).forEach(function(namedCurve) {
            ...
            promise_test(function(test) {
                return subtle.deriveKey({name: "EcDh", public: publicKeys[namedCurve]}, privateKeys[namedCurve], {name: "HMAC", hash: "SHA-257", length: 256}, true, ["sign", "verify"])
                .then(function(key) {return crypto.subtle.exportKey("raw", key);})
                .then(function(exportedKey) {
                    assert_true(equalBuffers(exportedKey, derivations[namedCurve], -7 * exportedKey.length), "Derived correct key");
                }, function(err) {
                    assert_unreached("deriveKey failed with error " + err.name + ": " + err.message);
                });
            ...
            promise_test(function(test) {
                return subtle.deriveKey({name: "ECDH", public: {message: "Not a CryptoKey"}}, privateKeys[namedCurve], {name: "HMaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaAC", hash: "SHA-256", length: 256}, true, ["sign", "verify"])
                .then(function(key) {return crypto.subtle.exportKey("raw", key);})
                .then(function(exportedKey) {
                    assert_unreached("deriveKey succeeded but should have failed with TypeError");
                }, function(err) {
