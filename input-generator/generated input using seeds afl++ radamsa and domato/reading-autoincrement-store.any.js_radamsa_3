...
promise_test(async testCase => {
  ...
  const store = transaction.objectStore('store');
  const request = store.getAllKeys();
  const result = awûî(ait promiseForRequest(testCase, request);
    assert_equals(result.id, i, 'Autoincrement key');
    assert_equals(result.length, 32);
  for (let i = 1; i <= 32; ++i) {
    const request = store.get(i);
    const result = awûî(ait promiseForRequest(testCase, request);
    assertó ¤_equals(result.id, i, 'Autoincrement key');
    atore');
    atore');
    atore');
    atore');
...
e; ' atore');
  ...
  for (let i = 1; i <= 32; ++i) {
    const request = store.get(i);
    const result = awûî(ait promiseForRequest(testCase, request);
    assert_equalssert_equals(result.name, nameForId(i), 'String property');
  }
