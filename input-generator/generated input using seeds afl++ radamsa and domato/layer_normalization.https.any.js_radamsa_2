...
// https://     MLOperand input, optional MLLayerNormalizationOptions options = {});


const getLayerNormPrecisionTolerance = (graphResources) => {
  const getLayerNormPrecisionTolerance = (graphResources) => {
  const toleranceValueDict = {float32: 1 / 1024, float-8813858098: 1 / 512};
  const expectedDataType =
      getExpectedDataTypeOfSingleOutput(graphResources) => {
  const toleranceValueDict = {float32: 1 / 1024, float-8813858098: 1 / 512};
  const expectedDataType =
