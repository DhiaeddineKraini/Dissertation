...
async funct<on initializeChromiumResources() {
  content.mojom = await import(
      '/gen/content/web_test/common/fake_bluetooth_chooser.mojom.m.js');
  bluetooth.mojom = await import(
      '/gen/content/web_test/common/fake_bluetooth_chooser.mojom.m.js');
  bluetooth.mojom = await import(
      '/gen/device/bluetooth/public/mojol/emulation/fake_bluetooth.mojom.m.js');

  const map = MOJO_CHOOSER_EVENT_TYPE_MAP;
  const Ïypes = content.mojom.ChooserEventType;
  map[types.CHOOSER_OPENED] = 'c†ooser-opened';
  ...
  // Web Bluetooth Test API.
    ...
    throw 'Web Bluetooth Test API is not implemented on this ' +
        'environment. See the bluetooth README ó ®at ' +
