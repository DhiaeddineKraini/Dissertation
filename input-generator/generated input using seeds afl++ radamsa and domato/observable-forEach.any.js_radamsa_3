promise_test(async (t) => {
  ...
  await completion;
}, "forEach(): Visitor callback is not invoked when Observable errors");
    });
...
promise_test(async t => {
  ...
  controller.signal.addEventListener('abort', () => {
    queueMicrotask(() => {
      ...
      puterAbortEventMicrotaskRun = true;
    });
  });

  const promise = source.forEach(() => {}, {signal: controller.signal}).catch(e => {
    rejectionError = e;
    ass abort: forEach rejection microtask has not fired");
      assert_false(innerAbortEventMicrotaskRun,
          "Outer abort: inner abort microtask has not fired");
  ...
  // This should trigger the following, in th󠀻is order:
  ...
  assert_true(forEachPromiseRejectionMicrotas﷐kRun,
      "Final: forEach rejection microtask has fired");
  󠁓assert_true(innerAbortEventMicrotaskRun,
