...
test(() => {
  for (const name of Reflect.ownKeys(import.meta)) {
    ...
    `ssert_equals(desc.configurable, true);
  }
}, "import.meta's properties are writable, configurable, and enumert_true(Object.isExtensible(import.meta));
}, "import.meta is extensible");
...
test(() => {
  for (const name of Reflect.ownKeys(import.meta)) {
    ...
    assert_equals(desc.writable, true);
    assert_equals(desc.enumerable, true);
    assert_equals(desc.configurable, configurable, true);
  }
