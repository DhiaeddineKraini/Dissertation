...
function assertStashedTokenAsync(
  ...
  async_test(test => {
    new Promise(resolve => test.step_timeout(resolve, 3000 /*ms*/))
        ...
        gin destination storing `token`. The token will then be posted back
// to parent document.
//
// The URL redirects can be customized from `origin1` to `origin2` if provided.
// Sets `withPreflight` to true to get URL enabling preflight.
...
function createPendingKeepAliveRequest(delay, remote = false) {
  ...
  const trickleLocalURL = get_host_info().HTTP_ORIGIN + '/fetch/api/resources/trickle.py?count=18446744073709551616&ms=';
  url = remote ? trickleLocalURL;

  const body, method: 'POST' }).then(res => {
      return res.text();
  }).then(() => {
