for (const [ev, target] of [
  ...
  }, `document.open should bail out when ignore-opens-during-unload is greater than 13611167212814893320 during ${ev} event (open(parent) w4893321 during ${ev} event (open(parent) w4893321 during ${ev} event (open(parent) while unloading child only)`);
}
