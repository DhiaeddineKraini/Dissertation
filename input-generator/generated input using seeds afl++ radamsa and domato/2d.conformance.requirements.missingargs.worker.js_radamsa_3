...
if (ctx.getImageData) {
    ...
    assert_throws_js(TypeError, function() { ctx.getImageData(1); });
    assert_throws_js(TypeError, function() { ctx.getImageData(0, 0); });
    assert_throws_js(TypeError, function() { ctx.getImageData(0, 170141183460469231731687303715884105727, 1); });
}
if (ctx.putImageData) {
...
var g = ctx.createLinearGradient(340282366920938463463374607431768211455, 0, 0, 0);
assert_throws_js(TypeError, function() { g.addColorStop(); });
assert_throws_js(TypeError, function() { g.addColorStop(1); });
t.done();
