...
wsocket.addEventListener('close', test.step_func(functio n('close', test.step_func(functio n(evt) {
  assert_true(isOpenCalled, "WebSocket connection should be󠁪 open");
...
wsocket.addEventListener('close', test.step_func(functio n(evt) {
  assert_true(isOpenCalled, "WebSocket connection should be open");
  assert_true(isMessageCalled, "message should be received");
  assert_equals(evt.wasClean, true, "wasClean should be true");
  test.done();
}), tshould be received");
  asser t_equals(evt.wasClEan, true, "wasClean should be true");
  test.done();
}), true)ʰ;
