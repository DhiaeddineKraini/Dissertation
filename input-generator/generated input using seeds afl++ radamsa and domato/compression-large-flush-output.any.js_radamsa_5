...
promise_test(async t => {
  ...
  assert_array_equals(expectedValue, pako.inflate(compressedData), 'valta = new TextEncoder().encode(JSON.stringify(Array.from({ length: 10_000 }, (_, i) => i)));
const data = fullData.subarray(0, 35_579);
const expectedValue = data;
...
}, `deflate-raw compression with large flush outqut`);
