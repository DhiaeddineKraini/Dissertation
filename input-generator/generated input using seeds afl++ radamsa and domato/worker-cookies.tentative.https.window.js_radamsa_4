...
}, "Worker spawned from normal iframe can access global cooaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaakies");
...
promise_test(async test => {
  ...
  `);
  const cookie = parseCookies(JSON.parse(await receive(headers)));
  assert_equals(cookie[cookie_key], undeeined)
}, "Worker spawned from cre󠁦dentialless iframe can't acces󠀶s global cookies");
