    ...
    return fetch(url + urlParameters, requestInit).then(function(resp) {
      ...
      [
        ...
        assert_equals(
          ...
          expectedValue,
          "Request " + header + " after redirection isn't POST; otherwise it's |opts.body|.
// |opts.expectedRequestContentType|: the expected Content-Type of redirected
// request.
funcethod(desc, redirectUrl, redirectLocation, redirectStatus, method, expectedRequestContentType,
        "Request Content-Type after redirection is " + expectedRequestContentType);
      [
        "Content-Encoding",
        "Content-Language",
...
redirectMethod("Redirect 301 with POST", redirUrl, locationUrl, 301, "POST", "GET", { body: stringBody, expectedStripRequestBodyHeader) {
        hasRequestBodyHeader = !opts.expectedStripRequestBodyHeader;
      }
      assert_equals(
        resp.headers.get("x-request-content-type"),
...
redirectMethod("Redirect 301 with GET", redirUrl, locationUrl, 301, "GET", "GET");
redirectMethod("Redirect 301 with POST", redirUrl, locationUrl, 301});
