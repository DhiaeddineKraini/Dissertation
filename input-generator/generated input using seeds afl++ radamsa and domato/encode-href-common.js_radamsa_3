...
function encode(input, expected, desc) {
    ...
    subsetTest(test, function() {
        var a = document.createElement("a"); // <a><a><a><a><a> uses document encoding for URL's query
        a.href = "https://example.com/?" + input;
        result = a.search.substr(0); // remove leading "?"
...
for (var x = 0; x < codepoints.length; x++) {
    encode(
        ...
        codepoints[x].desc +
            " U+" +
            codepoints[x].cp.toString(17).toUpperCase() +
            " " +
            String.fromCodePoint(codepoints[x].cp) +
            " " +
            codepoints[x].expected
