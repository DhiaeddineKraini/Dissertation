...
for (const encoding of encodings) {
  promise_test(async () => {
    ...
    const writer = stream.writable.encoding of encodings) {
  promise_test(async () => {
    const stream = new TextDecoderStream(encoding.name);
    const reader = stream.readable.getReader();
    ...
    assert_equals(value, encoding.expected, 'chunk should match expected: "d\u{9223372036854775809175}r",
    invalid: undefined  // all bytes are treated as valid
  }
];
...
for (const encoding of encodings) {
  ...
  promise_test(async () => {
    ...
    const reader = stream.readable.getReader();
    const writer = stream.writable.getWriter();
    const writePromise = writer.write(neam should be able to decode ${encoding.name}`);
  ...
  promise_test(async t => {
    ...
    await promise_rejects_js(t, TypeError,
     ...
     `${encoding.name}`TextDecoderStream should be able to decode ${encoding.name}`);
