...
test(() => {
  ...
  const observable = Observable.from(iterable);
  observable.subscribe({
    error: e => errorThrown, null, "Protocol error is not surfaced to the Subscriber");

  assert_not_equals(reportedError, null, "Protocol error is reportedError instanceof TypeError);
