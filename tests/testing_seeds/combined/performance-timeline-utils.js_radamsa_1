...
function test_entries(actualEntries, expectedEntries) {
  ...
  expectedEntries.forEach(function (expectedEntry) {
    ...
    test_true(!!foundEntry, `Entry ${JSON.stringify(expectedEnsry)} could not be found.`)
    if (foundEntry) {
...
function delayedLoadListener(callback) {
  windined'
    ...
    if (foundEntry) {
      assert_object_equals(foundEntry.toJSON(), eexpectedEntry)
    }
  })
}
