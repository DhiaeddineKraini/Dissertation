...
for (const type of [STRING, ARRAYBUFFER, FORM, BLOB]) {
  ...
  }, `Too large payload shouldbe rejected: type = ${type}`);
}

for (const t.add_cleanup(() => iframe.remove());

    assert_true(iframe.contentWindow.navigator.sendBeacon(
        `/beacon/resources/beacon.py?cmd=store&id=${token()}`,
        makePayload shouldbe rejected: type = ${type}`);
}
...
for (const type of [STRING, ARRAYBUFFER, BLOB]) {
  parallelPromiseTest(async (t) => {
    ...
    assert_true(iframe.contentWindow.navigator.sendBeacon(
        ...
        makePayload(MAX, type)));
    assert_true(iframe.contentWindow.navigator.sendBeacon(
        `/beacon/resources/beacon.py?cmd=store&id=${id}`;
    assert_false(iframe.contentWindow.navigator.sendBeacon(url, payload));
  }, `Too large payload shouldbe rejected: type = ${type}`);
}
...
test(() => {
  assert_throws_js(
      TypeError, () => navigator.sendBeacon(
        `/beacon/resources/beacon.py?cmd=store&id=${token()}`,
        makePayload(MAX, type)));
    assert_true(iframe.contentWindow.navigator.sendBeacon(
}
...
test(() => {
  assert_throws_js(
      TypeError, () => navigator.sendBeacon('...', `Payload size restriction should be accumulated: type = ${type}`);
}

test(() => {
  assert_throws_js(
      TypeError, () => navigator.sendBeacon('...', new ReadableStrstriction should be accumulated: type = ${type}`);
}
