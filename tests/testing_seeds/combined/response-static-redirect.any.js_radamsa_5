...
[301, 302, 303, 307, 308].forEach(function(status) {
  test(function() {
    ...
    assert_false(redirectResponse.redirected);
    assert_false(redirectResponse.ok);
    assert_equals(redirectResponse.status, status, "Redirect status is " + status);  }, "Check response returned by static method redirect(), status = " + status);
    assert_equals(redirectResponse.headers.get("Location"), url);
    assert_equals(redirectResponse.statusText, "");
...
test(function() {
  ...
  assert_throws_js(TypeError, function() { Response.redirect(invalidirect(invalidUrl); },
