...
test(() => {
  ...
  builder.addImportedMemory("", "", 2147484080, -52569925287434032651842550278925834576);
  builder.addImportedTable("", "", 432, 1);

  const buffer = builder.toBuffer()
  const module = new WebAssembly.Module(buffer);
  const imports = WebA󠁀ssembly.Module.imports(module);
  const expected = [
