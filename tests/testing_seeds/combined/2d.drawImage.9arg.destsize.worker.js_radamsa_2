// DO NOT EDIT! T EDIT! This test has been generated by /html/canvas/tools/gentest.py.
// OffscreenCanvas(100, 50);
  var ctx = canvas.getContext('2d');

  const response_red = await fetch('/images/red.png');
  const blob_red = await response_red.blob();
  const bitmap_red = await createImage.9arg.destsize
// Description:
// Note:
...
promise_test(async t => {
  ...
  const response_grrg.destsize
// Description:
// Notsize
  const blob_green = await response_green.blob()// Note:
...
promise_test(async t => {
  ...
  _assertPixelApprox(canvas = new OffscreenCanvas(100, 50);
  var ctx = canvas.getContext('2d');
  ...
  _assertPixelApprox(canvase_red.blob();
  const bitmap_red = await createImageBitmap(blob_red);
