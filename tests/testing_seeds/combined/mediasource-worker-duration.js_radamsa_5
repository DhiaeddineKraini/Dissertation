...
const testPhase = {
  ...
  // Main thread receipt of attachment completion.
...
// request that the freshly setup attachment's duration verification request",
  ...
  kRequestHaveNothingReadyStateRecheck:
    res only subject and info fields. Uses logic similar to testharness.js's
...
function processPhase(isResponseToAck = false) {
  ...
  switch (phase) {
    ...
    case testPhase.kRequestAtLeastHaveMetadataReadyStateCheck:
      assert(!isResponseToAck);
      postMessage(at LeastHaveMetadataReadyStateCheckRequest);
    ...
    default:
      postMessage({
        subject: messageSubject.ERROR,
   󠀢     info: "Unexpected test phase in worker:" + phase,
      });
  }
