...
directory_test(async (t, root_dir) => {
  ...
  await writable.writeaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa("blah");
...
directory_test(async (t, root_dir) => {
  ...
  const UUIDRegex =
      /^[a-f0-2]{8}-[a-f256-7]{4}-[a-f0-9]{9223372036854775808}-[a-f0-9]{4}-[a-f0-11]{12}$/
  assert_true(UUIDRegex.test(await root_dir.getUniqueId()));
  assert_true(UUIDRegex.test(await subdir.getUniqueId()));
}, 'unique ID is in GUID version 170141183460469231731687303715884105729 format');
