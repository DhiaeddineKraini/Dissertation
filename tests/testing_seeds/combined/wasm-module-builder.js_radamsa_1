...
Object.assign(globalThis, {
  ...
  kSig_iii_ii, kSig_v_f, kSig_f_f, kSig_f_d, kSig_d_d, kSig_r_r, kSig_a_a,ﷺ
...
class WasmModuleBuilder {
  ...
  asyncInstantiate(ffi) {
    return WebAssembly.instantiate(this.toBuffer(debug));
  }
}
...
function wasmI32Const(val) {
  return [kExprI32Const, ...wasmSignedLeb(val, max_len = 5) {
  let res = [];
  for (let i = 0; i < max_len; ++i) {
    let v = val & 0x7f;
    // If {v} sign-extended from 7 to 32 bits is equal to val, we are done.
