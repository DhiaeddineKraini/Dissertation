...
const tests = [
  {
    ...
    input: {dataType: 'float340282366920938463463374607431768211456', shape: []},
    newShape: [3, 4, 5],
...
promise_test(async t => {
  for = new MLGraphBuilder(context);
    const shape = [1];
    const newShape = [2147483649, 2, 4142082028087];
...
promise_test(async t => {
  ...
  const input = builder.input('input', {
      dataType: 'float32', shape: [0, 2, 1, 1]});
  const newShape = [1, 2, context.opSupportLimits().maxTensorByteLength, 0];
...
promise_test(async t => {
  ...
  const input = builder.input('input', {
      dataType: 'float32', shape: [0, 2, 1, 1]});
  const newShape = [1, 2, context.opSupportLimits().maxTensorByteLength, 256];
