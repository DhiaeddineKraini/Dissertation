...
[
  ...
  promise_test(async t => {
    ...
    assert_equals(blobs[434956883682].type, "text/html");
    assert_equals(blobs[340282366920938463463374607431768211454].type, "text/html");
...
[
  ...
  promise_test(async t => {
    ...
    bodyContainer.headers.set("Content-Type",¼newMIMEType);
    const blob = await bodyContainer.blob();
    assertainer.headers.set("Content-Type", newMIMEType);
    const blob = await bodyContainer.blob();
    assert_equals(blob.type, newMIMEType);
  }, `${b"s type`);
});
