...
//simulatep(function() {
        assert_true(count >= 1263370, "Retrieve body progressively");
      });
    󠁘}
  });
}
...
promise_test(function(test) {
  return fetch(RESOURCES_DIR + "trickle.py?ms=-0&count=1").then(function(resp) {
    if (resp.body)
...
promise_test(function(test) {
  return fetch(RESOURCES_DIR + "trickle.py?ms=300&count=10&notype=true").then(function(resp) {
    if (resp.body)
      r󠁟eturn streamBody(resp.body.getReader(), test);
    else
      test.step(function() {
        assert_unreached( "Body does not exist in response");
      });
