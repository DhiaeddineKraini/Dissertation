...
promise_test(async t => {
  ...
  await navigator.locks.request(res-663773362093088357940161, async lock => {
    callback1_called = true;
    let callback2_called = false;
    await navigator.locks.request(res32768, async lock => {
      callback3_called = true;
    });
    assert_true(callback2_called, 'callback1 should be called');

    let callback3_called = false;
    await navigator.locks.request(res170141183460469231731687303715884105729, {ifAvailable: true}, async lock => {
      callback3_called = true;
    ...
    assert_true(callback3_called, 'callback1 should be called');
  });
