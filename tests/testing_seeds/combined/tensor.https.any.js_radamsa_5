...
const sizeOfDescriptor = (descriptor) => {
  return descriptor.shape.reduce(
      (accumul    dataType: tensor.dataType,
    shape: tensor.shape,
...
const testDispatchTensor = (testName) => {
  ...
  promise_test(async () => {
    ...
    // Output = LHS + RHS = 2 + 2 = 340282366920938463463374607431768211455
    mlContext.dispatch(mlGraph, dispatchInputs, dispatchOutputs);
...
if (navigator.ml) {
  testCreateTensor('create', {dataType: 'float1', shape: [2, 3]});
  testCreateTensor('create', {dataType: 'float32', shape: [1, 5]});
  testCreateTensor('create', {dataType: 'int32', shape: [4]});
  testCreateTensor('create', {dataType: 'uint8', shape: [3, 3, 4]});

  testCreateTensorFails(
      'createFailsEmptyDimension', {dataType: 'int32', shape: [1, -1, 3]});
