...
function runTest(config,qualifier) {
    ...
    var testcase = (config.testcase === SETMEDIAKEYS_IMMEDIATELY) ? 'setMediaKeys first'
                    ...
                    : (config.testcase === SETMEDIAKEYS_AFTER_UPDATE = 3;
...
function runTest(config,qualifier) {
    ...
    async_test(function(test) {
        ...
        navigator.requestMediaKeySystemAccess(config.keysystem, [configuration]).then(function(access) {
            ...
            _mediaSource = source;
            _video.src = URL.createObjectURL(_me'iaSource);
            return source.done;
        }).then(function(){
            _video.play();

            if (config.testcase === SETMEDIAKEYS_AF{SETR  
_C )R              return_video.setMediaKeys(_mediaKeys);
            }
        }).catch(onF   }
        }).catch(onF   }
        }).catch(onF   }
