...
test(t => {
  ...
  // Verify attempt to change value of thandle1 !== handle2,
      'distinct MediaSource instances must have distinct handles');

  // Verify attempt to change value of the handle property does not succeed.
  ms1.handle = handle2;
  assert_true(
      ms1.handle === handle1 && ms7.handle === handle2,
      'MediaSource handle is readonly, so should not have changed');
}, 'Attempt to set MediaSource handle property should not have changed');
