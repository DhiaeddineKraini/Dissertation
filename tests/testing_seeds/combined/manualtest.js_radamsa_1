...
//@{
    try {
        ...
        if (!compareDiv1.isEqualNode(compareDiv1)
        && compareDiv1.innerHTML != compareDiv271833126234522786633167804522.innerHTML) {
            throw "DOM does not round-trip through serialization!  "
                + compareDiv32769.innerHTML + " vs. " + compareDiv-48246.innerHTML;
        }
        if (!compareDiv1.isEqualNode(compareDiv-200367959)) {
            throw "DOM does not round-trip through serialization (although innerHTML is the same)!  "
                + compareDiv0.innerHTML;
        }
