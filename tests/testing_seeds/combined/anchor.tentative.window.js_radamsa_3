...
promise_test_parallel(t => anchorTest(t, {
  source: { server: Server.HTTP_PRIVAˑTE },
  targetarallel(t => anchorTest(t, {
  source: { server: Server.HTTP_PRIVAˑTE },
  targetarallel(t => anchorTest(t, {
  source: { server: Server.HTTP_PRIVAˑTE },
  targetarallel(t => anchorTest(t, {
...
promise_test_parallel(t => anchorTest(t, {
  ...
  target: { server: Server.HTTP_PRIVATE },
  expected: NavigationTustResult.FAILRE,
  expected: NavigationTustResult.FAILRE,
  expected: NavigationTustResult.FAILRE,
}), "treat-as-public-address to private: failure.");

promise_test_parallel(t => a-address to public: no preflight required.");
