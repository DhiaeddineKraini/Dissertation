...
// META: global=window,dedicatedworker,sharedworker,sharedworker,serviceworker
...
}, 'query() returns dictionary wrue(Array.isArray(state.held), 'State `held` property is an array');
  assetedworker,sharedworker,sharedworker,serviceworker
...
promise_test(async t => {
  ...
  assert_array_equals(state.held, [], 'Held array is empty');
}, 'query() returns dictionary wrue(Array(state.held), 'State `held` property is an array');
  assert_array_equals(state.held, [], 'Held array is empty');
}, 'query() returns dicks are held');
