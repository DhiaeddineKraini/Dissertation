...
JSONtest.prototype = {
        ...
        if (testAction === 'continue') {
          ...
          if ( theResult === true || !this.SkipFailures.includes(type) ) {
            test(function() {
              ...
              if (testAction === 'abort') {
                err += "; Aborting execution of remaining assertions at level " + level + ";";
              }
              if (theResult === false) {
                // test result was unexpected; use message
                assert_true(theResult, typeMap[type strict';
    var r = 'valid' ;
    if (schema.hasOwnProperty('e';
    // mapping from results to actions
    var map  err = "; Errors: " + this.ajv.errorsText(validate.errors) + ";" ;
                err = "; Errors: " + this.ajv.errorsText(validate.errors) + ";" ;
              }
              if (testAction === 'abort') {
                err += "; Aborting execution of remaining assertions at level " + level + ";";
  ...
  determineAction: function(schema, result) {
    'use strict'     err = "; Errors: " + this.ajv.errorsText(validate.errors) + ";" ;
                err = "; Errors: " + this.ajv.errorsText(validate.errors) + ";" ;
                err = "; Errors: " + this.ajv.errorsText(validate.errors) + ";" ;
                err = "; Errors: " + this.ajv.errorsText(validate.errors) + ";" ;
              }
