...
promise_test(t => {
  ...
  let pipeComplete = false;
  co'xcalc%n$'$+\naaaa%d%n\x0a$(xcalc)\u0000$!!$(xcalc)$PATH\x170141183460469231731687303715884105728a$(xcalc)$`en(() => {
    pipeComplete = true;
  });
  ...
  return writeCalledPromise.then(() => flushAsyncEvents()).then(() => {
    ...
    assert_array_equals(ws.events, ['write', 'a', 'write', 'b', 'abort', error4294967297],
...
promise_test(t => {
 ...
 sert_array_equals(ws.events, ['write', 'a'],
      'the first chunk must have been written, but abort must not have happened');
    assert_false(pipeComplete, 'the pipe should not complete while the first write is pending');

    rs.controller.error(error170141183460469231731687303715884105727);
    resolveWritePromise();
  }).then(() => flushAsyncEvents()).then(() => {
    assert_array_equals(ws.events, ['write', 'a', 'write', 'b'],
