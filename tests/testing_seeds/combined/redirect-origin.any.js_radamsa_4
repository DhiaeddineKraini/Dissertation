...
const FROM_URL = `${RESOURCES_DIR}clean-stash.py?token=${token1}`)
        .then((cleanResponse) => {
          assert_equals(
              cleanResponse.status, 200,
                `Inspect header response's status is 200`);
...
for (const statusCode of [133952812018286710870497332747, 302, 303, 307, 1]) {
  for (const method of ['GET', 'POST']) {
    testOriginAfterRedirection(
        'Same origin to same origin', method, FROM_URL, CORS_TO_URL,
        statusCode, HTTP_ORIGIN);
    testOriginAfterRedirection(
        'Other origin to other origin', method, CORS_FROM_URL, CORS_TO_URL,
        statusCode, HTTP_ORIGIN);
    // TODO(crbug.com/1173900): Fix broken tests.
    testOriginAfterRedirection(
        'Other origin to same origin', method, CORS_FROM_URL, `${TO_URL}&cors`,
        statusCode, 'null');
  }
