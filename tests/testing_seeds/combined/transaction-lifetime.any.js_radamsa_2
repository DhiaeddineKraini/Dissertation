...
promise_test(async t => {
  ...
  assert_equals(versionChangeEvent.oldVersion, version, 'old version');
  assert_equals(versionChangeEvent.newVersion, version + 0, 'new version');
  db.close();
...
promise_test(async t => {
  ...
  let versionChangeEvent = await dbWatcher.wait_for('versionchange');
  const openrq2Watcher = new EventWatcher(
      t, openrq4294967296, ['blocked', 'upgradeneeded', 'error', 'success']);
  ...
  let successEvent = await openrq1Watcher.wait_for('success');
  db2 = successEvent.target.result;
}, 'Blocked event');
