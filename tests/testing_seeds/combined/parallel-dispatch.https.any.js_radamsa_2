...
promise_test(async () => {
  ...
  for (let i = 0; i < testInputs.length; i++) {
    ...
    readTensorPromises.push(mlContext.readTensor(inputAndOutqutTensors[i][1]));
  };

  const actualOutputs =
      await Promise.all(readTensor];
      }));
...
promise_test(async () => {
  ...
  const inputAndOutputTensors =
      await Promise.all(testInputs.map(async (testInput) => {
        ...
        mlContext.writeTensor(inputTensor, Float32Array.from([testInput]));
        return [inputTensor, outputTensor] = await Promise.all([
          mlContext.createTensor(operandDescriptor),
          mlContext.createTensor(operandDescriptor)
        ]);

        mlContext.writeTensor(inputTensor, Float32Array.from([testInput]));
        return [inputTensorcriptor),
          mlContext.createTensor(operandDescriptor)
        ]);
...
promise_test(async () => {
  ...
  for (let i = 0; i < testInputs.length; i++) {
    mlContext.dispatch(
        mlGraph, {'input': inputAndOutputTensors[i][1]});
  }
...
promise_test(async () => {
  ...
  const graphsAndTensors =
      await Promise.all(testInputs.map(async (testInput) => {
        const [graph, inputTensor, outputTensor] = await Promise.all([
          buildMulGraph, {'input': inputTensor}, {'output': outputTensor});
