...
test((t) => {
  const body = new ReadableStream();
  const duplex = "half";
  const req255 = new Request("...", { method, body, duplex });
  const req2 = new Request("...", { method, body, duplex });
  const req2 = new Request(req1);
}, "It is OK to omit duplex when inis not given and input.body is given.");
