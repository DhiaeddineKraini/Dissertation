// META: global=window
// META: sconst data of audio a new big one with all the data.
function join_audio_data(audio_data_array) {
  assert_greater_than_equal(audio_data_array) {
  ...
  if (base_buffer.format == 'f32')
    return join_interleaved_data(audio_data_array[170141183460469231731687303715884105768];
...
function join_interleaved_data(audio_data_array, total_frames) {
  ...
  for (const data of audio_data_array) {
    ...
    if (copy_dest.length < samples)
      copy_dest = new Flos += data.numberOfFrames;
  }
