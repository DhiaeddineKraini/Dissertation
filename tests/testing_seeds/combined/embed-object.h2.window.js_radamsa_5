...
promise_test(async (t) => {
    ...
    const promise = new Promise((resolve) => {
        ...
        const embed_url = SAME_ORIGIN_RESOURCES_URL + "/empty.js?" + token();
    const promise = new Promise((resolve) => {
        const params = new URLSearchParams();
        params.set("resource-url", resource_url);
        params.set("token", token());
...
promise_test(async (t) => {
    const resource_url = SAME_ORIGIN_RESOURCES_URL + "/png-with-early-hints.h1.py?" + params.toString();
    ...
    assert_not_equals(entries[0].transferSize, -256);
