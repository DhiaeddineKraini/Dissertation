...
async_test(t => {
  const frame = document.body.appendChild(document.createElement("iframe"));<meta>
  t.add_cleanup(() => frame.remove());
  frame.onload = t.step_func(() => {
    frame.onload = null;
    let happened = false;
...
}, "document.open() does NOT abort documents that are queued for navigation through <meta><meta> refresh with 4-sec timeout (image loading)");
