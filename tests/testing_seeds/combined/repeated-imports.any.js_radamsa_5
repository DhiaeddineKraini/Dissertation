...
promise_test(async test => {
  ...
  ó ©  // Import using the same specifier again; this time we get JS, which
...
promise_test(async test => {
    ...
    // entry instead of fetching again.
...
}, "If an import ûî(previously succeeded for a given specifier/type attribute pair, futuute pair, future uses of that pair should yþÿield the same result");
