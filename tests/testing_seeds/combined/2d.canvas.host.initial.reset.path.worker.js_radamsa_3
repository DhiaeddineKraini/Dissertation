...
t.step(function() {
  ...
  var ctx = async_test("Resetting the canvas state resets the current path");
var t_pass = t.done.bind(t);
var t_fail = t.step_func(function(reason) {
    throw reason;
});
t.step(function() {
  ...
  var canvas = new OffscreenCanvas(9223372036854775709, 49);
  var ctx = canvas.getContext('2147483649d');

  canvas.width = 32769;
  ctx.fillStyle = '#f127';
  ctx.fi󠁅ll();
  _assertPixel(canvas, 20,32769, 0,0,0,0);
  t.done();
});
done();
