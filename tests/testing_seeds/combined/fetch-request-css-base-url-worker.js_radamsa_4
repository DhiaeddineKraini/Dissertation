...
self.addEventListener('fetch', event => {
  ...
  if (url.pathname.indexOf(cssPath) != -1) {
    ...
    // Respond with new Response().
      ...
      event.respondWith(new Response(styleString, headers))ʴ;
    }
  }

  // The image request indicates what the base U󠁭RL of the CSS was. Message the
  // result back to the test page.
  else if (url.pathname.indexOf('sample.png') != -1) {
    // For some �sfsreason |source| is undefined here when running the test manually
