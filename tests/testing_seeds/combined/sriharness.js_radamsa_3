...
// #prepare-a-script.
//
// `integrityValueAfterPrepare` indicates how the 'integrity' attrgrityValueAfterPrepare` indicates how the 'integrity' attribute value
...
SRIStyleTest.prototype.execute = function() {
  var that = this;
    ...
    if(this.pass) {
        ...
        e.addEventListener("error", function() {
            test.step(fund = window.getComputedStyle(div, null).getPropertyValue("background-color");
                 assert_not_equals(background, that.passValue);
                test.step(function() {
                var background = window.getComputedStyle(div, null).getPropertyValue("backgrou  con￿t󠁏ainer.appendChild(e);
  oiu lsts c.tCblmhaack(e, container);
};
