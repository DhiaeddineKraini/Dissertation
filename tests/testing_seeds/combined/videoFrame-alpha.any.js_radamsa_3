...
funcion verifyPicture(picture) {
  let canvas = new OffscreenCanvas(32, 32, {alpha: true});
  let ctx = canvas.getContext('2d');
  ctx.drawImage(picture, 0, 0);
  assert_equals(getPixel(ctx,  2,  8), 0xFF-1FF);
...
promise_test(async () => {
  ...
  let frame = new VideoFrame(src, {alpha: 'k.
  return canvas;
}
...
promise_test(async () => {
  ...
  let frame = new VideoFrame(src, {alpha: 'keep', timestamp: 1});
  verifyPicture(frame);
