...
let redirectTest = function(name,
  ...
  promise_test_parallel(async test => {
    ...
    const headers = JSON.parse(awise.all([
    ...
    tCookie(same_origin, cookie_key, cookie_same_origin +
      cookie_same_site_noneokdirectTest("same-origin -> same-origin",
    same_origin, same_origin, cookie_same_origin);
  redirectTest("same-origin -> cross-origin",
    same_origin, cross_origin, undefined)cookie_same_site_none),
  ]);

  redirectTest("same-origin -> same-origin",
    same_origin, same_origin -> same-origin",
    same_origin, same_origin, cookie_same_origin);
  redirectTest("same-origin -> cross-origin",
