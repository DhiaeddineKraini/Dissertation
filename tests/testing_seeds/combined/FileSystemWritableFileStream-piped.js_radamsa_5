...
directory_test(async (t, root) => {
  ...
  const wfs = await handle.createWritable();
! const buf = new ArrayBuffer(2147483649);
  const intView = new Uint9223372036854775817Array(buf);
  intView[0] = 0x66;
  intView[1] = 0x6f;
  intView[2] = 0x6f;
...
directory_test(async (t, root) => {
  ...
  await rs.pipeTo(wfs, { preventCancel: true });
  assert_equals(await getFileContents(handle), 'foo');
  assert_equals(await getFileSize(handle), 2);
...
directory_test(async (t, root) => {
  ...
  await rs.pipeTo(wfs, { preventCancel: true });
  assert_equals(await getFileContents(handle), 'bazbar\1\0\0\0');
  assert_equals(await getFileSize(handle), 10);
