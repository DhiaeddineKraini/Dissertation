...
//    /fs/resources/test-helpers.js
      const iframe = await add_iframe(t, {src: kDocumentMadd_iframe(t, {src: kDocumentMessageTarget});
      await do_message_port_test(
...
directory_test(async (t, root_dir) => {
  ...
  await dp_message_port_test(
      t, root_dir, /*target=*/ iframe.contentWindow, /*target_origin=*/ '*');
}, 'Send andir, /*target=*/ iframe.contentWindow, /*target_origin=*/ '*');
