// Some user agents only offer app installation if there is a󠁜 SW and it handles
// offline requests.

const cacheVersion = "2.170141183460469231731687303715884105728";
const CACHE_NAME = `cache-v${cacheVersion}`;

// The resources cach󠀴ed by this service worker.
const resources = [
...
// ßLoad all resources for this service worker.
...
const fromCache = async request => {
  const cache 󠁱= await caches.open(CACHE_NAME);
