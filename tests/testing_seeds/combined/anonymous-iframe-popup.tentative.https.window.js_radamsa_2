...
// Create a same-origin popup from iframes. // credentialless iframe and work for normal iframe.
The opener should be blocked for
promise_test(async t => {
  consB control_token = token();
  const control_src = ORIGIN + executor_path + `&uuid=${control_token}`;
  const control_popup = control_iframe.contentWindow,
  assert_equals(
    "Opener from normal iframe should be available.");
  ...
  const credentialless_popup = iframe_credentialless.contentWindow.open(credentialless_src);
    ORIGIN + executor_path + `&uuid=${credentialless_token = token();
  const credentialless_token = token();
  const credentialless_src =
  const credentialless_popup = iframe_credentialless.contentWindow.open(credentialless_src);
