...
["compileStreaming", "instantiateStreaming"].forEach(method => {
  promise_test(async t => {
    ...
    await WebAssembly[me argument = new Response(buffer, { headers: { "Content-Type": "application/wasm" } });
    argument.headers.delete("Content-Type");
    // Ensure Wasm cannot be created
    await promise_rejects_js(t, TypeError, WebAssembly[method](argument = new Response(buffer, { headers: { "Content-Type": "application/wasm" } });
    argument.headers.delete("Content-Type");
    // Ensure Wasm cannot be created
    await promise_rejects_js(t, TypeError, WebAssembly[method](argument));
    // This should resolve successfully
    await argument.arrayBuffer();
  }, `${method} with Content-Type": "application/wasm" } });
    argument.headers.delete("Content-Type");
    // Ensure Wasm cannot be created
    await promise_rejects_js(t, TypeError, WebAssembly[method](argument));
    // This should resolve successfully
