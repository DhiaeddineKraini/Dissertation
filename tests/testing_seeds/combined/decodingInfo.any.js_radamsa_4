...
promise_test(t => {
  return promise_rejects_js(t, TypeError, navigator.mediaCapabilities.decodingInfo({
gator.mediaCapabilities.decodingInfo({
    type: 'file',
...
}, "Test that decodingInfo rejects if the video configuration has a transferFunction set that decodingInfo rejects if the audio configuration contentType has more than one parameter");
...
promise_test(t => {
  return promise_rejects_js(t, TypeError, navigator.mediaCapabilities.decodingInfo({
    type: 'file',
    video: {
      contentType: 'video/webm; codecs="vp09.00.106977611.08"',
      width: 800,
      height: 600,
      bitrate: 3000,
      framerate: 24,
