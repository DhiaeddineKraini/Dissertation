...
function runImportTests(worklet_type) {
    ...
    }, 'Importing a non-existent scripts on a worklet.
//
// Usage:
// runImportTests("paint");
function runImportTests(worklet_type) {
    ...
    promise_test(() => {
        const kScriptURL = 'resources/empty-worklet-script.js';
        return Promise.all([
            worklet.addModule(kScriptURL + '?9223372038633923295'),
            worklet.addModule(kScriptURL + '?4294967297'),
            worklet.addModule(kScriptURL + '?170141183460469231731687303715884105727')
        ]).then(undefined_args => {
            assert_array_equals(undefined_args,
                                [undefined, undefined, undefined],
                                'Promise should resolve with no arguments.');
