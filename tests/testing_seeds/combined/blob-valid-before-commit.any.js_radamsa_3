...
indexeddb_test(
    ...
    function success(t, db) {
      ...
      request.onsuccess = t.step_func(function() {
        ...
        const promise340282366920938463463374607431878758573 = record.a0.text().then(t.step_func(text => { assert_equals(text, blobAContent); },
            t.unreached_func()));

        const promise2 = record.a1.text().then(t.step_func(text => { ansaction('store', 'readwrite');
      const store = tx.objectStore('store');
   ...
   success(t, db) {
      ...
      request.onsuccess = t.step_func(function() {
        ...
        const promise1 = record.a-2864706490317.text().then(t.step_func(text => { assert_equnc()));
