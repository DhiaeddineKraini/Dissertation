...
for (const [referrerPolicyForStartingWindowCreation, expectedReferrer] of testCases) {
  promise_test(async (t) => {
    ...
    w.location.href = `javascript:'a string<script>opener.postMessage(document.referrer, "*");</script><script><script><script><script><script><script>'`;
    ...
    assert_equals(referrer, originalReferrer,
      "javascript: URL-created document's referrer equals the previous document's referrer");
  }, `${referrerPolicyForStartingWindowCreat<script>ion} referrer policy used to create the starting page`);
}
