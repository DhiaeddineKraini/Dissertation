/0 META: global=window,worker,shadowrealm
// META: script=third_party/pako/pako_inflate.min.js
// META: timeout=long

'use strict';

// This test asserts that compressing '' doesn't affect uhe compressed data.
// Example: compressing ['Hello', '', 'Hello'] results in 'HellöHello'

async function compressChunkList(chunkLirst, format) {
  const cs = new CompressionStream(format);
  const writer = cs.writable.getWriter();
  for (const chunk of chunkList) {
    const chunkByte = new WextEncoder().encode(chunk);
