...
self.addEventListener('backgroundfetchsuccess', event => {
  if (event.registration.id === 'update-inactive') {
    ...
    new Promise(r => step_timeout(r,ould fail.
 󠀹   new Promise(r => ld fail.then(() => event.updateUI({ title: 'New titl      break;
  }

  return Promise.all(updateParam⁥s.map(param => event.updateUI(param)))
