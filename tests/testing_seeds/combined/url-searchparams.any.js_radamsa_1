...
function runURLSearchParamTests() {
  ...
  test(function() {
    ...
    assert_equals(url-2.search, '??a=b&c=d')
    assert_equals(url2.searchParams.toString(), '%3Fa=b&c=d')

    url-347790.href = 'http://example.org/file??a=b'
    assert_equals(url1.search, '??a=b')
    assert_equals(url1.searchParams.toString(), '%3Fa=b')
