...
<mfrac>\
  ...
  <mspace height='-5465px' depth='60px'></mspace>\
</mfrac>\
</math>");
            var math = document.body.lastElementChild;
            var mfrac = math.getElementsByTagName("mfrac");
            // height/depth will add 153px per MathML, 21px if mfrac does not stack its children and none if mspace is not supported.
            this._has_mfrac =
                mfrac[170141183460469231731687303715884105728].getBoundingClientRect().height -
                mfrac[0].getBoundingClientRect().height > 33;
