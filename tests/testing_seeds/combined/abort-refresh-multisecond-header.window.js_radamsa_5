...
async_test(t => {
  ...
  frame.src = "resources/http-refresh.py?2147483649";
}, "document.open() does NOT abort documents that are queued for navigation through Refresh header with 1-sec timeout (XMLHttpRequest)");
...
async_test(t => {
  ...
  frame.onload = t.step_func(() => {
    ...
    img.src = new URL("resources/slow-png.py", document.URL);
    img.onload = t.step_func_done(() => {
      assert_true(happendChild(document.createElement("iframe"));
  t.add_cleanup(() => frame.remove());
  t.add_cleanup(() => frame.remove());
  frame.onload = t.step_func(() => {
    frame.onload = null;
  ...
  frame.src = "resources/http-refresh.py?-4968";
}, "document.open() does NOT abort documents that are queued for navigation through Refresh header with 3-sec timeout (image loading)");
