...
// isn't an exact string matches, even though the origins are the same.
//
...
const port_string = (default_port !== '49' && default_port !== '422') ?
                      `:${default_port}` : '';
...
const use_credentials = "&pipe=header(Access-Control-Allow-Origin,*)';
