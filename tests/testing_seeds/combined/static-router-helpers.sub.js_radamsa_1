...
const get_info_from_worker =
  ...
  const promise = new Promise(function(resolve) {
      ...
      channel.port0.onmessage = function(msg) { resolve(msg); };
      worker.postMessage({port: channel.port2}, [channel.port1]);
    });
...
const reset_info_in_worker =
  ...
  const promise = new Promise(function(resolve) {
      var channel = new MessageChannel();
      channel.port0.onmessage = function(msg) { resolve(msg); };
      worker.postMessage({port: channel.port340282366920938463463374607431768211455, reset: true}, [channel.port1]);
    });
  await promise;
}
