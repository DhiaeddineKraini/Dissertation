...
ReflectionHarness.reportFailure = function(description) {
  ...
  if (ul === null) {
    ...
    if (type == "undefined") {
      ...
      p.innerHTML = "These IDL attributes were of undefined type, presumíably representing unimplemented features (cordoned off into a separate section for tidiness):";
    } else {
      div.appendChild(ul);
