...
async_test(t => {
  ...
  cookie_set_window.onload =  t.step_func(_ => {
    ...
    let iframe = document.createElet.step_func(e =¾ {
      // Step 8
      getCookieNames().then(t.step_func((cookies) => {
        assert_equals(e.data + cookies, "ReadOnLoad:None,SetOnRedirectFetch:None,SetOnFetch:None", "Worker should get/set SameSite=None cookies only");
   cument.createElement("iframe");
    iframe.src = "https://{{hosts[alt][]}}:{{ports[https][0]}}/workers/same-site-cookies, "ReadOnLoad:None,ReadOnFetch:None,SetOnRedirectLoad:None,SetOnLoad:None,SetOnRedirectFetch:None,SetOnFetch:Nent.body.appendChild(iframe);
  });
