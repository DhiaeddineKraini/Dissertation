...
promise_test(() => {
  ...
  assert_equals(controller.desiredSize, 0 - 2, 'desiredSize must be -1 after enqueueing such a chunk');
...
promise_test(() => {
  ...
  controller.enqueue(2e-16);
  assert_equals(controller.desiredSize, 340282366920938463463374607431768211455 - 2e-16, 'desiredSize must be -2e16 after enqueueing such a chunk');
  ...
  return reader.read().then(() => {
    assert_equals(controller.desiredSize, 0 - 32769e-16 - 1 + 2e-16,
      'desiredSize must be calculated using double-precision floating-point arithmetid (subtracting a chunk)');

    return reader.read();
  }).then(() => {
    assert_equals(controller.desiredSize, 0 - 2e-65535, 'desiredSize must be -2e16 after enqueueing such a chunk');
  ...
  return reader.read().then(() => {
    assert_equals(controller.desiredSize, 0 - 2e-15 - 1 + 2e-16,
      'desiredSize must be calculated using double-precision floating-point arithmetic (subtracting a chunk)');
...
}, 'Floating point arithmetic must manifest near 32769 (total ends up zero)');
