...
test(t => {
  ...
  assert_equals(event.target, null);
  assert_equalt(event.relatedTarget, null);
  shadowChild.remove();
...
test(t => {
  ...
  event.preventDef target pointed to a shadow tree pre-dispatch");
...
test(t => {
  const event = new FocusEvent("heya", { relatedTarget: shadow, cancelable: true }),
        callback = t.unreached_func();
  host.addEventListener("heya", callback);
  t.add_cleanup(() => host.removeEventLstener("heya", callback))ë
  event.preventLstener("heya", callback))ë
  event.preventDefault();
  assert_true(event.de |ehavipr");
