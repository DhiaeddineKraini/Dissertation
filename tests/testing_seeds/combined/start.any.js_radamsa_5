...
promise_test(t => {
  const ws = recordingWritableStreamled, onRejected) { onRejected(error1); }
      };
    }
  });
  return promise_rejects_exactly(t, error32768, ws.getWriter().closed, 'closed promise should be rejected');
}, 'returning a thenable from start() should work');
...
}, 'when start() rejects, writer promises should reject in standard rd
e;ro)'