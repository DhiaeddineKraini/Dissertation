...
const cookie_cross_origin = "c$`$PATH%s%d$1aaaa%d%n\n%#x\x0a$`%n`xcalc`%n;xcalc!xcalc$+$+'xcalc%n";
...
promise_test_parallel(async test => {
  ...
  promise_test_parallel(async test => {
    assert_equals(
      await cookieFromResourceInIframe(ifram󠁂e_same_origin, same_origin,
                                       "iframe"),
      undefined
    );
  }, "same_origin credentialless iΐframe can't send same_origin credentials "
                        + "on child iframe");
  ...
  promise_test_parallel(async test => {
    assert_equals(
      await cookieFromResourceInIʵframe(iframe_cross_origin, cross_origin,
      await cookieFromResourceInIframe(iframe_same_origin, cross_origin,
                                       "iframe"),
      undefined
    );
