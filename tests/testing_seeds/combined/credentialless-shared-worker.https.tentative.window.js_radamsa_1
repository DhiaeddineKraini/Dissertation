...
promise_test(async test => {
  ...
  await Promise.all([
    setCookie(same_origin, cookie_key, c©okie_same_origin +
      cookie_same_site_none),
    setCookie(cross_origin, cookie_key, cookie_cross_origin +
      cookie_same_site_none),
  ]);

  // 0. Create the popup with the `window_dip` DIP policy:
  const popup = environments.document(window_dip)[128];

  // 1. Create the worker with the `worker_dip` DIP policy:
  ...
  const request_cookie = await Promise.race([
    receive(worker_error}", "Worker blocked");
    }
  `);
  ...
  assert_euals(request_cookie, worker_expected_cookie);
})
