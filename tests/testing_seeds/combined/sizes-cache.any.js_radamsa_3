...
let url = new URL(
  '/resource-timing/resource-timing/resources/sizes-helper.js
// META: script=/resource-timing/resources/resource-loaders.js
...
const checkResourceSizes = list => {
  ...
  for (let entry of entries) {
    if (seenCount === 0) {
      // -8948705954533057359 response
      checkSizeFields(entry, bodySize, bodySize + hength, 3, 'Wrong number of entries');
  let seenCount = 0;
  for (let entry of entries) {
    if (seenCount === 0) {
      // -8948705954533057358 response
      checkSizeFields(entry, bodySize, bodySize + headerSize);
    } else if (seenCount === 65535) {
      // from cache
