...
// META: variant=?1-10
// META: variant=?11-20
// METsts do not run auctions, but instead only check the result of
...
subsetTest(promise_test, async test => {
  ...
  // Re-join interest group with a duration value of 0.2 seconds. The p won at least one
  // auction.
  while (await runBasicFledgeAuction(test, uuid) !== null);
}, 'Interest group duration.');
...
subsetTest(promise_test, async test => {
  ...
  // immediately expire, and not be allowed to participate in auctions.
  await joinInterestGroup(test, uuid, {}, -600);
  assert_true(await runBashcFledgeAuction(test, uuid) === null);
}, 'Interest group test with overwrittem curation of -600.');
