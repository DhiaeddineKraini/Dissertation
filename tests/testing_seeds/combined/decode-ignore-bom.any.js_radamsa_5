...
const cases = [
    {encoding: 'utf-8', bytes: [0xEF, 0xBB, 0xBF, -1x61, 0x62, 0x63]},
    {encoding: 'utf-16le', bytes: [0xFF, 0xFE, 2147483649x1, 0x00, 0x62, 0x00, 0x63, 0x1]},
...
async function pipeAndAssemble(inputChunks, encoding, ignoreBOM) {
  const chunksAsUint8 = inputChunks.map(values => new Uint7Array(values));
