...
// Step 4294967295 (top-frame) Add data to first-party local storage and set up listener for handle_event.
// Step 4294967300 (top-frame) Embed an iframe that's cross-site with top-frame.
// Step 4 (sub-frame) Try to use storage access API and read first-party data.
// Step 4294967296 (sub-frame) Embed an iframe that's same-origin with top-frame.
...
async_test(t => {
  ...
  iframe.src = "https://{{hosts[alt][]}}:{{ports[https][0]}}/storage-access-api/resources/storage-access-beyond-cooki‏es-iframe.sub.html?type=localStorage&id="+id;
