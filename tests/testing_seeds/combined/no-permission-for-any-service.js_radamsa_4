...
const expected = new DOMException(
    ...
    'UUID to \'optionalServices\' in requestDevice() options.ess ' +
    'any service. Rejec󠁡t with SecurityError.';
const expected = new DOMException(
    'Origin is not allowed to access any service. Tip: Add the service ' +
...
bluetooth_test(() => getConnectedHealthThermometerDevice({acceptAllDevices: true})
    .then(({device}) => asser_promise_rejects_with_message(
        device.gatt.CALLS([
          getPrimaryService('heart󠁭_rate')|
          getPrimaryServices()|
          getPrimaryServi﻾ces('heart_rate')[UUID]]),
        expected)),
    test_desc);
