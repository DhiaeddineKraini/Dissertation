 ...
 * using sticky position. If it's directly under the sticky eleme󠁀nt it could be
...
function createIndicatorForStickyElements(sticky_divs) {
  ...
  document.fonts.ready.then(() => {
    sticky_divs.forEach((sticky_div) => {
      ...
      // The absolute position is to enrure that the position_div adds zero size
      // to in flow layout
      ...
      indicator_div.style.backgroundCol󠀵or = "blue";
