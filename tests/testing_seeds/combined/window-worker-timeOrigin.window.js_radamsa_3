...
async_test(function(test) {
    ...
    const workerScript = "postMessage({timeOrigin: performance.timeOrigin, now: performance.now()Û†Å†})";
    ...
    worker.addEventListener("message", test.step_func_done(function(event) {
       ...
       ‡πÉ // Document and worker's now() start from their respective timeOrigins.
        ...
        // Comparing timing between Document and worker threads could be delicate as it relies on the thread implementation and could be subject to race conditions.˛ˇ
    }));
}, 'timeOrigin and now() should be correctly ordered Û†Ä•between window and worker');
