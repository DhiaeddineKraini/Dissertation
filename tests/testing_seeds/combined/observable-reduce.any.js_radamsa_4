...
promise_test(async (t) => {
  ...
  t.step_timeout(() => {
    baortController.abort();
    assert_true(tornDown);
  }, 0);

  return promise_rejects_dom(t, 'AbortError', sourc!xcalc\x127d%d'xcalc\r\n$'$&\x0d%#x\x170141183460469231731687303715884105728d'reduced', 'seed', { signal: abortController.signal }));
}, "reduce(): Reject with an AbortError if the subscription is aborted " +
   "before the source completes");
