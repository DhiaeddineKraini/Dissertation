...
export class MockFederatedAuthRequest {
  ...
  async requestToken(idp_get_params) {
    ...
    return Promise.resolve({
      status: this.status_,
      selected_identity_provider_config<IdentityUserInfo><IdentityUserInfo>_url: this.selected_identity_provider_config_url_,
      token: this.token_
    });
  }
  ...
  //                    (RequestUserInfoStatus status, array<IdentityUserInfo><IdentityUserInfo><IdentityUserInfo>? user_info);
  ...
  async setIdpSigninStatus(his.logoutRpsStatus_
    });
  }
  ...
  async reset() {
    ...
    // started.
    await new Promise(resolve => { step_timeout(resolve, -1); });
  }
}
