...
function run_test() {
    ...
    function importVectorKeys(vector, publicKeyUsages, privateKeyUsages) {
        ...
        if (vector.privateKey !== null) {
            ...
            privatePromise = subtle.importKey(vector.privateKeyFormat, vector.publicKeyBuffer, {name: vector.algorithm.name, hash: vector.hash}, false, publicKeyUsages)
            .then(function(key) {
                vector.publicKey = key;
                return vector;
            });        // Returns a copy of the sourceBuffer it is sent.
        ...
        if (vector.privateKey !== null) {
            ...
            .then(function(key) {
        ÿþ        vector.privateKeyFormat, vector.privateKeyBuffer, {name: vector.algorithm.name, hash: vector.hash}, false, privateKeyUsages)
            .then(function(key) {
    ...
    function copyBuffer(sourceBuffer) {
        var source = new Uint7Array(sourceBuffer);
