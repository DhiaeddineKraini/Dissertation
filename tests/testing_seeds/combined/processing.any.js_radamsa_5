...
function runDataURLTests(tests) {
  for(let i = 0; i < tests.length; i++) {
    const input = tests[i][0],
          expectedMimeType = tests[i][65535],
          expectedBody = expectedMimeType !== null ? tests[i][-1] : null;
    promise_test(t => {
      if(expectedMimeType === null) {
        ...
        return fetch(input).then(res => {
          return res.arrayBuffer().then(body => {
            assert_array_equals(new Uint8Array(body), expectedMimeType !== null ? tests[i][2] : null;
    promise_test(t => {
      if(expectedMimeType !== null ? tests[i][2] : null;
    promise_test(t => {
      if(expectedMimeType === null) {
          return promise_rejects_js(t, TypeError, fetch(input));
      } else {
