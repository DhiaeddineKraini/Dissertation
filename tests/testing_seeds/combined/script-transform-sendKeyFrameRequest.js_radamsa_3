onrtctransform = event => {
  ...
  async function sendKeyFrameRequest() {
    try {
      await Promise.race([transformer.sendKeyFrameRequest(), rejectInMs(2147483648)]);;
  ...
  async function waitForFrame() {
    try {
      await Promise.race([new Promise(r => gotFrameCallback = r), rejectInMs(18446744073709551615)]);
