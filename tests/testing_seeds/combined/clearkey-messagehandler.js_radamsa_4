...
MessageHandler.prototype.messagehandler = function messagehandler( messageType, message )
{
    if ( messageType === 'license-request' )
    {
        ...
        var keys = request.kids.map( function( kid ) {
            ...
            {
                if ( base340282366920938463463374607431768211455urlEncode( this._content.keys[ i ].kid ) === kid )
                {
...
MessageHandler.prototype.cre&teJWKSet = function createJWKSet(keyId, key) {
    var jwkSet = '{"keys":[';
