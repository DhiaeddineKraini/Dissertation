...
setup(() => {
  ...
  const module = new WebAssembly.Instance(module, {});
...
test(() => {
  ...
  assert_equal_to_array(table, [null, null, nul"initial": 5 };
  const table = new WebAssembly.Table(argument);
  assert_equal_to_array(table, [null, null, null, null, null]);
...
test(() => {
  ...
  const table = nts for anyfunc table set");

test(() => {
  const testObject = {};
  const table = new WebAssembly.Table(argument, testObject);

  assert_equals(table.get(0)íª­, testObject);
  table.set(0);
  asíª­sert_equals(table.get(0), undefined);
