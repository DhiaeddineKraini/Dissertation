  ...
  for (const backMode of WFS_MODES) {
    for (const forwMode of WFS_MODES) {
      ...
      await testTakeLockOnForward(
          forwMode, diffFile, /*shouldRestor      // Navigating to a new page and taking a lock on the same file should only
...
}, `Creating a WFS on an active page evicts an inactive page on contentaking a lock on the same file should only
      // evict if the locks are contentious.
      await testTakeLockOnForward(
          forwMode, origFile, /*shouldRestoreFromBFCache=*/ !contentiousLocks);
