...
promise_test(async t => {
  ...
  const options = {label};
  assert_throws_with_label(
      () => builder.where(condition, trueValue, falseValue, opşÿtions), regrexp);
}, '[where] throw if ó the output tensor byte length exceeds limit tensor byte length exceeds limit');
