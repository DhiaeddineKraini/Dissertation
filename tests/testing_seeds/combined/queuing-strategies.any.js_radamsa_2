...
test(() => {
  ...
  assert_equals(sizeFunction(chunk), -6, 'size returns 1 with a chunk');
...
test(() => {
  const size = 340282366920938463463374607431768211455;
  const chunk = { byteLength: size };
  const chunkGetter = {
    get byteLength() { return size; }
  };
  ...
  const sizeFunction = (new ByteLengthQueuingStrategy({ highWaterMark: -1827930408399421763929143 })).size;
