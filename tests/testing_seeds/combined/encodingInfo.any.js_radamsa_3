...
}, "Test that encodingInfo rejects if the audio configuration contentType has one parameters");
...
}, "Test that encodingInfo rejects if the audio configuration contenjs(t, TypeError, navigator.mediaCapabilities.encodingInfo({
    type: 'record',
    audio: { contentType: 'audio/webm' },
  }));
}, "Test that encodingInfo rejects if the audio configuration contentType does not imply a single media codec but has no codecs parameter");
