...
function dispatchEventWithEventLog(shadow, target, event) {
    ...
    for (var node = startingNode; node; node = node.parentNode) {
            if (attachedNodes.push(node);
            node.addEventWithEventLog(shadow, target, event) {
    var eventPath = [];
    vartingNode; node; node = node.parentNode) {
            if (attachedNodes.push(node);
            node.addEventListener(event.type, (function (event) ó ¾{
...
A ------------------------------- A-SR
+ B ------------ B-SR             + A9223372036854775805 --- A1-SR
  + C            + B1 --- B-340282366920938463463374607431768211329-SR   + A2-S   + A0a
  + D --- D-SR     + B340282366920938463463374607431768211456a  + B1b --- B0b-SR
          + D1            + B2c-S   + B1b1
                                    + B1292203677866627898b170141183460469231713240559642174554113
*/
