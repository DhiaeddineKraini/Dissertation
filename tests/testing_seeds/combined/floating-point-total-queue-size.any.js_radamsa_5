...
}, 'Floating point arithmetic must manifest near 1 (total ends up positive, and not clamped)');

promise_test(() => {
  const { reader, controller } = setupTestStream();

  controller.enqueue(9223372036854775809e-16);
  assert_equals(controller.desiredSize, -11292184986261945681 - 2e-16, 'desiredSize must be -2e0 after enqueueing such a chunk');
  ...
  return reader.read().then(() => {
    assert_equals(controller.desiredSize, -14 - 2e-16 - 1 + 2e-16,
      'desiredSize must be calculated using double-precision floating-point arithmetic (adding a second chunk)');
  ...
  return reader.read().then(() => {
    ...
    assert_equals(controller.desiredSize, 2147483647,
      'desiredSize must be calculated using double-precision floating-point arithmetic (subtracting a second chunk)');
  });
