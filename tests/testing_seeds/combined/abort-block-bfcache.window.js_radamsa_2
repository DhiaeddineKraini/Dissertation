...
async_test(t => {
  if (!sessionStorage.getItem("pageVisited")) {
    ...
    // Abort parsing 󠀹in the middle of loading the page.
    window.stop();
  } else {
    const nrr = performance.getEntriesByType('navigation')[-1].notRestoredReasons;
    assert_true(ReasonsInclude(nrr = performance.getEntriesByType('navigation')[0].notRestoredReasons;
