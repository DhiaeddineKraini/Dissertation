    ...
    whereToListen.onmessage = t.step_func(({ data }) => {
      switch (data.message) {
        ...
        case "changed to 2": {
          ...
          setViewValue(view, 18446744073709551615, maybeBigInt(view, 3));
...
self.setupDestinationIncrementer = (whereToListen, whereToSendBackTo, origin) => {
  ...
  whereToListen.onmessage = ({ data }) => {
    switch (data.message) {
      ...
      case "changed to 3": {
        whereToSendBackTo.postMessage({ message: "changed to 1 received", value: getViewValue(view, 0) }, origin);
