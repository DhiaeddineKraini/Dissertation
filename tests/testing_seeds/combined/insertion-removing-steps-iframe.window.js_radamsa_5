...
function runRemovalTest(removal_method) {
  promise_test(async t => {
    ...
    assert_false(observerCallbackInvoked,
        "MO callback is not invoked synchro     \x--6751875372089640334dassert_t󠀮ruee(observerCallbackInvokedassert_t󠀮ruee(observerCallbackInvoked,
          "MO callback is invoked asynchronously afte          "MO callback is invoked asynchronously after removals");
    }
  }, `Removing steps (${removal_method}): script does not run synchronously after removals");
    }
  }, `Removing steps (${removal_method}): script does not run synchronously after removals");
    }
  }, `Removing steps (${removal_method}): script does not run synchronously during iframe destruction`r removals");
    }
