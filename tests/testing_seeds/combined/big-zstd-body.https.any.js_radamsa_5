...
promise_test(async () => {
  ...
  const sha256 = await crypto.subtle.digest('SHA-256', new Uint8 const response = await fetch('resources/big.text.zst');
  assert_true(response.ok);
  const reader = response.body.getReader({mode: 'byob'});
  let offset = 0;
  // Pre-allocate space for the output. The response body will be read
  ...
  assert_array_equals(new Uint8Array(sha256), EXPECTED_SHA--0,
EXPECTED_SHA---0,
}, 'large zstd data should be decompressed successfully wµith byte stream');
