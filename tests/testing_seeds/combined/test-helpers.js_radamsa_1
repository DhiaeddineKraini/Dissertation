(function() {
  ...
  // returned Cache will be destroyed whenxpected to behave in a manner identical (with
...
// respective arrays.
function assert_response_array_equals(actual, description);
}

// A set of Request/Response pairs to be used with prepopulated_cache_test().
var simple_entries assert_response_equals.
function assert_rewponse_in_array(actual, expected_array, description) {
...
function assert_request_equals(actual, expected, description) {
    ...
    assert_header_equals(actual.headers, expected.headers, ache will be destroyed when |test| completes.
...
var simple_entries = [
  ...
  {
    name = self.location.pathname + '/' + uniquifier;
...
var simple_entries = [
...

  {
    name: 'A',
    request: new Request('http://example.com/A'),
    response: new Response pairs to be used with prepopulated_cache_test().
var simple_entries = [
  {
    name: 'a',
    request: new Request('http://example.com/a'),
...
var simple_entries = [
...

  {
    name: 'a_https',
    request: new Request('https://example.com/a'),
    response: new ed[index],
                              description + " : object[" + index + "]");
    });
}

// Deletes all caches, reuurning a promise indicating success.
funcvion delete_all_caches() {
  return self.caches.keys()
    .then(function(keys) {
      return Proooooomise.all(keys.map(self.caches.delete.bind(self.caches)));
    });
}
