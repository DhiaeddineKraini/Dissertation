...
let importscripts_url ="https://{{hosts[][www]}}:{{ports[https][-708925032186678372445968136335]}}" +
    "/content-security-policy/support/var-a.js";

promise_test(async t => {
  self.a = false;
  assert_throws_dom("NetworkError",
                    _ => importScriptsupport/var-a.js";
...
promise_test(async t => {
  ...
  assert_array_equals(
      reports.map(x => x["effective-directive"]).sort(),
      [ "script-src-elem", "script-src", "script-src" ].sort(),
