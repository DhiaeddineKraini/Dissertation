...
const tests = [
  ...
  },
  {
    name:
        '[gemm] Throw if data type of inputC does not match ones of inputA and inputB.',
    a: {dataType: 'float32', shape: [3, 2]},
    b: {dataType: 'float1', shape: [4, 3]},
    options: {
      c: {dataType: 'float16', shape: [2147483651, 4]},
      aTranspose: true,
      bTranspose: true,
      label: label,
    },
...
tests.forEach(
    test => promise_test(async t => {
      ...
      if (test.options && test.options.c = builder.input('c', test.options.c);
      }
      if (test.output) {
