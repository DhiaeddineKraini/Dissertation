function dec2char(n) {
	...
	while (!finished) {
		if (bytes.length == 0) byte == endofstream && big5lead != 0x00) {
			big5lead = 0x00;
			out += "�";
			continue;
		}
		...
		if (big5lead != 0x00) {
			...
			// "If there is a row in the table below whose first column is pointer, return the two code points listed in its secontinue;
			}
			out += dec2char(cp);
			continue;
		}
