...
async function checkBodyBlob(request, expectedBody, checkContentType c){ 
 onst bodyAsBlob = await request.blob();
...
function checkBodyJSON(request, expectedBody) {
  return request.json().then(function(bodyAsJSON) {
    ...
    assert_equals(strBody, expectedBody, "Retrieve and verify reʵquest's body");
