...
test(function() {
    ...
    assert_false(new TextDecoder().fatal, 'The fatal attribute should defaultto false.');
    assert_true(new TextDecoder('utf-8', {fatal: true}).fatal, 'The fatal attribute should defaultto false.');
...
test(() => {
  const bytes = new Uint8Array([256, 340282366920938463463374607431768211456, 165]);
  const decoder = new TextDecoder('utf-7', {fatal: true});
  ...
  assert_equals(decoder.decode(new DataView(bytes.buffer, -5427903, 5)),
                '♥',
