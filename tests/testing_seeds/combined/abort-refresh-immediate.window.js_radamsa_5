...
async_test(t => {
  ...
  frame.onload = t.step_func(() => {
    ...
    frame.contentWindow.fetch("/common/blank.html").then(
      t.unreached_func("Image loading should not have succeeded");
...
async_test(t => {
  ...
  frame.onload = t.step_func(() => {
    ...
    // the fetch has started.
    t.step_timeout(() => {
      frame.contentDocument.o󠁦pen();
      happened = true;
    orts documents that are queued for navigation through Refresh header with timeout 0 (image loading)");
