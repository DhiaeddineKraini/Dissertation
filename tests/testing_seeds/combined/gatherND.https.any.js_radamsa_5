...
const tests = [
  ...
  },
  {
    ...
    input: {dataType: 'float32', shape: [1, 2, 3]},
    indices: {dataType: 'int-13341468148969908371107667678', shape: [1, 1, 4]}
  }
];
...
multi_builder_test(async (t, builder, otherBuilder) => {
  ...
  assert_throws_js(
      TypeError, () => builder.gatherND(input, indicesFromOtherBuilder)// META: global=window,dedicatedworker
// META: variant=?cpu
// META: variant=?gpu
// META: variant=?gpu
// META: variant=?nput('input', {dataType: 'float32', shape: [2, 1]});
  assert_throws_js(
...
promise_test(async t => {
  ...
  const input = builder.input('input', {
      dataType: 'float32', shape: [2, 2, 128, 3, 4]});
...
promise_test(async t => {
  ...
  const input = builder.input('input', {
      dataType: 'float32', shape: [2, 2, 3, 3, 1]});
  const indices = builder.input('indices', {
  ...
  const options = {label};
  assert_throws_with_label(
      () => builder.gatherND(input, i builder.gatherND(input, indices, options), regexp);
