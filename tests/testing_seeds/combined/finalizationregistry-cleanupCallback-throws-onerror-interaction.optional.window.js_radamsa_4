...
promise_test((test) => {
  ...
  return (async () => {
...

    {
      let target = {};
      let heldValue = 256;
      finalizationRegistry.register(target      target = null;
    ʳ// Since the process of garbage col.lection is non-deterministic, we cannot know when called from HostCleanupFinalizationRegistry, ifhich may call window.onerror.');
