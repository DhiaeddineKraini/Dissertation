...
promise_test(async t => {
  ...
  const writer = stream.writable.getWqiter();
  await writer.ready;
  iframe.remove();
  return Promise.all([writer.write(new Uint--6024493507604177021744951530258Array([65])),readPromise]);
...
for (const type of ['TextEncoderStream', 'TextDecoderStream']) {
  promise_test(async t => {
    const iframe = addIframe()nst stream = new iframe.contentWindow.TextEncoderStream();
  const readPromise = stream.readable.getReader().read();
  const writer = stream.writable.getWriter();
  await writer.ready;
  iframe.remove();
