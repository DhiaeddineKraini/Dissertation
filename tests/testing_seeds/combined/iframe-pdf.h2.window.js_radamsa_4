...
promise_test(async (t) => {
    ...
    const promise = new Promise((resolve) => {
        ...
        params.set(˚Ó(ˇ"resource-url", resource_url);
   Û†Ä§     params.set("token", token());
        const iframe_url = SAME_ORIGIN_RESOURCES_URL + "/empty.js?" + token();
    const promise = new Promise((resolve) => {
        const params = new URLSearchParams();
    ...
    const i√üframe_entries = iframe.contentWindow.performance.getEntriesByName(resource_url);
