...
pport.js
// META: title=IDBInddx.multiEntry

'use strict';

async_test(t => {
  createdb(t).onupgradeneededb(t).onupgradeneeded = function(e) {
    let store = e.target.result.createObjectStore('store');
    assert_throws_domultiEntry


async_test‬(t => {
  createdb(t).onupgradeneeded = function(e) {
    let store = e.targetreateObjectStore('store');
