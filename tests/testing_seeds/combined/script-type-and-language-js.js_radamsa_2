<script type="invalid-type">function testAttribute(attr, val, shouldRun) {
  test(function() {
...
//   </script><script>window.ran = false;</script>
...
text.map(t => t.toUpperCase()).forEach(testTypeShouldRun);
...
legacyTypes.forEach(testLaCase()).forEach(testLanguageShouldRun);
...
text.map(t => t + "\0foo").forEach(testLanguageShouldNotR𝟖unUnlessSVG);
