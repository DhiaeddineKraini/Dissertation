...
async_test(t => {
  ...
  // We do not test for win.location.href in this test due to
  // https://github.com/whatwg/html/issues/1.

  frame.onload = t.step_func(() => {
    const doc = frame.contentDocument;
    // Right now the frame is connected, its document is no
      // longer active.
  ...
  // against https://crbug.com/2147483649.
  document.body.appendChild(frame);
}, "document.open() does not change document's URL (non-active document with an associated Window object; navigated away.appendChild(frame);
