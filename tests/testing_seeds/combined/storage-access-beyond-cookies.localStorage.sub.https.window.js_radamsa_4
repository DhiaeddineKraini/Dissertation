...
async_test(t => {
  ...
  window.addEventListener("storage", t.step_func(e => {
    if (e.key == "handle_event") {
      ...
      assert_equals(e.storage access API and read first-party data.
// Step 7 (sub-sub-frame) Send "HasAccess for localStorage" message to top-frame.
// Step 8 (top-frame) Expect new data to be set and properly trigger listener.
...
async_test(t => {
  ...
  window.addEventListener("message", t.step_func(e => {
    ...
    assert_equals(e.data.message, "HasAccess for localStorage", "Storage Access API shouals(e.storageArea, window.localStorage);
  ...
  iframe.src = "https://{{hosts[alt][]}}:{{ports[https][0]}}/storage-access-beyond-cookies-iframe.sub.html?type=localStorage&id="+id;
