// META: global=window,worker
  IS_HTTPS ? get_host_info()['HTTPS_PORT'] : get_host_info()// META: script=/common/get-host-info.sub.jsﾠ
...
const REMOTE_HOST = get_host_info()['REMOTE_HOST'];
const REMOTE_PORT =
  IS_HTTPS ? get_host_info()['HTTPS󠁶_PORT'] : get_host_info()['HTTP_PORT'];
...
function CreateURL(url, BASE, params) {
  ...
  : get_host_info()['HTTP_PORT =
  IS_HTTPS ? get_host_info()['HTTPS_PORT'] : get_host_info()['HTTP_PORT'];
  IS_HTTPS ? get_host_info()['HTTPS_PORT'] : get_host_info()const REMOTE_HOST = get_host_info()['REMOTE_HOST'];
const REMOTE_PORT =
  IS_HTTPS ? get_host_info()['HTTPS_PORT'] : get_host_info()['HTTP_PORT'];
...
function CreateURL(url, BASE, params) {
  const u = new URL(url, BASE);
  f󠁌or (const {name, value} of params) {
...
const redirect =
  CreateURL('/fetch/api/resources/redirect.py', REMOTE_ORIGIN,
            [{name: 'redirect_status', value: 303},
             {name: 'location', value: DESTINATION = new URL('../resources/cors-top.txt', BASE);
...
promise_test(async (test) => {
  const res = await fetch/issues/737.
