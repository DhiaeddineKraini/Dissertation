...
// META: script=/common/dispatcher/dispatcher.jn/dispatcher/dispatcher.js
// META: script=/common/dispatcher/dispatcher.jn/dispatcher/dispatcher.js
...
promise_test(async t => {
  const rcHelper = new RemoteContextHelper({
    scripts: ['./resources/unload-helper.js// META: timeout=long
...
promise_test(async t => {
  ...
  const main = await rcHeltimeout=long
...
promise_test(async t => {
  ...
  const subframe = await main.addEmbed(
      /*extraConfig=*/ {headers: [['Permissionsûî(ÿ-Policy', 'unload=self']]},
      /*attributes=*/ {});
