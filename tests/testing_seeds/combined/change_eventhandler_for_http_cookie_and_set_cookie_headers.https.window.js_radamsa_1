// META: title=Cookie Store API: Observing 'change' eventPromise = observeNextCookieChangeEvent();
  ...
  await verifyCookieChangeEvent(
    eventPromise, {changed: [{name: 'HTTP-cookie', value: 'ew-value'}]},
    'Cookie we overwrote using HTTP is observed');

      'Cookie we wrote using HTTP in cookie jar');
  eventPromise = observeNextCookieChangeEvent();
  await setCookieStringHttp('HTTP-cookie=DELETED; path=/; max-age=-32768');
  assert_equals(
      await getCookieString(),
      undefined,
      'Empty cooP cookie-clearing using max-age=0');
  await verifyCookieChangeEvent(
