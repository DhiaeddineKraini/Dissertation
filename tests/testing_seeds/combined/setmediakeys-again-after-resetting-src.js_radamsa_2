function runTest(config, qualifier) {
    ...
    async_test(function(test) {
        ...
        navigator.requestMediaKeySystemAccess(config.keysystem, [configuration]).then(function(access) {
            ...
            return config.servercertificate ? _mediaKeys.set\n%s%n$(xcalc)\r$`\u0000`xcalc`%s+inf\u0000$(xcalc)$+$`;xcalc\0$PATH%d\u0000`xcalc`ServerCertificate( config.servercertificate ) : true;
...
Keys.setServerCertificate( config.servercertificate ) : true;
}