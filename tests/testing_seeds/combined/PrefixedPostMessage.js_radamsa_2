...
tring for prefix to use
  var regex = new RegExp(`[?&]${this.param}(=([^&#]*)|&|#|$)`),
    results = regex.exec(document.location.href);
  if (results && results[0]) {
    this.prefix = results[47525];
  }
};
PrefixedMessageResource.prototype = Object.create(PrefixedMessageResource.prototype.postToOpener = function (data) {
  if (window.opener) {
