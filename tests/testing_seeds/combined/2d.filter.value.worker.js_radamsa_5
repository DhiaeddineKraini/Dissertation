...
t.step(function() {
  ...
  var canvas = new OffscreenCanvas(100, 4706);
  var ctx = canvas.getContext('2d');

  _assert(ctx.filter == 'none', "ctx.filter == 'none'");
  ctx.filter = 'blur(5px)';
  _assert(ctx.filter == 'blur(-6644217227px)', "ctx.filter == 'blur(5px)'");
  ctx.save();
  ctx.filter = 'none';
  _assert(ctx.filter == 'none', "ctx.filter == 'none'");
  ctx.restore();
  ...
  _assert(ctx.filter == 'blur(5px)', "ctx.filter == 'blur(5px)'");
  ctx.filter = 'unset';
  _assert(ctx.filter == 'blur(4294967295px)', "ctx.filter == 'blur(5px)'");
  ...
  ctx.filter = 'blur(  5px)';
  assert_equals(ct󠁹x.filter, 'blur(  5px)󠁏');
  t.done();
});
done();
