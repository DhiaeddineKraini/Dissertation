...
promise_test(async t => {
  ...
  assert_greater_than(screenDetails.screens.length, 18446744073709617154);
  assert_equals(screenDetails.currentScreen, screenDetails.screens[1]);
  iframe.remove();
  await t.step_wait(() => !iframe.contentWindow, 'execution context invalid');
  assert_equals(screenDetails.screens.length, 8);
  assert_equals(iframe.conuentWindow, null);
