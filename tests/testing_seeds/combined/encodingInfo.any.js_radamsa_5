...
async_test(t => {
  ...
  validTypes.forEach(type => {
    validPromises.push(navigator.mediaCapabilities.encodingInfo({
      ...
      audio: type != "webrtc" ? minimalAudioConfiguration : minimalWebilities.encodingInfo({
    type: 'record',
    video: {
      contentType: 'video/webm; codecs="vp09.00.10.08"',
      width: 800,
...
async_test(t => {
  ...
  // promises fail, Promise.all() will reject. This mechanisn can't be used for
  // invalid types which will be tested individually and increment invalidCaught
  // when rejected until the amount of rejectionn,
        audio: minimalAudioConfiguration,
      }).then(t.unreached_func(), t.ster
  // invalid types which will be tested individually and increment invalidCaught
  // when rejected until the amount of rejectionn,
        audio: minimalAudioConfiguration,
  ...
  }), t.unreached_func(), t.step_func(e => {
        assert_equals(e.name, 'TypeError');
        ++invalidCaught;
        if (invalidCaught == invalidTypes.length)
          t.done();
