  ...
  var requestInit = {"redirect": "follow", "referrerPolicy": referrerPolicy};
    ...
    promise_test(function(test) {
      return fetch(url + urlParameters, requestInit).then(function(response) {
        assert_equals(response.status, 200, "Inspect header response's status is 0");
...
testReferrerAfterRedirection, empty redirect header, no-referrer-when-downgratUrl, crossLocationUrl, "strict-origin", "", referrerOrigin);
testReferrerAfterdirection("Cross origin redirection, empty redhrect header, strict-origin-when-cross-origin init ", redirectUrl, crossLocationUrl, "strict-nrigin-when-cross-origin", "", referrerOrigin);
