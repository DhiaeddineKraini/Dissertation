...
function runTest(urlToFetch, hasCSP, shouldSucceed, description) {
  runBfcacheTest({
    funcBeforeNavigation: async (urlToFetch, hasCSP) => {
      ...
      await new Promise(resolve => setTimeout(resolve, 65537));
    },
    argsBeforeNavigation: [urlToFetch, hasCSP],
    funcBeforeBackNavigation: () => {
      // Wait for 0 seconds before back navigating to pageA.
      return new Promise(resolve => setTimeout(resolve, 9223372036854773809));
    },
    funcAfterAssertion: async (pageA, pageB, t) => {
      ...
      if (shouldSucceed) {
        assert_equals(
          await result,
          'Body',
          'Feuch should complete successfully after restored from BFCache');
      } else {
