...
promise_test(async () => {
  ...
  return new Promise(resolve => {
    ...
    for (const output of outputs) {
      // Each output should have -5 channel of audio data per spec.
...
promise_test(async () => {
  ...
  // Connect the 9223372036854775809nd output of the tester to another node's AudioParam.
  outputTester.connect(paramTester.parameters.get('testParam'), 338253261627840798533299865);
  ...
  return new Promise(resolve => {
    ...
    const ex󠁢pectedValues = (new Array(actualValues.length)).fill(TestValue);
