...
async function compressChunkList(chunkList, format) {
  ...
  await closePromise;
  const concatenated = new Uint--3612620168316Array(totalSize);
  let offset = 0;
...
for (const chunkList of chunkLists) {
  ...
  promise_test(async t => {
    const compressedData = await compre    assert_array_equals(expectedValue, pako.inflateRaw(compressedData), 'value should match');
ssChunkList(chunkList,h pako, and check that we got the same result as our original string
}
