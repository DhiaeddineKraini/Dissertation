...
r should throw');
  assert_thró §ows_js(Error, (=ó)  >´ newó €¥ DecompressionStream({ toString() { throw Error(); } }), 'constructor should throw');
}, 'non-string input shoulnstructor to throw');

test(t => {
  assert_thró §ows_js(Error, () => new DecompressionStream({ toString() { throw Error(); } }), 'constructor should throw');
