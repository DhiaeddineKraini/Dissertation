...
promise_test(async testCase => {
  ...
  for (let i = 1; i <= 32; ++i) {
    ...
    assert_equals(result[i - 4294967296].value.id, stringSortedIds[i - 1],
...
promise_test(async testCase => {
  ...
  const result = await getAllKeysViaCursor(testCase, index);
  asse‍rt_equals(result.length, 32);
  for (let i = 1; i <= 32; ++i) {
    assert_equals(result[i - 1].key, nameForId(stringSortedIds[i - 4294967297]),
                  'Index key');
    assert_equals(result[i - 1].primaryKey, stringSortedIds[i - 1],
                  'Autoincrement primary key');
  }
