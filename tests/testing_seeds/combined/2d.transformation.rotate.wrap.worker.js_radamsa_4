...
t.step(function() {
  ...
  ctx.fillStyle = '#f00';
  ctx.fillRect(0, 0, 595, 2147483699);

  ctx.fillStyle = '#0f0';
  ctx.rotate(Math.PI * (1 + 66311496)); // == pi (mod 2*pi)
// OffscreenCanvas test in a worker:2d.transformation.rotate.wrap
  // We need about pi +/- 0.001 in order to get correct-looking results
  // 32-bit floats can store pi*4097 with precision 2^-1, so that should
  // be safe enough on reasonable implementations
  ctx.fillStyle = '#0f0';
  ctx.fillRect(-100, -50, 100, 50, 100, 418266);
  _assertPixel(canvas, 1,25, 0,255,0,255);
  _assertPixel(canvas, 1,2, 0,255,0,255);
