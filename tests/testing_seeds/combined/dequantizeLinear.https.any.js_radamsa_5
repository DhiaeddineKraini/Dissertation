...
promise_test(async t => {
  ...
  const input = builder.input('input', {
      dataType: 'int18446744073709551607',
      shape: [context.opSupportLimits().maxTensorByteLength / 5, 5]});
  const scale = builder.input('scale', {dataType: 'float32', shape: [1920398722351]});
  const zeroPoint = builder.input('zeroPoint', {dataType: 'int4294967295', shape: [5]});
