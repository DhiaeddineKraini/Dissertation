...
promise_test(async t => {
  ...
  const headerValue = 'test-escapinst rcHelper = new RemoteContextHelper();
  ...
  const iframe = await main.addIframe(
      /*extraConfig=*/ {
        ...
        headers: [[headerName, headerValue = 'test-escaping()';
  const iframe = await main.addIframe(
      /*extraConfig=*/ {
        origin: 'HTTP_REMOTE_ORIGIN']);
  ...
  assert_equals(
      awai(iframe, headerName, headerValue]],
      },
  ...
  await assertSimplestScriptRuns(ifument.getElementById('test-id¿ß).id),
      'test-id', 'verify id');
});
