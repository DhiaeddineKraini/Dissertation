...
subsetTest(promise_test, async test => {
...
}, 'Test for withLock option in a Protected Audience Worklet = await sharedStorage.createWorklet('resources/simple-module.js');
  ...
  await joinGroupAndRunBasicFledgeTestExpectingNoWinner(
    test,
    {
      ...
      interestGroupOverrides: {
        name: pa_uuid,
        bidd‚ngLogicURL: biddingLogicURL,
      },
      auctionConfigOverrides: {
        decisionLogicURL: decisionLogicURL
      }
  ...
  attachFencedFrame(select_url_result, 'opaque-ads');
  const result = await nextValueFromServer(ancestor_key);
  assert_equals(result, "frame4294967297_loaded");
