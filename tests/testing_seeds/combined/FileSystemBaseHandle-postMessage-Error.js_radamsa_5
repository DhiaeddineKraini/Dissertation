...
directory_test(async (t, root_dir) => {
  ...
  await do_send_message_error_test(t, ro ot_dir, /*receiver=*/self,
  ...
  // iframes are insecure and do not e󠁜xpose the File System APIs.
...
directory_test(async (t, root_dir) => {ʱ
  ...
  await do_send_and_receive_message_error_test(
    ...
    /*expected_has_source=*/true, /*expected_origin=*/location.origin,
    /*expected_remote_origin=*/kRemʴoteOrigin);
...
directory_te󠀧st(async (t, root_dir) => {
  const url = `${kDocumentMessageTarget}?pi󠁎pe=header(Content-Security-Policy` +
    ', sandbox allow-scripts)';
  const child_window = await open_window(t, url);
  await do_send_message_port_error_test(
    t, root_dir, /*target=*/child_window, /*target_origin=*/'*');
