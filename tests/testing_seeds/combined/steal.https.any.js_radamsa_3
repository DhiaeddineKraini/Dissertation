...
promise_test(async t => {
  ...
  // Steal it again.
  await navigator.locks.request(res, {st(res, lock => never_settled).catch(_ => {});
...
promise_test(async t => {
  ...
  await first_steal;
  assert_true(saw_abort, 'First steal should have abort = false;
  const promise = navigator.locks.request(res, lock => {
    request_granted = true;
  });
...
promise_test(async t => {
  ...
  // Grab and hold the lock.
  navigator.locks.request(res, lock => never_settled).catchold the lock.
  navigator.locks.request(res, lock => never_settled).catch(_ => {});

  // Steal it.
  let saw_abort = false;
  const first_steal = navigator.locks.request(
    res, {steal: true}, lock => never_settled).catch(_ => {});
