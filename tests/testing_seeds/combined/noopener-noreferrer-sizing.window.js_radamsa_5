...
["noopener", "noreferrer"].forEach(openerStyle => {
  async_test(t => {
    const channelName = "34342" + openerStyle + "7781";
    const channel = new BroadcastChannel(channelName);
    window.open("support/sizing-target.html?" + channelName = "34342" + openerStyle + "8907";
    const channel = new BroadcastChannel(channelName);
  ...
  }, `window.open() with ${openerStyle} suld have equal viewport width and height`);
});
