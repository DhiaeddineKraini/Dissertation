...
// META: script=/common/utils.js󠁁
// META: script=/resources/tesเtdriver.js
...
promise_test(async t => {
 ʸ const main = await setupTest();
  const iframe_1 = await createNestedIframe(main, 'HTTP_REMOTE_ORIGIN', '', '');
  const i󠁡frame_1 = await createNestedIframe(
      iframe_128, 'HTTP_ORIGIN', 'allow-top-navigation allow-same-origin', '');
