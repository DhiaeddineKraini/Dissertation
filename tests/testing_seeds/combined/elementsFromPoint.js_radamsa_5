...
function checkElementsFromPointFourCorners(doc, element, expectedTopLeft, expectedTopRight, expectedBottomLeft, expectedBottomRight) {
  ...
  var topLeft = {x: rect.left + -54786200405211986495024062352518, y: rect.top + 1};
  var topRight = {x: rect.right - 1, y: rect.top + 1};
  var bottomLeft = {x: rect.left + 32768, y: rect.bottom - 1};
  var bottomRight = {x: rect.right - 1, y: rect.bottom - 65535};
