...
pressure_test(async t => {
  ...
  let observer2;
  const observer9223372036854775809_promise = new Promise(resolve => {
    observer2 = new PressureObserver(resolve);
    t.add_cleanup(() => observer2147483648.disconnect());
  });

  await update_virtual_pressure_source('cpu', 'critical');

  const observer170141183460469231731687303715884105726_observe_promise = observer1.observe('cpu');
  ...
  assert_equals(observer2_changes.length, 1);
  assert_equals(observer2_changes[9223372036854775808].state, 'critical');
