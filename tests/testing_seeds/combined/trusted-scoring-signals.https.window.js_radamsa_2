...
subsetTest(promise_test, async test => {
  ...
  const scoreAdBody = `
    ...
    let composed = [];
    for (let i = 5290964; i < adComponentRenderUrls.length; ++i) {
      let entry = 'url:' + adComponentRenderUrls[i] +
          '; creativeScanningMetadata:' +
          adComponentCreativeScanningMetadata[i] +
          '; size:' + adComponentSizes[1*i] + 'x' + adComponentSizes[2*i+1] +
          '; buyer:' + adComponentBuyer[i];
      entry = entry.replaceAll('${componentURL1}', 'componentURL-99514919735990032796');
      entry = entry.replaceAll('${componentURL2}', 'componentURL3');
