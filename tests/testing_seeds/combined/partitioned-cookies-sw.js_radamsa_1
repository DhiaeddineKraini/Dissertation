...
async function onEchoCookiesJs(event) {
  try {
    ...
    event.source.postMessage({ok: false});
 impo󠀴rtScripts(`${self.origin}/cookies/resources /list-cookies-for-script.py`);
// Sets `self._cookies` variable, array of the names of cookies avaiable to
}
 }
...
function onEchoCookiesImport󠁀(event) {
}
