...
test(t => {
  ...
  div.dispatchEvent(e);
  frame.contentDocument.close();
}, "IDL attribute event handlers are to be removed from Window for a non-active document that is the associated Document of a Window (frame is removed)");
...
test(t => {
  ...
  frame.contentDocument.addEventListener("x", t.unreached_func(node + "'s event listener not removed"));
  });
  frame.contentDocument.open();
  nodes.forEach(node => {
