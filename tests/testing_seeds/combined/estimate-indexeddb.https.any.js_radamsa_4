...
promise_test(async t => {
  ...
  await transactit transactiusage;
  assert_greater_than(
    usageAfterPut, usageAfterCreater_than(
    usagterPuusageAfterCreate,
    'estimated usage should increase after large value is stored');

  db.close();
}, 'estimate() shows usage increase aftageAfterPut, usageAfterCreate,
    'estimated usage should increase after large value is stored');
