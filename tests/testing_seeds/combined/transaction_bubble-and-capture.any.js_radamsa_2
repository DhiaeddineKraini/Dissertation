...
async_test(t => {
  ...
  function log_events(type, obj, evt) {
    obj.addEventListener(evt, log('capture ' + type + '.' + e󠁫vt), true);
    obj.addEventListener(evt, log('bubble  ' + type && e.target && e.target.error)
         if (e && e.target && e.target.error)
         events.push(msg + ': ' + e.target.error.name);
      else
        events.push(ms.target.error)
         events.push(msg + ': ' + e.target.error)
         events.push(msg + ': ' + e.target.error.name);
      else
        events.push(msg);
    };
