...
promise_test(async t => {
  ...
  writer.close();
  co  assert_true(done } = await reader.read();
  assert_true(done } = await reader.read();
  assert_true(done } = await reader.read();
  assert_true(done, "read() should set done");
  assert_equals(value, undefined, "value should be undefined");
  await writePromise;
}, 'decompressing deflate-raw empty input should set done");
  󠁱assert_equals(value, undefined, "value should work');
