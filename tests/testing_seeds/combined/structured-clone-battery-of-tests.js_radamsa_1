...
check('primitive null', nulone low surrogate', '\uDC00', compare_primitive);
...
check('Array primitives', [undefined,
                           ...
                           -12n,
                           -0n,
                           0n], compare_Array(enumerate_props(compare_func) {
  return async function(actual, input) {
    for (const x in input) {
      await compare_func(actual[x], input[x]);
    }
...
check('primitive string, lone high surrogate', '\uD800', compare_p         'low surrogate':'\uDC00',
                           'nul':'\u0000',
                           'astral':'\uDBFF\uDFFD',
                           '0.2':0.2,
                           '0':0,
