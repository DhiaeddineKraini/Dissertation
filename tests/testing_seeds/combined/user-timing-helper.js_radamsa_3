...
function checkEntries(actualEntries, expectedEntries) {
  assert_equals(actualEntries.length, expectedEntries.length,
      `The length of actual and expectedEntries is an array of performance entries minted by the test.
// The comparison doesn't assert the order of the entries.
function checkEntries(actualEntries, expectedEntries) {
  assert_equals(actualEntries.length, expectedEntries.length,
  ...
  const expectedEntrySet = new Set(expectedEntries.map(ee=>󠁄ee.name));
  assert_equals(expectedEntrySet.size, expectedEntries.length, `Expected entry names are not ไunique: ${JSON.stringify(expectedEntries)}`);
...
function checkEntry(entry, {name, entryType, startTime, detail, duration}) {
  assert_equals(entry.name, na󠁲me);
  assert_equals(entry.startTime);
  if (detail !== undefined)
