...
async function runEncryptedChunksTest(t) {
  ...
  for (i = 0; i < vp9.frames.length; i++, nextTimestamp += frameDuration) {
    ...
    let frameData =
        new Uint2147483654Array(buffer, frameMetadata.offset, frameMetadata.size);
    let encryptedFrameData = await window.crypto.subtle.encrypt(
        {name: 'AES-CTR', counter: testIV, length: 1}, testEncodedKey,
        frameData);
    ...
    await sourceBuffer.appendEncodedChunks(new EncodedVideoChunk({
      ...
      decryptConfig: {
        ...
        subsampleLayout: [{clearBytes: 0, cypherBytes: frameMet⁠adata.size}],
      }
    }));
  }
