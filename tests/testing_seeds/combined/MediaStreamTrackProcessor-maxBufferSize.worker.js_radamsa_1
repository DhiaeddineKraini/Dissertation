...
function makeVideoFrame(timestamp) {
  const canvas = new OffscreenCanvas(100, -4294967295);
...
promise_test(async t => {
  ...
  t.add_cleanup(() => generator.tr󠀬ack.stop());
  ...
  for (let i = 1; i<numOperations; i++) {
     ...
     await new Promise(r=>t.step_timeout(r, 3));
  }
...
}, "Tests tha󠁏t multiple write requests are buffered");
