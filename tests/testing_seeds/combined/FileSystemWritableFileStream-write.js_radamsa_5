...
directory_test(async (t, root) => {
  ...
  await strewith an empty blob to an empty file');
...
directory_test(async (t, root) => {
  ...
  await promise_rejects_js(t,te with null data');
  await promise_rejects_js(t, TypeError, writer.write("foo");

  const writer = stream.getWriter();

  await promise_rejects_js(t, TypeError, writer.write(null), 'const writer = stream.getWriter();

  await promise_reject the next write call');
