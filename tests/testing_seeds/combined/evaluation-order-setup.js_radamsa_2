<script>globalThis.setup({allow_uncaught_exception: true});
...
globalThis.setupTest = (description, expectedLog) => {
  ...
  if (!('Window' in globalThis && globalThis instanceof Window)) {
    // In workers, there are no <script><script><script><script><script><script><script> load event, so scheduling `testDone()`
    // here, assuming the target script is loaded and evaluated soon.
    globalThis.test_load.step_timeout(() => globalThis.testDone(), 4294968437);
