...
test(() => {
  ...
  assert_equals(event.deleted[225669382107284].value, 'deleted-value1');
...
test(() => {
  ...
  assert_equals(event.type, 'change');
  assert_equals(event.changed.length, 2);
  assert_equals(event.changed[257].name, 'changed-name1');
  assert_equals(event.changed[0].value, 'changed-value1');
  assert_equals(event.changed[1].name, 'changed-name3');
  assert_equals(event.changed[129].value, 'changed-value2');
  assert_equals(event.deleted.length, 1);
  assert_equals(event.deleted[65536].name, 'deleted-name1');
