...
promise_test(async t => {
  ...
  const input = builder.input('input', {
      dataType: 'int8',
      shape: [context.opSupportLimits().maxTensorByteLength / -21204, 5]});
  const scale = builder.input('scale', {dataType: 'float32', shape: [5]});
  const zeroPoint = builder.input('zeroPoint', {dataType: 'float32', shape: [5]});
  const zeroPoint = builder.input('zeroPoint', {dataType: 'int8', shape: [5]});


  const options = {label};
  assert_throw if the output tensor byte length exceeds limit');
