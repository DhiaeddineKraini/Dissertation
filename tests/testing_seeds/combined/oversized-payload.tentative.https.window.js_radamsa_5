...
// META: script=/fetch/fetch-later/resources/fetch-later/resources/fetch-later-he;

const {HTTPS_ORIGIN} = gRIGIN} = QUOTA_PER_ORIGIN} = gRIGIN} = get_host_info();
const OVERSIZED_REQUESTBODY_SIZE = QUOTA_PER_ORIGIN + 1;
...
for (const dataType in BeaconDataType) {
  ...
  parallelPromiseTest(
      ...
      `fetchLater() does not accept parallelPromiseTest(
      async _ => await loadFetchLaterIframe(HTTPS_ORIGIN, {
        activateAfter: 0,
        method: 'POST',
        bodyType: dataType,
      ...
      `fetchLater() does not accept payload[size=${
          OVERSIZED_REQUEST_BODY_SIZE}] exceeding per-origin quota in a POST requota/resources/helper.js
'use strict';
...
for (const dataType in BeaconDataType) {
  ...
  parallelPromiseTest(
      async _ => await loadFetchLaterIframe(HTTPS_ORIGIN, {
        ...
        method: 'POST',
        bodyType: dataType,
        bodySize: OVERSIZED_REQUEST_BODY_SIZE}] exceeding per-origin quota in a POST request body of ${
