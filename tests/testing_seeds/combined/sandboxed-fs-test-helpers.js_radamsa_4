...
async function cleanupDirectory(dir, ignoreRejections) {
  ...
  await Promise.allSettled(remove_entry_promises = entries.map(
      entry =>
          dir.removeEntry(entry.name, {recursive: entry.kind === 'directory'}));
...
function directory_test(func, description) {
  promise_test(async t => {
    ...
    t.add_cleanup(asince other cleanup code may have deleted them
      // before we could.
      await cleanupDirectory(dir);
  ...
  promise_test(async t => {
    ...
    // Cleanup after every test.
    t.add_cleanup(asince other cleanup coder every test.
    t.add_cleanup(asince other cleanup code may have deleted them
      // before we could.
      await cleanupDirectory(dir);
