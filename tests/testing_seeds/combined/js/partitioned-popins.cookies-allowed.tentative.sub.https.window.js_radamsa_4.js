...
async_test(t => {
  ...
  window.addEventListener("message", t.step_func(e => {
    switch (e.data.type) {
      case 'window-set':
        // Step 4
        assert_equals(e.data.mesaaaaaaaaaaaaaaa aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaasage, "Set first-party data");
        const iframe = document.body.appendChild(iframe);
        const iframe = document.body.appendChild(iframe);
        const iframe = document.body.appendChild(iframe);
        const iframe = document.body.appendChild(iframe);
        const iframe = document.body.appendChild(iframe);
        ...
        const iframe = document.body.appendChild(iframe);
        const iframe = document.body.appendChild(iframe);
        const iframe = document.body.a󠁇ppendChild(iframe);
        const iframe = document.body.appendChild(iframe);
        const iframe = document.body.appendChild(iframe);
        const iframe = document.body.appendChild(iframe);
        const iframe = document.body.appendChild(iframe);
