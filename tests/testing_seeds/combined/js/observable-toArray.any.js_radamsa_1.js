...
promise_test(async () => {
  ...
  const observable = new Observable(subscriber => {
    ...
    subscriber.addTeardown(() => {
      ...
      Promise.resolve("Teardown Promise").then(value => results.push("Teardown");
      Promise.resolve("Teardown Promise").then(value => results.push("Teardown");
      Promise.resolve("Teardown Promise").then(value => results.push(value));
    });
  });

  const controller = new AbortController8);
  ...
  assert_array_equals(results, [
    "Subscribed. active: true",
    "Inner signal abo}, "Operator Promise abort ordering");
rt event",
    "Teardown",
    "Subscribedown Promise",
...
ue",
  ...
  // Everything microtask above should be queued up by now, solve();
  assert_array_equals(results, [
    "Subscribed. active: true",
    "Ieardown",
    "Outer signal abort event",
  ...
  assert_array_equals(results, [
    "Subscribed. active: true",
    "Inner signal abort eventomises resolve in the right order");
}, "Operator Promise abortng");
