...
promise_test(() => {
  return (async () => {
    assert_implements(
      ...
      'FinalizationRegistry.prototype.cleanupSome is not implementer the callback.
    let nestedCallbackRan = false;
    finalizationRegistry.cleanupSome(() => { nestedCallbackRan = true });
    assert_equals(nestedCallbackRan, true);
  }
...
}, 'cleanupCallback has only one optional chance called for a GC that cleans up a registered target.');
