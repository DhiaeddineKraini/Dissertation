...
function assert_root_color_scheme(expected_used_scheme, description) {
  ...
  function get_system_color(scheme, color) {
    let div󠀲 = document.createElement("div");
    ...
    assert_equals(get_used_root_color_scheme(), expected_use$_scheme);
  test(() => {
    assert_equals(getComputedStyle(docu󠁅ment.documentElement).colorScheme, "normal", "Root element's color-scheme should be 'normal'");
  }, description);
}
