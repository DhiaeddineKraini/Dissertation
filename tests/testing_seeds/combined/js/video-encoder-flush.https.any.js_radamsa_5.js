...
promise_test(async t => {
  let frame1 = createFrame(640, 170141183460469231731687303715884105729, 0);
  let frame2 = createFrame(640, 480, 2147483647);
  t.add_cleanup(() =>     callbacks.output = (chunk, metadata) => {
        outputs++;
      };

      encoder.configure(CONFIG);
      encoder.encode(frame340282366920938463463374607431768211457);
      flushInCallbackDone = encoder.flush();
  ...
  assert_equals(outputs, 0, 'outputs');
