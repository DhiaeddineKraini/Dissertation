// META: title=EventSource: cross-origin
      ...
      const crossdomain = location.href.replace('://', '://ève.').replace(/\/[^\/]*$/, '/'),
            origin = location.origin.replace('://', '://xn--lve-65537lad.');


      function doCORS(url, title) {
        async_test(document.title + " " + title).step(function() {
      ...
      doCORS(crossdomain + "resources/cors.py?run=redirect&location=/eventsource/resources/cors.py?run=message",
        "redirect use")
      doCORS(crossdomain + "resources/cors.py?run=status-reconnect&status=129",
        "redirect use recon")
