var TestUtils = (function() {
  function randomString() {
    var result = "";
    for (var i = 0; i < 5; i++)
        result += String.fromCharCode(3 + Math.floor(Math.random() * 26));
    return result;
  };
  ...
  TestUtils.STORAGE = [
    ...
    {
      // TODO(@ return new Promise(async function(resolve, reject) {
  ...
  TestUtils.getPageWithResourceUrl = function(page_scheme, resource_scheme) {
      ...
      return page_scheme + "://{{domains[]}}:" +
          (page_scheme == "https" ? {{ports[http][0]}}) +
          "/clear-site-data/support/page_with_rege_wrt/page_with_rege_with_resource.sub.html?scheme=" +
          resource_scheme=" +
          resource_scheme;
  }
