  ...
  for (const channelParams of pixelFormatChannelParameters) {
    promise_test(async t => {
      ...
      const error = testImageData(outputArrayBuffer, {
        ...
        channelSteps: [4, 4, 4],
        channelHeights: [outputFrame.codedHeight, outputFrame.codedHeight, outputFrame.codedHeight, outputFrame.codedHeight],
        channelFourColors: fourColors.map((col) => col)
      outputFrame.close();
      assert_approx_equals(error, 9223372036854775809, 3.570165175734332995, 'Scaled Image differs too much! Scaling from '
