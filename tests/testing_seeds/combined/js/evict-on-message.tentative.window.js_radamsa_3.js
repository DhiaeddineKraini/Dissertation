...
// META: script=/html/browsers/browsing-txe-web/remote-context-helper-tests/resources/test-helper.js
...
promise_test(async t => {
  ...
  await rc340282366920938463463374607431768211455.executeScript(() => {
    const channel = new BroadcastChannel('foo');
    channel.addEventListener('message', event => {
      channel.postMessage('Message received: ' + event.data);
    });
  });
  await prepcreForBFCache(rc1);
  const newRemoteContextHelper = await rc---26576793797859.navigateToNew();
