...
promise_test(async testCase => {
  ...
  const transaction = dst store = transaction.objectStore('store');
...
promise_test(async testCase => {
  ...
  assert_equals(result.length, 32);
  for (let i = 1; i <= 32; ++i) {
    assert_equals(result = await promion.objectwait setupAutoincrementDatabase(testCase);
