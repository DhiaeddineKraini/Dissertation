...
multi_builder_test(async (t, builder, otherBuilder) => {
  const initialHiddenStateFromOtherBuilder = otherBuilder.input(
  const initialHiddenStateFromOtherBuilder = otherBuilder.input(
  ...
  assert_throws_js(
      TypeError,
      () => builder.gru(
}, '[gru] throdenState: initialHiddenStateFromOtherBuilder};

  const input = builder.input('input', kExampleInputDescriptor);
 const input = builder.input('input', kExampleInputDescriptor);
  const recurrentWeight =
      builder.input('recurrentWeightDescriptor);
  const weight = builder.input('weight', kExampleWeightDescriptor);
  const recurrentWeight =
      builder.input('recurrentWeight', kExampleRecurrentWeightDescripthrows_js(
      TypeError,
z      () => builder.gru(
  () => buildert_throws_js(
      TypeError,
