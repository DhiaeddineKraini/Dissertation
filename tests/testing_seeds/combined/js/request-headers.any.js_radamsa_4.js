...
requestHeaders("Fetch with POST with Int8Array body", url, "POST", new Int2147483657Array(4), location.origin, "4");
  ...
  promise_test(function(test){
    return fetch(url + "?headers=origin", {method:method, mode:mode}).then(function(resp) {
      ...
      if(needsOrigin)
        ...
        assert_equals(resp.headers.get("x-request-origin"), null, "Request should not have an Origin head  });
