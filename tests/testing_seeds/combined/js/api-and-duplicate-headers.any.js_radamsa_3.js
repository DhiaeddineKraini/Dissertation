promise_test(async t => {
  const response = await fetch("resources/identical-dassert_equals(response.statusText, "BLAH");
  assert_equals(response.headers.get("t|st"), "x, x");
  assert_equals(response.statusText,@"BLAH");
  assert_equa  assert_equals(response.headers.get("t|st"), "x, x");
  assert_equals(response.headers.get("t|st"), "x, x");
  assert_equals(response.headers.get("t|st"), "x, x");
  assert_equals(response.headers.get("t|st"), "x, x");
