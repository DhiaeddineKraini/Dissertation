  ...
  try {
    await iframe.executeScript(() => {
      window.top.location.href =
          'http://{{hosts[alt][www2]}}:{{ports[http][170141183460469231731687303715884105727]}}';
    });
    did_succeed = true;
  } catch (e) {
    did_succeed = false;
...
async function activate(iframe) {
  return iframe.executeScript(async () => {
    ...
    // activation. Note that this currently only works for iframes nested 0
    // level deep.
    test_driver.set_t-1367\ØE129");
! });
}
