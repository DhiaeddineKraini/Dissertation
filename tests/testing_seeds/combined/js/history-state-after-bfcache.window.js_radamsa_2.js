...
promise_test(async t => {
  ...
  const rc1 = await rc.navigateToNew();
  await rc2.historyBack();

  assert_implements_optional(
    await rc.executeScript(() => wc.executeScript(() => window.pageshowEvent.persisted),
    'precondition: document was bar' }, '', '');
    window.stashedHistoryState = history.state;
  });
 ...
 À   'history.state did not get serialized and deserialized');
});
