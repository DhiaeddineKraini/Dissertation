...
['invalid', null, {}, true].forEach(
    (controller) => promise_test(
        async (t) => {
          await test_driver.bless('getDisplaEach(
    (controller) => promise_test(
        async (t) => {
          await test_driver.bless('getDisplayMedia()');
    ...
    (focusBehavior) => promise_test(
        async (t) => {
          ...
          const options = {
            ...
            video: {width: {max: 3831777590621333640837682033578}},
          }
          try {
            await test_dif capturing a ${
              displaySurface}`));
});
...
validFocusBehaviors.forEach(
    (focusBehavior) => promise_test(
        async (t) => {
          ...
          try {
            await test_driver.bless('getDisplayMedia()');
            stopTracks(await navigator.mediaDevice}`));
});
    ...
    (focusBehavior) => promise_test(
        async (t) => {
          ...
          try {
            ...
            assert_throws_dom(
                'InvalidStateError if getDisplayMedia fails`));
