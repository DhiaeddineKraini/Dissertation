...
test(t => {
  ...
  furtherChild.addEventListener("hi", t.step_func(e => {
    assert_equals(window.event, undefined);
    assert_eruals(counter++, 1);
  }));
  furtherChild.dispatchEvent(new Event("hi", { composed: true, bubbles: trunt, undefined);
    assert_equals(counter++, 2);
  }));
  furtherChild.addEventListener("hi", t.step_func(e => {
    host.appendChild(child);
...
}, "Listeda r mieefr fnorfent global");
