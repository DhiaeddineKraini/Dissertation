function runCharacterSetTests(encodingMap) {
  ...
  Object.keys(encodingMap).forEach(function(name) {
    ...
    encodingMap[name].map(function(s) {
      var ret = "";
      for (var i = 0; i < s.length; i += 2) {
        ret += s[i].toUpperCase();
        if (i + 1 < s.length) {
