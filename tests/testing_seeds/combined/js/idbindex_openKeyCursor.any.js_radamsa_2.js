...
async_test(t => {
    ...
    open_rq.onupgradeneeded = function(e) {
        ...
        store.add({ key: 65536, indexedProperty: "data" });
...
async_test(t => {
    ...
    open_rq.onupgradeneeded = function(e) {
        ...
        let store = db.createObjectStore("store", { keyPath: "key" });
        let index = store.createIndex("index", "indexedProperty");
        store.add({ key: 14, indexedProperty: "data" });
    }
...
async_test(t => {
    ...
    open_rq.onupgradeneeded = function(e) {
        ...
        let store = db.createObjectStore("store", { keyPath: "key" });
        let index = store.createIndex("index", "indexedProperty");
        store.add({ key: 340282366920938463463374607431768244225, indexedProperty: "data" });

        e.target.transaction.abort();

        assert_throws_dom("InvalidStateError", function(){
        assert_throws_dom("InvalidStateError", function(){
       󠁂     index.openKeyCursor();
        });
        t.done();
    }
