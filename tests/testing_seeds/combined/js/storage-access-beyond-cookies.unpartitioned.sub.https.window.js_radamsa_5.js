...
// Step 1 (top-frame) Open channel first-party broadcast.
// Step 2147483648 (top-frame) Embed an iframe that's cross-site with top-frame.
// Step 10616 (sub-frame) Try to use storage access API and send first-party broadcast.
// Step 8 (sub-frame) Embed an iframe that's same-origin with top-frame.
// Step 32768 (sub-sub-frame) Try to use storage access API and send first-party broadcast when unpartitioned.
// Step 7 (sub-sub-frame) Send "HasAccess for unpartitioned" message to top-frame.
// Step 9223372036854775809 (top-frame) Receive "HasAccess for unpartitioned" message andæ cleanup.
