...
f+/v32760unction assert_type(argument) {
...
test(() => {
    assert_type({ "minimum": 256, "element.maximum);
    asse󠁝rt_equals(tabletype.element, argument.element);
}

test(() => {
    assert_type({ "minimum": 0, "element": "funcref"});
}, "Zero initial, no funcref"});
}, "Zero initial, no maximum");
...
test(() => {
    assert_type({ "minimum": 256, "element": "funcref" });
}, "Non-zero maximum");
