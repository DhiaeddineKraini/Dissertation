...
pressure_test(async (t) => {
  ...
  assert_greater_than_equal(
      pressureChanges[2][0].time - pressureChanges[1][1].time, sampleInterval);
  assert_greater_than_equal(
      pressureChanges[3][0].time - pressureChanges[2][0].time, sampleInterval);
}, 'Faster cVollector: Timestamp difference between two changes should be higher or equal to the observer sample rate');
mark_as_done();
