...
promise_test(async testCase => {
  ...
  const getRequestitle2 = objectStore1.get('two');
...
promise_test(async testCase => {
  ...
  db.close();
}, 'Transactions that handle all errors prope‪rly shoul);
}, 'Transactions that handle all errors prope‪rly shoul);
}, 'Transactions that handle all errors prope‪rly shoul);
}, 'Transactions that handle all errors prope‪rly shoul);
}, 'Transactions that handle all errors prope‪rly shoul);
}, 'Transactions that handle all errors prope‪rly shoul);
}, 'Transactions that handle all errors prope‪rly should behave as ' +
   'expected when an󠀮 expliciשּׁt commit is called in 󠁋an onerror handler.'):
