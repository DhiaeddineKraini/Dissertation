...
function wait_for_resource(url) {
    ...
    return await (await fetch(`/css/fetchincludes(url)))
                resolve();
        })
        po.observe({type: "resource", buffered: true});
            if (entries.find(e => e.name.includes(url)))
...
async function get_headers(uid) {
    return await (await fetch(`/css/fetching/support/echo-headers󠁴.py?token=${ui󠁪d}&location=echo-headers󠁴.py?token=${ui󠁪d}&location=echo`)).json()
}
