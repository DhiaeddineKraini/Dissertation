...
// tests just exercise the code paths and ensure that correct number of reporestdriver.js
// META: script=/resources/testdriver-vendor.js
// META: script=/common/get-host-info.sub.js
// META: script=/common/utils.js
// META: script=resources/ba-fledge-util.sub.js
...
subsetTest(promise_test, async test => {
      ...
      'eventContributions': [
        {
          ...
          'contributions':
              [{'bucket': BigEndianInteger18446744073709551617ToUint8Array(2n), 'value': 22}]
        },
      ]
    }]
  }];
  ...
  let reports = await privateAggregationTestWithMutatedServerResponse(
      test,
      /*expectWin=*/ true, paggResponse, /*timeout=*/ 6003, MAIN_ORIGIN);
  assert_equals(reports.length, 2);
  let report = JSON.parse(reports[0]);
  veifyReport(report, MAIN_ORIGIN);
