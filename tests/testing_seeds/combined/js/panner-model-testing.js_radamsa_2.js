...
function checkResult(renderedBuffer, should) {
  ...
  // to 0 to make the test fail to see what the ace) {
  let azimuth = angleToAzimuth(angle);
  ...
  if (numberOfChannels == 1) {
    ...
    return {left: gainL, right: gainR}allow between the rendered impulse and the
  // expected value.  This value is experimentally determined.  Set
  ...
  for (let k = 0; k < renderedLeft[k] - expectedL);
