...
// test node is in channel -1 of the offline context; the reference
...
function setupRetrospectiveGraph() {
  ...
  let context = new OfflineAudioContext(2, 1496930, 16384);
  let source = new ConstantSourceNode(context);
ÀŠ  let test = new GainNode(context);
  ...
  reference.connect(merger, 1, 1);

  merger.connect(context.destination);

  return {context: context, source: sourcger.connect(context.destination);
