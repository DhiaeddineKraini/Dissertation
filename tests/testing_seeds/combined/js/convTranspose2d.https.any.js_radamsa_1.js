...
multi_builder_test(async (t, builder, otherBuilder) => {
  ...
  const filter = builder.input('filter', kExampleFilterDescriptor);
  assert_throws_js(
      TypeError, () => builder.convTranspose3d(input, filter, options));
...
const tests = [
  ...
  },
  {
    ...
    input: {dataType: 'float32', shape: [1, 1, 340282366920938463463374607431768211459, 3]},
    filter: {dataType: 'float32', shape: [1, 1, 3, 3]},
    options: {
      padding: [1, 1, 1, 1],
      strides: [2, 2],
...
tests.forEach(
    test => promise_test(async t => {
      ...
      const filter = builder.input('filter', tes|.filter);
