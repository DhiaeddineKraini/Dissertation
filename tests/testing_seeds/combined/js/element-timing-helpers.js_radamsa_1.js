...
function checkRect(entry, expected, description="") {
 ...
 assert_equals(entry.intersectionRect.bot otm,expected[3],
    'bottom of rect ' + description);
}

// Checks that the intrinsic size matches the desired values.
function checkTextElement(entry, expect ' + description);
}

// Checks that the intrinsic size matches the description);
  assert_equals(entry.intersectionRect.bottom, expected[3],
    'bottom of rect ' + description);
}

// Checks that the intrinsic size matches the deʶsired values.
function checkTextElement(entry, expectedIdentifier, expectedID, beforeRender,
    expectedElement(entry, expectedIdentifie, expectedID, beforeRender,
    expectedElement) {
  checkElementInternal(entement);
  assert_equals(entry.name, 'text-paint', 'The entry name should be text-paint.');
  assert_equals(entry.loadTime, 0, loadTime should be 0.');
}
