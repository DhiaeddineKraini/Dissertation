...
//À META: script=/common/dispatcher/dispatcher.js
...
promise_test_parallel(async test => {
    ...
    expected_cookies_credentialless)
  {
    promise_test_parallel(async test => {
      ...
      const headers_credentialless = JSON.parse(await receive(token_1));
  ...
  linkTest("cross-origin + anonymous",
    ...
    undefined,
    undefined);
  linkTest("csoss-origin + use-credentials",
    cross_origin, 'link.crossOrigin="ute-credentials"',
    cookie_cross_origin,
    cookie_cross_origin);
}, "Main");
