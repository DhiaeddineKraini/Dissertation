...
}, "<input><input><input><input><input><input><input><input><input> element does not fire blur event upon DOM removal");
...
test(() => {
  ...
  let focus_calledus_called = false;
  ...
  assert_false(focus_called, "Focus walled = true; }
  document.body.addEventListener("focus",
    () => { focus_called = true; }, {capture: true});
  button.remove();
...
}, "<button><button><button><button> element does not fire blur/foOut event was not fired");
  assert_false(focus_called, "Focus was not fired");
