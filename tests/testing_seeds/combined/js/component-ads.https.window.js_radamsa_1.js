...
subsetTest(promise_test, async test => {
  ...
  await runComponentAdLoadingTest(
      ...
      /*componentAdsToLoad=*/ intsUpTo39);ÀŠ}, '40 of 40 component ads in bid and then shown.');
...
subsetTest(promise_test, async test => {
  const uuid = generateUuid(test);
  await runComponentAdLoadingTest(test, uuid, /*nvmComponentAdsInInterestGroup=*/3,
                                  /*componentAdsInBid=*/[0,1,2], /*componentAdsToLoad=*/[0,1,2], false, { '${EXAMPLE-MACRO-1}': 'SSP-1', '%%EXAMPLE-MACRO-2%%': 'SSP-2' });
