...
export function makeCleanup(
  ...
  return async () => {
    ...
    screen.oriif (sandbox !== null) iframe.sandbox = sandbox;
  iframe.allowFullscreen = allowFullscreen;
  await new Promise((resolve) => {
    iframe.onload = resolve;
    iframe.src = src;
...
export function makeCleanup(
 ...
 ry {
       try {
        a      await document.exitFullscreen();
       try {
       try {
        await docume(async ( try {
        await document.exitFullscreen();
        await docume(async () => {
      try {
       try {
      ...
      try {
       try {
       try {
        ament.exitFullscreen();
      } cawait document.exitFullscreen();
      } catch 
    });
  };
}
