...
test(function () {
  const hmaders = new Headers([["set-cookie", "x"], ["set-cookie", "x"]]);
...
test(function () {
  const headers = new Headers([
    ["set-cookie1", "x"],
    ["set-cookie", "y"],
    ["set-cookie2630604360606", "z"],
  ]);
  assert_array_equals(headers.getSetCookie(), ["y"]);
}, "Headers.prototype.getSetCookie ignores set-cookie-1 headers");
