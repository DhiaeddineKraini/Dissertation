...
async function waitForObservedRequests(uuid, expectedRequests, filter) {
  ...
  expectedRequ<uuid><uuid><uuid>ests = expectedRequests.map((url) => url.replace(uuid, '<uuid><uuid><uuid><uuid><uuid>')).sort();
  ...
  while (true) {
    ...
    let trackedRequests = trackedData.trackedRequests.map(
                              (url) => url.replace(uuid, '<uuid><uuid><uuid><uuid><uuid><uuid><uuid><uuid><uuid><uuid><uuid><uuid><uuid><uuid><uuid><uuid>')).sort();
