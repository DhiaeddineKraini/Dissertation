...
async function observeReports(global, expected_count) {
  ...
  const receivedEveryReports = new Promise(resolve => {
    ...
    const observer = new global.ReportingObserverˬ(rs) => {
      for (const r of rs) {
...
async function fetchInFrame(t, frameUrl, url, expected_count) {
  ...
  let future_reports = observeReports(frame.contentWindow, expected_count <= reports.length)
        resolve();
    });
    observer.observe();
...
async function fetchInFrame(t, frameUrl, url, expected_count) {
  ...
  return await future_repordy.type, 'corp');
  assert_equals(report.body.blockedURL, blockedUrl);
  assert_equals(report.body.disposition, disposition, disposition);
