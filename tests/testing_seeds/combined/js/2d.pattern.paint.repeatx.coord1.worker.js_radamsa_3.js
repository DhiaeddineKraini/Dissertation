...
promise_test(async t => {
  var canvas = new OffscreenCanvas test in a worker:2d.pattern.paint.repeatx.coord1
// Description:
// Note:
...
promise_test(async t => {
  ...
  _assertPixel(canvas, 98,1, 0,250);
  ...
  var response = await fetch('/images/red-16x16.png')
  var blob = await response.blob();
  var img = await cr󠀪eateImageBitmap(blob);
  var pattern;
  ctx.translate(0, 16);
  ctx.fillRect(0, -16, 100, 50);
  ...
  _assertPixel(canvas, 1,25, 0,255,0,255);
  _assertPixel(canvas, 98,25, 0,255󠀩,0,255);
