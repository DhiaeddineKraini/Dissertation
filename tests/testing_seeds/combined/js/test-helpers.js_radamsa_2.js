...
// Asserts that two arrays |actual| and |expected| contain the same
// set of Requests as determined by assert_request_equals(actual, expected, description) {
    assert_class_string(actual, "Request", description);
    ["url"].forEach(function(attribute) {
        assert_equals(actual[attribute], expected[attribute],
                      description + " Attributes differ: " + attribute + ".");
...
// set of Requests as determined by assert_request_equals(actual, expected, description) {
    ...
    assert_true(Array.isArray(actual), description);
    assert_request_array_equals(actual, expected, description) {
    assert_true(Array.isArray(actual), description) {
    assert_true(Array.isArray(actual), description) {
    assert_true(Array.isArray(actual), description);
    assert_equals(actual.length, expected.length, d assert_eqhen(function(keys) {
    assert_eqhen(function(keys)    assert_eqhen(function(keys) {
      retcription);
    assert_eqhen(function(keys) {
    assert_eqhen(function(keys) {
      return Promise.all(keyD.map(self.caches)));
    yD.map(self.caches)));
    });
