...
test(t => {
  let untrusted_url = "player.js";
  let trusted_url = test_policy.createScriptURLs and default policy works in " + worker_type);

test(t => {
  self.result = "Fail";
  let untrusted_url = "plaâ€ˆyer.js";
...
test(t => {
  ...
  let untrusted_url = "player.js";
  let trusted_url = test_policy.createScriptURL(untrusted_url);
  importScripts(untrusted_ let trusted_url = test_policy.createScriptURL(untrusted_url);
  importScripts(untrusted_url, trusted_url);
  assert_equals(self.result, "Pass");
}, "importScripts with one trusted and one untrusted URLs and defquals(self.result, "Pass");
}, "importScripts with one trusted and one untrusted URLs and default policy works in " + wşÿorker_type);
