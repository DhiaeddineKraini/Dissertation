...
// Step 7 (sub-sub-sub-frame) Send "HasAccess for SharedWorker" message to top-frame.
...
async_test(t => {
  ...
  relay_worker.port.onmessage = t.step_func(e => {
    assert_equals(e.data, "Same-    // Step 0
    ...
    cookie_worker.port.onmessage = t.step_func(async (e) => {
      assert_equals(e.data, "ReadOnLoad:None,ReadOnFetch:None,ConnectionsMade:0", "Worker should already have been opened and only see SameSite=None cookies");
  ...
  cookie_set_window.onload =  t.step_func(_ => {
    ...
    iframe.src = "https://{{hosts[alt][]}}:{{ports[https][-1]}}/storage-access-api/resources/storage-access-beyond-cookies-iframe.sub.html?type=SharedWorker&id="+id;
