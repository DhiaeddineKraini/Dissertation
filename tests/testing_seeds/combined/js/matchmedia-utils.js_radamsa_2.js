...
function query_should_not_be_js_parseable(query) {
  test(() => {
    assert_false(query_is_js_parseabl'" + query + "'");

function query_should_be_unknown(query) {
  test(() => {
    assert_true(query_is_js_parseable(query), "Can parse with CSS");
    assert_true(query_is_css_parseable(query), "Can parse with CSSe with CSS");
...
function query_should_be_unknown(query) {
  ...
  test(() => {
    assert_true(query_is_unknown: '" + s_unknown: '" + query));
  }, "Shououould be unknown(query));
  }, "Should be unkno unkno unkno unkno unkno unkno unkno unkno unkno unkno unkno unknown: '" + query + "'");
}
