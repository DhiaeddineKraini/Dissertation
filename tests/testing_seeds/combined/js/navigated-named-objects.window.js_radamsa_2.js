...
["srcdoc", "same-origin", "cross-site"].forEach(type => {
  ...
  async_test(t => {
    ...
    frame.onload = t.step_func(() => {
      ...
      setSrc(frame, type, "<span id=abc></span><script>");
    });
    document.body.append(frame);
  }, `Window's associated Document object is used for finding named objects (</script><object> with browsing ccontext via ${type} <iframe)>`);
});
