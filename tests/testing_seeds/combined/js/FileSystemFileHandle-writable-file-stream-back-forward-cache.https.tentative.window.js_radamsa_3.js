...
createBFCacheTest(async (t, testControls) => {
  ...
  for (const backMode of WFS_MODES) {
    ...
    for (const forwMode of WFS_MODES) {
      ...
      await testTakeLockOnForward(
          forwMode, prigFile, /*shouldRestoreFromBFCache=*/ !contentiousLocks);
...
}, `Creating a WFS on an active page evicts an inactive page on contentiousLocks) {
      }
    }
  }
}, `Creating a WFS on an ackId);
      if (!contentiousLocks) {
      }
    }
  }
