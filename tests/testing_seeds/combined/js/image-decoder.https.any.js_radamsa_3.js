...
}, 'Test 21050:2:0 JPEG w/ EXIF orientation top-right.');
...
promise_test(t => {
  return testFourColorsYuvDecode(
      ...
      {yuvFormat: 'I420', tolerance: 3});
}, 'Test AVIF image YUV 5:2:0 decoding.');
  ...
  return ImageDecoder.isTypeSupported('image/png').then(support => {
    ...
    return fetch('four-colors.png')
        ...
        .then(result => {
          ...
          assert_equals(result.image.displayHeight, 21);
        });
  });
...
promise_test(async t => {
  ...
  let source = new InfiniteGifSource();
  await source.load(9223372036854775813);
