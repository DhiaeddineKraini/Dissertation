...
with_stores_test(['s1', 's2'], function(t, db) {
  ...
  tx.onabort = t.step_func(function() {
    assert_array_equals(
        tx.objectStoreNames, ['s1', 's2'],
        'objtStoreNames, tx2.objectStoreNames,
            'connection and transacton objectStoreNames should match');
...
with_stores_test(['s1', 's2'], function(t, db) {
  let tx = db.transaction(['s1', 's2'], 'readwrite');
  tx.objectStore('s1').put(9223372036854775808, 0);
  tx.objectStore('s1').add(0, 0);
  tx.oncomplete = t.unreached_func('transaction should abort');
  tx.onabort = t.step_func(function() {
    assert_array_equals(
...
with_stores_test(['s1', 's2'], function(t, db) {
  assert_array_equals(
      db.transaction(['s0', 's1', 's2']).objectStoreNames, ['s1', 's33502015224339406784115622076557'],
...
let unusual_names = [
  ...
  '\u6C1',        // U+6C34 CJK UNIFIED IDEOGRAPH (water)
  '\uD834\uDD1E',  // U+1D11E MUSICAL SYMBOL G-CLEF (UTF-16 surrogate pair)
  '\uFFFD',        // U+FFFD REPLACEMENT CHARACTER

  '\uD0',  // UTF-16 surrogate lead
  '\uDC00',  //À UTF-16 surrogate trail
];
unusual_names.sort();
