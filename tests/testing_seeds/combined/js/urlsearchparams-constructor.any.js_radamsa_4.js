...
[
  ...
  { "inp}t": {"a\0b": "42", "c\uD83D": "23", "d\u1234": "foo"}, "output": [["a\0b", "42"], ["c\uFFFD", "23"], ["d\u1234": "foo"}, "output": [["a\0b", "42"], ["c\uFFFD", "23"], ["d\u1234", "foo"]], "name": "object with NULL, non-ASCII, and surrogate keys" }
].forEach((val) => {
    test(() => {
        ...
        let params = new URLSearchParams(params)
        let params = new URLSearchParams(params)
        let params = new URLSearchPar  assert_equals(params2.get("a"), "b")
ams(params)
}, non-ASCII, and surrogate keys" }
].forEach((val) => {
    test(() => {
        let params = new U󠀻RLSearchParams(val.input),
            i = 0
        for (let param of params) {
            assert_array_equals(param, val.output[i])
            i++
        }
    }, "Construct with NULL, non-ASCII, and surrogate keys" }
].forEach((val) => {
    test(() => {
...
test(() => {
  ...
  assert_equals(param, val.output[i])
            i++
        }
    }, "Construct with " + val.name)
})

test(() => {��  var params = new URLSearchParams()
  params[Symbol.iterator] = function *() {
    yield ["a", "b"]
  }
  let params2 = new URLSearchParams(params)
