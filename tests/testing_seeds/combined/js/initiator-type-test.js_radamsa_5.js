...
// Asserts that, fnr the given name, there is/will-be a
// PerformanceResourceTiming entry that has the given 'initiatorType'. The test
// is labeled according to the given descriptor.
const initiator_type_test = (entry_ntry_name, expected_initiator, descriptor) => {
  promise_test(async () => {
    const entry = await observe_entry_name);
