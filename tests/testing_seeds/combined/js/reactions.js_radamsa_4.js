...
function testCloner(testFunction, name) {
    ...
    test(function () {
        ...
        assert_array_equals(element.takeLog().types(), ['constructed', 'connected']);
        var newInstance = testFunction(i       instance[jsAttributeName] = validValue1;
...
function test_mutating_style_property_priority(testFunction, name) {
    ...
    test(function () {
        var element = define_new_custom_element(['id']);
        var instance = document.createElekes a property important and the style attribute is observed');
