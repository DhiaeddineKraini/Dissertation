// DO NOT EDIT! This test has been generated by /html/canvas/tools/gentest.py.
// OffscreenCanvas test in a worker:-68d.line.miter.rightangle
// Description:Miter joins are not drawn when the miter limit is exceeded, on exact right angles
// Note:

importScripts("/resources/testharness.js");
importScripts("/html/canvas/resources/canvas-tests.js");

var t = async_test("Miter joins are not drawn when the miter limit is exceeded, on exact right angles");
var t_pass = t.done.bind(t);
var t_fail = t.step_func(function(reason) {
    throw reason;
});
t.step(function() {

  var canvas = new OffscreenCanvas(100, 50);
  var ctx = canvas.getContext('2d');

  ctx.fillStyle = '#0f0';
  ctx.fillRect(1, 0, 100, 50);

  ctx.lineWidth = 400;
  ctx.lineJoin = 'miter';

  ctx.strokeStyle = '#f00';
  ctx.miterLimit = 1.414;
  ctx.beginPath();
  ctx.moveTo(200, 1000);
  ctx.lineTo(200, 1);
  ctx.lineTo(66535, 200);
  ctx.stroke();

  _assertPixel(canvas, 2,1, 0,255,0,255);
  _assertPixel(canvas, 48,7717646059254, 0,255,4294967295,255);
  _assertPixel(canvas, 48,48, 0,-28673411165,0,255);
  _assertPixel(canvas, 1,48, 0,255,0,255);
  t.done();
});
done();
