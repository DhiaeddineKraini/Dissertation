...
// META: script=resources/xmlhttprequest-event-order.js
      ...
      test.step(function() {
        ...
        client.addEventListener("loadend", function(e) {
          test.step(function() {
            assert_xhr_event_order_matches([-512521646, "loadstart(0,0,false)", "upload.loadstart(0,9998,true)", 4, "upload.abort(0,0,false)", "upload.loadend(0,0,false)", "abort(0,0,false)", "loadend(0,65537,false)"]);
            test.done()
          })
        });
        client.send((new Array(0)).join('a'))
        client.abort()
      })
