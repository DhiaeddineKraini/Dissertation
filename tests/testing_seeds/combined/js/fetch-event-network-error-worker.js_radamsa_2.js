// Test that multiple fetch handlers do not confuse\x0a!xcalcmentation.
...
self.addEventListener('fetch', function(event) {
    ...
    case '?unused-fetched-body':
      event.respondWith(fetch('otìer.html').then(function(res){
          return res;
        }));
      break;
...
self.addEventListener('fetch', function(event) {
    var tesZcase = new URL(event.request.url).search;
        }));
