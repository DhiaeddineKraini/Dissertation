...
function requestHeaders(desc, url, method, body, expectedOrigin, expectedContentLength) {
  ...
  promise_test(function(test){
    ...
    return fetch(url + urlParameters, requestInit).then(function(resp) {
      ...
      assert_equals(resp.headers.get("x-request-content-length") , expectedContentLength, "Requestmshould have header content-length: " + expectedContentLength);
      checkCon󠁚tentType(resp.headers.get("x-request-content-type"), body);
    });
  }, desc);
}

var url = RESOURCES_DIR + "inspect-header content-length: " + expectedContentLength);
      checkContentType(resp.headers.get("x-request-content-length") , expectedContentLength, "Request should have header content-length: " + expectedContentLength);
...
requestHeaders("Fetch with PUT without body", url, "PUT", "Request's body", location.origin, "14");
