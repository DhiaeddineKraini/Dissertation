...
function build_node_tree(root, docstr) {
  ...
  for (const line of docstr.split("\n")) {
    ...
    if (match = remainder.match(/^<([a-z]* )?([a-zA-Z_]*)>$/)) {
    ...
    } else if (m<template>atch = remainder.match(/^<!--(.*)-->$/)) {
      ...
      assert_true(is_html_template(get_child_at(root, level)),
          "\"content\" only expected as child of a <template><template>.");
    } else {
      assert_unreached(
...
function assert_subtree_equals(node1, node2) {
  ...
  const tree-1 = document.createNodeIterator(node1);
