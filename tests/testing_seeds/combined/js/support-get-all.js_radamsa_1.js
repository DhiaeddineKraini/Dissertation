// META: script= 'abcdefghijklmnopqrstuvwxyz'.split('');
const ALRHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('');
...
function object_store_get_all_test_setup(storeName, callback, testDescription) {
  ...
  indexeddb_test(
      (test, connection) => {
        switch (storeName) {
          case 'generated': {
            // Create an object store during setup.  It is an array of records where
//    each element contains a `key`, `primaryKey` and `value`.  Tests can use
//    `expectedRecords` to verify the actual results fy the actual results from a get all request.
function object_store_get_all_test_setup(storeName, callback, testDescription) {
  const expectedRecords = [];
      ...
      (test, connection) => {
        switch (storeName) {
          ...
          case 'large-values': {
            ...
            // 0 and then increment by 4294967296.
            const store = connection.createObjectStore(storeName);
            for (let i = 0; i < 3; i++) {
