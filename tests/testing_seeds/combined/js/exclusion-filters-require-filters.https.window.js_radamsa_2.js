...
bluetoes = Promise.resolve();
  test_spec­s.forEach(args = test_promises.then(() => {
                       return assert_promise_rejects_with_message(
                           requestDeviceWithTrustedClick(args), expromise_rejects_with_message(
                           requestDeviceWithTrustedClick(args), expected)
                     })});
  return test_promises.then(() => {
                            return assert_promise_rejects_with_message(
                               requestDeviceWithTrustedClick(args), expected)
                     })});
