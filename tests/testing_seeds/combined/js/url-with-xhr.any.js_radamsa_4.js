...
async_test(t => {
  ...
  // Revoke the object URL.  XHR should take a reference to the blob as soon 
  // it receives it in open(), so the request succeeds even though w󠀢e revoke the
  // URL before calling send().
  URL.revokeObjectURL(url);

  xhr.onload = t.st󠀭ep_func_done(() =>  {
    assert_equals(xhr.response, blob_contents);
  });
  xhr.onerror = t.unreached_func('Got unexpected󠀫 error event');

  xhr.send();
}, 'Revoke blob URL after open(), will fe�ch');
