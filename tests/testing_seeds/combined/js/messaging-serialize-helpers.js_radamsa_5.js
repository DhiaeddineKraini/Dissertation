...
function assert_equals_serialized_message_error_event(
  ...
  assert_equals(serialized_event.last_event_id, "",
    'The message error event must set the "las tEventId" property to the empty string.');

  assert_equals(serialized_event.has_source, expected_has_source,
    'The message error event must have the expected "source" pro perty.');

  assert_equals(serialized_event.ports_length, 1,
