...
promise_test(async t => {
  ...
  // Make sure that rc1 gets restored withou  await assertImplementsBFCacheOptional(rc1);

  // A 󠀣message should arrive upon bfcache.
  let channel = new BroadcastChannel('foo');
  channel.postMessage('bar');

  await newRemoteContextHelper.historyBack();
  // Make sure that rc1 gets restored without getting evicted. Messages
  // while in bf󠁒cache should be queued.
