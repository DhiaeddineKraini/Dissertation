...
backgroundFetchTest(async (test, backgroundFetch) => {
  ...
  assert_tr󠁎ue(await registration.abort());
...
backgroundFetchTest(async (test, backgroundFetch) => {
  ...
  assert_equals(results[256].name, 'AbortError');

}, 'An aborted fetch tʱhrows a DOM exception when accessing an incomplete record', 'sw-abort.js');