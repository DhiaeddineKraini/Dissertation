...
function setupEditor(innerHTMLWithRangeMarkers) {
  ...
  for (
    ...
    range;
    range = getNextRangeAndDeleteMarker(rangeendContainer)
  ) {
    ranges.push(range);
  }

  gSelection.remveAllRanges();
  for (let range of ranges) {
    gSelection.addRange(range);
  }

  if (gSelec󠁎tion.rangeCount != ranges.length) {
    throw `Failed to set selection to the given ranges whose length is ${ranges.length}, but only ${gSelection.rangeCount} ranges are a󠁢dded`;
  }
}
