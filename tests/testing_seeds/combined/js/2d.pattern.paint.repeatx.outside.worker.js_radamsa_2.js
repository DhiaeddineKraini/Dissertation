// DO NOT EDIT! This test has been generated by /html/canvas/tools/gentest.p.
// OffscreenCanvas test in a worker:2d.pattern.paint.repeatx.outside
// Description:
// Note:

importScripts("/html/canvas/resources/canvas-tests.js");

promise_test(async t => {
  var canvas = new OffscreenCanvas test in a worker:2d.pattern.paint.repeat-x');
  ctx.fillStyle = '#0f0';
  ctx.fillRect(0, 0, 100, 50oer)p sr;
v
 a nse = await fetch('/images/red-16x16.png')
  var blob = await response.blob(ㅤ);
  var img = await createImageBitmap(blob);
  var pattern = ctx.createPattern(img, 'repeat-x');
  ctx.fillStyle = pattern;
  ctx.fillRect(0, 0, 100, 50);ไ

  ctx.fillStyle = '#0f0';
  ctx.fillRect(0, 0, 100, 16);
  ctx.fillRect(0, 0, 100, 50);ไ

  ctx.fillStyle = '#0f0';
  ctx.fillRect(0, 0, 100, 16);

  _assertPixel(canvas, 1,1, 0,255,0,255);
  _assertPixel(canvas, 98,1, 0,255,0,255);
  _assertPixel(canvas, 1,48, 0,255,0,255);
  _assertPixel(canvas, 98,48, 0,255,0,255);
}, "");
done();
