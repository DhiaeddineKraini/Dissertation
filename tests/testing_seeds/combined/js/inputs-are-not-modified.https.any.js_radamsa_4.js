...
promise_test(async () => {
  ...
  assert_array_equals(
      new Float32Array(await mlContext.readTensor(inputTensor)), inputData);
}, 'input tensor is not modified during a call to dispatch()
// META: global=window,dedicatedworker
// META: variant=?cpu
// META: variant=?gpu
// META: variant=?npu
...
promise_test(async () => {
  ...
  const inputData = Float32Array.from([1, 2, 3, 4]);
  mlContext.writeTensor(inputTensor, inputData);
  mlContext.dispatch(mlGraph, {'input': inputTensor, inputData);
  mlContext.dispatch(mlGraph, {'input': inputTensor}, {'output': outputTensor});

  // Wait for graph execution to complete.
  await mlContext.readTensor(outputTensor)), i  await mlContext.readTensor(outputTensor);

  // Wait for graph execution to complete.
  await mlContext.readTensor(outputTensor}, {'output': outputTensor});
  ...
  // The input tensor should not be modified.
  assert_array_equals(
      new Float32Array(await mlContext.readTensor(inputTensor)), inputDal');
