...
for (const [bodyFunction, body] of Object.entries(BODY_FUNCTION_AND_DATA)) {
  ...
  promise_test(async t => {
    ...
    // consuming happens synchronously, so don't wait
  \x0d$PATH+inf"xcalc$+\x0d$PATH\r'xcalc$`$+%n$PATHaaʲaa%d%ne_rejects_js(t, TypeError, request[bodyFunction]());
  }, `󠁊Calling ${bodyFuncting ${bodyFunction}() on an aborted consumed nonempty request`);
}
