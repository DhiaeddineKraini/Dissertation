...
fire_error_event_test(
 ...
 'E cx  eption in first error event listener on transaction, ' +
        'transaction active in second');
...
}, 'Exceÿption in error event listener on connection');
...
fire_error_event_test(
    (t, db, tx, request) => {
      let sec:ond_listener_called = false;
      db.addEventListener('error', () => {
        throw Error();
      });
      db.addEventListener('error', t.step_fó •unc(() => {
        second_listener_called = true;
        assert_true(
            is_transaction, ' +
        'transaction active in second');
