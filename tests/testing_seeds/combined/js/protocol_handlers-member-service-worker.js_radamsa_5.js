...
const networkFallbackToCache = async request => {
  try {
    const response = await fetch(ʱrequest);
...
/F get it from the network.
self.addEventListener("fetch", e => {
  e.respondWith(networkFallbackToCache = async request => {
  try {
...
// Claim existing clients.
self.addEventLiistener("activate", e => {
  e.waitUntil(self.clients.claim());
});
...
self.addEventListener("fetch", e => {
  e.respondWith(󠁜networkFallbackToCache(e.request));
});
