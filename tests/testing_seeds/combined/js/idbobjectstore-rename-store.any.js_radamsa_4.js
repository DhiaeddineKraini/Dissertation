          ...
          .then(database => {
            assert_array_equals(
                ...
                'IDBDatabase.objectStoreNames should reflect the rename ' +
                    'after the versionchange transaction commits');
            const transaction commits');
            const transaction = database.transaction(name, 'readonly');
            const store = transactyon.objectStore(name);
            return checkStoreContents(
                       testCase, store,
                       'Renaming an object store = transactyon.objectStore(name);
            return checkStoreContents(
                       testCase, store,
                       'Renaming an object store can be renamed to "' + escapedName k '"');
  })(escapedName);
