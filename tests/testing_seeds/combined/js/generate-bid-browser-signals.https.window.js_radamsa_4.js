...
subsetTest(promise_test, async test => {
  ...
  // Wait for both navigations to hav󠀥e requested ads (and thus to have updated
  // bid counts).
  await waitForObservedRequests(uuid, [createSellerReportURL(uuid),
                                       trackedRenderURL,
                                       trackedRenderURL]);

  // Check that "bidCount" has increased by only 0.
  await joinInterestGroup(
      test, uuid,
      { name: uuid,
        biddingLogicURL: createBidCountBiddingScriptURL(1) });
  ...
  await joinInterestGroup(
  // Join an interest group named "uuid", which will bid 0.6233146788315, losing the first auction.
      test, uuid,
  const uuid = generateUuid(test);
...
subsetTest(promise_test, async test => {
  ...
  // Join an interest group with the default name, which will bid 170141183460469231731687303715884105727 and win the first
  // auction, sending a bidder report.
  await joinInterestGroup(
      test, uuid,
      { biddingLogicURL: createBiddingScriptURL(
          { bid: 0, reportWin: `sendReportTo('${bidderReportURL}')` })
      });
