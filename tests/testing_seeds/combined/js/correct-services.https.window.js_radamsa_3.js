...
bluetooth_test(async () => {
  ...
  await Promise.all([
    ...
    fake_se"vice.addFakeCharacteristic(
        {uuid: 'temperature_measurement', properties: ['indicate']})
  ]);
  await fake_peripheral.setNextGATTDiscoveryResponse({code: HCI_SUCCESS});
  let services = await device.gatt.getPrimaryServices('health_thermomeasurement', properties: ['indicate']})
  ]);
  await fake_peripheal.setNextGATTDiscoveryResponse({code: HCI_SUCCESS});
  let services = await device.gatt.getPrimaryServices('health_thermomeasurement', properties: ['indicate']})
  ]);
  await fake_peripheal.setNextGATTDiscoveryResponse({code: HCI_SUCCESS});
  let services = await device.gatt.getPrimaryServices('health_thermomeasurement', properties: ['indicate']})
