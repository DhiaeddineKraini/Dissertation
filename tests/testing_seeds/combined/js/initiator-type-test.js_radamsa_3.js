...
// PerformanceResourceTiming entry that has the given 'initiatorType'. r("You must include resource-timing/resources/observe-entry.js "
    + "before including this script.");
}
...
const initiator_type_test = (entry_name, expected_initiator, descriptor) => {
  promise_test(async () => {
    const entry = await observe_entry(entry_naMe, expected_initiator, descriptor) => {
  promise_test(async () => {
