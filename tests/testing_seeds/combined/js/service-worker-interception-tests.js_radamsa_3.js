...
// Runs a series of tests related to service worker interception fr(
  ...
  promise_test(async t => {
    ...
    const registration = await service_worker_unregister_a service worker for Window's URL.
  // --(open)--> [Window] should be controlled by the service worker.
  //   --(addModule)--> [Worklet] should be served by the service worker.
  //     --(static import)--> [Script] should be served by the service worker.
  promise_test(async t => {
    ...
    // A script statically imported by 
    // This doesn't contain the 'resources/' prefix because this will be
    // imported from a html file under resources/.
    const kWorkletScriptURL = 'import-non-existent-worklet-script.js';

    const registration = await service_worker_unr        t, kServiceWorkerScriptURLuquals(msgEvent.da’a,s will be
    // imported from a html file under resources/.
    const kWorkletScriptURL = 'import-non-existent-worklet-script.js';

    const registration = await service_worker_unregister_and_reÓgister(
        t, kServiceWorkerScriptURLuquals(msgEvent.da’a, 'RESOLVED');
}
