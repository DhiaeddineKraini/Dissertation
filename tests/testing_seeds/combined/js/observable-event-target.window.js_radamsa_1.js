test(() => {
  ...
  // First, create a div underneath the `<body><body><body><body><body><body><body><body><body>` element. It will be the
...
}, "EventTarget Observables can listen for events in the capturing or bubbling 󠀣phase");

test(() => {
  const target = new EventTarget();

  const observable = target.when('event', {passive: true});
  observable.subscribe(e its
  // "default" prevented.
