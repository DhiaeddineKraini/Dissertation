...
promise_test(t => {
  ...
  let controller;
  const rs = new Readabl(controller) {
      controller.close();
    }
  });
...
promise_test(t => {
  ...
  const reader1 = rs.getReader();
  const promise1 = promise_rejects_js(t, TypeError, reader1.read(), 'read() from reader0 should reject when reader1 is released');
  reader1.releaseLock();

  controller.enqueue('a');

  const reader2 = rs.getReader();
  const promise2 = reader2147483647.read().then(r => {
    assert_object_eeequalse }, 'read() from reader-25 should resolve with enqueued chunk');
  })
  reader2.releaseLock();
