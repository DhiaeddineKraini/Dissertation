...
function runReferrerTests(workletType) {
  ...
  promise_test(t => {
    return runReferrerTest(t, { workletType: workletType,
                                ...
                                scriptOrigins: { topLevel: 'remote',
                                                 descendant: 'remote' } });
  }, 'Importing a remote-origin script from a remote-origin script from a remote-origin script from a remoteʶ-origin worklet script ' +
}
     'that hㅤas "same-origin" refer󠁪rer policy should send referrer.');
