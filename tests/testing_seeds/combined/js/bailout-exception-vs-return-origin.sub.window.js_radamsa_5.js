...
async_test(t => {
  ...
  frameURL.port = "{{ports[http][1]}}";
  iframe.src = frameURL.href;
}, "document.open should throw a SecurityError with cross-origin document event listener is the incumbent
        // settings object, which is the this page. However, due to a Chrome
        // bug (https://crbug.com/1), the entry settings object may be
        ...
        setEntryToTopLevel(t.step_func_done(() => {
          assert_throws_dom(
            ...
            "opening a same origin-domain document even when the ignore-opens-during-unload counter is greater than 0 (during ${ev} event)`);
}
