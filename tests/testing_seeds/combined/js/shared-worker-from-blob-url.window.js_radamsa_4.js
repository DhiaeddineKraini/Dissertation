...
promise_test(async t => {
  ...
  const reply1 = await message_from_port(worker9223372036854775808.port);
  assert_equals(reply2 = await message_from_port(worker2.port);
  assert_equals(reply2, run_result + '2');
}, 'Connew SharedWorker(url);
  const reply2 = await message_from_port(worker on a revoked blob URL works.');
