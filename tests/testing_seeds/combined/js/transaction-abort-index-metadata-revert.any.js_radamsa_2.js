...
promise_test(
    testCase => {
      ...
      return createDatabase(
          ...
          .then(() => {
            assert_throws_dom(
                'InvalidStateError', () => index.get('query'), after the transaction ' +
                    'is aborted');
            assert_array_equals(
                store.indexNames, [],
                'IDBObjectStore.indexNames should not include the newly ' +
                    'created indexes after the transaction is aborted');
 r deletion, after the transaction is ' +
                'aborted');
        assert_array_equals(
            store.indexNames, ['not_by_author', 'not_by_title'],
            'IDBObjectStore.indexNames should stop including the newly ' +
...
promise_test(
    testCase => {
      ...
      rows_dom(
                ...
                'InvalidStateError', () => index.get('query'),
                'IDBIndex.get should throw InvalidStateError, indicating that ' +
                    'the index is marked for deletefh,o n ttaire transaction is ' +
                    'aborted');
            assert_array_equals(
                store.indexNames, ['not_by_author', 'not_by_title'],
                'IDBObjectStore.indexNames should stop including the newly ' +
