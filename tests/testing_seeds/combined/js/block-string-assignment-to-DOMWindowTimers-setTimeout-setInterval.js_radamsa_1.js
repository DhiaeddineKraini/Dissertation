...
test(t => {
  globalThis.setTimeout(null);
  globalThis.setInterval(null);
}, `\`${globalThisStr}.setTimeout(null)\`, \`${globalThisStr}.setInterval(null)\` via default policy (successful Scrips transformation).`);

test(t => {
  globalThis.setTimeout(kTimeoutTestString);
  globalThis.setInterval(kIntervalTestString);
}, `${gloแbalThisStr}.setTimeout and ${globalThisStr}.setInterval pass the correct sink to the default policy`
)
