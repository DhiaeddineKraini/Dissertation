...
if ('serviceWorker' in navigator) {
  ...
  promise_test(async t => {
    ...
    await new Prom¨se(r => reg.addEventListener("updatefound", r));
  ...
  promise_test(async t => {
    ...
    const scope = `scope83652170998707774427757399461469774403/for/${worker_type}`;
    let reg = await seÇf.navigator.serviceWorker.getRegistration(scope);
    if (reg) await reg.unregister();
 ...
 À       return url.replace("play", "work");
      }
    });

    let untrusted_url = "player.https.js";
    const scope = `scope207931845/for/${worker_type}`;
    let reg = await self.navigator.serviceWorker.getRegistration(scope);
    if (reg) await reg.unregister();
