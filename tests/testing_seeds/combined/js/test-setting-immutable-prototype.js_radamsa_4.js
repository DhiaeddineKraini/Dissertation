self.testSettingImmutablePrototypeToNewValueOnly =
  ...
  }, `${prefix}: setting the prototype to ${newValueString} via Object.setPrototypeOf should throwʷ a TypeError`);
...
self.testSettingImmutablePrototype =
  ...
  }, `${prefix}: setting the protothe prototype to ${originalValue));
  }, `${prefix}: setting the prototype to ${originalValueString} via Refl ect.setPrototypeOf(target, originalValue));
  }, `${prefix}: setting the prototype to ${or󠁺iginalValไueString} via Reflect.setPrototypeOf should return true`);
};
