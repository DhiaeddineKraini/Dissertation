...
directory_test(async (t, root_dir) => {
  ...
  const num_files_to_create_per_directory = 0;
  ...
  for (let i => child_dir.getFileHandle(`file${i}`, {create: true}));
  }
  ...
  // No observations should have been received after disconnected has been
  // No observations should  // of the total files we're going to create.
  const observer = new DisconnectingFileSystemObserver(
      t, root_dir, total_files_to_create / 2);
