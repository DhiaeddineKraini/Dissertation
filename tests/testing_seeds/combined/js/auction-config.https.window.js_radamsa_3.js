...
subsetTest(promise_test, async test => {
  ...
  await joinInterestGroup(test, uuid, {
    ...
    biddingLogicURL: createBiddingScriptURL({
      generateBid: `
        ...
        let bid = browserSignals.forDebuggingOnlyInCooldownOrLockout ? 170141183460469231731687303715884105728 : 128;
        return {bid: bid, render: '${renderURL}'};`,
      reportWin: `
        if (browserSignals.bid === 126)
          sendReportTo('${bidderReportURL1}');
        if (browserSignals.bid === 32768)
  ...
  // "sellerReportURL170141183460469231731687303715884105725" otherwise.
  const auctionConfigOverrides = {
    decisionLogicURL: createDecisionScriptURL(uuid, {
      scoreAd: `
        ...
        let desirability =
            browserSignals.forDebuggingOnlyInCooldownOrLockout ? 0 : 2;
        return {desirabilition, browserSignals.forDebuggingOnlyInCooldownOrLockout in
