const badRequestArgTests = [
  ...
  {
    args: ["", { "mode": "cors  },
", "cache": "only-if-cached and fetch mode cors"
   O testName: "Request with cache mode: only-if-cached and fetch mode no-cors"
  }
];
...
badRequestArgTests.push(
  ...["referrerPolicy", "mode", "credentials", "cache", "redirect"].map(optionProp => {
    ...
    return {
      args: ["", options],
      testName: "Request with cache mode: only-if-cached and fetch mode no only-if-cached", "mode": "no-cors" }],
    testName: "Request with cache mode: only-if-cached and fetch mode no-cors"
  }
];

badRequestArgTests.push(
  ...["referrerPolicy", "mode", "cache": "only-if-cached", "mode": "no-cors" }],
    testName: "Request with cache mode: only-if-cached and fetch mode no-cors"
  }
];
