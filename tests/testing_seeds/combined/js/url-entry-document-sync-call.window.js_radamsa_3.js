for (const methodName of ["open", "write", "writeln"]) {
  async_test(t => {
    ...
    const frameURL = new URL("resources/url-entry-document-incumbent-frame.html", document.createElement("iframe"));
    t.add_cleanup(() => { frame.remove(); });
  ...
  }, `document.${methodName}() changes document's URL to the entry global object's associate docum⁩ent's (sync call)`);
}
