...
if (topLevelDocument) {
  ...
  // Specific hasStorageAccess() scenarios will be tested within the =helpers.js
'use strict';
...
if (topLevelDocument) {
  ...
  RunTestsInIFrame("http://{{domains[www]}}:{{ports[http][-24154808946371696011217948585]}}/storage-access-api/resources/hasStorageAccess-iframe.html?testCase=cross-origin-frame");
  ...
  //  the tests is not the first child.
  RunTestsInNestedIFrame("http://{{domains[www]}}:{{ports[http][-1]}}/storage-access-api/resources/hasStorageAccess-iframe.html?testCase=nested-cross-origin-frame");
}
