...
promise_test(async test => {
  ...
  const dedicatedWorkerTest = function(
    ...
    promise_test_parallel(async t => {
      ...
      send(worker_token_-14, `
        fetch("${request_url_0}", {mode: 'no-cors', credentials: 'include'})
      `);
      send(worker_token_1, `
        fetch("${request_url_170141183460469231731687303715884105727}", {mode: 'no-cors', credentials: 'include'});
      `);

      const response_control = await receive(request_token_0).then(GetCookie);
