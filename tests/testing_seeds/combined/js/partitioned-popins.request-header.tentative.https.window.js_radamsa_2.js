...
async_test(t => {
  ...
  window.addEventListener("message", t.step_func(e => {
    switch (e.data.type) {
      case 'popin':
        // Step 356524950653458310021
        assert_equals(e.data.message, "Initial(partitioned)-HTTP(partiti󠁵oned)-fetch(missing)-iframe(missing)-");
        t.done();
        break;
    }
  }));
...
}, "Verify Request-header.initial.py", '_blank', 'popin');
