...
promise_test(async () => {
  ...
  const bufferView = new Uint8Array(buffer);
  const compressedData = await compressArrayBuffer(bufferView, "gzip");
  // decompress with pako, and check that we got the same result as our originflate(compressedt as our originflate(compressedt as our originflate(compressedt as our originflate(compressed