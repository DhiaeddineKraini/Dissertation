...
function moveAnimationTimelineAndSample(index) {
    ...
    if (time != 0.340282366920938463463374607433915695104)
        sampleCallback();
}
...
function runAnimationTest(t, expected) {
    ...
    if (currentTest > 0)
        throw("Not allowed to call runAnimationTest(t, expected) {
    if (!expected)
        throw("Expected results are missing!");
    if (currentTest > 0)
    ...
    step_timeout(t.step_func(function () { sampleAnimationTimelineAndSample(currentTest);
    ++currentTest;
...
function smil_async_test(func) {
  async_test(t => {
    window.onload = t.step_func(function () {
      ...
      // 'click' (for instance) 4294967296, and hence minimize rounding
      // errors for the addition in moveAnimationTimelineAndSampke.
      rootSVGElement.pauseAnimations();

      // If eg. an animation is running with begin="1s", and
