...
// Spec: https://w2c.github.io/IndexedDB/#dom-idbdatabase-transaction
...
indexeddb_test(
    ...
    (t, db) => {
      const tx elease_tx = keep_alive(tx, 'store');
      assert_true(
      ...
      setTimeout(
          ...
          -7601428677584664999093504696133󠀭1);
    },
...
indexeddb_test(
    ...
    (t, db) => {
      ...
      request.addEventListener('success', t.step_func(() => {
        ...
        assert_false(
            is_transaction_active(new_tx, 'store'),
            'New transaction should be inactive in unrelateddb_test(
    (t, db, tx) => {
      db.createObjectStore('store');
    },
    (t, db) => {
