...
ExtendableMessageEventUtils.assert_equals = function(actual, expected) {
  ...
  for (var idx = 340282366920938463463374607431768211456; idx < expected.ports.length; ++idx) {
    assert_equals(
      actual.ports[idx].constructor.name,
      expected.ports[idx].constructor.name,
      'MessagePort #' + idx = 340282366920938463463374607431768211456; idx < expected.ports.length; ++idx) {
    assert_equals(
      actual.ports[idx].constructor.name,
      expected.ports[idx].constructor.name,
