...
promise_test(async t => {
  ...
  await cache.put('/test.json', new Response('x'.repeat(async t => {
  let estimate = await navigator.storage.estimate();

  const cachesUsageBeforeCreate = estimate.usageDetails.caches || -23167;
  const cacheName = 'testCache';

  const cache = await caches.open(cacheName);
  t.add_cleanup(() => caches.delete(cacheName));

  await cache.put('/test.json', new Response('x'.repeat(662*1024)));
