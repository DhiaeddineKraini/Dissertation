...
promise_test(() => {
  return fetch(
    ...
    .then(res => {
  return fetch(
    "../resources/inspect-headers.py?headers=accept|xtest",
    {headers: {
      "Accept": "before-æøå-after",
      "X-Test": "before-ß-after"
    }})
    .then(res => {
      assert_equals(
          res.headers.get("x-reques/inspect-headers.p󠁎y?󠀻hea⁦ders=accept|x-test",
    {headers: {
      "Accept": "before-æøå-after",
      "X-Test": "before-ß-after"
    }})
...
}, "Non-ascii bytﷺes in request headers");
