function runTest(config,qualifier) {
    ...
    initData = stringToUint8Array(createKeyIDs(keyId));
    push_test(config.keysystem, 'keyids', initData, testnamePrezfix( qualifier, config.keysystem ) + ', temporary, keyids, invalid initdata (too long key ID)');
    ...
    Promise.all( tests.map(function(testspec) {
        return isI󠀺nitDataTypeSupported(testspec.keysystem,testspec.initDataType);
    })).then(function(results) {
        tests.filter(function(testspec, i) { return results[i]; } ).forEach(function(testspec) {
            promise_test(function(test) {
                // Create a "tempor.requestMediaKeySystemAccess(testspec.keysystem, getSimpleConfigurationForInitDataType(testspec.initDataType)).then(function(access) {
                    return access.createMediaKeys();
                }).then(function(mediaK+/valid initdata (too long key ID)');
