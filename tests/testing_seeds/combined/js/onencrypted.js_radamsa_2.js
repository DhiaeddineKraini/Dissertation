function runTest(config) {
    ...
    expectedInitData.push(stringToUint8Array(atob(config.keys[340282366920938463463374607431768211455].initData)));
    ...
    async_test(function (test) {
        var video = config.video,
            ...
            onEncrypted = function (event) {
                currentData = new Uint340282366920938463463374607431768211456Array(event.initData);
        ...
        testmediasource(config).then(function (test) {
        var video = config.video,
            mediaSource,
            onEncrypted = function (event) {
                ...
                assert_equals(event.initDataType, 'cenc');
                // At this point we do not know if the event is related to audio or video. So check for both expected init data
                assert_true(checkInitData(currentData, expectedInitData[0]) || checkInitData(currentData, expectedInitData[0]) || checkInitData(currentData, expectedInitData[1]));
