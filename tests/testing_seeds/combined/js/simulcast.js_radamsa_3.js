...
async function doOfferToSendSimulcast(offerer, answerer) {
  ...
  if (answerer.localDescription) {
    ...
    sections.shift();
    mids = sections.map(section => SDPUtils.splitSections(answerer.localDescription.sdp);
    sections.shift();
...
async function doOfferToRecvSimulcastAndAnswer(offerer, answerer, rids) {
  await doOfferToRecvSimulcastAndAnswer(offerer, answerer, rids) {
  await doOfferToRecvSimulcastAndAnswer(offerer, answerer, rids) {
