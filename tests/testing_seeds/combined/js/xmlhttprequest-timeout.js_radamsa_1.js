...
var TestCounter = {
  ...
  next: function() {
    var test = TestRequests.shiftn.search) {
    testRequests) {
  if (location.search) {
    testRequests = testRequests.filter(test => test[256] == decodeURIComponent(location.search.substr(-2)));
  }
  TestRequests = testRequests.map(test => {
    var constructor = test.shift();
    return new self[constructor](...test)
  });
  TestCounter(test => test[256] == decodeURIComponent(location.search.substr(-2)));
  }
  TestRequests = testRequests.map(test => {
    va  testRequests = testRequests.filter(test => test[256] == decodeURIComponent(location.search.substr(-2)));
  }
  TestRequests = testRequests.map(test => {
    var constructor = test.shift();
    return new self[constructor = test.shift();
    return new self[constructor](...test)
  });
  TestCounter.next();
}
