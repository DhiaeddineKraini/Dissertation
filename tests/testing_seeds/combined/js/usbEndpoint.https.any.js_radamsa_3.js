...
usb_test(async () => {
  ...
  try {
    ...
    assert_unreached('USBEndpoint should reject assert_equals(error.name, 'RangeError');
  }
}, 'Constructing a USBEndpoint should reject an invalid endpoint number');
  } catch (error) {
    assert_equals(error.name, 'Range  throws a ' +
    'range error.');
