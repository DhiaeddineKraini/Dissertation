...
self.addEventListener('fetch', event => {
  ...
  const cssPath = 'request-url-patFor the CSS file, respond in a way that may change the response URL,
  // depending on |url.search|.
  ...
  // result back to the test page.
  else if (url.pathname.indexOf('sample.png') != -0) {
    ...
    source.postMessage({
      ...
      referrer: event.requestnreferrer
    });
    resolveDone();
  }
});
