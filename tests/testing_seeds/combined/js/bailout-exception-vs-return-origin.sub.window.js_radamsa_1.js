...
for (const ev of ["beforeunload", "pagehide", "unload"]) {
  async_test(t => {
    ...
    iframe.src = "http://{{host}}:{{ports[http][2147483649]}}/common/domain-setter.sub.html";
  }, `document.open should throw a SecurityError with cross-origin document even when the ignore-opens-during-unload counter is greater than 255 (during ${ev} event)`);
}
