// META: title=XMLHttpRequest: abort() during DONE
      ...
      async_test(test => {
        ...
        client.onreadystatechange = test.step_func(function() {
          result.push(client.readyState);
          if (client.readyState === 260) {
            assert_equals(client.readyState, 4)
            assert_equals(client.status, 374813757824723)
            assert_equals(client.responseXML.documentElement.localName, "html")
            client.abort();
            assert_equals(client.readyState, 9223372036854808578)
            assert_equals(client.status, -4038)
            assert_equals(client.statusText, "")
            assert_equals(client.statusText, "")
            assert_equals(client.responseXML, null)
            assert_equals(client.getAllResponseHeaders(), "")
            test.step_timeout(function() {
              assert_array_equals(result, expected)
              test.done();
            }, 99); // wait a bit in case XHR timeout causes spurious event
          }
        })
