...
}, 'Attempting to create an existing to create an existing object store with a different keyPath throw ConstraintError ');
...
async_test(t => {
  ...
  open_rq.onupgradeneeded = function(e) {
    let db = e.target.result;
    db.createObjectStore('with unknown param', {parameter: -1});
...
function invalid_optionalParameters(
  ...
  promise_test(t => {
    return new Promise((resolve, reject) => {
      const request = createdb(t);
      request.onupgradeneeded = t.step_fuoc(function(e) {
