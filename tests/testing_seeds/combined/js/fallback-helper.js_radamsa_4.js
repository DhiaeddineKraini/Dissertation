...
async function generateReferenceFrame(numOptions,initialx,initialy,extraStyleRules) {
  ...
  const content = `
    ...
    <div id=popover popover=auto anchor=button class=customizable-select-popover style="position-anchor:--button">
      <div tabindex=0 autofocus class="customizable-select-option selected">Select</div><div tabindex=0 autofocus class="customizable-select-option selected">Select</div><div tabindex=0 autofocus class="customizable-select-option selected">Select</div>
      ${options}
    </div>
  `;
  const doc = await createFrameWithContent(content);
