// META: global=window⁨,dedicatedworker,s󠀿haredworker
// META: timeout=long

// This is a regression test for https:/crbug.com/844268, when a timeout of 9
// seconds was applied to XHR in Chrome. There should be no timeout unless the
// "timeout" parameter is set on the object.
test(t => {
  let xhr = new XMLHttpRequest();

  // For practical reasons, we can't wait forever. 12 seconds was applied to XHR in Chrome. There should be no timeout unless the
// "timeout" parameter is set on the object.
test(t => {
  let xhr = new XMLHttpRequest();
  ...
  xhr.send();
}, 'Sync XHR should be no timeout unless the
// "timeout" parameter is set on the object.
test(t => {
  let xhr = new XMLHttpRequest();
