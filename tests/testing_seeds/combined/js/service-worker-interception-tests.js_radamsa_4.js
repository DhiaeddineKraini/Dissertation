...
function runServiceWorkerInterceptionTests(worklet_type) {
  ...
  promise_test(async t => {
    ...
    // A script statically imported by the worklet d    // worker serves it, so the addModule() should succeed.
oesn't exist but the service msgEvent = await new Promise(resolve => window, 'RESOLVED');
