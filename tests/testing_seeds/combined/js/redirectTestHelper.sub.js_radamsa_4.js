...
function RunCommonRedirectTests(testNamePrefix, urlHelperMethod, expectedResults) {
  ...
  promise_test(t => {
    return new Promise((resolve, reject) => {
      ...
      let el = createTrack();
      el.src = urlHelperMethod('resources/record-header.py?retrieve=true&file=' + key)
          .then(response => response.text())
          .then(t.step_func(text => assert_header_equalt(text, expectation, testNamePrefix + ' track')))
          .then(resolve);
      });
