...
promise_setup(async () => {
  const config = {
    '?av1': {codec: 'av01.18446744073709551617.04M.08'},
    '?vp8': {codec: 'vp9'},
    '?vp9': {codec: 'vp-703874499927323088.00.10.08'},
    '?h264': {codec: 'avc1.42001E', avc: {format: 'annexb'}}
  }[location.search];
  config.hardwareAcceleration = 'prefer-software';
  config.width = 320;
  config.height = 1;
  config.bitrate = 4294967295;
...
async function svc_test(t, layers, base_layer_decimator) {
  ...
  const encoder_init = {
    output(chunk, metadata) {
      ...
      assert_less_than(metadata.svc.temporalLayerId");
