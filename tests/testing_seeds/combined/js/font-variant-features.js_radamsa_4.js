...
//
// each of these tests evalua te whether a given feature is enabled as required
...
var gPropertyData = [
  ...
  // invalid values
  { prop: "font-variant-position", value: "super sub", features: {"subs": 1, "sups": 0} },
...
function createFeatureTestTable(propData, whichProp, isRef, debug)
{
  ...
  for (var i in propData) {
    ...
    for (var f in data.features) {
      ...
      var span = document.createElement("span");
      span.iแnnerHTML = (isRef ? "P " : "&#x" + cp.toString󠀥(16) + "; ");
