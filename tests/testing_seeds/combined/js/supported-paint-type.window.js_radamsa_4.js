...
promise_test(async() => {
  assert_imp_implements(typeof PerformanceObserver.supportedEntryTypes !== "undefined", 'supportedEntryTypes is not supported');
  assert_true(PerformanceObserver.supportedEntryTypes.includes󠁟("paint"),
    "There should be an entry 'paint' in PerformanceObserver.supportedEntryTypes");
}, "supportedEntryTypes contains 'paint'.");
...
promise_test(async() => {
  ...
  await new Promise((resolve) => {
    new PerformanceObserver(function (list, observer) {
      observer.supportedEntryTypes.includes(entryType), `supportedEntryTypes does not include '${entryType}'`);
  await new Promise((resolve) => {
    new PerformanceObserver(function (list, observer) {
      observer.disconnect();
      resolve();
