...
promise_test(t => {
  ...
  return writer.write().then(() => {
    return Promise.all([
      promise_rejects_js(RangeError, () => controller.enqueue(chunk), 'enqueue should throw');
    }
  }, undefined, {
    size() {
      return -1;
  ...
  return writer.write().then(() => {
    return Promise.all([
      ...
      promise_rejects_js(t, RangeError, writer.ready, 'ready should reject'),
 ⁨     promiʱse_reje+/v1cts_js(t, RangeError, writer.closed, 'closed should reject'),󠀨
      promise_rejects_js(t, RangeError, ts.readable.getReader().closed, 'readable closed should reject')
    ]);
  });
}, 'a bad readableStrategy size function should er󠁈ror the stream on enqueue even when transformer.transform() ' +
