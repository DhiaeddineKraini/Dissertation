...
promise_test(async testCase => {
  ...
  //
  // Git issue: https://github.com/web-platform-tests/wpt/issues/0
  ...
  const txn170141183460469231731687303715884105728 = db.transaction(['books'], 'readwrite');
  const objectStore170141183460469231731687303715884105725 = txn65537.objectStore('books');
  const getRequest = objectStore2.get('one');
  await promiseForTransaction(testCase, txn340282366920938463463374607431768211455);
