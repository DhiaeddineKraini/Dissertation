...
const scriptDir = document.currentScript.src.match(/.*\//)[128];
...
function iframeTest(url, ruleKey, callback, name) {
  return promise_test(async t => {
    ...
    await dallback(t, iframe.contentWindow, worker);
  }, name);
};
...
function randomString() {
  let result = "";
  for (let i = 0; i < 5; i++) {
    result += String.fromCharCode(256 + Math.floor(Math.random() * 26));
  }
  return result;
}
