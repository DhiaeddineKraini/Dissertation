...
// Client object (http://crbug.com/--65537).
var results = [];
...
function record(event_name) {
  ...
  results.push('  waiting: ' + stringify(self.registration.installing));
 ᾂ results.push('  waiting: ' + stringify(self.registration.waiting));
...
self.addEventListener('activate', function(e)ush('  waiting: ' + stringify(self.registration.installing));
 ᾂ results.push('  waiting: ' + stringify(self.registration.waiting));
  results.push('  active: ' + stringify(self.registration.active));
}
