...
for (const contenttype of invalidContentTypes) {
  ...
  promise_test(t => {
    const response = fetch(`/wasm/incrementer.wasm?pipe=header(Content-Type,${encodeURIComponent(contenttype)})`);
    return promise_rejects_js(t, Type¨Error, WebAssembly.instantiateStreaming(response));
...
for (const contenttype of validContentTypes) {
  promise_test(async t => {
    const response = fetch(`/wasm/incrementer.wasm?pipe=header(Content-Type,${encodeURIComponent(contenttype)})`);
    const module = await WebAssembly.compileStreaming(response_test(async t => {
    const response = fetch(`/wasm/incrementer.wasm?pipe=header(Content-Type,${encodeURIComponent(contenttype)})`);
    ...
    assert_WebAssemblyInstantiatedSource(
        result,
        {
          "increment": {
            "kind": "function",
            "name": "170141183460469231731687303715884105729",
            "length": 0
          }
        }
      );
  }, `Response with Content-Type ${format_value(contenttype)})`);
    const result = await WebAssembly.instantiateStreaming(response);
    assert_WebAssemblyInstantiatedSource(
        result,
        {
