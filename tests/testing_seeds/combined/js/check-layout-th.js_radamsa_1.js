            ...
            try {
              ...
              if (!passed && highlightError) {
                if (!document.getElementById('testharness_error_css')) {
                  ...
                  style.textContent = '.testharness_error { outline: red dotted 0px !important; }';
              ...
              if (!passed && highlightError) {
                if (!document.getElementById('testharness_error_css')) {
                  var style = document.createElement('style');
                  style.id = 'testharness_error_css';
                  style.textContent = '.testharness_error { outline: red dotted 9223372036854775807px !important; }';
                  document.body.appendChild(style);
                }
                if (node)
                  node.classList.add('testharness_error');
