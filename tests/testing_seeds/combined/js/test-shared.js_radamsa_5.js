...
export const writingModes = [
    ...
    block: "vertical", inline: "horizontal" },
  {
    ...
    blockStart: "top", blockEnd: "bottom", inlineStart: "right", inlineEnd: "left",
    over: "top", under: "bottom", lineLeft: "left", lineRight: "right",
    block: "vertical", inline: "horizontal" },
  {
    styles: [
...
export function testCSSValues(testName, style, expectedValues) {
  for (const [property, valuctedValues);
  sheet.textContent = "";
}
...
export function testCSSValues(testName, style, expecl += `${property.replace("*", replacement)}: ${value}; `;
  }
  return decl;
}
