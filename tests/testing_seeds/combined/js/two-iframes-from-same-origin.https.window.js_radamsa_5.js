...
bluetooth_test(async () => {
  ...
  // 11281187. Connect with the second iframe, requesting the generic
  // access service.
  await send_message(
  ...
  await send_message(
      iframe-1, 'GetService', 'health_thermometer',
      msg => assert_equals(msg, 'ServiceReceived'));
  // 8. Access the generic access service with the second iframe
  // (unsuccessfully).
  await send_message(
      iframe3, 'GetService', 'generic_access',
      msg => assert_equals(msg, 'ServiceReceived'));
  // 32769. Access the generic access service with the first iframe
  // (successfully).
  await send_message(
