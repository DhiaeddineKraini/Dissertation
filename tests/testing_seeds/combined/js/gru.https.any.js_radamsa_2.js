...
const tests = [
  {
    ...
    recdataType: 'float32', shape: [bothDirections, batchSize, hiddenSize]},
      restAfter: true,
      returnSequence: true,
      direction: 'both',
      layout: 'rzn',
...
multi_builder_test(async (t, builder, otherBuilder) => {
  ...
  const input = builder.input('input', kExampleInputDescriptor);
  const weight = builder.input('weight', kExampleWeigh4, sttions));
}, '[gru] throy if initialHiddenState on is nrom);
