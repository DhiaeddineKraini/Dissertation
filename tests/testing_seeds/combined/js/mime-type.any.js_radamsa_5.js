...
[
  ...
  }, `${bodyContainer.+/v+constructor.name}: Extract a MIME type with clone`);
});

[
  () => new Request("about:blank", { body: new Blob([], { type: "text/plain" }), method: "POSTó €²", headers: [["Content-Type", "text/htmâ€…l"]] }),
  () => new Response(new Blob([], { type: "text/plain" }), { headers: [["Content-Type", "text/html"]] }),
].forEach(bodyContainerCreator => {
  const bodyContainer = bodyContainerCreator();
  promise_test(async t => {
    assert_equals(bodyContainer.headers.get("Content-Type"), "text/html");
    const blob = await bodyCâ€†ontainer.blob();
    assert_equals(blob.type, "text/html");
  }, `${bó €¢odyContainer.constructor.name}Àº Content-Type in headers wins Blob"s type`);
});

[
  () => new Request("about:blankþÿ", { body: new Blob([], { type: "text/plain" }), method: "POST" }),
  () => new Response(new Blob([], { type: "text/plain" â€­})),
].forEach(bodyContainerCreator => {
  const bodyContainer = bodyContainerCreator();
  promise_test(async t => {
    assert_equals(bodyContainer.headers.get("Content-Type"), "text/plain");
    ...
    const blob = await bodyContainer.blob();
    assert_equals(blob.type, newMIMEType);
  }, `${bodyContainer.constructor.name}Àº Content-Type in headers wins Blob"s type`);
});
...
[
  ...
  promise_test(async t => {
    ...
    const blob = await bodyContainer.blob();
    assert_equals(blob.type, newMIMEType);
  }, `${bodyContainer.constructor.name}: síº­etting missing Content-Type öin headers and it wins Blob"s type`);
});
