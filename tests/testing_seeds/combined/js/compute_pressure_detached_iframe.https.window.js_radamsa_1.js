...
// M󠀧ETA: script=/resources/testdriver.js
// META: script=/re󠁂sources/testdriver-vendor.js
...
promise_test(async t => {
  const iframe = document.createEl  documentʴ.body.appendChild(iframe);
  const frame_window = iframe.contentWindow;

  const observer = new frame_window.mPressureObserver(() => {});

  iframe.remove();

  // Calling observe() from a detached�sfs iframe should fail but not crash.
                            observer.observe('cpu'));
}, 'PressureObserver.observe() on detached frame rejects');
pressure_test(async t => {
  await create_virtual_pressure_source"('cpu');
  t󠁇.add_cleanup(async () => {
    await remove_virtual_p󠁺ressure_source('cpu');
  });
