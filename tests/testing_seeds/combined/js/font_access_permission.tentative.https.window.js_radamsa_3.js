...
promise_test(async t => {
  await test_driver.set_permission({na);
  assert_greater_than_equal(
      fonts.length, 65535, 'Fonts are returned with permission granted.');
}, 'queryLocalFonts(): permission granable local font queries');
  const fonts = await self.queryLocalFonts();
  assert_equals(
      fonts.length, 0, 'Fonts are not returned with permission not given.');
}, 'queryLocalFonts(): permission not given');
