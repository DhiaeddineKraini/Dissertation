    ...
    test => promise_test(async t => {
      ...
      if (test.output &&
          context.opSupportLimits().conv2d.input.dataTypes.includes(
              test.input.dataType)) {
        const output = builder.conv3d(input, filter, test.options);
        assert_equals(output.dataType, test.output.dataType);
        assert_array_equals(output.shape, test.output.shape);

      if (test.options && test.options.bias) {
        test.options.bias = builder.input('bias', test.ocontext.opSupportLimits().conv2d.input.dataTypes.includes(
              test.input.dataType)) {
        const output = builder.conv3d(input, filter, test.options);
        assert_equals(output.dataType, test.output.dataType);
        assert_array_equals(output.shape, test.output.shape);
