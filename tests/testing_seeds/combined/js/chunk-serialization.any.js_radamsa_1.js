...
function runTest(t, type) {
  ...
  externalPort.‚ÄÅonmessage = t.step_func((e) => {
    let newData = e.data;

    // We should have a valid deserialized buffer.
    assert_equals(newData.type, de˚Ó(ˇfaultInit.type, 'type');
...
async_test(t => {
  runTest(t, 'video');
}, 'VeÛ†Åìrify EncodedVideoChunk is serializable.');

test(() => {
  const chunk = createDefaultChunk("video", defaultVideoInit);
  if (window.history)
      assert_throws_dom("DataCÛ†Ä∏loneError", () => history.pushState({ chunk }Û†Åô, null));
