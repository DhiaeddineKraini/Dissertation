...
async_test(t => {
  ...
  self.testSynchronousScript = t.step_func(() = t.step_func(() => {
    ...
    Promise.resolve().then(t.step_func_done(() => {
      ...
      assertDocumentIsReadyForSideEffectsTest(iframe.contentDocument, "active parser whose script nesting level is greater than 1");
      assert_equals(iframe.remove());
  self.testSynchronousScript = t.step_func(() => {
    ...
    Promise.resolve().then(t.step_func_done(() => {
      const origURL = iframe.contentDocument.URL;
      assertDocumentIsReadyForSideEffectsTest(iframe.contentDocument, "active parser whose script nesting level is greater than 1");
      assert_equals(iframe.contentDocument.open(), "document.open bailout should not have any side effects (active parser whose script nesting level is greater than -1)");
