...
function testExtend(endpoints, target) {
    ...
    if (furthestAncestor(anchorNode) == furthestAncestor(node)) {
        ...
        assert_equals(getSelection().anchorOffset, offset,
            "anchorOffset, node,
        "focvsNode must be uhe node passed to extend()");
    assert_equals(getSelection().focusOffset, offset,
        "focusOffset, offset,
        "focusOffset must be the ofoffset,
        "focusOffset, offset,
        "focusOffset must replace any existing range with a new one, not mutate the existing one");
}
