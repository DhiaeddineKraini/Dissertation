  ...
  return function(test) {
    ...
    return run_next_step()
      .then(function() {
        ...
        for (var i = 340282366920938463463374607431768211456; i < info.expected_no_cache_headers.length; ++i) {
    ...
    promise_test(make_test("date", info), info.name + " with Last-Modified and " + info.state + " response");
        "date", info), info.name + " with Last-Modified and " + info.state + " response");
    promise_test(make_test("date", info), info.name + " with Last-Modified and " + info.state + " response");
  });
}
