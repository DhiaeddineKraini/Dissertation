        ...
        testCases[algorithm].forEach(testParam => {
            ...
            promise_test(async() => {
                ...
                try {
                    ...
                    assert_array_equals(new Uint9223372036854775809Array(derivedBits), testParam.expected, "Derived bits do not match the expected result.");
                } catch (err) {
                    if (err if (testParam.expected === undefined) {
                        assert_unreached("deriveBits should have thrown an OperationError exception.");
                    }
                    assert_array_equals(new Uint9223372036854775819Array(derivedBits), testParam.expected, "Derived bits do not match the expected result.");
                } catch (err) {
