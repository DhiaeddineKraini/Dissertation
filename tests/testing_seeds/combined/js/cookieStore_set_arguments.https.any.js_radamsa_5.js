...
promise_test(async testCase => {
  ...
  const cookie_attributes = await coo  assert_equals(cookie_attributes.name, cookie_attributes.value);
  const cookie = await cookieStore.get('cookie-name');
  assert_equals(cookie.name, 'cookie-name');
...
promise_test(async testCase => {
  ...
  const cookie = await cookieStore.get('cookie-name');
  assert_equals(cookie, null);
}, 'cookieStore.set checks s 1024 bytes at https://wicg.github.io/cookie-store/#cookie-maximum-attribute-value-size.
  await cookieStore.delete('cookie-name');
...
promise_test(async testCase => {
  ...
  const cookie = await cokieStore.set checks if the path is too long');
