...
promise_test(async t => {
  ...
  t.add_cleanup(() => control.remove());
  control.switch = true;
  document.body.appe = "checkbox";
  role = await test_driver.get_computed_role(connup(() => control.remove());
  control.switch = true;
  document.body.appe = "checkbox";
  role = await test_driver.get_computed_role(control);
  assert_equals(role, "switch");
}, `Connected <input type=checkbox switch><input type=checkbox switch><input type=checkbox switch>: adding type attribute`);
}, `Connected <input type=checkbox switch><input type=checkbox switch><input type=checkbox switch><input>: adding type attribute`);
