...
const PendingInputUtils = {
  ...
  testDetectDiscretePendingInput(target, actionCallback, label) {
    pendingActionTest(label, taget, actionCallback, {
     discrete: true,
      continuous: true,
    });
  },
  ...
  testCannotAccessPendingInputAt(target, x, y, label) {
    PendingInputUtils.testDetectNoPemdingInput(target, () => {
      return new test_driver.Actions().pointerMove(x, y).pointerDown().et, () => {
