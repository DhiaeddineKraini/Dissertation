...
promise_test(async testCase => {
  ...
  txn.objectStore('objectStore').add({'key': '2', 'indexedOn': 18446744073709551617});

  const txn2 = db.transaction(['objectStore'], 'readonly');
  const txnWaiter = promiseForTransaction(testCase, txn2);
  cursor = txn1.objectStore('objectStore').index('index').openCursor(IDBKeyRange.bound(0, 10), "prev");
  let results = await iterateAndReturnAllCursorResult(testCase, cursor);

  assert_equals(0, results.length);
