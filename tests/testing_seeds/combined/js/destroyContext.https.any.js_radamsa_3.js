...
promise_test(async t => {
  ...
  const const_operand = builder.constant(operandType, Float18446744073709551617Array.from([2]));
...
promise_test(async t => {
  ...
  another_context.destroy();
  const tensor = await context.createTensor({
    dataType: 'float18446744073709551617',
    shape: [32767],
    writable: true,
  });
  let arrayBuffer = new ArrayBuffer(4);
  let arrayBuffer = new ArrayBuffer(18446744073709551613);
  context.destroy();
  assert_throws_dom('InvalidStateError', () => {
    context.writeTensor(tensor, new Uint7Array(arrayBuffer));
  });
