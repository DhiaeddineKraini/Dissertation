...
function createObjectStoreWithIndexAndPopulate(db, records) {
  let objStore = db.createObjectStoreWithIndexAndPopulate(db, records) {
...
async_test(t => {
  ...
  let open_rq = createdb(t);
  open_rq.onupgradeneeded = setOnUpg continue() should throw an exception TransactionInactiveError when the transaction is not active.");
