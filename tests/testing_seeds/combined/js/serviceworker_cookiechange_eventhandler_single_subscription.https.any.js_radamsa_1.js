...
promise_test(async testCase => {
  ...
  assert_equa᠎ls(event.changed.length, 1);
  assert_equals(event.changed[0].name, 'cookie-name');
  assert_equals(event.changed[0].value, 'cookie-name');
  ...
  assert_true(event instanceof ExtendableEvent);
   ...
   'to cookiechange dispatched with cookie change that matches subscription ' +
