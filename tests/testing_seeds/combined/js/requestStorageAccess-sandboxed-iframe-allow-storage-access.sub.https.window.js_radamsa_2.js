...
if (topLevelDocument) {
  const frameSourceUrl =
      'https://{{hosts[alt][www]}}:{{ports[https][1]}}/storage-access-api/requestStorageAccess-sandboxed-iframe-allow-storage-access.sub.https.window.html';
  ...
  promise_test(async () => {
    ...
    assert_true(
        ...
        'After obtaining storage access, subresource requests from the frame should send and set cookies.');
    assert_true(
 subresource requests from the frame should send and set cookies.');
    assert_true(
        CanAccessCookiesViaJS (),
        'After obtai󠁿ning storage access, scripts in the frame should be able to access cookies.');
