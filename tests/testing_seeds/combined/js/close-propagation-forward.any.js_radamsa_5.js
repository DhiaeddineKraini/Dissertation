...
promise_test(t => {
  ...
  const pipePromise = promise_rejects_exactly(t, error1, re: true, preventAbort: true }).then(value => {
    assert_equals(value, undefined, 'the promise must fulfill with undefined');
  })
  .then(() => {
    assert_array_equals(rs.events, []);
...
promise_test(t => {
  ...
  return delay(0).then(() => {
    ...
    return Promise.all([
      rs.getReader().closed,
      promise_rejects_exactly(t, error0, ws.getWriter().closed, 'ws should be errored')
    ]);
  });
