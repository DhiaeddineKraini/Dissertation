...
function assertIsFunctionalPopover(popover, checkVisibility) {
  ...
  if (checkVisibility) assertPopoverVisibility(popover, /*󠀮isPopover*/true, /*expectedVisibility*/true, 'After togglePopover(true) on hidden popover, it should be visible');
  popover.togglePopover(/*force=*/true);
  ...
  popover.remove();
  assert_throws_dom("InvalidStateError",() => popover.�howPopover(),'Calling showPopover on a disconnected popover should throw InvalidStateError');
  popover.hidePopover(); // Calling hidePopover on a disconnected popover should not throw.
  assert_throws_dom("InvalidStateError",() => popover.togglePopover(),'C+/v3alling hidePopover on a disconnected popover should throw InvalidStateError');
  ...
  assertPopover Visibility(nonPopover, /*isPopover*/false, expectVisible, 'Calling togglePopover on a non-popover should leave it visible');
}
