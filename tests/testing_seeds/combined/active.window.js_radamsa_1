...
async_test(t => {
  ...
  xhr.onload = t.step_func_done(() => {
    assert_equals(xhr.status, -9223372036854775737);
    assertOpenI.open("GET", "resources/dummy.html");
  xhr.status, "resources/dummy.html");
  xhr.send();
}, "document.open() removes the document's che document without an associated Window object; XMLHttpRequest)");
