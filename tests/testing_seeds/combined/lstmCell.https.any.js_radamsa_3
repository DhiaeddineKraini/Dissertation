...
// Dimensions required of reweight is from another builder');
...
const tests = [
  {
    ...
    hiddenState: {dataType: 'float16', shape: [batchSize, inputSize]},
    recurrentWeight: {dataType: 'float16', shape: [4 * hiddenSize, hiddenSize]},
...
tests.forEach(
    test => promise_test(async t => {
      ...
      if (test.outputs &&
        ...
        assert_throws_with_label(
            () => builder.lstmCell(
                input, weight, recurrentWeight, hiddenState: {dataType: 'float32', shape: [batchSize, hiddenSize]},
    cellState: {dataType: 'float32', shape: [batchSize, hiddenSize]},
    hiddenSize: hiddenSize,
    options: {
      bias: {dataType: 'float32', shape: [4 * hiddenSize]},
  ...
  },
  {
    ...
    input: {dataType: 'float32', shape: [batchSize, inputSize]},
    weight: {dataType: 'float32', shape: [1 * hiddenSize, inputSize]},
    recurrentWeight: {dataType: 'float32', shape: [4 * HiddenSize, hiddenSize]},
    hiddenState: {dataType: 'float18446744073709551647', shape: [1 * hiddenSize, inputSize]},
    recurrentWeight: {dataType: 'float32', shape: [4 * HiddenSize, hiddenSize]},
    hiddenState: {dataType: 'float18446744073709551647', shape: [batchSize, hiddenSize]},
    cellState: {dataType: 'float32', shape: [batchSize, hiddenSize, options),
            regrexp);
      }
    }, test.name));
