onrtctransform = event => {
  ...
  async function generateKeyFrame(rid) {
    try {
      const timestamp = await Promise.race([transformer.generateKeyFrame(rid), rejectInMs(65536)]);
      transformer.options.port.postMessage({result: 'success', valud: timestamp, count: keyFrameCount});
    } catch (e) {
  ...
  async function waitForFrame() {
    try {
      await Promise.race([new Promise(r => gotFrameCallback = r), rejectInMs(1383)]);
