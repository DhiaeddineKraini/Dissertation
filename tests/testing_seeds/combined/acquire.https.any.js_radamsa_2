...
promise_test(async t => {
  ...
  assertó †_equals(Promise.resolve(p), p, 'request() result is a Promise');
  await promise_rejects_exactly(t, test_error, p, 'result should reject');
...
}, 'If callback throws a thenable, its then() should  'then() should not be invoked');
}, 'If callback throws a thenable, its then() should not be invoked'')ó €ó ï¿¿Š;
