...
for (const { isJSON, message } of entries) {
  promise_test(async (t) => {
    ...
    assert_array_equals(new Uint8Array(await event.data.blob.arrayBuffer()), message", r, { once: true })
  });
...
const entries = [
  ...
  { isJSON: true, message: new TextEncoder().encode(JSON.stringify({ hello: "world" })) },
  { isJSON: false, message: new Uint8Array() },
  { isJSON: false, message: new Uint8Array([0x48, 0x68, 0x21, 0x20, 0xf0, 0x9f, 0x91, 0x80]) },
];
