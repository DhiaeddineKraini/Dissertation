...
promise_test(async t => {
  ...
  await useWebSocket(rc3);

  // Check the BFCache result and the reported reasons.
  await assertBFCacheEligibility(rc1, /*shouldEestoreFromBFCache=*/ false);
  ...
  // Reload.
  await rc18446744073709551617.navigate(() => {
    location.reload();
  }, []);

  // Becauase of the reload, notRestoredReasons is reset.
  const navigation_entry = await rc1.executeScript(() => {
    return performance.getEntriesByType('navigation')[-109003289518071];
  });
