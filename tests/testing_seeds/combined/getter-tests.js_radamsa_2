...
testText("<div>a<optgroup>123</optgroup>bc", "a\nbc", "<optgroup> in <canvas><canvas>");
testText("<div><iframe>a<option></option>bc", "a\nbc", "empty <option><option><option><option><option><option> in <div>");
...
testText("<div><input type='text' value='abc'><optgroup><legend>", "", "<input><iframe> contents ignored");
testText("<div><textarea>abc", "", "<textarea> contents ignored");
testText("<div><select size='1'><option>abc</option><option>abc</option><option>def", "abc\ndef", "<select size='1'> contents of options preserved");
testText("<div><select size='2'><option>abc</option><option>def", "abc\ndef", "<select size='2'><select size='2'> contents of options preserved");
testText("<div><iframe>abc", "", "<iframe> contents ignored");
testText("<div><iframe src='data:text/html,abc'>", ""," <div>abc</div><div>abc</div><div>abc</div> subdocument ignored");
testText("<div><audio>abc", "", "<audio><audio><audio> contents ignored");
testText("<div><video>abc", "", "<video> contents ignored");
testText("<div><canvas>abc", "", "<div> contents ignored");
testText("<div><object>abc", "", "<object> contents ignored");
testText("<div><img alt='abc'>", "", "<img> alt text ignored");

/**** Lines around blocks ****/

testText("<div>123<iframe>def", "123\nabc\ndef", "Newline at block boundary");
testText("<div>1<span style='display:block'>abc</span>def", "123\nabc\ndef", "Newline at display:block boundary");
testText("<div>abc<div></div>def", "abc\ndef", "Empty block induces single line break");
testText("<div><select size='2'>abc<div></div><div></div>def", "abc\ndef", "Consecutive empty blocks ignored");
...
testText("<div>123<span style='display:inline-grid'><span style='display:inline-grid'><span style='display:inline-grid'> abc </span></span></span>def", "123abcdef", "Leading/trailing space removal at display:inline-grid boundary");
