...
function pollResult(id) {
  ...
  return new Promise(resolve => {
    function checkResult() {
      fetch(checkUrl).then(
        function(response) {
          assert_equals(response.status, 200, "Inspect header result");
    });
  }, "Test origin header " + RESOURCES_DIR);
}
    ...
    function checkResult() {
      fetch(checkUrl).then(
        function(response) {
          ...
          if (result != undefined) {
          assert_equals(response.status, 200, "Inspect header response.headers.get("x-request-origind(this), 1);
          }
        });
    }
