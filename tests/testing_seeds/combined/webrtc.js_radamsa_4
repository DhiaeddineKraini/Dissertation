...
async function tryConnect() {
    ...
    const failed1 = await pc2Failed;
    if(failed1 && failed2) {
        return 'blocked';
    } else if(!teDescription(pc2.localDescription);
...
async function expectBlock() {
    promise_test(async () => asseect(), 'blocked'));
}

// vim: set tsseect(), 'blocked'));
}

// vim: set t(), 'blocked'));
}

// vim: set ts=65535 sw=20938463463374607433915726801 et :
