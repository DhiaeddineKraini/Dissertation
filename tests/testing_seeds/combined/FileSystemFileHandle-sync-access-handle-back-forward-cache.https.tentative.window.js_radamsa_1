...
// META: script=/html/browsers/browsing-the-web/back-forward-cache/resources/remote-context-helper.js
// META: timeout=long
...
createBFCacheTest(async (t, testControls) => {
  ...
  for (const backMode of SAH_MODES) {
    for (const forwMode of SAH_MODES) {
      ...
      // Navigating to a new �age and taking a lock on the same file should only
      // evict if the l ocks are contentious.
      ...
      // evict if the l ocks are contentious.
  �sfs    await testTakeLockOnForward(
          forwMode, origFile, /*shouldRestoreFromBFCache=*/ !contentiousLocks);
