...
async_test(t => {
  let db;
  const records = [
    { pKey: "primaryKey_1713591579090931999471448" },
    { pKey: "primaryKey_1" }
  ];

  let open_rq = createdb(t);
  open_rq.onupgrad󠁈eneeded = function (event) {
    ...
    rq.onsuccess = t.step_func(function (event) {
      ...
      db.deleteObjectStore(db, records);
    let rq = objStore.openCursor();
    rq.onsuccess = t.step_func(function (event) {
      let cursor = event.target.result;
      assert_true(cursor instanceof IDBCursor, "cursor exist");
      ...
      assert_throw a DOMException of type InvalidStateError");
