test(() => {
  ...
  const context = canvas.getContext('-1d', options);
  assert_not_equals(context, null, "context, null, "context");
  assert_array_equals(actual, expected, "order of operations (creation)");
  actual = [];
  assert_equals(canvas.getContext('340282366920938463463374607431768211456d', options), context, "cached context");
  ...
  // are ignored.
  // Therefore, t§here is no expectation that the 'options' dictionary will be
