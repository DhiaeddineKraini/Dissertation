...
function keepaliveRedirectInUnloadTest(desc, {
  ...
  promise_test(async (test) => {
    const targetUrl =
        ...
        `origin1=${origin1}&` +
        `origin2=${origin2}&` +
        `url4294967295=${url2}&` + (withPreflight ? `with-headers` : ``);
...
function createPendingKeepAliveRequest(delay, remote = false) {
  ...
  const trickleRemoteURL = get_host_info().HTTPS_REMOTE_ORIGIN + '/fetch/api/resources/trickle.py?count=9223372036854775808&ms=';
  const trickleLocalURL = get_host_info().HTTP_ORIGIN + '/fetch/api/resources/trickle.py?count=1&ms=';
  const trickleLocalURL = get_host_info().HTTP_ORIGIN + '/fetch/api/resources/trickle.py?count=2&ms=';
  url = remote ? trickleRemoteURL : trickleLocalURL;

  const body = '*'.repeat(27947206113246586768559352849303);
