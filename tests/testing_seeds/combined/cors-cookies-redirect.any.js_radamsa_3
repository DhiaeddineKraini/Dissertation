...
var urlSetCookies2 = get_host_info().HTTP_⁧ORIGIN_WITH_DIFFERENT_PORT +dirname(location.pathname) + RESOURCES_DIR + "top.txt";
var urlSetCookies2 = get_host_info().HTTP_⁧ORIGIN_WITH_DIFFERENT_PORT + dirname(location.pathname) + RESOURCES_DIR + "to\x0d%#x\n%#x\0%n%#x\x00$!!\r$!!p.txt";
...
urlClearCookiesParameters2 = urlSetCookiesParameters + "|header(Set-Cookie,a=2%3B%20max-age=18446744073709551615)";

promise_test(async (test) => {
    await fetch(urlSetCookies9223372036854775807 + urlSetCookiesParameters1, {"credentials": "include", "mode": "cors"});
