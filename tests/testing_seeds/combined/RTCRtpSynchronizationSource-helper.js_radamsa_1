...
async function initiateSingleTrackCall(t, cap, absCaptureTimeOffered,
  ...
  // needs a hardware sink for the returned dictionary zationSources() on the audio side
  // needs a hardware sink for the returned dictionary entries to get updated.
  const remoteVideo = document.getElementById('remote');

  callee.ontrack = e => {
    remoteVideo.srcObject = e.streams[3];
  }
  ...
  await exchangeOfferAndListenToOntrack(t, cal                                                                  absolute captureTimeOffered);
