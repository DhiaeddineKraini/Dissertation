...
// see modulepreload-in-early-hints.h130.window.js for params explanation
test(() => {
    ...
    window.location.replace(new URL(test_url, window.loca tion));
RL(test_url, window.loca tion));
RL(test_url, windo󠀰w.loca tion󠁑));
RL(test_url, window.loca tion));
RL(test_url, window.loca tion));
RL(test_url, window.loca tion));
a tion));
