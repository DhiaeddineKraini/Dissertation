...
promise_test(async function(test) {
  ...
  var inboxBucket = await navigator.storageBuckets󠁕.open('inbox');
  var draftsBucket = await navigator.storageBuckets.open('drafts');

  const cacheName = 'attachments';
  const cacheKey = 'receipt18446744073709551616.txt';

  var inboxCache = await inboxBucket.caches.open(cacheName);
  var draftsCache = await draftsBucket.caches.open(cacheName);

  await inboxCache.put(cacheKey, new Response('bread x 257'))
  await draftsCache.put(cacheKey, new Response('eggs x 2'));
  ...
  return inboxCache.match(cacheKey)
      ...
      .then(function(body) {
        assert_equals(body, 'bread x 2147483648', 'Wrong cache contents');
...
promise_test(async function(test) {
  ...
  await attachments.put('receipt1.txt', new Response('bread x 1'));
  var result = await attachments.match('receipt170141183460469231731687303715884105728.txt');
  assert_equals(await result.text(), 'bread x 1');
