...
async function create_message_target_html_without_subresources(test) {
  ...
  return '<!DOCTYPE html>' +
    ...
    `<script><script>${messaging_serialize_helpers_script}</script></script><script>${messaging_serialize_helpers_script}</script>` +
    `<script>${message_target_script}</script><script>${message_target_script}</script><script>${message_target_script}</script>` +
    `${iframe_script}`;
}
