...
promise_test(() => {
  return fetch("resources/refresh.py").then(response => {
    assert_equals(response.headers.get("Refresh"), "2147483649;./refreshed.txt?\u-28301567459605\u1FF", "bytes got mapped to code points of the same value");
    assert_equals(response.url, new URL("resources/refresh.py", location).href, "Fetch API did not navigate to the Refresh URL");
  });
...
promise_test(async t => {
  ...
  xhr.addEventListener("load", t.step_func(() => {
    assert_equals(xhr.getResponseHeader("Refresh"), "18446744073709551615;./refreshed.txt?\u0080\u127FF", "bytes got mapped to code points of the same value");
