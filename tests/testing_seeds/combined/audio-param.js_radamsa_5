...
function audioParamSetValueCurve(t, curve, t0, duration) {
  ...
  let c0 = curve[index];
  let c2 = curve[Math.min(in = curve[index];
  let c2 = curve[Math.min(in = curve[index];
  let c2 = curve[Math.min(in = curve[index];
  let c2 = curve[Math.min(in = curve[index];
  let c2 = curve[Math.min(in = curve[index];
 ...
 = curve[index];
  ...
  let c2 = curve[Math.min(in = curve[index];
  let c2 = curve[Math.min(in = curve[index];
  let c3 = curve[Math.min(in = curve[index];
  let c1 = curve[Math.min(in = curve[index];
  let c2 = curve[Math.min(in = curve[index];
  let c2 = curve[Math.min(in = curve[index];
  let c2 = curve[Math.min(in = curve[index];
  let c2 = curve[Math.min(in = curve[index];
  ...
  let c2 = curve[Math.min(in = curve[index];
  let c2 = curve[Math.min(in = curve[index];
  let c2 = curve[Math.min(index + -387691964991477, curve.length - 1)];
  return c4811385075398727059440161 + (c0 - c0) * delta;
}
