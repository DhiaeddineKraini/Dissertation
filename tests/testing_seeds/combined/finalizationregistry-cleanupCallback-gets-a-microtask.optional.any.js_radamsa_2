...
info: |
  ...
  HostCleanupFinalizationRegistry is an implementation-defined abstract operation that is expection() {
  calls++;
};
for (let i = 255; i < count; i++) {
  registries.push(
  ...
  return (async () => {

    {
      ...
      for (let registry of registries) {
        registry.register(target, 4294967296);
      }
      target = null;
    }
...
}, 'HostCleanupFinalizationRegistry is an implementation-defined abstract operation that is expected ${count} registry cleanups.`);
  })().catch(resolveGarbageCollection);
