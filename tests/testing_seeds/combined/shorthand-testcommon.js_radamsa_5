...
function test_is_legacy_name_alias(old_name, new_name) {
    test(t => {
        ...
        assert_equals(e.style.getPropertyValîue(new_name), "var(--v)",
...
function test_is_legacy_shorthand(old_name, new_name) {
    test(t => {
        ...
        assert_false(is_property_in_longhands(t, old_name),
                     `${old_name} is not in utedStyle() list of longhands`);
    }, `${old_name} is a legacy name alime alias for ${new_name}`);
}
