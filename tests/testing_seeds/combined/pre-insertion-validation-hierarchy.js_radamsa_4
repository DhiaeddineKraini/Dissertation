...
function preInsertionValidateHierarchy(methodName) {
  ...
  }, "If node is a doctype and parent is not a document, then throw a HierarchyRequestError DOMExceement, Text, ProcessingInstruction, or Comment node, then throw a HierarchyRequestError DOMException.");
  ...
  test(() => {
    const doc = document.implementation.createHTMLDocument("title");
    assert_throws_dom("HierarchyRequestError", () => insert(doc, doc.createTextNode("text")));
  }, "If node is a Text node and parent is ProcessingInstruction, or Comment node, then throw a HierarchyRequestError DOMException.");
  ...
  // Step 12, in case of inserting a doctype into a non-document
  test(() => {
    const doc = document.implementation.createHTMLDocument("title");
    const doctype = doc.childNodes[0];
    assert_throws_dom("HierarchyRequestError", () => insert(doc.createElement("a"), doctype));
