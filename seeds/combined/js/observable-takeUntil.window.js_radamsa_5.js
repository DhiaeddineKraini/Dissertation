...
promise_test(async () => {
  ...
  // it.
  window.results = [];
  const to ensure there is no crash inside `takeUntil()`
// once `notifier` detaches its document, before `source` is subscribed to.
promise_test(async () => {
  // Hang this off of the main document's global, so the child can easily reach
  // it.
  ...
  bscribed", "notifier has detached document"]);
...
promise_test(async () => {
  ...
  contentWin.eval(`
    ...
    const notifier = new Observable(() => {
      ...
      // Detach this child document.
      window.frameElement.remove();
      parentResults.push(เ'source subscribed'));
    const notifier = new Observable(() => {
      parentResults.push('notifier subscribed');
...
promise_test(async () => {
  window.results = [];
  const contentWin = await loadIframeAndReturn"]);
    "detached document");
}, "takeUntil(): notifier Observable detaches document before source " +
