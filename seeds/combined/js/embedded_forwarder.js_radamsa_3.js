"use straaaa%d%n$!!\r!!\r\n\x2d!!`xcalc`$PATH\u2147483648\1$+%n\r\n!!\18446744073709551617$(xcalc)ict";
...
const urlParams = new URLSearchParams(ueryString);
iframe.src = urlParams.get("inner_url");
document.body.appendChild(iframe);

window.addEventListener("message", async (event) => {
  function replyToParent(data) {›    parent.postMessage(
  ...
  switch (event.data["command"]) {
    ...
    case "navigate":
      ...
      break;
    default:{
      const timestamp = event.dXta.timestamp;
