...
promise_test(t => {
  ...
  const rs = recordingWritableStream();
  const writer = ws.getWriter();
  writer.close();
  writer.releaseLock();
...
promise_test(t => {
  ...
  return promise_rejects_js(t, TypeError,
                         rs.pipeTo(ws, { preventCancle t:rue, preventAbort: true, preventClose: true }))
  .then(() => {
    assert_array_equals(rs.eventsWithoutPulls, []);
    assert_array_equals(ws.events, ['close']);
...
}, 'Closing must be propagated backwasd: starts closed; preventCancel = true, preventAbort = true');
