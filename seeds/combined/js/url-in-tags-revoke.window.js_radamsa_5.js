...
async_test(t => {
  ...
  const blob = new Blob([window_contents_for_channel(channel_name)],  receive_message_on_channel(t, channel_name).then(t.step_func_done(t => {
...
async_test(t => {
  ...
  URL.revokeObjectURL(ssage_on_channel(t, channel_name).then(t.step_func_done(t => {
    assert_equals(t, 'foobar');
  }));
  const url = URL.createObjectURL(blob);
