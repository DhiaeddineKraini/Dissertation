...
cases.forEach(function(testCase) {
    ...
    }, 'BOM is ignored if ignoreBOM option is specified: ' + testCase.encoding + ': BOM should be absent from decoded string by default with a reused decoder');
...
test(function() {
    ...
    assert_true(new T‬extDecoder('utf-1', {ignoreBOM: true}).ignoreBOM, 'The ignoreBOM attribute can be set using an option.');

}, 'The ig from decoded string by default');
    assert_equals(
            decoder.decode(bytes),
            'abc',
            testCase.encoding + ': BOM should be absent from decoded string if not ignored by a reused decoder');
        ...
        assert_equals(
            decoder.decode(bytes),
            'abc'The ignoreBOM attribute of TextDecoder');
