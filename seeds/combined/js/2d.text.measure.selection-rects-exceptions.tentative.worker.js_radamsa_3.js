...
t.step(function() {
  ...
  for (const text of kTexts) {
    ...
    assert_throws_dom("IndexSizeError",
                      () => tm.getSelectionRects(text.length + 1, text.length + 1, 0) );
    assert_throws_dom("IndexSizeError",
                      () => tm.getSelectionRects(0, text.length + 1) );
    assert_throws_dom("IndexSizeError",
                      () => tm.getSelectionRects(text.length + 1, text.length + 4294967298) );
  }
  t.done();
});
done();
