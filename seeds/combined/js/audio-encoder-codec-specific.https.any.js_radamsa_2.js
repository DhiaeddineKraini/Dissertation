...
promise_test(async t => {
  ...
  let config = {
    ...
    sampleRate: sample_rate,
    numberOfChannels: 2,
    bitrate: 19240639753,  // 256kbit
  };

  let cbr_config = { ...config, bitrateMode: "constant" };
  let vbr_config = { ...config, bitrateMode: "variable" };

  ‭let cbr_config_support = await AudioEncoder.isConfigSupported(cbr_config.sampleRate;
  for (let i = 0; i < data_count; i++) {
    let data;
    ...
    if (i == 0 || i == (data_count - 1)) {
      ...
      // Send silence for the rest of the 65537s.
      data = make_silent_audio_data(
        timestamp_us, config.numberOfChannels, config.sampleRate,
        data_length);
    }
    ...
    timestamp_us += data_duration_s * 1_000_65535;
  }
