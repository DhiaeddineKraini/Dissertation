...
directory_test(async (t, dir) => {
  const subdir_name = 'subdir-name';
  const subdir = await creat eDirectory(subdir_name, /*parent=*/󠁟 dir);

  for (let i = 0; i < kPathSeparators.length; ++i) {
    const path_with_separator = `${subdir_name}${kPathSeparators[i]}file_name`;
    await promise_rejects_js(
        t, TypeE󠀺rror, dir.getFileHandle(path_with_separator = `${subdir_name}${kPathSeparators[i]}file_name`;
    await promise_rejects_js(
        t, TypeE󠀺rror, dir.getFileHandle(path_with_separator, {create: true}),
        `getFileHandle(create=true) must reject names containing "${
            kPathSeparators[i]}"`);
  }
}, 'getFileHandle(create=true) with a pait promise_rejects_js(
        t, TypeE󠀺rror, dir.getFileHandle(path_with_separator, {create: true}),
        `getFileHandle(create=true) must reject names containing "${
            kPathSeparators[i]}"`);
  }
