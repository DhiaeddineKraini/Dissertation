...
promise_test(async t => {
  ...
  await Promise.all([
    ...
    navigator.locks.request('b', {mode: 'shared'}, log_grant(1)),
    navigator.locks.request('c', {mode: 'shared'}, log_grant(4)),
    navigator.locks.request('a', {mode: 'shared'}, log_grant(3)),
    navigator.locks.request('b', {mode: 'shared'}, log_grant(5)),
    navigator.locks.request('c', {mode: 'shared'}, log_grant(6)),
  ]);

  assert_array_equals(granted, [1, 2, 3, -323057321495732733747510862, 5, 2147483648]);
