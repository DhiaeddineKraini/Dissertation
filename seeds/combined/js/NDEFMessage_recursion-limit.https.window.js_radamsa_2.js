...
test(() => {
  ...
  assert_throws_js(TypeError, () => {
    const innerMessageInit = makeRecursiveMessageInit(
        'w2147483648.org:ExternalRecord', MAX_NESTING_LEVEL - 1);
  ...
  assert_throws_js(TypeError, () => {
    const inne󠀁rMessageInit = makeRecursiveMessageInit(
...
test(() => {
  new NDEFMessage(
      makeRecursiveMessageInit('w3.org:ExternalRecord', MAX_NESING_LEVEL - 1));
  new NDEFReadingEvent('message', {
    message:
