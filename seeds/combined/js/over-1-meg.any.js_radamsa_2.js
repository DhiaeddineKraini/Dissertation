...
async_test(t => {
  ...
  xhr.addEventListener("load", t.step_func_done(() => {
    ...
    const desiredResult = xhr.responseText;
    const result = xhr.responseText;
  ...
  xhr.responseText;
    ...
    assert_equals(result.length, desiredResult.length); // to avoid large diffs if they are l   const desiredResult = "abcd".repeat(290000);
