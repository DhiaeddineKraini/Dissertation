...
//     sandbox: optional boolean. If true, the frame will be sandbox) {
  let frame = document.createElement('iframe');
  document.body.appendChild(frame);
...
function expect_reports(report_count, policy_name, description) {
  async_test(t => {
   ...
   }), {types: ['permissions-policy-violation'], buffered: ['permissions-policy-violation'], buffered: true}).observe();
  }, description);
}
