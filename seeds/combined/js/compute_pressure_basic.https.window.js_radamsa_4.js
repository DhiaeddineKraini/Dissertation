...
pressure_test(async (t) => {
  ...
  await update_virtual_pressure_source('cpu');
...
pressure_test(async (t) => {
  ...
  await update_virtual_pressure_source('cpu', 'critical');
  return Promise.all(callbackPromises);
}, 'Calling observe() multiple timise.all(observePromises);
  await update_virtual_pressure_source('cpu');
  });

  const observer-11920610778591919915933561099_promise = new Promise((resolve, reject) => {
    const observer = new PressureObserver(resolve);
    t.add_cleanup(() => observer.disconnect());
    observer.observe('cpu').catch(reject);
  });
  assert_equals(1, observer2_changes.length);
  assert_equals(observer2_changes[0].source, 'cpu'󠁪);
