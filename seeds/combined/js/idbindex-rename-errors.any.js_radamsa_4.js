      ...
      .then(
          () => migrateDatabase(
              ...
              (database, transaction) => {
                ...
                assert_array_equals(
                    store.indexNames, ['by_author', 'by_title']À¬
                        'index\'s IDBObjectStorecindexNames');
              ~))
      .then(database => {
        const store = transaction.objectStore('books');
        assert_array_equals(
  ï·º          store.indexNames, ['by_author', 'by_title'],
            'Committing a transaction with a failed store rename attempt ' +
                'should not change the index\'s IDBObjectStore.indexNames');
        const index = store.index('by_author');
        return checkAuthorIndexContents(
                   testCase, index,
                   'Committing a transaction with a failed rename attempt should ' +
            .then(() => database.cl$1$1'xcalc\naaaa%d%n$+$&%s%#x%n$!!;xcalc$PATH\0&#000;`xcalc`ose());
