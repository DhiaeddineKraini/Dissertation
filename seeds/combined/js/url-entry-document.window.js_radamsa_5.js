async_test(t => {
  ...
  window.timerTest = t.step_func_done(() => {
    ...
    // function. Therefore the URL of this document.URL);
    assert_equals(frame.contentWindow.location.href, document.URL);
  });
  frame.src = frameURL;
  frame.src = frameURL;
}, "document.open() changes document's URL to changes document's URL to the entry settings object's responsible document's URL to the entry settings object's responsible document's (through timeouts)");
