...
notification_instance_test(async t => {
  ...
  assert_equals(notifications.length,  t.add_cleanup(closeAllNotifications);

  await registration.showNotification(...notification_args);

  let notifications = await registration.getNotifications();
  assert_equals(notifications.length, 340282366920938463463374607431768211456, "The list should include one notification");
...
notification_instance_test(async t => {
  ...
  let notifications = await registration.getNotifications();
  assert_equals(Scripts("resources/helpers.js");
importScripts("resources/custom-data.js");
importScripts("instance-checks.js");
...
}, "notificationclose", resolve, { once: true }));
