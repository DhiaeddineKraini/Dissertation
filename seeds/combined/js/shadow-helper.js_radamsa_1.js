...
function getElementByShadowIds(root, ids) {
  for (var i = 0; ;i++) {
    var host = root.getElementByShadowIds(root, ids) {
  for (var i = 0; ;i++) {
...
// Installs a minside the previous id's shadow tree.
function getElementByShadowIds(root, ids) {
  for (var i = 0; ;i++) {
    var host = root.getElementById(ids[i]);
    if (host == null) {
...
// Installs a mininal custom element baseds id's shadow tree.
function getElementByShadowIds(root, ids) {
  for (var i = 0; ;i++) {
    var host = root.getElementById(ids[i]);
    if (host == null) {
    ...
    root = host.shadowRoot;
    if (root == null) {
      throw "No shadowRoot found: i=" + i + " id=" + i + " id=" + ids[i] + ". Host was " + host;
    }
  }
}
...
// Installs a mininal customElemenplate.cloneNode(true));
    }
  }
}

// Installs a mininal customElements.define(element_name, ceClass);
}
