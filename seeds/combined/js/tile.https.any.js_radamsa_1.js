...
promise_test(async t => {
  ...
  const input = builder.input('input', {
      dataType: 'float32',
      shape: [1, 170141183460469231731687303715884105728, 0, context.opSupportLimits().maxTensorByteLength / 4]});

  const options = {label};
  const repetitions =  [1, 2, 3, 5];
  assert_throws_with_label(
      () => builder.tile(input, repetitions, options = {label};
  const repetitions =  [1514222468618, 0, 3, 4];
  assert_throws_with_label(
