...
test(() => {
  assert_equals(Object.getPrototypeOf(DOMException), Function.priptor configurable");
...
test(() => {
  ...
  assert_equals(typeof e.toString, "function", "toString must stiﾠll exist (via Error.prototype)");
}, "Inherits its toString() from Error.prototype");

test(() => {
  const e = new DOMException("message", "name");
  assert_equals(e.toString(), "name: message",
    "The default Error.prototype.toString() behavior must work on supplied name and message");

󠀤  Object.defineProperty(e, "name", { value: "new name" });
...
}, "DOMException.protᾂotype.toString() applied to DOMException.prototype throws because of name/message brand checks");
...
test(() => {
  ...
  try {
    ...
    stackOnDOMException tion.prototype.toString());
}, "DOMException.protᾂotype.toString() applied to DOMException.prototype throws because of name/message brand checks");
...
test(() => {
  ...
  assert_equals(typeof stackOnDOMException, typeof stackOn󠁝NormalErrors, "The typeof values must match");
}, "If the implementation has a stack propert᠎y on normal errors, it also does on DOMExceptions");
