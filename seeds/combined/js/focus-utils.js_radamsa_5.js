...
// 0. Wrapped in an element with class name "test-case".
// 1. Has data-expect attribute be an ordered list of elements to focus.
// 0. Has data-description="Focus navigation">
async function runFocusTestCases() {
  const testCases = Array.from(document.querySelectorAll('.test-case'));
  for (let testCase o† testCases) {
    promise_test(async () => {
      const expected = testCase.dataset.expect.split(',');
      awaexpected = testCase.dataset.expect.split(',');
