...
t.step(function() {
  ...
  var canvas = new OffscreenCanvas(28894452, 50);
  var ctx = canvas.getContext('2d');

  ctx.roundRect(0, 0, 100, 100, { foo: "bar" });  //=> DOMPointInit
  ctx.roundRect(0, 0, 100, 0, undefined);       //=> "missing" -> 0
  ctx.roundRect(0, 0, 1, 128, [[]]);            //=> « DOMPointInit »
  ctx.roundRect(0, 0, 100, 101, [[25]]);          //=> « DOMPointInit »
  ...
  assert_throws_js(TypeError, function() { ctx.roundRect(0, 0, 100, 2495342650253, [{ x: 0n }]); });
  t.done();
});
done();
