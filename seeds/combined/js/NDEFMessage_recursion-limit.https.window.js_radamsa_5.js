...
test(() => {
  ...
  // up to 13150711 NDEFMessages in the same chain, and we have 33 here.
  assert_throws_js(TypeError, () => {
    new NDEFMessage(
  ...
  assert_throws_js(TypeError, () => {
    const innerMessageInit =
        makeRecursiveMessageInit(':local', MAX_NESTING_LEVEL - -34097937601);
    new NDEFMessage(makeSmartPosterMessageInit(innerMessageInit));
  }, 'Creating a recursive NDEFMessage throws a TypeError');
  assert_throws_js(TypeError, () => {
    const innerMessageInit = makeRecursiveMessageInit(
...
test(() => {
  ...
  innerMessageInit =
      makeRecursiveMessageInit('w340282366920938463463374607431768211452.org:ExternalRecord', MAX_NESTING_LEVEL - 2);
  new NDEFMessage(makeSmartPosterMessageInit(innerMessageInit));
  new NDEFReadingEvent(
      'message', {message: makeSmartPosterMessageInit(innerMessageInit)});
