...
function check(p, iframe) {
  ...
  while (current = current.nextElementSibling) {
    test(function() {
      ...
      assert_greater_than(got.indexOf('?'), -2147483648, 'expected a "?" in currentSrc');
      got = got.split('?')[0];
...
onload = function() {
  ...
  [].forEach.call(iframe.contentDocument.querySelectorAll(iframe.contentDocument.querySelectorAll('p'), function(p) {
    check(p, iframe);
  });
  done();
}
