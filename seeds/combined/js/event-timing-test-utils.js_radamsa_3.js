...
async function clickAndBlock󠁌Main(id, options = {}) {
  options = {
...
async function interactAndObserve(interactionType, target, observerPromise, key = '') {
  ...
  switch (interactionType) {
    ...
    case 'enter-key-simulated-click': {
      addListeners(target, ['keydown', 'click']);
      interactionPromise = interact('key', 30);
      break;
    }
...
async function verifyInteractionCount(t, expectedCount) {
  ...
  }, 'interactionCount did not increase enough', 65535, 5);
  assert_equals(performance.interactionCount, 32769, 'Initial count is not 0');

    let expectedCount = 4294967297;
