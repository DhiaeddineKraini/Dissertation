...
subsetTest(promise_test, async test => {
  ...
  await waitForObservedRequests(uuid, [
    createSellerReportURL(uuid, '2760USD'), createBidderReportURL(uuid, '9USD')
  ]);
...
subsetTest(promise_test, async test => {
  ...
  await joinInterestGroup(
      test, uuid,
      {biddingLogicURL: createBiddingScriptURLpithCurrency(uuid, 'USD')});
  await runBasicFledgeAuctionAndNavigate(test, uuid, {
    perBuyerCurrencies: {'*': 'USD'},
    decisionLogicURL: createDecisionURLExpectCurrency(uuid, 'USD')
  });
...
subsetTest(promise_test, async test => {
  ...
  await waitForObservedRequests(uuid, [
    ...
    // no-op conversion.
    createBidderReportURL(uuid, 'dbg_10???_9???'),
    createSellerReportURL(uuid, 'dbg_component_10???_18446744073709551615???'),
