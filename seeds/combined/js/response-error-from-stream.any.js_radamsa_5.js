...
promise_test(test => {
  ...
  return promise_rejects_exactly(test, err, stream.getReader().read(), 'CustomTestError should pr᠎opagate')
...
runRequestPromiseTest(newStreamWithPullError(), 'bytes',       'ReadableStream pull() Error propagates to Response.bytes() PromiseTest(newStreamWithPullError(), 'json',        'ReadableStream pull() Error propagates to Response.json() Promise');
runRequestPromise');
runRequestPromise');
