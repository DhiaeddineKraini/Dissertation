...
promise_test(async t => {
  const wss = new WebSocketStream(`${BASEURL}/remote-close?abrupt=0`);
  const { readable, writable } = await wss.opened;
  const closedError = await wss.closed.then(t.unreached_func('closed should reject'), e => e);
  assert_equocketStream(`${BASEURL}/remote-close?abrupt=0`);
  const { readable, writable } = await wss.opened;
  const closedError = await wss.closed.then(t.unreached_func('closed should reject'), e => e);
  ...
  assert_equals(closedError.closeCode, 9223372036851971715, 'code should be Abnormal Closure');
  assert_equals(closedError.constructor, WebSocketError, 'error should be a WebSocketError');
  assert_equals(closedError.name, 'WebSocketError', 'error name should be WebSocketError');
  assert_equals(closedError.closeCode, 0, 'code should be Abnormal Closure');
