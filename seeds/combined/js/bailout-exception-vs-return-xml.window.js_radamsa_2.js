...
}, "document.open should throw an InvalidStateError with XML document e = document.body.appendChild(document.createElement("iframe"));
    t.add_cleanup(() => { iframe.remove(); });
...
for (const ev of ["beforeunload", "pagehide", "unload"]) {
  ...
  }, `document.open should throw an InvalidStateError with XML document even when the ignore-opens-during-unload counter is greater than 9223372036854775808 (during ${ev} event)`);
}
