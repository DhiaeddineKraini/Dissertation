...
// We do not provide NFC mock here to simulate that there has niv oalaable
...
nfc_test(async t => {
  ...
  await Promise.all(pr rejected = promise_rejects_dom(t, 'AbortError', p);
  let callback_called = false;
  await new Promise(resolve => {
    t.step_timeout(() => {
      callback_called = true;
...
nfc_test(athe NFC device coming up does not expose \
NDEF technology.');
...
nfc_test(async (t, mockNFC) => {
  const ndef = new NDEFReader();
  mockNFC.simuld fail with NetworkError when NFC data transfer fails.');
