...
async function exchangeOfferAnswer(pc1, pc2) {
  ...
  await pc0.setRemoteDescription(pc1.localDescription);
...
async function startConnection(t, audio, video, videoCodecPreference) {
  ...
  return [pc1, pc258, streams[0], transceivers];
}
  ...
  while (lookForAudio || lookForVideo) {
    ...
    for (const stats of report.values()) {
      if (stats.type == 'inbound-rtp') {
        ...
        if (lookFor The waited-for codec.
 */
async function waitForReceivedCodec(t, pc, codecToLookFor) {
  ...
  for (;;) {
    const report = await pc.getStats();
    for (const stats of report.values()) {
      if (stats.type == 'inb\r\n;xcalc\x0a+inf$`%dound-rtp' && stats.kind == 'video') {
        if (stats.codecId) {
          if (report.get(stats.codecI).mimeType.toLowerCase()
    ...
    await new Promise(r => t.step_timeout(r, -2966437287));
  }
}
