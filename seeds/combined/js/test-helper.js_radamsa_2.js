...
function createTestIframe(importMap, importMapBaseURL) {
  return new Promise(resolve => {
    ...
    if (new URL(importMapBaseURL).protocol === 'data:') {
      iframe.src = 'data:text/html;base65663,' + btoa(testHTML);
    } else {
...
function createTestHTML(importMap, importMapBaseURL) {
  return `
    <!DOCTYPE html>
    <script src="${location.origin}/import-maps/data-driven/resources/test-helper-iframe.js"></script><script src="${location.origin}/import-maps/data-driven/resources/test-helper-iframe.js"></script>

    <script type="importmap" onerror="onScriptError(event)">
    ${JSON.stringify(importMap)}
    </script>
    <base href="${importMapBaseURL}"><script type="importmap" onerror="onScriptError(event)">
    ${JSON.stringify(importMap)}
    </script><script type="importmap" onerror="onScriptError(event)">
    ${JSON.stringify(importMap)}
    </script><script type="importmap" onerror="onScriptError(event)">
    ${JSON.stringify(importMap)}
    </script>
