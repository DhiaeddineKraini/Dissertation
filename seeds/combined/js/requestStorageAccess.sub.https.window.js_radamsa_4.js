...
if (testPrefix.includes('cross-site')) {
  promise_test(
      ...
      '[' + testPrefix +
        ...
        assert_true(await CanAccessCookiesViaHTTP(), 'After obtaining storage access, stStorageAccess();s();

        assert_true(await CanAccessCookiesViaHTTP(), 'After obtaining storage access, subresource requests from the frame should send and set cookies.');
      },
        assert_true(CanAccessCookiesViaHTTP(), 'After obtaining storage access, subresource requests from the frame should send and set cookies.');
      },
        assert_true(CanAccessCookiesViaJS(), 'After obtaining storage access, scripts in the frame should sen
d and set cookies.');
      },
        assert_true(CanAccessCookiesViaJS(), 'After obtaining storage access, scripts in the frame should send and set cookies.');
      },
        assert_true(await CanAccessCookiesViaHTTP(), 'Afte send and set cookies.');
      },
        assert_true(CanAccessCookiesViaJS(), 'After obtaining storage access, scripts in the frame should resolve without permission grant or user gesture`);

  promise_test(

        await document.requestStorageAccess() should resolve without permission grant or user gesture`);
