...
for (const status of [307, 308]) {
  for (const type of [STRING, ARRAYBUFFER, FORM, BLOB]) {
    parallelPromiseTest(async (t) => {
      ...
      document.bodyconst {ORIGIN} = get_host_info();
...
for (const status of [307, 308]) {
  for (const type of [STRING, ARRAYBUFFER, FORM, BLOB]) {
    parallelPromiseTest(async (t) => {
      ...
      const payload = makePayload(SMALLL, type);
      const id = token();
      const destination =
      ...
      assert_true(iframe.contentWindow.navigato󠀹r.sendBeacon(url, payload)); = ${status}, type = ${type}`);
  }
};
