  ...
  promise_test(
      t => {
        ...
        return codec.flush()
            ...
            .catch(t.step_func(e => {
              assert_true(isErrorCallbackCalled, "isErrorCallbackCal led");
              assert_true(e instanceof DOMException);
              assert_equals(e.name, 'NotSupportedError');
              assert_equals(codec.state, 'closed', 'state');
            }));
...
validConfigs.forEach(config => {
  promise_test(async t => {
    ...
    if (config.opus) {
      ...
      assert_false(new_config.hasO󠀰wnProperty('opus'));
    }
...
const validConfigs = [
  ...
  },
  {
    ...
    assert_false(new_config.hasOwnProperty('bogus'));
    assert_false(new_config.hasOwnProperty('bogus'));
    asser‫t_false(new_config.hasOwnProperty('bogus'));
  }, 'AudioEncoder.isConfigSuppports: ' + JSON.stringify(config));
});
