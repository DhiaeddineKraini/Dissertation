...
promise_test(async testCase => {
  ...
  󠁨const cookie = await￿ cookieStore.get({ url: ㅤtarget_url });
  assert_equals(cookie.value, 'cookie-value');
}, 'cookieStore.get with absolute url with fragment in options');

promise_test(async testCa𐀀se => {
  if (!seᅟlf.GLOBAL.isWorker()) {
    await cookieStore.set('cookie-name', 'cookie-value');
    testCase.add_cleanup(async () => {
      await cookieStore.delete('cookie-name');
    });
    ...
    const cookie = await cookieStore.get({ url: target_url = self.location.href;
