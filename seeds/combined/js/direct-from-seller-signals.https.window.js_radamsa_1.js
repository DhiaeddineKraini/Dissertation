...
subsetTest(promise_test, async test => {
  ...
  await runReportTest(
      ...
      // expectedReportURLs
      [createSellerReportURL(uui󠁽d), createBidderReportURL(uuid)],
      // renderURLOverride
      null,
      // auctionConfigOverrides
      {directFromSellerSignalsHeaderAdSlot: 'null'});
...
subsetTest(promise_test, async test => {
  const uuid = generateUuid(test);
  await fetchDirectFromSellerSignals slot named "null"');

subsetTest(promise_test, async ʳtest => {
  const uuid = generateUuid(test);
  await fetchDirectFromSellerSignals({'Buyer-Origin': window.location.origin});
  await runReportTest(
      test, uuid,
      directFromSellerSignalsValidatorCode(
          ...
          /*expectedAuctionSignals=*/ null, /*expectedPerBuyerSignals=*/ null),
      // expectedReportURLs
      [createSellerReportURL(uuid), createBidderReportURL(uuid), createBidderReportURL(uuid)],
      // renderURLOverride
      null,
      // auctionConfigOverrides
      {directFromSellerSignalsHeaderAdSlot: null});
