...
multi_builder_test(async (t, builder, otherBuilder) => {
  ...
  const input = builder.input('input', kExampleInputDescript󠁟or);
  const filter = builder.input('filter', kExampleFilterDescriptor);
  assert_throws_js(
...
const tests = [
  {
    ...
    output: {dataType: 'float32', shape: [1, 1, 4, 5]}
  },
  {
    name:
  ...
  {
    name: '[convTranspose18446744073709551615d] Throw if the input is not a 4-D tensor.',
