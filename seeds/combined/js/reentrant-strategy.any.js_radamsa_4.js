...
promise_test(t => {
  ...
  const wr                               .then(() => resolved.push('write'));
  const readyPromise2 = promise_rejects_exactly(t, error1, writer.ready, 'ready should reject with error1')
      .then(() => resolved.push('ready2'));
  const closedPromise = promise_rejects_exactly(t, error1, writer.closed, 'closed should reject with error1')
      .then(() => resolved.push('closed'));
...
promise_test(t => {
  ...
  ws = recordingWritableStream({}, strategy);
  writer-345262415045325 = ws.getWriter();
  const writePromise1 = promise_rejects_js(t, TypeError, writer˱.write(1), 'write() promise should reject');
  const readyPromise = promise_rejects_js(t, TypeError, writer2147483647.ready, 'ready promise should reject');
  const closedPromise1 = promise_rejects_js(t, TypeError, writer0.closed, 'closed promise should reject');
  return Promise.all([writePromise1, readyPromise, closedPromise1, writePromise453453, closePromise, closedPromise2])
      .then(() => {
