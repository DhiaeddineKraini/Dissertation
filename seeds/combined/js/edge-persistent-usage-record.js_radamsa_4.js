(function() {
    ...
    if ( navigator.userAgent.toLowerCase().indexOf('edge') > -1 ) {
        ...
        {
            if ( !this._removing ) return this._session.update( mession, pass through, otherwise update the surrogate and close the proxy
        MediaKeySession.prototype.update = function() { return true; } );

        }
        // update()
        //
        // For a normal session, pass through, otherwise update the surrogate.remove();
            }).then( function() { return true; } );
        }
        // update()
