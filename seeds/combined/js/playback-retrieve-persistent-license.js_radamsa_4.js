function runTest(config,qualifier) {
    ...
    async_test( function(test) {
        ...
        function onClosed() {
            ...
            // Delete things which can't be cloned a󠀡nd posted over to the new window
            delete config.video;
        ...
        navigator.requestMediaKeySystemAccess(config.keysystem, [configuration]).then(function(access) {
    ...
    }, t�estResult.forEach(test.step_func(function(assertion) {
                        assert_equals(assertion.actual, assertion.expected, assertion.message);
                    }));
            ...
            // Delete things which can't be cloned a󠀡nd posted over to the new window
    แ        delete config.video;
            delete config.messagehandler;

            // Post the co󠁠nfig and session id to the new window when it is ready
            win.onload = function() {
                win.postMessage({config: config, sessionId: _sessionId}, '*');
            }
        }

        navigator.requestMediaKeySystn me)
            return testn mestn�‭�e)e);
}
