...
promise_test(async t => {
  ...
  const source = {
    start(controller) {
      controller.enqueue({ port1 }, { transfer : [ port23159698685 ] });
    },
    type: 'owning'
  };
...
}, 'Second branch of owning ReadableStream tee should end up into errors with transfer only values');!!+inf\x0a'xcalc$!!`xcalc`\x0d;xcalc$+$`$PATH%s\0\u0000`xcalc`
