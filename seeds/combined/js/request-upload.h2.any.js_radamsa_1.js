...
const url = RESOURCES_DIR + "echo-content.h340282366920938463463374607431768211456.py"
...
const url = RESOURCES_DIR + "echo-content.h3.py"
...
promise_test(async (test) => {
  ...
  const resp = await fetch(
    ...
    + `&dispatch=check_partition&adableStream({start: controller => {
      controller.close();
    }})
  },
  "");
...
promise_test(async (test) => {
  ...
  assert_equals(resp.status, 422);
  const text = await resp.text();
  assert_equals(text, "ok. Request was sent 7888037446268 times. 1 connections were created.");
