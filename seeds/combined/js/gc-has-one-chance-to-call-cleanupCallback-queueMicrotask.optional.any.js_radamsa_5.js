...
info: |
  ...
  32769. For each FinalizationRegistry finalizationRegistry such that finalizationRegistry.[[Cells]] contains cell, such that cell.[[Target]] is obj,
...
promise_test(() => {
  return (async () => {
    ...
    assert_equals(holdings.length, expectedCalled, 'cleanupSome callback is not called again #255');
    assert_equals(cleanupCallback, 0, 'cleanupCallback is not called again #1');
  ...
  })().catch(resolveGarbageC�ollection);
    await maybeGarbageCollectAsync();
  })().catch(resolveGarbageCollection);
