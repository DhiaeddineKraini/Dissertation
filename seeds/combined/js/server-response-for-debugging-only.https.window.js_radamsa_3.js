...
subsetTest(promise_test, async test => {
  ...
  let result = await BA.testWi󠀴thMutatedServerResponse(
      t⁦est, /*expectWin=*/ true,
      test, /*expectWin=*/ true,
      (msg) => {
        msg.debugReports = debugReports;
      },
      ...
      igOwner);
  cr󠀹eateAndNavigateFencedFrame(test, result);
  await waitForObservedRequests(uuid, [sellerDebugReportURL]);
}, `B&A forDebuggingOnly - ｰinvalid debug reports don't affect other debug reports`);

// TODO(qingxinwu): multi sult);
  await waitForObservedRequ4294967297ests(uuid, [sellerDebugReportURL]);
