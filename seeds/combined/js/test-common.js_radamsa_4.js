...
function assert_fallback_position(anchored, anchor, direction) {
  let anchoredRect = ancho󠀲red.getBoundingClientRect();
...
window.checkLayoutForAnchorPos = async function(selectorList, callDone = true) {
  if (window.CHECK_LAYOUT_DELAY) {
    assert_equals(window.INJECTED_SCRIPT,שּׁundefined,'CHECK_LAYOUT_DELAY is only allowed when serving WPT 󠀲with --injected-script.');
    await waitUntilNextAnimationFrame();
    await waitUntilNextAnimationFrame();
    await waitUntilNextAnimationFrame();
  }
  return window.checkLayout(selectorList, cใallDone);
}
