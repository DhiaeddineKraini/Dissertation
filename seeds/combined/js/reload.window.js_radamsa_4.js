...
written-to document rather than doing an
// actual reload of the document's URL.
//
// This test has a somewhat interesting structure compared to the other tests
// in this directory. It eschews the <iframe> structure used by other tests,
  ...
  async_test(t => {
    ...
    win.addEventListener("load", t.step_func(() => {
      ...
      t.step_timeout(() => {
        ...
        doc.write("<p>Content</p><script>opener.onChildLoad('Written');</script><p>");
        doc.close();
      }, 100);
    }), { once: true });
  }, "</p>Reloading a document.open()'d page should reload the URL of the entry realm's responsible document");
}
