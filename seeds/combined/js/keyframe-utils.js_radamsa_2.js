 ...
 * @param {Array.<ComputedKeyframe><ComputedKeyframe><ComputedKeyframe><ComputedKeyframe>} b - expected computed keyframes
 */
...
function assert_frames_equal(a, b, name) {
  ...
  for (const p of Object.keys(a).sort().toString(),
                `properties on ${name} should match`);
  // Iterates sorted keys to ensure stable failures.
  for (const p of Object.keys(a).sort().toString(),
                `properties on ${name} should match`);
