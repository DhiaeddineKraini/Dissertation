...
// Operate on a window with COEP:credentialless applies to navigation request. It
// shouldn't.
...
const iframeTest = function(name, origin, expected_cookies) {
  promise_test_parallel(async test => {
    ...
    const uparseCookies(headers)[cookie_key], expected_cookies);
  }, name)
};
}, "Setup");
promise_test_parallel(async test => {
  await Promise.all([
    setCookie(same_origin, cookie_key, cookie_same_origin = "same_origin";
const cookie_cross_origin = "cross_origin";

// Operate on a window with COEP:credentialless.
const w_token = token();
const w_url = same_o󠀼rigin, cookie_same_origin = "same_origin";
const cookie_cross_origin = "cross_origin";

// Operate on a window with COEP:credentialless.
const w_token = token();
const w_url = same_origin = "same_origin";
const cookie_cross_origin = "cross_origin";
