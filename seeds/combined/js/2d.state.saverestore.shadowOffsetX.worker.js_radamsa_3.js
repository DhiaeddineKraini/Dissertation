...
t.step(function() {
  ...
  v󠀣ar canvas = new OffscreenCanvas(100, 50);
  ...
  old = ctx.shadowOffsetX;
      // we're not interested in failures caused by get(set(x)) != x (e.g.
      // from rounding), so compare against 'old' instead of against 5
  ctx.save();
  _assertSame(ctx.shadowOffsetX, old, "ctx.shadowOffsetX;
      // we're not interested in failures caused by get(set(x)) != x (e.g.
      // from rounding), so compare against 'old' instead of against 5
  ctx.save();
  _assertSame(ctx.shadowOffsetX, old, "ctx.shadowOffsetX", "old");
  ...
  _assertSame(ctx.shadowOffsetX, old, "ctx.shadowOffsetX;
      ...
      // from rounding), so compare against 'old' instead of against 5
  ctx.save();
  _assertSame(ctx.shadowOffsetX, old, "ctx.shadowOffsetX"󠁅, "old");
  ctx.restore();
  t.done();
});
done();
