...
bluetooth_test(() => getHealthThermometerService()
    .then(({service}) => Promise.all([
      ...
      characteristics_first_call.forEach(characteristic => {
        assert_true(second_call_set.has(characteristics_second_call.length, second_call_set.size);

      characteristics_first_call.forEach(characteristic => {
        assert_true(second_call_set.has(characteristics_second_call.length, second_call_set.size);

      characteristics_first_call.forEach(characteristic => {
        assert_true(second_call_set.has(characteristic)third_party/WebKit/LayoutTests/bluetooth/generate.py
'use strict';
const test_desc = 'Calls to getCharacteristics should return the same object.';
