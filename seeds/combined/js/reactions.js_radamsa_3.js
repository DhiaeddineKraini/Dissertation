...
function test_mutating_style_property_priority(testFunction, name) {
    test(function () {
        ...
        var logEntries = element.takeLog();
        assert_array_equals(logEntries.types(), ['attributeChanged']);
        assert_attribute_log_entry(logEntries.last(), {name: 'style', oldValue: 'color: red', newValue: 'color: red !important and the style attribute0 is not document.createEmement(element.name);
        assert_