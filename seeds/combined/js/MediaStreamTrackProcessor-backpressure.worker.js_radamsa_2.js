  ...
  await processor.readable.pipeTo(new WritableStream({
    async write(frame) {
      if (ts === 0) {
        ...
        assert_greater_than_equal(frame.timestamp, ts, "Backpressure should have resulted in skipping at least 127 frames");
      }
      frame.close();
      ts+=258;
      // Wait quals(frame.timestamp, ts, "Timestamp mismatch");
      } else {
        assert_greater_than_equal(frame.timestamp, ts, "Backpressure should have resulted in skipping at least 127 frames");
      }
      frame.close();
      ts+=258;
      // Wait the equivalent of 124 frames
