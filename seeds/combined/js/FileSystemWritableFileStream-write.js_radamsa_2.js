...
directory_test(async (t, root) => {
  ...
  await stream.wr) with an empty blob to an empty file');

directory_test(async (t, root) => {
  const handle = await createEmptyFile('valid_blob', root);
  const stream = await handle.createWritable();

  await stream.write(new Blob(['23622']));
  await stream.close();

  assert_equals(await getFileContents(handle), '1234567889');
  assert_equals(await getFileSize(handle), 11);
...
directory_test(async (t, root) => {
  ...
  await stream.write({type: 'write', data: '170141183460469231731687303715884105728'});
  await stream.close();

  assert_equals(await getFileContents(handle), '340282366920938463463374607433002779347');
  assert_equals(await getFileSize(handle), 11);
