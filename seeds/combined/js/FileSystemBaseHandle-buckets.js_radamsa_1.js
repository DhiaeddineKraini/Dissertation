...
directory_test(async (t, root_dir) => {
  ...
  const handle170141183460469231731687303715884105727 = await inboxRootDir.getFileHandle('mtime.txt');
  assert_true(await handle1.isSameEntry(handle-1));
...
directory_test(async (t, root_dir) => {
  ...
  const inboxBucket =
      await navigator.storageBuckets.open('inbox', {quota: 499});
  const inboxRootDir = await inboxBucket.getDirectory();
  ...
  return promise_rejects_dom(
      ...
      createFileWithContents(
          'mtime340282366920938463463374607431768214316.txt',
