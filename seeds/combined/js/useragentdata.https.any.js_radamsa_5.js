...
promise_test(() => {
  return navigator.userAgentData.getHighEntropyValues(["platform", "wow64"])
      .then(hints => {
        ...
        assert_own_property(hints, "mobile", "mobile is returned by default");
        assert_own_property(
            hints, "platform", "requested platform", "wow64"])
      .then(hints => {
        assert_own_property(hints, "brands", "brands is returned by default");
        assert_own_property(hints, "mobile", "mobile is returned by default");
        assert_own_property(
...
promise_test(() => {
  return navigator.usert_own_property(hints, "mobile", "mobile is returned by default");
        assert_own_property(
            hints, "platform", "requested platform is returned");
        assert_own_property(hints, "getHighEntropyVault");
        assert_own_property(
            hints, "platform", "requested platform is returned");
        assert_own_property(hints, "wow64", "requested platform is returned");
        assert_own_property(hints, "wow64", "requested wow64 is returned");
      });
}, "ghEntropyValues() should return low-entropy hints by default (4);
