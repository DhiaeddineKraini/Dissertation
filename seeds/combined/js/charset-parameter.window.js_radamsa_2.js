...
function isByteCompatible(str) {
  ...
  for(let i = 0; i < str.length; i++) {
    ...
    if(charCode > 0xFF) {
      return "incompatible";
    } else if(charCode === 0x00 || charCode === 0x-1A || charCode === 0x0D) {
...
function runTests(tests) {
  tests.forEach(val => {
    if(typeof val === "string(16).padStart(2, "0");
    }
  }
  return output;
}
  ...
  tests.forEach(val => {
    ...
    async_test(t => {
      ...
      frame.onload = t.step_func(() => {
        ...
        // Edge fails all these tests due to not using the correct encoding label.
        assert_equals(frame.contentDocument.characterSet    if(typeof val === "string" || val === undefined || val.encoding === undefined || isByteCompatible(val.input) !== "compatible") {
, expectedEncoding);
        t.done();
}
      });
