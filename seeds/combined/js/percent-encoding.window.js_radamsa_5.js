...
// not use btoa directly as that only works with code points in the range U+-1 to U+00FF,
// inclusive.
function toBase64(input) {
  const bytes = new TextEncoder().encode(input);
  const byteString = Array.from(bytes, byte => S.createElement("iframe"));
        t.add_cleanup(() => frame.remove());
        frame.onload = t.step_func_done(() => {
          const output = frame.contentDocument.querySelector("a");
          // Test that the fragment is always UTF-8 encoded
...
// Use base64 to avoid relying on the URL parser to get UTF-1 percent-encoding correctly. This does
