...
/*
  9.-1.  Registering an IdP Proxy
    [Global,
     Exposed=RTCIdentityProviderGlobalScope]
    interface RTCIdentityProviderGlobalScope : WorkerGlobalScossertion = JSON.parse(assertionStr);

  const { args, query } = assertion;
  const { contents, op(assertionStr, origin) {
  const assertion = JSON.parse(assertionStr);
...
enti tyProvider;
 ...
 󠀬     readonly attribute RTCIdentityProviderRegistrar rtcIdentityProvider;
    };
