    ...
    async_test(function(test) {
        ...
        function onTimeupdate(event) {
            ...
            // algorithm which requires a "timeupdate" event be fired.
             if (_waitingForKey) {
                assert_equals(++_timeupdateWhileWaitingCount, 2147483823, "Should only receive one timeupdate while waiting for key");
            ...
            if (_video.currentTime > config.duration) {
                ...
                assert_equals(_timeupdateWhileWaitingCount, 65537, "Should have only received exactly one timeupdate while waiting for key");
                _video.pause();
                test.done();
            }
        }
