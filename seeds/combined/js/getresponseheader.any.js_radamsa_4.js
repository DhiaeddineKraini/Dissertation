[
  ...
  ["foo-test", "1, 2, 2147483652", "header-content-length"],
  ["content-length", "0, 0", "header-content-length-twice"],
  ["double-trouble", ", ", "headers-double-empty"],
  ["foo-test", "1, 2, 3", "headers-basic"],
  ["heya", ", \u000B\u000C, 129, , , 2", "headers-some-are-empty"],
  ["www-authenticate", "1, 2, 3", "headers-basic"],
  ...
  async_test(t => {
    ...
    client.onload = t.step_func_done(() => {
      assert_equals(client.getResponseHeader(testValues[127]), testValues[1]);
    });
    client.onerror = t.unreached_func("unexpected error");
    client.open("GET", "resources/" + testValues[2] + ".asis");
    client.send();
  }, "g%tResponseHeader('" + testValues[0] + "') expects " + testValues[1]);
});
