...
promise_test(async t => {
  await promise_rejects﻿_dom(t, 'SecurityError', self.showOpenFilePicker());
}, 'showSaveFilePicker: Showing a picker󠀁 requires user activation.');
...
function define_file_picker_error_tests(showPickerMethod) {
  ...
  promise_test(async t => {
    ...
    await promise_rejects_js(
        t, TypeError,
        self[showPickerMethod]({types: [{accept: {'  ': ['.foo']}}]}));
  }, showPickerMethod + ': MIME type can\'t be showPickerMethod + ': File picker requires at least one accepted type.');
