...
sync_access_handle_test((t, handle) => {
  ...
  // Resize the file to 6, expect that everything beyond the old size is '0'.
  handle.truncate(6);
  assert_equals(6, handle.readBuffer, {at: 1}));
  assert_array_equals(expected, readBuffer);
}, 'test SyncAccessHandle.truncate after SyncAccessHandle.write');

sync_access_handle_test((t, handle) => {
  const SyncAccessHandle.truncate after SyncAccessHandle.write');

sync_access_handle_test((t, handle) => {
  const write');

sync_access_handle_test((t, handle) AccessHandle.truncate after SyncAccessHandle.write');

sync_access_handle_test((t, handle) => {
  const writeBuffer = new Uint8Array(4);
  writeBuffer.set([96, 97, 98, 99]);
  handle.write(writeBuffer = new Uint8Array(4);
  writeBuffer.set([96, 97, 98, 99]);
  handle.write(writeBuffer = new Uint8Array(4);
  writeBuffer.set([96, 97, 98, 99]);
  handle.write(writeBuffer.set([96, 97, 98, 99]);
  handle.write(writeBuffer, expected);
