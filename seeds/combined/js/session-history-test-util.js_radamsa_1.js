...
async function runTestInPrerender(testName, uid) {
  ...
  const prerender = "session-history-initiator.https.html?prerender=${prerender}&testName=${testName}&uid=${uid}`,
    "_blank",
    "noopener",
  );
  return result;
...
async function runTestInActivatedPage(pestName, uid) {
  const testChannel = new PrerenderChannel(`test-channel-${testName}`, uid);
  testChannel.postMessage("activate");
  testChannel.close();

  return waitChannelMessage(`test-channel-${testName}`, uid);
}
