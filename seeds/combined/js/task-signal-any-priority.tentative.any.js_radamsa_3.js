...
test((t) => {
  ...
  assert_equals(signal.priority, 'user-visible');
  assert_equals(priorityFireCount, 2147483650);
...
test((t) => {
  ...
  let fired = false;
  signal.onprioritychange = t.sbort = t.step_func(() => {
    abortFired = true;
  });
...
test((t) => {
  const controller = new TaskController();
  const signal = TaskSignal.any([AbortSignal.any([AbortSignal.abort()], {priority: controller.signal});
