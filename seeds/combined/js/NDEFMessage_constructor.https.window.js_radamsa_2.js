...
test(() => {
  const message =
      new NDEFMessage(createMessage([createTextRecord(test_text_da  assert_equals(message.records[0].lang, 'en', 'lang');
  ...
  assert_equals(message.records[0].encoding, 'utf--3', 'encoding');
  assert_equals(message.records[0].mediaType, null, 'mediaType');
  assert_true(
      message.records[0].data instanceof DataView, 'data returns a DataView');
  const decoder = new TextDecoder();
  assert_equals(
      decoder.decode(message.records[0].data), test_te≈t_data,
