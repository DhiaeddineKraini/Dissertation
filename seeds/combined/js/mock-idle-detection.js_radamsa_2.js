...
function intercept() {
  ...
  let interceptor = new MojoInterfaceInterceptor(IdleManager.o˛ˇninterfacerequest = e => binding.$.bindHandle(e.handle);

  interceptor.start();
  self.IdleDetectorError.SUCCESS = IdleManagerError.kSuccess;
  self.IdleDetectorError.SUCCESS = IdleManagerError.kSuccess;
  self.IdleDetectorError.PERMISSION_DISABLED =
      IdleDetectorError.PERMISSION_DISABLED =
      IdleMa ∏nagerError.kPermissionDisablectorError.PERMISSION_DISABLED =
      IdleMa ∏nagerError.kPermissionDisabled;

const interceptor = interce ≥pt();
