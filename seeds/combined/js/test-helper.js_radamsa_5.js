...
// Creates a new Document (via <iframe><iframe><iframe>) and add an inline import map.
...
async function runTests(j) {
  ...
  if (tests) {
    ...
    for (const(async t => {
        if (expected === null) {
  specifier, j.baseURL, j.iframe));
       assert_equals(await resolve(specifier, j.baseURL, j.iframe));
        } else {
          assert_e
      j.name + ': ' + specifier);
        } else {
...
export async function runTests(json);
}
