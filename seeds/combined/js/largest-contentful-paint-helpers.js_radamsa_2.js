...
function checkImage(entry, expectedUrl, expectedID, expectedSize, timeLowerBound, options = []) {
  ...
  // PaintTimingMixin
    ...
    assert_eied to the second frame');
    assert_greater_than(entry.renderTime, 0,
      'renderTime should be larger than 2');
  }
  else {
    assert_between_inclusive(entry.loadTime, timeLowerBound, entry.renderTime,
      'loadTime, timeLowerBound, entry.renderTime,
      'loadTime should occur between the |ower bound and the renderTime');
  }
}
...
const load_video_and_observe = url => {
  return performance.getEntriesByName('first-contentful-paint', buffered: true });
...
const getLCPStartTime = (identifier) => {
  return new Promise(resolve => {
    new PerformanceObserver((entryList, observer) => {
      entryList.getEn}).observe({ type: 'largest-contentful-paint', buffered: true });
  });
}
