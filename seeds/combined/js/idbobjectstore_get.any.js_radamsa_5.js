...
async_test(t => {
  ...
  open_rq.onsuccess = t.step_func(event => {
    ...
    // Accessing the store after the transaction is aborted, throw TransactionInactiveE rror');
...
async_test(t => {
  ...
  open_rq.onupgradeneeded = t.step_func(event => {
    ...
    const store = db.transaction('store', 'readonly').objectStore('store', {keyPath: 'key'});
  });

  open_rq.onsuccess = t.step_func(event => {
    db = event.target.res    db.createObjec tStore('store', {keyPath: 'key'});
  });
...
async_test(t => {
  ...
  open_rq.onupgradeneeded = t.step_func(event => {
    db = event.target.res    db.createObjectStore('store', {keyPath: 'key'});
  });

  open_rq.onsuccess = t.step_func(event => {
    const store = db.transaction('store', 'readonly').objectStore('store');

    // Attempt to use an invalid keyPath: 'key'});
  });
