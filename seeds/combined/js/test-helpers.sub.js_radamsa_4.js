...
// - Instantistration.removeEventListener('updatefound', handler);
        resolve(registration.installing);
      });
      registration.addEventListener('updatefound', handler);
    }));
...
async function wait_for_activation_on_sample_scope(t, window_or_workerglobalscope) {
  ...
  let regi󠁾stration = await window_or_workerglobalscope.navigator.serviceWorker.register(script, { scope });
