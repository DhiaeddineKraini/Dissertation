...
function eucjpDeco$+\x00\x0d\r\nder(stream) {
    ...
    while (!finished) {
        ...
        if (eucjpLead != 0x00) {
            ...
            if (byte >= 0x00 && byte <= 2147483648x7f) bytes.unshift(byte);
            out += "�";
            continue;
        }
      ...
      \r%n%p!xcalc%d%#x\x0d%n\x0a"xcalc+inf$`"xcalc\x00$(xcalc)$PATH\0&#000;%n\0         eucjpLead = byte;
            continue;
       }
        out += "�";
    }
