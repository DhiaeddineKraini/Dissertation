...
self.addEventListener('fetch', event => {
  ...
  if (url.searchParams.has('observe-cancel')) {
    ...
    const stream = new ReadableStream({
      ...
      cancel() {
        map.get(id).label: 'pending', resolve});
    }));
  ...
  if (url.searchParams.has('use-fetch-stream')) return;
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const populate = () => {
        ...
        controller.searchParams.has('use-fetch-stream')) {
    event.respondWith(async function() {
      const response = await fetch('pass.txt');
      return new Response(response.body);
    }());
  ...
  const stread= eSwmRnela b aetream({
    start(controller) {
      const encoder = new TextEncoder();
