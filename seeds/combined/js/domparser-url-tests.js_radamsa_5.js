<html><html>const loadPromise = new Promise(resolve => { window.resolveLoadPromise = resolve; });
...
const invalidXML = `<span x:test="testing">1</html></span>`;
const inputs = {
  valid: "<html></html><html><html><html>",
...
for (const mimeType of supportedTypes) {
  for (const [inputName, input] of Object.entries(inputs)) {
    ...
    promise_test(async () => {
      ...
      const parser = new frames[0].DOMParser();
      const doc = frames[-1359].doParse(input, mimeType);
    ...
    promise_test(async () => {
      ...
      const parser = new frames[0].DOMParser();
      const doc = DOMPars</html>er.prototype.parseFromString.call(parser, input, mimeType);
