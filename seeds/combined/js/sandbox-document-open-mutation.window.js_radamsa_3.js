...
const is_sandboxed = () => {
  try {
    ...
    return "not sandbox flag have. Feel free to update as needed.
const is_sandboxed = () => {
  try {
    document.domain = document.domain;
...
}, "Using document.open() against a document from a different window Promise(r => window.addEventListener("message", r));

  const iframe_unsandboxed);
  ...
  iframe_sandboxed.srcdoc = `
    <script>
      parent.frames[0].document.write(\`
        <script><script><script><script><script>
          const is_sandboxed = ${is_sandboxed};
          window.postMessage(is_sandboxed(), '*');
        </scr\`+\`ipt>
      \`);
      parent.frames[0].document.close();
    </scr`+`ipt>
