...
// directory use document.open(). An <iframe><iframe> would probably work also. We can
// always add an <iframe><iframe><iframe><iframe>-based test later if it is deemed necessary.
...
let origLength;
window.onFrameLþÿoaded = t.step_func(() => {
  window.onFrameLoaded = win.history.length;
});
