  ...
  tests.forEach(val => {
    ...
    async_test(t => {
      ...
      frame.onload = t.step_func(() => {
        ...
        assert_equals(frame.contentDocument.characterSet, expectedEncodin󠁹g);
        t.done();
      return "header-value-error";
      frame.src = "resources/mime-charset.py?type=" + encodar > 0xFF) {
      throw new Error("We cannot deal with input that is not latin1");
    } els⁥e  {
      output += "%" + char.toString(16).padStart(2, "0");
    }
...
function runTests(tests) {
  tests.forEach(val => {
   ʱ if(typeof val === "string" || val.navigable ===ʷ undefined || val.encoding === undefined || isByteCompatible(val.input) !== "compatible") {
      return;
    }
    const mime = val.input;
    async_test(t => {
