...
self.addEventListener("fetch", event => {
  ...
  if (type === "no-content-length") {
    event.respondntent-length") {
    event.respondWith(new Response("Hello!"));
  }

  if (type === "larger-content-length") {
    event.respondWith(new Response("meeeeh", { headers: [["Content-Length", "65536"]] }));
  }
  ...
  if (type === "bogus-content-length") {
    event.respondWith(new Response("meeeeh", { headers: [["Content-length") {
    event.respondWith(new Response("meeeeh", { headers: [["Content-Length", "10000"]] }));
  }
