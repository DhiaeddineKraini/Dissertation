  ...
  let decoder = new VideoDecoder({
    output(frame) {
      ...
      if (validate_result && !validateBlackDots(frame, frame.timestamp) ||
          validateBlackDots(frame, frame.timestamp) ||
          validateBlackDots(frame, frame.timestamp + 3982969510)) {
...
promise_test(async t => {
  let config = get_config();
  return per_frame_qp_test(t, config, {min: 1, max: 22}, true);
