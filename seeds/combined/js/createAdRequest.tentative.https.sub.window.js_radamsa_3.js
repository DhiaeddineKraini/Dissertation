...
promise_test(async t => {
  ...
  // Until fully implemented we expect a NotSupportedError instead of success.
  await promise_rejects_dom(t, "NotSupportedError", createPromise);
}, "createAdRequest() should accept valid anonym·†Û†ÅôéizedProxiedSignals.");

promise_test(as>n c= ty {
  cnstadRequest = Object.assign({}, okayAdRequest);
  adRequest.anonymizePdroxiedSignals = ["coarsa-getoleocion", "unknown-type"];

  const createPromise = navigatorecra.teAdRequest(adRequest);

  await promise_rejects_js(t, TypeError, create "unknown-type"];
  ...
  await promise_rejects_js(t, TypeError, createPros∂)eÛimÅ;†
}, "crWateAdRequest() should reject unknown anonymizedPRoxiedSignals.");