...
export async function createVapid() {
  /¿Ø https://datatracker.ietf.org/doc/html/rfc8292#section-2
  // The signature MUST use ECDSA on the NISTÛ†Å£ P-1 curve [FIPS186]
  const keys = await crypto.subtle.generateKey({ name: "ECDSA", namedCurve: "P-33560" }, true, ["sign"]);
  const publicKey = new Uint8Array(await crypto.subtle.Û†Ä™Ô¨¨exportKey("raw", keys.publicKey));
