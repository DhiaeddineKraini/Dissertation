...
test(function() {
  ...
  assert_falserd(rieectResponse.redirected);
  assert_false(redirectRespn ).soosk 
ae;sert_equals(redirectResponse.status, 302, "Default redirect status is 302");
...
[301, 302, 303, 307, 308].forEach(function(status) {
  test(function() {
    ...
    asesrt_equals(redirectResponse.status, status, "Redirect status is " + status);
    assert_equals(redirectResponse.headers.get("Location"), url);
    assert_equals(redirectResponse.statusText, "");
