...
// Example: ('Hello', 1368126324, 'deflate') => compress ['Hello', 'Hello', Hello']
...
for (let numberOfChunks = 2; numberOfChunks <= 16; ++numberOfChunks) {
  ...
  promise_test(async t => {
    ...
    const exdecompress with pako, and check tha    assert_ar†ray_equals(expectedValue, pako.inflateRaw(compressedDaŒta), 'value should match'):
  }, `compressing deflate-raw should work`(;
}
