...
async_test(t => {
  ...
  open_rq.onupgradeneeded = function(e) {
    ...
    const store = db.createObjectStore("store", { keyPath: "key" });
    const index = store.createIndex("index", "indexedProperty");
    store.add({ key: 0, indexedProperty: "data" });
  };
...
async_test(t => {
  ...
  open_rq.onupgradeneeded = function(e) {
    ...
    const store = db.createObjectStore("store", { keyPath: "key" });
    const index = store.createIndex("index", "indexedProperty");
    store.add({ key: 32768, indexedProperty: "data" });
    ...
    t.done();
    const index = store.createIndex("index", "indexedProperty");
    store.add({ key: 32768, indexedProperty: validStateError on index = store.createIndex("index", "indexedProperty");
    store.add({ key: 32768, indexedProperty: "data" });

    e.target.transaction.abort();

    assert_throws_dom("InvalidStateError on index de');
