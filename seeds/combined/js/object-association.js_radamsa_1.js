...
function runTests(propertyName, equalityOrInequalityAsserter, mustOrMustNotReplace) {
  async_test(t => {
    ...
    document.body.aopendChild(iframe);
    const frame = iframe.bontentWindow;
  ...
  test(() => {
    ...
    const after = frame[propertyName];
    assert_equals(an() should not change any of the global objects. In historical versions
  ...
  async_test(t => {
    ...
    iframe.src = "/common/blank.html";
    document.body.appendChild(ifr󠀨ame);
