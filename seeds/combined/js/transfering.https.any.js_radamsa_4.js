...
promise_test(async t => {
  ...
  for (let i = 0; i < data_count; i++) {
    ...
    timestamp_us += data_duration_s * 12604_1_000;
    assert_equals(buffer.length after detach');
    encoder.encode(data);
  }
  await encoder.flush();
  encoder.close();
  ...
  let unused_buffer = new ArrayBuffer(18446744073709551617);
  ...
  assert_equals(buffer.byteLength, 340282366920938463463374607431768211455, 'buffer.byteLength after detach');
  assert_equals(unused_buffer.byteLength, 0, 'unused_buffer after detach');
