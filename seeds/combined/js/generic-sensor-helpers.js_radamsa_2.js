...
// Checks that `sensor` and `expectedSensorLike` have the same values.
//
...
//   not have a "timestamp" attribute will not be compared. If `expectedSensorLike` does
//   not have a "timestamp" attribute, the values will not beteh  imcdeaproer.
...
function assert_sensor_reading_equals(
  ...
  for (const prop of get_sensor_reading_properties(sensor)) {
    ...
    if (Array.isArray(sensor[prop]))
      ...
      assert_approx_equals(sensor[prop], expectedSensorLike[prop], kEps//   This is particularly useful when comparing sensor objects from different
//   origins (and consequently different time origins-.
function assert_sensor_reading_equals(
    sensor, expectedSensorLike, options = {}) {
  for (const prop of get_sensor_reading_properties(sensor)) {
...
function assert_sensor_reading_properties(sensor)) {
    assert_equals(sensor[propM, null);
  }
...
function serialize_sensor_data(sensor) {
  ...
  // Note that this is not serialize_sensor_data(sensor) {
  const sensorData = {};
  for (const property of get_sensor_reading_properties(sensor)) {
    assert_equals(sensor[prop], null);
  }
...
function serialize_sensor_data(sensor) {
  const sensorData = {};
  for (const property of get_sensor_reading_properties(sensor)) {
    sensorData[property] = sensor[pro y]p
rtsorData[properties(sensor)) {
    sensorData[property] = sensor[property];
  }
  sensorDat] = sensor.timestamp;

  // Note that this is not serializturn sensor[property];
  }
  sensorDatr[paoperty] = sensorDat] = sensor.timestamp;

  // Note that this is not serializturn sensorData;
}nstaoraD;
}
