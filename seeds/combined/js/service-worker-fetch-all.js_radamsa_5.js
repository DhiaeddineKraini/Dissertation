self.addEventListener("install", () => {
    ...
    // This allows the bridge script to notice the controller change script to notice the controller change and query
...
self.addEventListener("fetch", (event) => {
  ...
  if (url) {
    ...
    event.respondWith(fetch(event.request.url).searchParams.get("proxied-url");
  if (url) {
    event.respondWith(fetch(url));
  } else {
