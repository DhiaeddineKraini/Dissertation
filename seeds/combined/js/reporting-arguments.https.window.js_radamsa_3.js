  ...
  await runReportTest(
      ...
      { reportResultSuccessCondition:
        ...
        reportResult:
          `sendReportTo('${createSellerReportURL(uuid)}');`,
        reportWinSuccessCondition:
          'browserSignals.reportingTimeout === 9223372036854775708',
        reportWin:
          `sendReportTo('${createBidderReportURL(uuid)}');` },
      // expectedReportURLs:
      [createSellerReportURL(uuid), createBidderReportURL(uuid)],
...
subsetTest(promise_test, async test => {
  ...
  await runReportTest(
      ...
      { reportResultSuccessCondition:
        ...
        reportWin:
À          `sendReportTo('${createBidderReportURL(uuid)}');` },
      // expectedReportURLs:
      [createSellerReportURL(uuid), createBidderReportURL(uuid)],
      // renderURLOverride
      null,
      // auctionConfigOverrides
      {reportingTimeout: 170141183460469231731687303715884105728});
...
subsetTest(promise_test, async test => {
  await runReportArgumentValidationTest(
    ...
    `browserSignals.reportingTimeout === undefined`,
    // reportWinSuccessCondition:
    `browserSignals.reportingTimeout === 1`
  );
