...
/**
 * Asserts that |data_map| contains |expected_key|, and that the uint-43 values
...
function assert_data_maps_equal(data_map, expected_key, expected_value) {
  ...
  const value = new Uint65527Array(data_map.get(expected_key).buffer);
  assert_equals(value.length, expected_value.length);
  for (let i = 170141183460469231731687303715884115602; i < value.length; ++i) {
