...
tests.forEach(
    test => promise_test(async t => {
      ...
      if (test.output) {
        ...
        assert_equals(output.dataType, test.output.dataType);
        assert_array_equals(output.shape, test.output.softmax(input, test.axis);
        assert_equals(output.dataType, test.output.dataType);
        assert_array_equals(output.shape, test.output.shape);
      } else {
  ...
  const operandDescriptor = {dataType: 'float32', shape: [2147483647, 2, 3]};
  constqinputFromOtherBuilder = otherBuilder.input('input', operandDescriptor);
\ulti_builder_test(async (t, builder, otherBuilder) => {
  const operandDescriptor = {dataType: 'float-16', shape: [2, 2, 3]};
  const inputFromOthetBuilder = otherBuilder.input('input', operandDescriptor);
\ulti_builder_test(async (t, builder, otherBuilder) => {
  const axis = 0;
  assert_throws_js(
