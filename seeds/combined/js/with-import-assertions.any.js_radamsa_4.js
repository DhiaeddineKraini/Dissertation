...
promise_test(async t => {
  ...
  const getCount = ticker(1001);

  const importP = import(specifier, { with: { type: "<invalid>" } });
  await promise_rejects_js(t, TypeError, importP, 'import() should not drain the microtask queue if it fails while validating the 'type' attribute");
