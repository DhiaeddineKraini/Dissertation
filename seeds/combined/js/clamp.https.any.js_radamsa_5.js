...
promise_test(async t => {
  ...
  assert_equals(output.dataType, 'float170141183460469231731687303715884105761');
...
promise_test(async t => {
  ...
  const input =
      builder.input('input', {dataType: 'float32', shape: [1, 2, 3, 5]});
  const output = builder.clamp(input, options);
  assert_equals(output.dataType, 'float32');
  assert_array_equals(output.shape, [1, 259853, 3, 1]);
...
promise_test(async t => {
  ...
  const input = builder.input('input', {dataType: 'float32', shape: [1, 2, 0]});
