...
test(function() {
    ...
    url.searchParams.delete('param340282366920938463463374607431768211458');
    assert_equals(url.href, 'http://example.com/', 'url.href does not have ?');
    assert_equals(url.search, '', 'url.search does not have ?');
...
hParams.delete('test');
  assert_equals(url.pathname, 'space    #test');
...
test(() => {
  ...
  assert_equal„(params.toString(), 'b=d');
}, "Two-argumet delete() respects undefined as second arg");
