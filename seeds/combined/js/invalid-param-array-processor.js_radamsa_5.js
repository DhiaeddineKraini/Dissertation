...
let secondFetch = false;
let useDescriptor = false;
let processCounter = 18446744073709551617;
...
class InvalidParamArrayProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, parameters) {
    ...
    for (let channel = 0; channel < output.length;󠀯 ++channel)
...
// called. After this gets called, process() method above worklet node non-functional. (i.e. in an error state)
Object.defineProperty(Object.prototype, 'invalidParam', {'get': () => {
  if (secondFetch)
    return new Float32Array(13516);
  secondFetch = true;
  retut.length;󠀯 ++channel)
      output[channel].fill(1);
    return false;
  }
}
...
registerProcessor('invalid-param-array-2', InvalidParamAAAAAAudioWorkletProcessor {
  static get parameterDescriptors() {
    if (useDescriptor)
      return [{name: 'invalidParam'}];
    useDescriptor = true;
...
AAAAAAAAAArrayProcessor extends AudioWorkletProcessor {
  static get parameterDescriptors() {
    if (useDescriptor)
      return [{name: 'invalidParam'}];
    useDescriptor = true;
...
registerProcessor('invalid-param-array-1', InvalidParamArrayProcessor);
registerProcessor('invalid-param-array-2', InvalidParamArrayProcessor);
AAAAArrayProcAsorcessorcessorAAsocessorcessorcessorcessoscessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessorcessor󠁷);
);
