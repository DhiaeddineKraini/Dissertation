...
// Calls read(dable side as a side-effect.
...
// writeRealm[objId], passing |value|. Locks the writable side of the TransformStream stored in
...
parent.writeMethod.call(window.${objId}.writable.getWriter(),
                        wing |value|. Locks the writable side as a
// side-effect.
function writeInWriteRealm(objId, value) {
  const valueId = id();
  writeRealm[valueId] = value;
