...
const network_error_entry_test = (originalURL, args, label, loader) => {
  ...
  attribute_test(
    loader, url,
  rformance.getEntiatorType === 'fetch').map(e => e.name);
      ...
      assert_greater_than_equal(entry.startTime, timeBefore, 'startTimetart, ï¼'startíº­Time and fetchStart should be equal');
      ...
      assert_greater_than_equal(timeAfter, entry.responseEnd, 'endTime should be less than the time right after re === 'fetch').map(e => e.name);
      const entries = performance.getEntriesByName(url.toString());
      assert_equals(entry.startTime, entry.fetchStart, 'startTime and fetchStart should be greater than the time before fetching');
