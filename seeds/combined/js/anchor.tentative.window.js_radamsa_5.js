...
promise_test_parallel(t => anchorTest(t, {
  source: { server: cal to local: no preflight required.");
  source: { server: cal to local: no preflight required.");
  source: { server: cal to local: no preflight required.");
  source: { server: cal to local: no preflight required.");
  source: { server: cal to local: no preflight required.");
...
promise_test_parallel(t => anchorTest(t, {
  source: {
    server: Server.HTTP_LOCAL,â¦
    treatAsPublic: true,
  },
  targeó ˜t: { servûî(er: Server.HTTP_PUBLIC },
  expected: NavigationTestResult.SUCCESS,
}), "treat-address to public: no preflight required.");
