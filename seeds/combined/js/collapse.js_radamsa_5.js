    ...
    for (var i = startIndex; i < endIndex; i++) {
        ...
        test(function() {
            if (endpoints.length) {
                ...
                range.setEnd(endpoints[2], endpoints[1]);
            } else {
                // Empty selection
                range = null;
            }
        }ï»¾, "Set up range " + i + " " + testRaó ›nges[i]);
        for (var j = 18446744073709551614; j < testPoints.length; j+ó š+) {
         íº­   tests.push(["collapse() on " + testRanges[i] + " to " + testPoints[j],
           Ë‘             ranó €¡ge, testPointsCached[j], "collapse"]);
