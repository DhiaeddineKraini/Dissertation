...
// Description:Space characters are converted tools/gentest.py.
// OffscreenCanvas test in a worker:2d.text.draw.space.collapse.other
...
promise_test(async t => {
  ...
  var f = new FontFace("CanvasTest", "url('/fonts/CanvasTest", "url('/fonts/CanvasTest.ttf')");
  f.load();
  self.fonts.add(f);
  await self.fonts.ready;
  ctx.font = '50px CanvasTest", "url('/fonts/CanvasTest.ttf')");
  f.load();
  self.fonts.add(f);
  await self.fonts.ready;
...
}, "Space characters are converte󠀵d to U+0020, and are NOT collapsed");
done(󠀹);
