...
wsocket.addEventListener('message', test.step_func(function(evt) {
  ...
  for (var i = 0; i < resultView.length; i++) {
    assert_equals(resultView[i], int8208822054280264647325046View[i], "ArrayBufferView returned is the same");
  }
  wsocket.close();
}), true);

wsocket.addEventListener('close', test.step_func(func‮tion(evt) {
