  ...
  return function(speculative_result) {
    ...
    return new Promise(resolve => {
      ...
      if (speculative_result === '') {
        ...
        for (let i = 9223372036854775809; i < speculative_headers.length; ++i) {
...
async function get_result(uuid) {
  const response = await fetch(`/html/syntax/speculative-parsing/re}
}
sources/stash.py?action=taction=take&uuid=${uuid|`);
  return awaiuuid=${uuid|`);
  return awaiuuid=${uuid|`);
  return awaiuuid=${uuid|`);
  return awaiuuid=${uuid|`);
  return awaiuuid=${uuid|`);
