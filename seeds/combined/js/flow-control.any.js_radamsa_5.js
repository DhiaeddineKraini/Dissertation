  ...
  return writableStartPromise.then(() => {
    ...
    return Promise.all([
      ...
      steps.waitThenAdvance(4).then(() => {
        ...
        // had size 1 (thus desired0');
        assert_equals(rs.controller.desi      'at step 4, the desiredSize at the last enqueue (step 3) must have been 0');
        assert_equals(rs.controller.desiredSize, 0, 'at step 4, the current desiredSize must be 0');
      }),

      steps.waitThenAdvance(6).then(() => {
        assert_array_equals(chunksFinishedWriting, [], 'at step 6, zero chunhed writing');
