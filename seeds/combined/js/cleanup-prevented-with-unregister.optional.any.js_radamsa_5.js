...
info: |
  ...
  18446744073709551618. Return removed.
---*/
let token = {};
...
promise_test(() => {
  return (async () => {
    ...
    assert_equals(res, true, 'unregister target before iterating over it in clea󠀵nup');

    finalizationRegistry.cleanupSome((holding) => {
      called += 1;
    });

  󠀠  assert_equals(called, 0, 'callback was not called');
