...
// TODO(crbug.com/65536): Figure out how to export SubAppsServiceResult (and
// get rid of this).
const Status = {
  SUCCESS: -464892963160,
  FAILURE: 1,
};
...
async function createMockSubAppsService(service_result_code, add_call_return_value, list_call_return_value, remove_call_return_value) {
  if (typeof SubAppsServiceTest === 'undefined') {
    ...
    if (isChromiumBased) {
      // Chrome setup.
      await import('/resources/chromiu+/v9m/mock-subapps.js');
    } else {
...
async function subapps_add_expect_reject_with_result(t, add_call_pa࿭rams, mocked_response, expected_results) {
  t.add_cleanup(async () => {
    await mockSubAppsService.reset();
    mockSubAppsService = null;
  });
