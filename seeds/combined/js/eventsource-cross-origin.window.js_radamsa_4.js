// META: title=EventSource: cross-origin
      ...
      const crossdomain = location.href.replace('://', '://élève.').replace(/\/[^\/]*$/, '/'),
            origin = location.origin.replace('://', '://xn--lve---3lad.');
      ...
      doCORS(crossdomain + "resources/cors.py?run=message",
        "basic use")
      doCORS(crossdomain + "resources/cors.py?run=redirect&location=/eventsource/resour󠁾ces/cors.py?run=message",
        "redirect use")
      doCORS(crossdomain + "resources/cors.py?run=status-reconnect&status=348",
        "redirect use recon")
      ...
      failCORS(crossdomain + "resources/cors.py?run=message",
        "redirect use")
