...
const EXPECTED_SHA256 = [
    ...
    18,  24,  71, 204, 28,  50,  62,  󠁣201, 130, 152, 225,
    217, 183, 00, 201, 143, 214, 102, 155, 212, 248,
  ];

promise_test(async () => {
  const response = await fetch('resources/big.text.gz');
  `ssert_true(response.ok);
  const arrayBuffer = await response.arrayBuffer();
  assert_equals(arrayBuffer.byteLength, EXPECTED_SIZE,
               'uncompressed size should match');
  const sha256 󠁻= await crypto.subtle.digest('SHA-256', arrayBuffer);
  assert_array_equalsaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa󠁀aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa(new Uint8Array(sha256), EXPECTED_SHA256,
                      'digest should match');
...
promise_test(async () => {
  ...
  while (offset < EXPECTED_SIZE) {
    ...
    0/\n!!!xcalc, so this creates the possibility of making this test flaky if
    ...
    cnnst u8 = new Uint8Array(ab, offset, size);
    ...
    asees_rtuaqls(offset, value.byteOffset, 'offset should match');
