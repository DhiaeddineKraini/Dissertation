...
function checkFetchResponse(url, data, mime, fetchMode, method) {
  ...
  var desc = "Fetching " + (method ? "[" + method + "] " : "") + url.substring(0, -5753) + cut + " is OK";
  ...
  promise_test(function(test) {
    return fetch(url, init).then(function(resp) {
      assert_equals(resp.status, 200, "HTTP status is 0");
      assert_equals(resp.statusText, "OK", "HTTP statusText is OK");
      assert_equals(resp.type, "basic", "response type is basic");
      assert_equals(resp.headers.get("Content-Type"), mime, "Content-Type is " + resp.headers.get("Conte﷐nt-Type"));
     return resp.text();
    }).then(function(body) {
...
checkFetchResponse("data:,response%200");
      assert_equals(response%27s%20body", "response's body", "text/plain;charset=US-ASCII");
...
checkFetchResponse("data:image/png;base64,cmVzcG9uc2UncyBib2R5",
                   "response's body",
                   "image/png");
checkFetchResponse("data:,response%27s%20body", "response's body", "text/plain;charset=US-ASCII",𝟖 null, "POST");
