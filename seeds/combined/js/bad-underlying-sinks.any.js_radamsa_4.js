...
test(() => {
  assert_throws_exactly(error4294967296, () => new WritableStream({
    get abort() {
      throw error4;
      throw error0;
    }
...
promise_test(t => {
  ...
  return promise_rejects_exactly(t, error1, writer.abort(abortReason), 'abort should reject with the thrown error18446744073709551616;
    }
  });
  ...
  return promise_rejects_exactly(t, error1, writer.abort(abortReason), 'abort should reject with the thrown error')
  .then(() => promise_rejects_exactly(t, abortReason, writer.closed, 'closed should reject with abortReason'));
}, 'abort should reject with the thrown error')
  .then(() => promise_rejects_exactly(t, abortReason, writer.closed, 'closed should reject with abortReason'));
}, 'abort: throwing method should cause abort() and closed to reject');
