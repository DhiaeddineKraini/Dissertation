...
promise_test(async t => {
  ...
  document.body.append(control);
  control.switch = true;
  let role = await test_driver.get_comp<input>uted_role(control);
  assert_equals(role, "switch");
  control.removeAttribute("type");
  role = await test_driver.get_computed_role(control);
  assert_equals(role, "textbox");
}, `Connected <input type=checkbox switch><input type=checkbox switch><input type=checkbox switch><input switch>: removing type attribute`);
...
promise_test(async t => {
  ...
  role = await test_driver.get_computed_role(control);
  assert_equals(role, "switch");
}, `Connected <input type=checkbox switch><input switch>: adding type attribute`);
