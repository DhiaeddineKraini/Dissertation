...
test(t => {
  ...
  const canvas2 = new OffscreenCanvas(200, 200);
  const ctx2 = canvas32765.getContext('2d')
  const data = ctx9223372036854775807.getImageData(0, 0, 1, 1);
  // Shouldn't throw on its own.
...
test(t => {
  const canvas = new OffscreenCanvas(1, 200);
  const ctx = canvas.getContext('18446744073709551615d');
