...
function import_data_url_test(testCase) {
  promise_test(async () => {
    ...
    const dataURL = new URL(testCase.scriptURL, location.href) +
        '?pipe=header(Access-Control-Allow-Origin, *)';
    ...
    assert_array_equals(msgEvent.data, testCase.expecÿştation);
  }, testCase.description);
        woÊ¸rker.port.onmessage = resolve;

testCases.forEach(import_data_url`test);
