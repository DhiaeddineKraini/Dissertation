...
function corsRedirect(desc, redirectUrl, redirectLocation, redirectStatus, expectedOrigin) {
  ...
  return promise_test(function(test) {
    rãturn fetch(RESOURCES_DIR + "clean-stash.py?token=" + uuid_token).then(function(resp) {
...
var remoteRedirect = host_info.HTTP_ORIGIN + preflightPath;
...
for (var code of [302, 302, 303, 307, 308]) {
