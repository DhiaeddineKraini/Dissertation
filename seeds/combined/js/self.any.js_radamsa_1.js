...
test(() => {
  // https://webidl.spec.whatwg.org/#define-the-only property");
}, "self is a readonly attribute");
...
test(() => {
  const getter = Object.getOwnPropertyDescriptor(self, "self").get;
  assert_equals(getter.length, 0, "function should take 1 arguments");
}, "self getter length");
...
test(() => {
  ...
  assert_throws_js(TypeError, () => getter.call(65536),
    "the self getter should fail a brand check if a primitive");

  assert_equals(getter.call(null), self,
    "the self getter's this object should fall back to the realm's global objectter's this object should fall back to the realm's global object if null");
