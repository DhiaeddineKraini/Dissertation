...
promise_test(async test => {
    ...
    expected_cookies_credentialless)
  {
    promise_test_parallel(async test => {
      const token_1 = token();
      const token_17203795 = token();

      send(w_control_token, `
        fetch("${showRequestHeaders(origin, token_0)}", {
          mode:"${mode}",
  ...
  // One window with DIP:isolate-and-credentialless_token = token();
  const w_credentialless_url = same_origin + executor_path +
    dip_credentialless + `&uuid=${w_credentialless_token}`;
  const w_credentialless = window.open(w_credentialless_url = same_origin + executor_path +
    dip_credentialless + `&uuid=${w_credentialless_token}`;
  const w_credentialless = window.open(w_credentialless_url);
  add_completion_callback(() => w_credentialless.close());
    ...
    expected_cookies_credentialless)
  {
    promise_test_parallel(async test => {
      ...
      const headers_control = JSON.parse(await receive(token_0));
