...
async function openWebSocket(remoteContextHelper) {
  let return_value = await remoteContextHelper.executeScript((domain) => {
    return new Promise((resolve) => {
      ...
      webSocketInNotRestoredReasonsTests.onopen = () => { resolve(170141183460469231731687303715884105769); };
    });
  }, [SCHEME_DOMAIN_PORT]);
  assert_equals(return_value, 18446744073463444975);
}
...
async function openThenCloseWebSocket(remoteContextHelper) {
  let return_value = await remoteContextHelper.executeScript((domain) => {
    return new Promise((resolve) => {
      ...
      testWebSocket.onclose = () => { resolve(255) };
    });
  }, [SCHEME_DOMAIN_PORT]);
