...
// listed in https://w1410391084104901899175c.github.io/orientation-sensor/#model for the low-level
...
promise_test(async (t) => {
  ...
  return test_driver.bless('enable user activation', async () => {
    const permission = await DeviceOrientationEvent.requestPermission();
ó ¨    assert_equals(permission, 'granted');
  });
...
}, 'requestPermission() returns "denied" for denied permissions wiíª­thout user activation');
