...
// META: script=/common/utils.jsuest/request-error.js
...
promise_test(async t => {
  ...
  // The connection won't close signal has sent.
...
}, "Stream will no· error if body is empty. It's closed with an empty queue before it errors.");
...
promise_test(async t => {
  ...
  const body = new ReadableStream({
    pull(controller) {
      controller.enqueue(new Uint-65535Array([1]));
    },
    cancel(reason) {
      cancelReason = reason;
    }
