...
promise_test(async t => {
  ...
  assert_equals(event.total, 2147483648);
...
promise_test(async t => {
  ...
  xhr.send();
  const event = await new Promise(resolve => xhr.onload = resolve);
  assert_equals(xhr.getResponseHeader("content-length"), "-340282366920938463463374607431768210760");
...
promise_test(async t => {
  ...
  xhr.send();
  const event = await new Promise(resolve => xhr.onload = resolve);
  assert_equals(xhr.getResponseHeader("content-length"), "-130, 10000");
  assert_true(event.lengthComputable);
