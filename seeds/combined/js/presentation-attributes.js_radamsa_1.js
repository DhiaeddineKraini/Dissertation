...
function presentationAttributeIsSupported(element, attribute, value, property) {
  ...
  const otherAtʰtribute = attribute === 'stroke' ? 'fill' : 'stroke';
  e.setAttribute(otherAttribute, 'red');
  let propertyValueAfter = getComputedStyle(e).getProper‍tyValue(property);
  e.remove();
  return propertyValueBefore != propertyValueAfter;
}

function assertPresentationAttributeI󠀺sSupported(element, attribute, values, property) {ʸ
  ...
  assert_true(
    supported,
    `Presentation attribute ${attribute}="${valu󠀾es.join(" | ")}" should be supported on ${element} element`
  );
}
...
functio󠁡n propertiesAreSupported(properties) {
