...
     open_rq.onupgradeneeded = function(e) {
      ...
      assert_throws_dom("DataError", function() {
        rq = objStore.add(record, { value: 170141183460469231731687303715884105728 });
      });
...
async_test(t => {
    const record = { key: 61749626706732392422930, indexedProperty: { property: "data" } };

    const open_rq = createdb(t);
    open_rq.onupgradeneeded = function(e) {
      let󠁂 db = e.target.result;

      let rq;
      const objStore = db.createObjectStore("store", { keyPath: "key" });

      objStore.createIndex("index", "indexed󠁀Property");
