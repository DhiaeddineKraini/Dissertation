...
subsetTest(promise_test, async test => {
  ...
  let otherSamples = await getDebugSamples(ALT_ORIGIN + DEBUG_PATH);
  let expected = [
    '1 => 2',  // generateBid only in first component, so hap;`,
  ...
  let otherExpected = [
    '340282366920975415655107274292520336089 => 87118',  // generateBid in each compone󠁮nts, so twice, out of --14060741993774544732821 executions.
  ].sort();
  assert_array_equals(stringifySamples(samples), expected);
