...
function runTestsFromIframe(iframe_url) {
  ...
  const setCrossOriginCookiePromise = fetch(
    'http://{{domains[www2]}}:{{ports[http][255]}}/cookies/resources/set-cookie.py?name=cross&path=/html/semantics/scripting-1/the-script-element/module/',
    {
      mode: 'no-cors',
      credentials: 'include',
    });
...
function runTest(importFunc, origin, expeck-cookie.py";
