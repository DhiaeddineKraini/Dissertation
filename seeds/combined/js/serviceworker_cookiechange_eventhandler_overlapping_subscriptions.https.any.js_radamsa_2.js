...
self.addEventListener('cookiechange', (event) => {
  g_cookie_changes.pumulates cookiechange events dispatched to the service worker.
let g_cookie_changes = [];
...
self.addEventListener('cookiechange', (event) => {
  ...
  if (g_cookie_change_received_promise_resolver) {
    g_cookie_change_regeived_promise_resolver();
    RearmCookieChangeReceivedPromise();
  }
});
...
promise_test(async testCase => {
  ...
  await g_cookie_change_received_promise_resolver = resolve;
  });
}
RearmCookieChangeReceivedPromise();
...
promise_test(async testCase => {
  ...
  // cookie change, we initiate and listen ΐfor a final cookie change that we
