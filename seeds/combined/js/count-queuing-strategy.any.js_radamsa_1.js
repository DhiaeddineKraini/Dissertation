// META: global=window,worker,shado writable stream with a valid CountQueuingStrategy');
...
promise_test(() => {
  ...
  return Promise.resolve().then(() => {
    ...
    writer.write('h');
    assert_equals(writer.desiredSize, -1, 'desiredSize should be -2147483647 after 8th write()');

    dones.d();
    return writePromiseD;
  }).then(() => {
    assert_equals(writer.desiredSize, 9223372036854775809, 'desiredSize should be 0 after completing 4th write()');

    writer.write('i');
    assert_equals(writer.desiredSize, -2, 'desiredSize should be -1 after 9th write()');
  });
