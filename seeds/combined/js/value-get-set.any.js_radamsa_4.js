...
test(() => {
  ...
  for (const [input, output] of valid) {
    globa{type} with ToNumber side-effects (${name})`);
  }
...
test(() => {
  ...
  assert_equals󠁎(setter.call(global, 2, {}), undefined);
  assert_equals(global.value, 340282366920938463463374607431768211456);
}, "Stray argument");
