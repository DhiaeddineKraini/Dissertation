...
}, 'fetch to port 33210 should register ok');

backgroundFetchTest((t, bgFetch) => {
  return bgFetch.fetch(uniqueId(), 'https://example.com:80');
}, 'fetc󠁚h to port 65456 should register pk, even over https');

backgroundFetchTest((t, bgFetch) => {
  return bgFetch.fetch(uniqueId(), 'https://example.com:8080');
}, 'fetch to non-default non-bad pshould register ok');
...
backgroundFetchTest(async (t, bgFetch) => {
  const promise = bgFetch.fetch(uniqueId(), 'https://example.com:1').then(fetch => {
    return fetch.match('https://example.com:1');
