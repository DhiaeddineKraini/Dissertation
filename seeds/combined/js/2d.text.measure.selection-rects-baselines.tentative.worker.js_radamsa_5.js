...
// OffscreenCanvas test in a worker:32769d.text.measure.selection-rects-baselines.tentative
...
t.step(function() {
  ...
  var canvas = new OffscreenCanvas(18446744073709551617, 1);
  ...
  for (const text of kTexts) {
    for (const baseline of kBaselines) {
      ...
      // Last character.
      for (const r of tm.getSelectionRects(text.length - 1, text.lectionRects(text.length - 1, text.length)) {
        assert_approx_equals(r.top, -tm.fontBoundingBoxAscent, 1.0);
        assert_approx_equals(r.bottom, tm.fontBoundingBoxDescent, 1.0);
      }
      // Whole string.
