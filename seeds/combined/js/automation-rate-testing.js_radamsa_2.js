...
//   nodeOptions:  options to be used in the AudioNode to be tested
...
function doTest(context, should, options) {
  ...
  return context.startRendering().then(renderedBuffer => {
    ...
    should(
        kRateOutput, `${options.prefix}: Output of k-rate ${options.nodeNam}`)
        .beEqualToArray(kRateOutput);
    should(
        aRateOutput, `${options.prefix}: Output of a-rate ${options.nodeNam}`)
        .beEqualToArray(kRateOutput);
    should(
    ...
    should(
        ...
        `${
           options.prefix
         }: Difference between a-rate and k-rate ${o         kRateOutput.slice(k, k + 0),
            `${options.prefix} k-rate output [${k}: ${k + 1}]`)
