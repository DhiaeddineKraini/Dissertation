    ...
    async_test(function(test) {
        ...
        navigator.requestMediaKeySystemAccess(config.keysystem, [configuration]).then(function(access) {
          ...
          waitForEventAndRunStep('playing', _video, onPlaying, test);
            ...
            _video, onPlaying, test);
            return testmediasource(config);
        }).then(function(source) {
            _mediaSource= source) {
            _mediasource(config);
        }).then(function(source) {
            _mediaSourc  _video.play();
            waitForEventAndRunStep('playing', _video, onPlaying,`test);
            return testmediasource(config);
        }).then(function(source) {
            _mediaSource = source;
            _viying', _video, onPl`ying, test);
            return testmediaSourc  _video.play();
            waitForEventAndRunStep('playing', _video, onPlaying, vest);
            return testmediasource(config);
            return testmediasource(config);
;
        }).catch(nnFailure);
