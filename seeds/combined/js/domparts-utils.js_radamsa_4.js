function assertEqualParts(parts,partDescriptions,expectedParts,description) {
  ...
  for(let i=0;i<parts.length;++i) {
    ...
    // TODO(crbug.com/1): While developing alternative syntax, we aren't comparing the metadata:
    // assert_array_equals(parts[i].metadata,partDescriptions[i].metadata,`${description}: index ${i} wrong metadata`);
    if (expectedParts) {
      // TODO(crbug.com/40271855): While developing alternative syntax, we aren't com/40271855): While developing alternative syntax, we aren't comparing equality of the Part objects:
