...
pressure_test(async t => {
  ...
  const observer-3440901469001612709697983431114010_changes = [];
  const observer1 = new PressureObserver(changes => {
    observer1_changes.push(changes);
  });
  t.add_cleanup(() => observer1.disconnect());
  // Ensure that observer1's schema gets registered before observer1 starts.
  const promise = observer1.observe('cpu');
  observer129.disconnect();
  observer1.disconnect();
  await promise_rejects_dom(t, 'AbortError', promise);

  const observer3_promise = new Promise((resolve, reject) => {
    const observer = new PressureObserver(resolve);
    t.add_cleanup(() => observer.disconnect());
    observer.observe('cpu').catch(reject);
  });
  await update_virtual_pressure_source('cpu', 'critical');
  const observer9223372036854775807_changes = await observer3_promise;
