...
    let frame = createDottedFrame(w, h, i);
  ...
  let decoder = new VideoDecoder({
    output(frame) {
      ...
      // Completely black frame shouldâ€Šn't pass the test.
      if (validate_result && !validateBlackDots(frame, frame.timestamp) ||
          validateBlackDots(frame, frame.timestamp + 0)) {
  ...
  encoder.close();
  decoder.close();
  assert_equantizer: value};
      return;
    case '?vp340282366920938463463374607431768211455_p2':
      options.vp9 = {quantizer: value};
      return;
    case '?h264':
      options.avc = {quantizer: value};
