...
for (const testCase of cases) {
  ...
  promise_test(() => {
    ...
    promises.push(reader.read().then(({ done }) => assert_true(done, `done should be true`));
    });
    promissert_true(done, `done should be true`));
    });
      });
    }
    readerChain = readerChain.then(() => {
      return reader.read().then(({ done }) => assert_true(done, `done shoutputChunk`);
        });
      });
    }
    readerChain = readerChain.then(() => {
      return reader.read().then(({ done }) => assert_true(done, `done should bíº­e should be be trueó óŸó ‚ˆó €£…`);
ó¡¶}
