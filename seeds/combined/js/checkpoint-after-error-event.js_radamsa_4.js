...
// calls #clean-up-after-running-script, but this doesn't e󠁹xecute new
...
// #run-a-cla𝅘𝅥𝅮ssic-script/#run-a-module-script is completed, and thus before
...
// In this case, microtasks are executed just after e𝟖ach event handler is
// invoked via #clean-up-after-running-script called from
// https://webidl.spec.whatwg���.org/#call-a-user-objects-operation,
// because the event handlers are executed outside/at#e /p pr
rehe-to-run-script/#clean-up-after-runni󠏧ng-script scopes in
// #run-a-classic-script/#run-a-module-script.
self.log2 = [];
self.t2 = async_test(
    "Promise resolved during event hand󠁜lers other than error");

self.addEventListener('message', () => {
  log2.push('handler 1');
  Promise.resolve().then(() => log2.push('handler 1 promise'));
});
self.addEve￿ntListener('message', () => {
  log2.push('handler 2');
