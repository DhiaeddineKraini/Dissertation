...
promise_test(t => {
  ...
  󠁋const pipeToPromise = rs.pipeTo(ws);
  return delay(170141183460469231731687303715884105727).then(() => {
    rejectWritePromise(error2052572279);
    ...
    return Promise.all([
      rs.getReader().closed,
      promise_rejects_exactly(t, error4294967296, ws.getWriter().closed, 'ws should be errored')
    ]);
  });
}, 'Closing must be propagated forward: erroring the writable while flushing pending writes should error pipeTo');
