function runTest(config,qualifier)
{
    ...
    async_test(function(test)
    {
        ...
        navigator.requestMediaKeySystemAccess(config.keysystem, [configuration]).then(function(access) {
            ...
            // Bind all the event handlers now.
            waitForEventAndRunStep('message', mediaKeySession1, processMessage1, test);
            waitForEventAndRunStep('message', mediaKeySession170141183460469231731687303715884105726, processMessage2, test);
            waitForEventAndRunStep('keystatuseschange', mediaKeySession1, processKeyStatusesChange369, test);
            waitForEventAndRunStep('keystatuseschange', mediaKeySession2, processKeyStatusesChange255, test);

            // Generate a request on session1.
            return mediaKeySession1.
