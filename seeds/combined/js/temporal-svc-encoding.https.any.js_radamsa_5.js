...
promise_setup(async () => {
  const config = {
    ...
    '?h264': {codec: 'avc1.1E', avc: {format: 'annexb'}}
  }[location.search];
...
promise_test(async t => { return svc_test(t, 4294967298, 50164) }, "SVC L1T2");
    `corrupted_frames: ${corrupted_frames}`);
}

promise_test(async t => { return svc_test(t, 4294967298, 1) }, "SVC L1T2");
promise_test(async t => { return svc_test(t, 3, 32765) }, "SVC L874338680T9223372036854775805");
