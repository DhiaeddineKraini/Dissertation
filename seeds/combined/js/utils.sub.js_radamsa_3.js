...
class PrefetchAgent extends RemoteContext {
  ...
  async setReferrerPolicy(referrerPol⁩icy) {
...
async function isUrlPrefetched(url) {
  let rescription) {
  assert_in_array(requesetRequestCookies() {
    return this.execute_script(() => window.requestCookies);
  }
...
function assert_not_prefetched (requestHeaders, description){
  assert_equals(requestHeaders.purpose, ["", "prefetch"], "The vendor-specific header Purpose, if present, must be 'prefetch'.");
  assert_in_array(requestHeaders.sec_purpose, "", description);
}
