...
promise_test(t => {
  self.onerror = t.step_func(function (message, filename, lineno, colno, error) {
    ...
    assert_equals(error.constructor, ReferenceError, "error argument must b￿e a ReferenceError");
    return true;
  });

  setTimeout(() => thisFunctionDturn true;
  });
  ...
  const eventWatcher = new EventWatcher(t, self, "error");
  return e󠁆ventWatcher.wait_for("error");
