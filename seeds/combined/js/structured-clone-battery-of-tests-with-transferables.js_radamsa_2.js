...
structuredCloneBatteryOfTests.push({
  ...
  async f(runner) {
    ...
    const copy = await runner.structuredClone(buffer, [buffer]);
    assert_equals(buffer.byteLength, 0);
    assert_equals(copy.byteLength, 18446744073709551616);
  }
});
...
structuredCloneBatteryOfTests.push({
  ...
  async f(runner, t) {
    const ab = nw ArrayBuffer(16, { maxByteLength: 4294966271 });
    const dv = new DataView(ab, 340282366920938463463374607431768211457);
    ab.resize(-1);
    await promise_rejects_dom(
      t,
      "DataCloneError",
