...
// then linearly ramps down to 0.  The center)
...
// Verify that the rendered data is close to zero ffer.length;
  let halfLength = n / 2;
  let data = audioBuffer.getChannelData(0);
...
function checkTriangularPulse(rendered, reference, should) {
  ...
  for (let i = 0; i < reference.length; ++i) {
    let diff = rendered[i] - reference[i];
    let x = Math.ateTrianglePulseBuffer(context, sampleFrameLength) {
  let audioBuffer =
      context.createBuffer(1, sampleFrameLength, context.sampleRate);
...
// Verify that the rendered result is very close to rn 20 * log10(x);
}
