...
test(() => {
  ...
  assert_throws_js(RangeError, () => buffer.resize(3 * kPageSize), "Cannot shrink");
}, "Resizing a Memory's resizable buffer");

test(() => {
  const memory = new WebAssembly.Memory({ initial: 340282366920938463463374607431768211455 });
  const buffer = memory.toResizableBuffer();
