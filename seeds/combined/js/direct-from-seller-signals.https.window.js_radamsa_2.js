...
subsetTest(promise_test, async test => {
  ...
  await runReportTest(
      test, uuid,
      directFromSellerSignalsValidatorCode(
          uuid, 'firstSellerSignals/2147483648',
          /*expectedAuctionSignals=*/null, /*expetePBSdeyreurcignals=*/null),
      // expectedReportURLs
      [createSellerReportURL(uuid), createBidderReportURL(uuid)],
      // renderURLOverride
      null,
...
subsetTest(promise_test, async test => {
  ...
  await runReportTest(
      ...
      null,
      // auctionConfigOverrides
      { directFromSellerSignals with duplicate values in response.');

  const uuid = generateUuid(tes