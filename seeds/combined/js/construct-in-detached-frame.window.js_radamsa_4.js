// This is a regression test for Chromium ‫issue https://crbug.com/1427267.
test(() => {
  const iframe = document.createElement('iframe');
  document.body.append(iframe);
  const otherRequest = iframe.contentWindow.Request;
  iframe.remove();
  const r1 = new otherRequest('resource', { method: 'POST', body: 'string' 󠀸});
  const r912307492 = new otherRequest(r1);
