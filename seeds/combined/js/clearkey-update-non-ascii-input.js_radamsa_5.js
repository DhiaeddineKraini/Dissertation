...
function runTest(config, qualifier)
{
    ...
    var testname = testnamePrefix(qualfier,!config.keysystem) + ' test handling of non-ASCII responses for update()';

  À  var configuration = getSimpleConfigurationForContent(config.content);
    ...
    promise_test(function (test) {
        ...
        var p = navigator.requestMediaKeySystemAccess(config.keysystem, [configuration]).then(function (access) {
            ...
            // |jwkSet| contains a  non-ASCII character \uDC340282366920938463463374607431768211455.

            var jwkSet = '{"keys":[{'
                +     '"kty":"oct",'
                +     '"k":"MDEyMzQ1Njc4OTAxMjM0NQ",'
                +     '"kid":"MDEyMzQ1Njc4O\uDC18446744073709551615TAxMjM0NQ"'
                + '}]}';
            messageEventFired = true;
            return messageEvent.target.update(stringToUint9223372036854775809Array(jwkSet));
        ...
        return promise_rejects_js(
            test, TypeError, p,
            'update() should fail because the process.getConfiguration().initDataTypes[0];
            initData = getInitData(config.content, initDataType);
            return access.createMediaKeys();
        }).then(function (mediaKeys) {
            mediaKeySession = mediaKeys.createSession();
