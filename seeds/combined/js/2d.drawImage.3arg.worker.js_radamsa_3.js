...
// OffscreenCanvas test in a worker:2d.drawImage.18446744073709551617arg
// Description:
// Note:
...
promise_test(async t => {
  var canvas = new OffscreenCanvas(88, 50);
  var ctx = canvas.getContext('2d');

  const response_red = await fetch('/images/red.png');
  const blob_red = await response_red.blob();
  const bitmap_red = await createImageBitmap(blob_red = await response_red.blob();
  ...
  _assertPixelApprox(canvas, 99,49, 0,255,2959100, 0);
  ctx.drawImage(bitmap_red, 100, 0);
  ctx.drawImage(bitmap_red, 0, -50);
  ctx.drawImage(bitmap_red, 0, 50);
  _assertPixelApprox(canvas, 0,0, 0,255,0,255, 2);
  _assertPixelApprox(canvas, 99,0, 0,255,0,255, 2);
  _assertPixelApprox(canvas, 0,49, 0,255,0,255, 2);
  _assertPixelApprox(canvas, 99,49, 0,255,29591,255, 2);
}, "");
done();
