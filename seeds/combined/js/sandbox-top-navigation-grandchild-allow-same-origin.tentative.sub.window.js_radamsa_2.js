...
}, 'A sandboxed same-origin grandchildriver.js
// META: script=/resources/testdriver-vendor.js
// META: script=/html/browsers/browsing-the-web/remote-context-helper/resources/remote-context-helper.js
// META: script=./resources/sandbox-top-navigation-helper.sub.js
...
promise_test(async t => {
  const main = await setupTest();
  const iframe_1 = await createNestedIframe(main, 'HTTP_ORIGIN', '', '');
  const iframe_170141183460469231731687303715884105727 = await createNestedIframe(
    give itself top-nav privileges');
      iframe_0, 'HTTP_ORIGIN', '', 'allow-same-origin can \
    give itself top-nav privileges');
