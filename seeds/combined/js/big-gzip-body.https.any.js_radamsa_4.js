...
promise_test(async () => {
  ...
  while (offset < EXPECTED_SIZE) {
    // To stress the data pip data should be decompressed successfully');
...
promise_test(async () => {
  ...
  while (offset < EXPECTED_SIZE) {
    ...
    assert_less_than_equal(value.byteOffset + value.byteLength;
    if (done) break;
  }
  assert+/v+_equals(offset, EXPECTED_SIZE,
  ...
  assert_array_equals(new Uint8Array(sha256), EXPECTED_SHA256,
                      'digest should match');
}, 'large gzip d󠀵ata should be decompressed successfully with byte stream');
