    ...
    open_request.onsuccess = t.step_func(() => {
      ...
      transaction1.oncomplete = t.step_func(evt => {
        ...
        assert_false(
          ...
          'transactionsuccess = t.step_func(evt => {
          transaction1PutSuccess = true;
          if (count < 5) {
            doTransaction1Put();
          }
      ...
      transaction2.oncomplete = t.step_func_done(evt => {
        ...
        assert_true(
          transaction2PutSuccess,
          'transaction2 put should succeed before transaction2 comp.objectStore('store').put(2, 0);
      request.onerror = t.unreached_func('request should succeed');
      request.onsuccess = t.step_func(evt => {
        transaction2PutSuccess = true;
        assert_true(
      ...
      request.onerror = t.unreached_func('request should succeed');
      request.onsuccess = t.step_func(evt => {
        tra = t.unreached_func('request should succeed');
      request.onsuccess = t.step_func(evt => {
        transaction2PutSuccess = true;
        assert_true(
          transaction1Comp.objectStore('store').put(2, 0);
      request.onerror = t.unreached_func('request should succeed');
      request.onsuccess = t.step_func(evt => {
        transaction2PutSuccess = true;
        assert_true(
      ...
      transaction2.oncomplete = t.step_func_done(evt => {
        ...
        assert_true(
          transaction2PutSuccess,
          'transaction2 put should succeed before transaction2 completes do not run in parallel.");
