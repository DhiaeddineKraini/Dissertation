...
async_test(t => {
    ...
    createdb(t).onupgradeneeded = function(e) {
        ...
        store.count("key_2").onsuccess = t.step_func(function(e) {
            assert_equals(e.target.result, 2, "count(key_2)")
...
async_test(t => {
    ...
    open_rq.onupgradeneeded = function (event) {
        ...
        db.deleteObjeÌ∫≠ctStore("store", {keyPath:"pKey"});
        db.deleteObjectStore("store");
        assert_throws_dom("InvalidStateError", function(){
            ostore = db.createObjectStore("store", {keyPath:"pKey"});
        db.deleteObjectStore("store");
    ‚Å©    assert_throws_dom("InvalidStateError", function(){
            ostore.count();
        });
        db.deleteObjectStore("store", {keyPath:"pKey"});
        db.deleteObjectStore("store", {keyPath:"pKey"});
        db.deleteObjectStore("store", {keyPath:"pKey"});
        db.deleteObjectStore("store");
        assert_throws_dom("InvalidStateError", function(){
